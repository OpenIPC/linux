...
static irqreturn_t dma_irq_handler(int irq, void *arg)
{
	...
	reg_status.reg = DAI_GETREG(DAI_INTSTATUS_REG_OFS) & 0x10000;
	if (reg_status.reg == 0) {
		return IRQ_NONE;
	}
...
static int nvt_dma_open(struct snd_pcm_substream *substream)
{
	...
	struct snd_soc_component *component = snd_soc_rtdcom_lookup(prtd, "alsa_component_p");
...
static snd_pcm_uframes_t nvt_dma_pointer(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		bytes = DAI_GETREG(DAI_RX1DMACURRENT_REG_OFS) - DAI_GETREG(DAI_RX1DMASTART_REG_OFS);
	}
...
static int nvt_dma_prepare(struct snd_pcm_substream *substream)
{
	...
	if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK) {
		...
		dai_set_rx_dma_para(0, (UINT32)runtime->dma_area, frames_to_bytes(runtime, runtime->buffer_size)>>2);
...
int __init nvt_alsapcm_module_init(void)
{
	...
	pdev = platform_device_alloc("nvt,alsa_plat_p",-1);
	if (!pdev)
	{

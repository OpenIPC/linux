...
static const struct soc_enum aec_type =
	SOC_ENUM_SINGLE(ES8311_GPIO_REG44, 0, 7, aec_type_txt);    // change with dts
...
static int es8311_probe(struct snd_soc_component *component)
{
	...
	printk("lwx:es8311 0x44=%02x\n",snd_soc_component_read32(component, ES8311_GPIO_REG44));
	...
	if (!of_property_read_string(np, "aec-mode", &str)) {
		...
		for (i = 0; i < ARRAY_SIZE(aec_type_txt); i++) {
			if (strcmp(str, aec_type_txt[i]) == 0) {
				printk("lwx:str=%s, i=%d\n",str, i);
				es8311->aec_mode = i;
				break;
			}
		}
	}
	printk("lwx:dt->es8311->aec_mode=%d\n",es8311->aec_mode);

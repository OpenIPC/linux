...
s32 ffsMountVol(struct super_block *sb)
{
	...
	if (i < 53) {
        ...
        #else
    if (memcmp(p_pbr->oem_name, (u8*)"EXFAT", 3)) {
        printk("[EXFAT] it don't have the exfat's flag!\n");
        ret = FFS_FORMATERR;
        #endif
	} else {
...
/*****************************************************************
aj for exfat add check sum of the fdt  start
*******************************************************************/
    
const unsigned char anyka_exfat_updatecase[5836] = {
...
u16 anyka_exfat_GetUnicodeUpdateCase(u16 UniCode)
{
    u16 ret;
    if(UniCode >= sizeof(anyka_exfat_updatecase) / 2)
        return UniCode;
    ret = anyka_exfat_updatecase[UniCode * 2 + 1];
    ret = (ret << 8) + anyka_exfat_updatecase[UniCode * 2];
    return ret;
}
u16 anyka_exfat_GetFileNameHash( u16 *FileName, u16 length)
{
    u16 i, HashValue = 0, tmp;
    for(i = 0; i < length; i++)
    {
        tmp = FileName[i];
        if(tmp == 0 || tmp == '\\' || tmp == '/')
        ...
        tmp = anyka_exfat_GetUnicodeUpdateCase(tmp);            
        HashValue = (u16)(((HashValue << 15) + (HashValue >> 1)) + (tmp & 0xFF));
        HashValue = (u16)(((HashValue << 15) + (HashValue >> 1)) + (tmp >> 8));
    }
    return HashValue;    
}
/*****************************************************************
aj for exfat add check sum of the fdt  end
*******************************************************************/
...
u16 calc_checksum_2byte(void *data, s32 len, u16 chksum, s32 type)
{
	...
	case CS_DIR_ENTRY:
		...
		break;
    case CS_NAME_HASH:
		chksum = anyka_exfat_GetFileNameHash((u16 *)c, len >> 1);
        break;
	default
			:
		for (i = 0; i < len; i++, c++)

...
#if 0
	hi_sync_timeline_debug_add(obj);
#endif
...
#if 0
	hi_sync_timeline_debug_remove(obj);
#endif
...
struct sync_fence *hi_sync_fence_create(const char *name, struct sync_pt *pt)
{
	struct sync_fence *fence;
...
#if 0
	hi_sync_fence_debug_add(fence);
#endif

	return fence;
}
EXPORT_SYMBOL(hi_sync_fence_create);

struct sync_fence *hi_sync_fence_fdget(int fd)
{
...
EXPORT_SYMBOL(hi_sync_fence_fdget);

void hi_sync_fence_put(struct sync_fence *fence)
{
	fput(fence->file);
}
EXPORT_SYMBOL(hi_sync_fence_put);

void hi_sync_fence_install(struct sync_fence *fence, int fd)
{
	printk("<%s> Line %d: fd =%d, fence=%p, name=%s\n", __func__, __LINE__, fd, fence, fence->name);
	fd_install(fd, fence->file);
}
EXPORT_SYMBOL(hi_sync_fence_install);
...
struct sync_fence *hi_sync_fence_merge(const char *name,
...
EXPORT_SYMBOL(hi_sync_fence_merge);

int hi_sync_fence_wake_up_wq(wait_queue_t *curr, unsigned mode,
...
int hi_sync_fence_wait_async(struct sync_fence *fence,
			  struct sync_fence_waiter *waiter)
{
	...
	init_waitqueue_func_entry(&waiter->work, hi_sync_fence_wake_up_wq);
...
EXPORT_SYMBOL(hi_sync_fence_wait_async);

int hi_sync_fence_cancel_async(struct sync_fence *fence,
			     struct sync_fence_waiter *waiter)
{
	unsigned long flags;
	int ret = 0;
...
EXPORT_SYMBOL(hi_sync_fence_cancel_async);

int hi_sync_fence_wait(struct sync_fence *fence, long timeout)
{
	long ret;
...
EXPORT_SYMBOL(hi_sync_fence_wait);
...
static int sync_fence_release(struct inode *inode, struct file *file)
{
	...
	hi_sync_fence_debug_remove(fence);
...
static long sync_fence_ioctl_wait(struct sync_fence *fence, unsigned long arg)
{
	...
	return hi_sync_fence_wait(fence, value);
}
...
static long sync_fence_ioctl_merge(struct sync_fence *fence, unsigned long arg)
{
	...
	fence2 = hi_sync_fence_fdget(data.fd2);
	if (fence2 == NULL) {
		err = -ENOENT;
		goto err_put_fd;
	}
	...
	fence3 = hi_sync_fence_merge(data.name, fence, fence2);
	if (fence3 == NULL) {
		err = -ENOMEM;
		goto err_put_fence2;
	}
	...
	hi_sync_fence_install(fence3, fd);
	hi_sync_fence_put(fence2);
	return 0;

err_put_fence3:
	hi_sync_fence_put(fence3);

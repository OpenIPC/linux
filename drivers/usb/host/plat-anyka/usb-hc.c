...
//#define AKOTG_HS_DEBUG
//#define AKOTG_HS_VERBOSE_DEBUG
...
static u8 ep_fifos[] = { USB_FIFO_EP0, USB_FIFO_EP1, USB_FIFO_EP2, USB_FIFO_EP3, USB_FIFO_EP4};
struct akotghc_epfifo_mapping akotg_epfifo_mapping;
#ifdef AKOTG_HS_DEBUG
static char *trans_desc[4] = {"iso", "intterrup", "conroller", "bulk"};
static char *xfer_name[4] = {"control", "iso", "bulk", "int"};
#endif
...
)
{
	...
	if (ep->epnum == 0) {
        ...
        */
#if 0//#ifdef CONFIG_USB_AKOTG_DMA
        if((len == 512) && (remain >= len))
        {
            ...
            do
            {
                ...
                if (phyaddr == 0) {
                    printk(KERN_ERR"tx dma_map_single error!\n");
                    break;
                }
                ...
                if(!usbdma){
                    //printk(KERN_ERR"dma alloc fail for in_packet\n");
    ...
    */
    ...
    if((len == 512) && (remain >= len))
    {
        ...
        do
        {
            ...
            if (phyaddr == 0) {
                printk(KERN_ERR"rx dma_map_single error!\n");
                break;
            }
            ...
            usbdma = akotg_dma_alloc(akotghc, ep->epfifo);
            if(!usbdma){
                //printk(KERN_ERR"dma alloc fail for out_packet\n");
...
{
	if (akotghc->port_status & (1 << USB_PORT_FEAT_SUSPEND)){
		HDBG("start_transfer_epx, akotghc->port_status : 0x%x\n", akotghc->port_status);
		return;
	}
...
) __releases(akotghc->lock) __acquires(akotghc->lock)
{
	...
	VDBG("Finishing EP%d URB Request...\n", usb_pipeendpoint(urb->pipe));
...
done(struct akotg_usbhc *akotghc, struct akotghc_ep *ep)
{
	...
	if (err_occurred) {
		...
		case USB_PID_IN:
			...
			if (len > ep->length) {
				printk(KERN_ERR"USB_PID_IN(OverFlow): len=%d, ep->length=%d\n",
					len, ep->length);
				len = ep->length;
				urbstat = -EOVERFLOW;
			}
...
static void reset_otg(struct work_struct *work)
{
	...
	/* power up and set usb suspend bit, reset the transceiver of USB 2.0 HS controller */
...
irqreturn_t akotg_usbhc_irq(struct usb_hcd *hcd)
{
	...
	{
		if(rINTTX &USB_INTR_EP0)
			pr_info("USB_INTR_EP0 happen!,but port_status is not USB_PORT_FEAT_ENABLE\n");
	}
...
int akotg_usbhc_urb_dequeue(struct usb_hcd *hcd, struct urb *urb, int status)
{
	...
	if (retval) {
		//printk(KERN_ERR"Dequeue: check and unlink urb failed!\n");
		goto fail;
	}
...
) 
{
	...
	if (!__is_epnum_mapped(&akotg_epfifo_mapping, epnum, is_out)) {
		if (!__map_epnum_to_epfifo(&akotg_epfifo_mapping, epnum, is_out, &epfifo)) {
			...
			HDBG("ep%d can not mapping\n",epnum);
			return -ENOSPC;
		}

		HDBG("###ep%d mapping hostEP%d \n",epnum, epfifo);
	...
	if (!(akotghc->port_status & (1 << USB_PORT_FEAT_ENABLE))
		...
		retval = -ENODEV;
		HDBG("ep%d dead or disabled port!\n",epnum);
		goto fail_not_linked;
	}

	retval = usb_hcd_link_urb_to_ep(hcd, urb);
	if (retval) {
		HDBG("ep%d usb_hcd_link_urb_to_ep fail!\n",epnum);
		goto fail_not_linked;
	}
...
akotg_usbhc_hub_status_data(struct usb_hcd *hcd, char *buf)
{
	...
	if (!timer_pending(&akotghc->timer)) {
		if (akotg_usbhc_irq( /* ~0, */ hcd) != IRQ_NONE)
			pr_info("akotg_usbhc_hub_status_data call akotg_usbhc_irq return IRQ_HANDLED\n");//akotghc->stat_lost++;
	}
	local_irq_restore(flags);
...
int akotg_usbhc_reset(struct usb_hcd *hcd)
{
	unsigned long con;
	struct akotg_usbhc *akotghc = hcd_to_akotg_usbhc(hcd);

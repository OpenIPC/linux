...
#define FOTG210_MAX_FIFO_NUM	8 /* fifo0...fifo4 */
...
#define DCFESR_FIFO_EMPTY(fifo)	(1 << 8 << ((fifo)<<1))
...
#define DMISGR1_MF_IN_INT(fifo)	(1 << (16 + ((fifo)<<1)))
...
#define DMISGR1_MF_OUTSPK_INT(fifo)	(0x3 << (fifo) * 4)

/* Device Mask of Interrupt Source Group 2 Register (0x13C) */
#define FOTG210_DMISGR2		0x13C
#define DMISGR2_MDMA_WAKEUPVBUS	(1 << 14)
#define DMISGR2_MDMA_IDLE	(1 << 13)
...
#define DISGR1_OUT_INT(fifo)	(1 << (((fifo)<<0) * 2))
#define DISGR1_SPK_INT(fifo)	(1 << 1 << (((fifo)<<0) * 2))
#define DISGR1_IN_INT(fifo)	(1 << 16 << ((fifo)<<0))

/* Device Interrupt Source Group 2 Register (0x14C) */
#define FOTG210_DISGR2		0x14C
#define DISGR2_DMA_CMPLT5	(1 << 12)
#define DISGR2_DMA_CMPLT4	(1 << 11)
#define DISGR2_DMA_CMPLT3	(1 << 10)
#define DISGR2_DMA_CMPLT2	(1 << 9)
...
#define FOTG210_OUTEPMPSR(ep)	(0x160 + 4 * ((ep) - 1))

/* Device Endpoint 1~4 Map Register (0x1A0) */
#define FOTG210_EPMAP(ep)		(0x160 + 4 * (ep))
#define EPMAP_FIFONO(ep, dir)		\
	((((ep) - 1)<<1) << ((dir) ? 24 : 28))
#define EPMAP_FIFONOMSK(ep, dir)	\
	((0xF) << ((dir) ? 24 : 28))
#define EPMAP_DIROUT	(0x1 << 16)

/* Device FIFO Map Register (0x1A8) */
#define FOTG210_FIFOMAP(ep)		(0x1A8 + (((ep)>>1)<<2))
#define FIFOMAP_DIROUT(fifo)	(0x0 << 5 << (fifo) * 8)
#define FIFOMAP_DIRIN(fifo)	(0x101 << 5 << ((fifo)&0x1) * 16)
#define FIFOMAP_DOUBLEBLK(fifo)	(0x101 << 2 << ((fifo)&0x1) * 16)
#define FIFOMAP_SINGLEBLK(fifo)	(0x000 << 2 << ((fifo)&0x1) * 16)
#define FIFOMAP_BIDIR(fifo)	(0x2 << 5 << (fifo) * 8)
#define FIFOMAP_NA(fifo)	(0x3 << 5 << (fifo) * 8)
//#define FIFOMAP_EPNO(ep)	((ep) << ((ep) - 1) * 8)
#define FIFOCF_BLKSZ_1024(fifo)	(0x101 << 4 << ((fifo)&0x1) * 16)
//#define FIFOMAP_EPNOMSK(ep)	(0xF << ((ep) - 1) * 8)

/* Device FIFO Confuguration Register (0x1AC) */
#define FOTG210_FIFOCF		0x1AC
#define FIFOCF_TYPE(type, fifo)	(((type)+((type)<<8)) << ((fifo)&0x1) * 16)
...
#define FIFOCF_FIFO_EN(fifo)	(0x1 << ((fifo)&0x1) * 16 << 7)
...
#define FOTG210_DMACPSR2_EP3 0x204
...
struct fotg210_udc {
	...
	spinlock_t					lock2; /* protect the struct */
	void __iomem				*reg;

...
static ssize_t f_hidg_read(struct file *file, char __user *buffer, size_t count,
			   loff_t *ptr)
{
	struct f_hidg *hidg = file->private_data;
...
static void hidg_ssreport_complete(struct usb_ep *ep, struct usb_request *req)
{
	...
	if (req->status != 0 || req->buf == NULL || req->actual == 0) {
		ERROR(cdev, "%s FAILED: status=%d, buf=%p, actual=%d\n",
...
static void hidg_disable(struct usb_function *f)
{
	...
	if (hidg->out_ep) {
		...
		spin_lock_irqsave(&hidg->read_spinlock, flags);
		list_for_each_entry_safe (list, next, &hidg->completed_out_req,
					  list) {
...
static int hidg_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (hidg->use_out_ep && hidg->out_ep != NULL) {
		...
		for (i = 0; i < hidg->qlen && status == 0; i++) {
			struct usb_request *req = hidg_alloc_ep_req(
				hidg->out_ep, hidg->report_length);
			if (req) {
...
static int hidg_bind(struct usb_configuration *c, struct usb_function *f)
{
	...
	if (hidg->use_out_ep) {
		hidg_ss_out_ep_desc.wMaxPacketSize =
			cpu_to_le16(hidg->report_length);

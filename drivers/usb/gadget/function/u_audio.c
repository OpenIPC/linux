...
static int snd_uac_pcm_mute_info(struct snd_kcontrol *kcontrol,
...
static int snd_uac_pcm_mute_get(struct snd_kcontrol *kcontrol,
				  struct snd_ctl_elem_value *ucontrol)
{
	struct g_audio *g_audio= (struct g_audio *)kcontrol->private_data;
	int value = g_audio->mute;
	ucontrol->value.integer.value[0] = value;
	return 0;
}

static struct snd_kcontrol_new snd_uac_pcm_mute = {
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
	.name = "Playback Mute Control",
	.access = SNDRV_CTL_ELEM_ACCESS_READWRITE | SNDRV_CTL_ELEM_ACCESS_TLV_READ,
	.index = E_UAC_CONTROL_TYPE_PLAYBACK_MUTE,
	.info = snd_uac_pcm_mute_info,
	.get = snd_uac_pcm_mute_get,
	.put = NULL,
};
...
static struct snd_kcontrol_new snd_uac_pcm_volume = {
	.iface = SNDRV_CTL_ELEM_IFACE_MIXER,
...
void g_audio_notify(struct g_audio *g_audio, int index)
{
	...
	switch(index) {
	case E_UAC_CONTROL_TYPE_PLAYBACK_MUTE:
		ctl = g_audio->uac->mute_ctl;
		snd_ctl_notify(card, SNDRV_CTL_EVENT_MASK_VALUE, &ctl->id);
		break;
	case E_UAC_CONTROL_TYPE_PLAYBACK_VOLUME:
		ctl = g_audio->uac->volume_ctl;
		snd_ctl_notify(card, SNDRV_CTL_EVENT_MASK_VALUE, &ctl->id);
		break;
	default:
		dev_err(g_audio->uac->card->dev, "%d Error!\n", __LINE__);
		break;
	}
}
					...
					const char *card_name)
{
	...
	if ((err = snd_ctl_add(card, uac->volume_ctl)) < 0)
		return err;

	uac->mute_ctl = snd_ctl_new1(&snd_uac_pcm_mute, g_audio);
	if ((err = snd_ctl_add(card, uac->mute_ctl)) < 0)
		return err;

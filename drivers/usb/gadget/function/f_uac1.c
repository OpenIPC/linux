...
static int f_audio_set_alt(struct usb_function *f, unsigned intf, unsigned alt)
{
	...
	if (intf == uac1->as_out_intf) {
		if (alt && uac1->as_out_alt == alt) {
			printk(KERN_DEBUG"force reset intf %d alt %d", intf, alt);
...
static void mixer_cmd_work(struct work_struct *data)
{
	...
	switch (con->type)
	{
		case UAC_FU_VOLUME:
			...
			g_audio_notify(&uac1->g_audio, E_UAC_CONTROL_TYPE_PLAYBACK_VOLUME);
			break;
		case UAC_FU_MUTE:
			uac1->g_audio.mute = value;
			g_audio_notify(&uac1->g_audio, E_UAC_CONTROL_TYPE_PLAYBACK_MUTE);
			break;
		default:
			break;
	}
...
static int control_selector_init(struct f_uac1 *uac1)
{
	...
	capture_mute_control.data[UAC__CUR] = 0;
	...
	playback_mute_control.data[UAC__CUR] = 0;

...
EXPORT_SYMBOL(phy_utmi_reset);

static void phy_utmi_deinit(struct phy *phy)
{
    phy_utmi_reset();
    // power down utmi
...
static int phy_infinity6e_sata_init(struct phy *phy)
{
    ...
    SETREG16(dphy_base + 0x26*4, (tx_voltage_array[m_TxVoltageIdx].reg_biasi & 0x7) << 1);
    ...
    OUTREG16(dphy_base + 0x40*4, ((tx_voltage_array[m_TxVoltageIdx].reg_dem & 0x0F) << 12) |
                                 ((tx_voltage_array[m_TxVoltageIdx].reg_drv & 0x3F) << 6) | 0x19);
...
static int phy_infinity6e_reset(struct phy *phy)
{

...
#if   defined(CONFIG_GTP_FW_720JC)
#include "gt9xx_fw_720jc.h"
#elif defined(CONFIG_GTP_FW_720LX)
#include "gt9xx_fw_720lx.h"
#else
#define CTP_CUSTOM_CFG {0}
#endif
#endif
...
static void goodix_ts_work_func(struct work_struct *work)
{
	...
	//printk("pre_touch = %d    touch_num = %d,\n",pre_touch, touch_num);
	...
	/*
	if(enter_earlysuspend_flag == 1){
		printk("=====joey===== goodix_ts_work_func   powerkey\n");			
		msleep(50);
		input_report_key(ts->input_dev, KEY_POWER, 1);
		input_sync(ts->input_dev);
		input_report_key(ts->input_dev, KEY_POWER, 0);
		input_sync(ts->input_dev);
		enter_earlysuspend_flag = 0;
	}
	*/
#endif
	pre_key = key_value;
...
static s32 gtp_init_panel(struct goodix_ts_data *ts)
{
	...
	u8 send_cfg_buf[] = CTP_CUSTOM_CFG;
	u8 cfg_info_len = (sizeof(send_cfg_buf))/(sizeof(send_cfg_buf[0]));
...
#endif
	__set_bit(KEY_POWER,  ts->input_dev->keybit);
...
#ifdef CONFIG_OF
static const struct of_device_id gt911jc_of_match[] = {
    { .compatible = "allwinner,gt911" },
    { }
};
MODULE_DEVICE_TABLE(of, gt911jc_of_match);
#endif
...
static struct i2c_driver goodix_ts_driver = {
	...
	.driver = {
		...
		.owner  = THIS_MODULE,
#ifdef CONFIG_OF
		.of_match_table = of_match_ptr(gt911jc_of_match),
#endif
...
static int __init goodix_ts_init(void)
{
	...
	if (input_sensor_startup(&(config_info.input_type))) {
		printk("%s: ctp_startup err.\n", __func__);
		return 0;
	} else {
		ret = input_sensor_init(&(config_info.input_type));
		if (ret != 0)
			printk("%s:ctp_ops.init err.\n", __func__);
	...
	printk("===== goodix_ts_init =====i2c_add_driver is %d\n",ret);

...
/// @param <RET>        \b None :
////////////////////////////////////////////////////////////////////////////////
void HAL_HWI2C_SetIOMapBase(U32 u32Base,U32 u32ChipBase,U32 u32ClkBase)
{
    HWI2C_HAL_FUNC();
    ...
    //HWI2C_HAL_INFO("HWI2C IOMap base:%16lx Reg offset:%4x\n", u32Base, (U16)HWI2C_REG_BASE);
//    printk(" HWI2C IOMap base:%x \n", u32Base);
//    printk("HWI2C u32ChipBase base:%x\n ", u32ChipBase);
//    printk(" HWI2C u32ClkBase base:%x \n", u32ClkBase);
...
BOOL HAL_HWI2C_Write2Byte(U32 u32RegAddr, U16 u16Val)
{
    ...
    //HAL_HWI2C_WriteByte(u32RegAddr,LOWBYTE(u16Val));
    //HAL_HWI2C_WriteByte(u32RegAddr+1,HIBYTE(u16Val));
...
U8 HAL_HWI2C_ReadChipByte(U32 u32RegAddr)
{
	...
	HWI2C_HAL_FUNC();
	HWI2C_HAL_INFO("HWI2C IOMap base:%16lx u32RegAddr:%8lx u32RegAddr:%16lx\n", _gMChipIO_MapBase, u32RegAddr,(_gMChipIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2)));
    //return ((volatile U8*)(_gMIO_MapBase))[(u32RegAddr << 1) - (u32RegAddr & 1)];
    
    u16value = (*(volatile U32*)(_gMChipIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2)));
	HWI2C_HAL_INFO("u16value===:%4x\n", u16value);
	return ((u32RegAddr & 0xFF) % 2)? HIBYTE(u16value) : LOWBYTE(u16value);
}
...
BOOL HAL_HWI2C_WriteChipByte(U32 u32RegAddr, U8 u8Val)
{
	...
	if((u32RegAddr & 0xFF) % 2)
	{
		u16value = (((U16)u8Val) << 8)|((*(volatile U32*)(_gMChipIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2))) & 0xFF);
	}
	else
	{
		u16value = ((U16)u8Val)|((*(volatile U32*)(_gMChipIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2))) & 0xFF00);
	}
	
	(*(volatile U32*)(_gMChipIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2))) = u16value;
    return TRUE;
}
...
U16 HAL_HWI2C_ReadClk2Byte(U32 u32RegAddr)
{
    ...
    //return ((volatile U16*)(_gMIO_MapBase))[u32RegAddr];
    return (*(volatile U32*)(_gMClkIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2)));
}
...
BOOL HAL_HWI2C_WriteClk2Byte(U32 u32RegAddr, U16 u16Val)
{
	...
	HWI2C_HAL_FUNC();
	HWI2C_HAL_INFO("HWI2C IOMap base:%16lx u32RegAddr:%8lx u32RegAddr:%16lx\n", _gMClkIO_MapBase, u32RegAddr,(_gMClkIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2)));

    //((volatile U16*)(_gMIO_MapBase))[u32RegAddr] = u16Val;
    //HAL_HWI2C_WriteByte(u32RegAddr,LOWBYTE(u16Val));
    //HAL_HWI2C_WriteByte(u32RegAddr+1,HIBYTE(u16Val));
    (*(volatile U32*)(_gMClkIO_MapBase+((u32RegAddr & 0xFFFFFF00ul) << 1) + (((u32RegAddr & 0xFF)/ 2) << 2))) = u16Val;
    return TRUE;
}
...
/// @param <RET>        \b TRUE : ok, FALSE : fail
////////////////////////////////////////////////////////////////////////////////
BOOL HAL_HWI2C_SetPortRegOffset(HAL_HWI2C_PORT ePort, U16* pu16Offset)
{
    HWI2C_HAL_FUNC();
...
/// @param <RET>        \b TRUE : ok, FALSE : fail
////////////////////////////////////////////////////////////////////////////////
BOOL HAL_HWI2C_SelectPort(int ePort)
{
    U8 u8Value1=0;

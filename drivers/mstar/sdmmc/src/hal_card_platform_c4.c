...
#if (D_OS == D_OS__UBOOT)
        //SD0_CDZ for IP CDZ
        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7C), BIT07_T);           //[1] cdz gpio mode, [0] sd gpio mode
...
void Hal_CARD_InitGPIO(GPIOEmType eGPIO, PADEmType ePAD, BOOL_T bEnable)
{
    ...
    {
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7C), BIT07_T);           //[1] cdz mode, [0] sd gpio mode
...
BOOL_T Hal_CARD_GetGPIOState(GPIOEmType eGPIO, PADEmType ePAD)
{
    ...
    if( eGPIO==EV_GPIO2 ) //EV_GPIO1 for Slot 1
    {
    }
...
void Hal_CARD_SetGPIOState(GPIOEmType eGPIO, PADEmType ePAD, BOOL_T bOutputState)
{
    ...
    if( eGPIO==EV_GPIO1 ) //EV_GPIO1 for Slot 0
    {
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7E), BIT12_T);  //PMU_GPIO_OUT_EN

        if(bOutputState)
        {
            CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7D), BIT12_T);   //PMU_GPIO_OUT=1
        }
        else
        {
            CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_PMU_BANK, 0x7D), BIT12_T);  //PMU_GPIO_OUT=0
        }

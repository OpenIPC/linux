...
#define EN_BYPASSMODE     (TRUE)    //BYPASS MODE or ADVANCE MODE(SDR/DDR)
...
#define V_SD_BOOTCFG_INIT   0x0

#if (EN_MIU_WDEN_PATCH_DIS)
#define V_NC_REORDER_INIT   (R_MIU_WDEN_PATCH_DIS)
#else
#define V_NC_REORDER_INIT   0x0
#endif
...
static volatile U16_T   gu16_PAD_IN_SEL[3] = {0};
static volatile U16_T   gu16_EMMC_BOOTCFG_REG[3] = {0};
...
//***********************************************************************************************************
//  SDMMC Mechanism Detection Function
//***********************************************************************************************************
...
{
    gu16_PAD_IN_SEL[eIP] = 0;

    switch ( eBusTiming )
    {
        case EV_BUS_LOW:
            gu16_SD_MODE_DatSync[eIP] = R_SD_DATSYNC;
            gu16_EMMC_BOOTCFG_REG[eIP] = R_SDYPASSMODE;
            break;
        case EV_BUS_DEF:
        ...
        case EV_BUS_HS:
            gu16_PAD_IN_SEL[eIP] = R_CSREG_PAD_IN_SEL; // T-Mux SDR
            gu16_SD_MODE_DatSync[eIP] = R_SD_DATSYNC;
            gu16_EMMC_BOOTCFG_REG[eIP] = 0;
            break;
...
RspErrEmType Hal_SDMMC_SendCmdAndWaitProcess(IPEmType eIP, TransEmType eTransType, CmdEmType eCmdType, SDMMCRspEmType eRspType, BOOL_T bCloseClk)
{
    ...
    CARD_REG_SETBIT(A_NC_REORDER_REG(eIP), V_NC_REORDER_INIT | gu16_PAD_IN_SEL[eIP]);
    CARD_REG(A_EMMC_BOOTCFG_REG(eIP)) = V_SD_BOOTCFG_INIT | gu16_EMMC_BOOTCFG_REG[eIP];
...
/*#if(EN_BIND_CARD_INT)
    //Because IP has bug, and it may have chance to access the same register, so we need to do os lock outside
 ...
 *     @author jeremy.wang (2015/11/3)
 * Desc: Set SDIO Int end setting
 *
 * @param eIP : FCIE1/FCIE2/...
 * @param eRspErr : Response Error Type
 * @param u16BlkCnt : Block Cnt
 ----------------------------------------------------------------------------------------------------------*/
...
/*----------------------------------------------------------------------------------------------------------
 *
 * Function: Hal_SDMMC_SetMIEIntEn_ForV4
 *     @author jeremy.wang (2014/5/28)
 * Desc: Design a MIE Int enable funtion for host semaphone process to avoid access the same register issue
 *
 * @param eIP : FCIE1/FCIE2/...
 * @param eTransType : CIFD/DMA/NONE
 ----------------------------------------------------------------------------------------------------------*/
void Hal_SDMMC_SetMIEIntEn_ForV4(IPEmType eIP, TransEmType eTransType)
{
    U16_T u16MIE_PATH_MMA_EN = (U8_T)eTransType;
    U16_T u16MIE_TRANS_END = (u16MIE_PATH_MMA_EN<<14) | (u16MIE_PATH_MMA_EN<<11) | ((!u16MIE_PATH_MMA_EN)<<2);
    Hal_CARD_INT_SetMIEIntEn(eIP, EV_INT_SD, V_MIE_INT_EN_INIT | u16MIE_TRANS_END);

...
mhve_ios* mmfeios_acquire(
    char* tags)
{
    mfe_asic* asic = MEM_ALLC(sizeof(mfe_asic));
    if (asic)
    {
...
set_outbs_addr(
    ...
    int         size)
{
#define UPDATE_ADDR (1<<1) // reg3b_s_bspobuf_set_adr
#define SET_ADDRESS (1<<0) // reg3f_s_bspobuf_update_adr
    uint32  value;
    // address value
    value = addr;
    regs->reg3e = 0;
    ...
    REGWR(base,0x3b,regs->reg3b|SET_ADDRESS,"set bsp obuf");
}
...
{
    mfe_asic* asic = (mfe_asic*)ios;
    mfe_regs* regs = asic->p_regs;
    void* base = asic->p_base;
    uint bit = 0;
    do
    {
        while (!(bit = bit_irq(base)))
            msleep(3);
        // TODO: handle enc_done/buf_full
    }
    while (!(bit&BIT_ENC_DONE));
    // feedback
    ...
    regs->cycles = ((uint)(regs->reg77&0xFF)<<16) + regs->reg76;
    regs->bitcnt = ((uint)(regs->reg43_s_bsp_bit_cnt_hi)<<16) + regs->reg42_s_bsp_bit_cnt_lo;
    regs->sumofq = ((uint)(regs->reg29_s_mbr_last_frm_avg_qp_hi)<<16) + regs->reg28_s_mbr_last_frm_avg_qp_lo;
    regs->mjob.i_tick = (int)(regs->cycles);
    regs->mjob.i_bits = (int)(regs->bitcnt - regs->delta);
    clr_irq(base, 0xFF);
    return bit;
}
    ...
    int         irq)
{
    ...
    mfe_regs* regs = asic->p_regs;
    void* base = asic->p_base;
    uint bit = bit_irq(base);
    if (bit&BIT_BUF_FULL)
    {
        clr_irq(base, 0xFF);
        set_outbs_addr(base, regs, regs->obase[1], regs->osize[1]);
        return 1;
    }
    msk_irq(base, 0xFF);
    // TODO: handle enc_done/buf_full

...
#if 0
    send_trace("ptInBufCfg->nInBufYAddr[0]=0x%x, addr=0x%x\n", ptInBufCfg->nInBufYAddr[0], addr);
    send_trace("(0x%x,0x%x), nRegJpeGBufYAdr0Low:0x%x,  nRegJpeGBufYAdr0High:0x%x \n", \
        (addr>>8)&0xFFFF,\
        addr>>(8+16),\
        pJpeReg->uReg06.tReg06.nRegJpeGBufYAdr0Low,\
        pJpeReg->uReg07.tReg07.nRegJpeGBufYAdr0High);
#endif
    // Y1
...
u32 HalJpeSetClock(JpeHalHandle_t* pJpeHalHandle)
{
    JpeReg_t* pJpeReg = NULL;

    if(!pJpeHalHandle)
        return -1;

    pJpeReg = &pJpeHalHandle->tJpeReg;

    /* Enable Clk */
    //REGW(REG_CKG_MFE_BASE,0x00,regs->regClk,"Set Clk");

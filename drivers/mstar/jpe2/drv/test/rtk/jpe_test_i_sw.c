...
static int _DoTest(JPEG_TEST_FORMAT_e eRawFormat, int bDebugMsg, int nTaskId)
{
    ...
    for(eClkSelect = eCLK_288; eClkSelect <= eCLK_27; eClkSelect++)
    {
        ...
        {
            case eCLK_288:
                CamOsDebug("%s Task:%d Clock: eCLK_288 %s \n", BLUE, nTaskId, NORMAL);
                break;
            case eCLK_216:
                CamOsDebug("%s Task:%d Clock: eCLK_216 %s \n", BLUE, nTaskId, NORMAL);
                break;
            case eCLK_54:
                CamOsDebug("%s Task:%d Clock: eCLK_54  %s \n", BLUE, nTaskId, NORMAL);
                break;
            case eCLK_27:
                CamOsDebug("%s Task:%d Clock: eCLK_27  %s \n", BLUE, nTaskId, NORMAL);
                break;
        }
...
static int _TestJpeMultiTask(CLI_t *pCli, char *p)
{
    ...
    CamOsDebug("%s JpeTaskid[0]=%d, JpeTaskid[1]=%d\n", BLUE, JpeTaskid[0], JpeTaskid[1], NORMAL);

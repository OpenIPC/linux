...
#define MINMUN_WIDTH    16
#define MINMUN_HEIGHT   5
 ...
 *   none
 */
static ive_request_data* drv_ive_add_request(ive_drv_handle *handle, ive_file_data *file_data)
{
    struct platform_device *pdev = handle->pdev;
...
static IVE_IOC_ERROR drv_ive_check_config(ive_ioc_config *config)
{
    ...
    if (config->input.width < MINMUN_WIDTH || config->input.height < MINMUN_HEIGHT) {
...
static void ive_drv_sync_image(ive_drv_handle *handle, ive_ioc_image *image, enum dma_data_direction direct)
{
    ...
    // IVE_MSG(IVE_MSG_DBG, "sync buffer %p, format 0x%x\n", image->address[0], image->format);
...
int ive_drv_init(ive_drv_handle *handle, struct platform_device *pdev, phys_addr_t base_addr0, phys_addr_t base_addr1)
{
    ...
    handle->work_buffer = devm_kmalloc(&handle->pdev->dev, sizeof(ive_work_buffer), GFP_KERNEL);
    if (handle->work_buffer == NULL) {
        IVE_MSG(IVE_MSG_ERR, "can't allocate working buffer\n");
        return -ENOMEM;
    }

    IVE_MSG(IVE_MSG_DBG, "work buffer = 0x%p\n", handle->work_buffer);
...
static void ive_drv_start(ive_drv_handle *handle, ive_ioc_config *config)
{
    ...
    switch (config->op_type) {
        ...
        case IVE_IOC_OP_TYPE_MAP:
            memcpy(handle->work_buffer->map_buffer, config->coeff_map.map, sizeof(config->coeff_map.map));
            handle->work_buffer_dma = dma_map_single(NULL, handle->work_buffer, sizeof(ive_work_buffer), DMA_TO_DEVICE);
            dma_sync_single_for_device(NULL, handle->work_buffer_dma, sizeof(ive_work_buffer), DMA_TO_DEVICE);
            ive_hal_set_coeff_map(&handle->hal_handle, (void*)handle->work_buffer_dma);
            break;
        ...
        case IVE_IOC_OP_TYPE_NCC:
            handle->work_buffer_dma = dma_map_single(NULL, handle->work_buffer, sizeof(ive_work_buffer), DMA_FROM_DEVICE);
            dma_sync_single_for_device(NULL, handle->work_buffer_dma, sizeof(ive_work_buffer), DMA_FROM_DEVICE);
            ive_hal_set_coeff_ncc(&handle->hal_handle, (void*)handle->work_buffer_dma);
            break;
...
IVE_IOC_ERROR ive_drv_process(ive_drv_handle *handle, ive_file_data *file_data)
{
    ...
    if (drv_ive_add_request(handle, file_data) == NULL) {
...
ive_file_data* ive_drv_post_process(ive_drv_handle *handle)
{
    ...
    switch(handle->hal_handle.reg_bank1.op_type)
    {
        case IVE_IOC_OP_TYPE_MAP:
            dma_unmap_single(NULL, handle->work_buffer_dma, sizeof(ive_work_buffer), DMA_TO_DEVICE);
            break;

        case IVE_IOC_OP_TYPE_NCC:
            dma_unmap_single(NULL, handle->work_buffer_dma, sizeof(ive_work_buffer), DMA_FROM_DEVICE);
            break;
    ...
    if (previous_file_data != NULL) {
        ive_drv_copy_work_buffer(handle, &(previous_file_data->work_buffer));
...
{
    memcpy(work_buffer, handle->work_buffer, sizeof(ive_work_buffer));
}

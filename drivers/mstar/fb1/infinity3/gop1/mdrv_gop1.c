...
void GOP1_SwitchSubBank(int subbank)
{
    unsigned long GOP_Reg_Base=0;
    GOP_Reg_Base = mdrv_BASE_REG_GOP10_PA;

    if ((subbank>=0)&&(subbank<=2))
    {
        OUTREG16(GOP_Reg_Base+REG_GOP_7F,subbank);
        GOP1DBG( "[GOP1]%s: Switch to Subbank %d\n",__func__,subbank);
    }
    else
        GOP1DBG( "[GOP1]%s: Wrong Subbank %d\n",__func__,subbank);
}

void GOP1_Pan_Display(int Screen, U32  FB_Start_Addr )
{
    ...
    GOP1DBG ("[GOP1]%s: in, addr=0x%x \n",__func__,FB_Start_Addr);

    /// GOP0_1 settings
    GOP_Reg_DB = GOP_BANK_DOUBLE_WR_G1;
    ...
    while((INREGMSK16(GOP_Reg_Base+REG_GOP_7F, GOP_BANK_GOP1_WR_ACK_MSK)!=0))//wait double buffer write ack
    {
        wait_count++;
        msleep(3);
        if(wait_count>10)
    ...
    U32 Pixel_Format,U16 Bytes_Per_Pixel, int bInterlaceOut)
{
    ...
    //GOP1DBG ("GOP_Setmode Screen is %x.\r\n",Screen);
    GOP1DBG ("[GOP1]%s: FB_Width  = [%d]\n",__func__,FB_WIDTH);
    GOP1DBG ("[GOP1]%s: FB_Height = [%d]\n",__func__,FB_HEIGHT);
    GOP1DBG ("[GOP1]%s: FB_addr   = [0x%x]\n",__func__,FB_add);
    GOP1DBG ("[GOP1]%s: FB_Format = [%s]\n",__func__,PARSING_FB_FORMAT(DISPLAY_Format));
    GOP1DBG ("[GOP1]%s: FB_BPP    = [%d]\n",__func__,FB_Bytes_Per_Pixel);
    ...
    U32 Pixel_Format,U16 Bytes_Per_Pixel, int bInterlaceOut, int StretchRatio)
{
    ...
    GOP1DBG ("[GOP]%s: FB_Width    = [%d]\n",__func__,FB_WIDTH);
    GOP1DBG ("[GOP]%s: FB_Height   = [%d]\n",__func__,FB_HEIGHT);
    GOP1DBG ("[GOP]%s: FB_addr     = [0x%x]\n",__func__,FB_add);
    GOP1DBG ("[GOP]%s: FB_Format   = [%s]\n",__func__,PARSING_FB_FORMAT(DISPLAY_Format));
    GOP1DBG ("[GOP]%s: FB_BPP      = [%d]\n",__func__,FB_Bytes_Per_Pixel);
    GOP1DBG ("[GOP]%s: FB_StrRatio = [%x]\n",__func__,StretchRatio);
...
void GOP1_Set_OutFormat(int screen, int byuvoutput)
{
    U32 GOP_Reg_Base = 0;
    U16 GOP_Reg_DB   = 0;
...
void GOP1_Set_Constant_Alpha(int screen, int bEn, int alpha)
{
    U32 GOP_Reg_Base = 0;
    U16 GOP_Reg_DB   = 0;
    ...
    GOP1DBG("[GOP1]%s: screen:%d, bEn:%d, alpha:%x\n",__func__, screen, bEn, alpha);
...
void GOP1_Set_Color_Key(int screen, int bEn, int u8R, int u8G, int u8B)
{
    U32 GOP_Reg_Base = 0;
    U16 GOP_Reg_DB   = 0;
    ...
    GOP1DBG("[GOP1]%s: screen:%d, bEn:%d, (R=%x, G=%x, B=%x)\n",__func__, screen, bEn, u8R, u8G, u8B);
...
void GOP1_Set_PipeDelay(int screen, int delay)
{
    ...
    GOP1DBG("[GOP1]%s: screen:%d, delay:%d\n",__func__, screen, delay);
...
void GOP1_Set_Palette_RIU(int screen, int u8Index, int u8A, int u8R, int u8G, int u8B)
{
    U32 GOP_Reg_Base = 0;
...
void GOP1_Set_Outputmode(int Screen, int Panel_Width, int Panel_Height,U32 FB_Start_Addr, U32 Pixel_Format, U16 Bytes_Per_Pixel)
{
    U32 FB_add    = FB_Start_Addr;
    U16 FB_WIDTH  = Panel_Width;
    U16 FB_HEIGHT = Panel_Height;
...
void GOP1_Set_Enable_GWIN(int screen, unsigned char bEn)
{
    unsigned long GOP_Reg_Base = 0;
    ...
    GOP1DBG( "[GOP1]%s: Set enable_GOP = [%d]\n",__func__,enGOP);
...
void GOP1_Get_Enable_GWIN(int screen, unsigned char *bEn)
{
    unsigned long GOP_Reg_Base = 0;
    ...
    GOP1DBG( "[GOP1]%s: Get enable_GOP = [%d]\n",__func__,enGOP);

}

void GOP1_Get_Alpha(int screen, int *alpha_type, int *alpha_val)
{
    unsigned long GOP_Reg_Base = 0;
    ...
    *alpha_val = (unsigned char)INREGMSK16(GOP_Reg_Base+REG_GOP_0A, 0x00FF);
    ...
    GOP1DBG( "[GOP1]%s: Alpha_type = [%s], Alpha_value = [%d]\n",__func__,PARSING_ALPHA_TYPE(*alpha_type),*alpha_val);

}

void GOP1_Get_Color_Key(int screen, int *bEn, int *u8R, int *u8G, int *u8B)
{

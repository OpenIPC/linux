...
#include "gop1/mdrv_gop1.h"
#include "inv_color/mdrv_gop1_inv_color.h"
...
//  Defines
//--------------------------------------------------------------------------------------------------
#define FB1_DEBUG 0  //0 for only error message, 1 for normal debug message, 2 for more message

#if (FB1_DEBUG==0)
#define FB1DBGERR(fmt, arg...)     printk(KERN_INFO fmt, ##arg)
#define FB1DBG(fmt, arg...)
#define FB1DBGMORE(fmt, arg...)
#elif (FB1_DEBUG==1)
#define FB1DBGERR(fmt, arg...)     printk(KERN_INFO fmt, ##arg)
#define FB1DBG(fmt, arg...)        printk(KERN_INFO fmt, ##arg)
#define FB1DBGMORE(fmt, arg...)
#elif (FB1_DEBUG==2)
#define FB1DBGERR(fmt, arg...)     printk(KERN_INFO fmt, ##arg)
#define FB1DBG(fmt, arg...)        printk(KERN_INFO fmt, ##arg)
#define FB1DBGMORE(fmt, arg...)    printk(KERN_INFO fmt, ##arg)
#else
#define FB1DBGERR(fmt, arg...)
#define FB1DBG(fmt, arg...)
#define FB1DBGMORE(fmt, arg...)
#endif
...
//  Functions
//--------------------------------------------------------------------------------------------------
static int mdrvinfinityfb1_ioctl(struct fb_info *info, u32 cmd, unsigned long arg);
static int mdrvinfinityfb1_mmap(struct fb_info *info, struct vm_area_struct *vma);
...
static int mdrvinfinityfb1_suspend(struct platform_device *pdev,pm_message_t state);
...
//  Global variable
//--------------------------------------------------------------------------------------------------
struct clk **gop1_clks;                             /* clock define, one for palette sram clock, one for scaler fclk1 *///index=1 is scl_fclk
int num_parents_clocks_gop1                   = 0;  /* number of clocks*/
static void *sg_pG3D_fb2_vir_addr1_gop1       = 0;  /* virtual address of frame buffer */
static u_long sg_videomemorysize_gop1         = 0;  /* frame buffer size */
dma_addr_t sg_G3D_fb2_bus_addr1_gop1          = 0;  /* physical address of frame buffer */

FB_GOP_GWIN_CONFIG genGWinInfo_gop1           ={0}; /* global variable to record gwin information */
unsigned char genGWIN_gop1                    = 1;  /* global variable to record gwin is open or close *///open:1; close:0
FB_GOP_ALPHA_CONFIG genGOPALPHA_gop1          ={0}; /* global variable to record alpha blending information */
FB_GOP_COLORKEY_CONFIG genGOPCOLORKEY_gop1    ={0}; /* global variable to record color key information */

unsigned char genInv_Color_gop1               = 0;  /* global variable to record inverse color is open or close *///open:1; close:0
FB_GOP_INVCOLOR_AE_CONFIG genAE_info_gop1     ={0}; /* global variable to record AE information */
unsigned long genY_Thres_gop1                 = 0;  /* global variable to record Y Threshold */
FB_GOP_INVCOLOR_SCALER_CONFIG genSC_info_gop1 ={0}; /* global variable to record Scaler information */

FB_GOP_RESOLUTION_STRETCH_H_CONFIG genGOPGernelSettings_gop1={0}; /* global variable to record Stretch mode information */

static struct task_struct *pSWupdateInvThread_gop1=NULL;
unsigned char genTreadisError_gop1            = 0;

static struct task_struct *pSWupdateInvDebugThread_gop1=NULL;
...
FB_GOP_PaletteEntry stGOPCurrentPalInfo_GOP1[GOP_PALETTE_ENTRY_NUM]={};


static const char* KEY_DMEM_FB1_BUF="FB1_BUFFER";


//-------------------------------------------------------------------------------------------------
//  Data structure
//-------------------------------------------------------------------------------------------------
static struct fb_var_screeninfo sg_mdrvinfinityfb1_default_var =
{
...
static struct fb_fix_screeninfo sg_mdrvinfinityfb1_fix  =
{
    .id          = "FB1_gop1",            /* identification string eg "TT Builtin"*/
...
static struct fb_ops sg_mdrvinfinityfb1_ops =
{
    .owner = THIS_MODULE,
    ...
    .fb_mmap        = mdrvinfinityfb1_mmap, /* for non cached memory */
    .fb_ioctl       = mdrvinfinityfb1_ioctl,
    .fb_pan_display = mdrvinfinityfb1_pan_display,
    .fb_check_var   = mdrvinfinityfb1_check_var,
    .fb_set_par     = mdrvinfinityfb1_set_par,
};


static const struct of_device_id infinity_infinityfb1_dt_ids[] =
{
    { .compatible = "mstar,infinity-gop1" },
    { /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, infinity_infinityfb1_dt_ids);

static struct platform_driver sg_mdrvinfinityfb1_driver  =
{
    .probe   = mdrvinfinityfb1_probe,   /* initiailize */
    .remove  = mdrvinfinityfb1_remove,  /* it free(mem), release framebuffer, free irq etc. */
    .suspend = mdrvinfinityfb1_suspend,
    .resume  = mdrvinfinityfb1_resume,

    .driver  =
    {
        .name           = "mdrvinfinityfb1",
        .owner          = THIS_MODULE,
        .of_match_table = of_match_ptr(infinity_infinityfb1_dt_ids),
    },
};

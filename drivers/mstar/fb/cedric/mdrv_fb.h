...
static s32 mdrvfb2_ioctl(struct fb_info *info, u32 cmd, unsigned long arg);
static s32 mdrvfb2_mmap(struct fb_info *info, struct vm_area_struct *vma);
static s32 mdrvfb2_pan_display(struct fb_var_screeninfo *var, struct fb_info *info);
static s32 mdrvfb2_check_var(struct fb_var_screeninfo *var, struct fb_info *info);

static struct fb_var_screeninfo sg_mdrvfb2_default_var =
{
    .xres = defult_XRES,
    .yres = defult_YRES,
...
static struct fb_fix_screeninfo sg_mdrvfb2_fix  =
{
    .id = "mdrv FB",
...
static struct fb_videomode sg_mdrvfb2_default_fbmode;

static struct fb_ops sg_mdrvfb2_ops =
{
    ...
    .fb_mmap = mdrvfb2_mmap, /* for non cached memory */

    /* vma->vm_page_prot = pgprot_noncached(vma->vm_page_prot); */
    .fb_ioctl = mdrvfb2_ioctl,

    .fb_pan_display = mdrvfb2_pan_display,

    .fb_check_var = mdrvfb2_check_var,
...
#else
#define mdrvfb2_suspend    NULL
#define mdrvfb2_resume    NULL
#endif
...
/* device .name and driver .name must be the same, then it will call
   probe function */
static struct platform_driver sg_mdrvfb2_driver  =
{
    .probe  = mdrvfb2_probe,   /* initiailize */
    .remove = mdrvfb2_remove,  /* it free(mem),
                                    release framebuffer, free irq etc. */
    .suspend    = mdrvfb2_suspend,
    .resume        = mdrvfb2_resume,
...
static u64 sg_mdrvfb2_device_lcd_dmamask = 0xffffffffUL;
/* for 0x08000000 */
static struct platform_device sg_mdrvfb2_device =
{
    .name = "mdrvfb2",
    .id = 0,
    .dev =
    {
        .release = mdrvfb2_platform_release,
        .dma_mask = &sg_mdrvfb2_device_lcd_dmamask,

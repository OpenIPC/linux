...
//  Functions
//--------------------------------------------------------------------------------------------------
static int mdrvinfinityfb_ioctl(struct fb_info *info, u32 cmd, unsigned long arg);
static int mdrvinfinityfb_mmap(struct fb_info *info, struct vm_area_struct *vma);
...
static int mdrvinfinityfb_suspend(struct platform_device *pdev,pm_message_t state);
...
{
    .id          = "FB_gop0",             /* identification string eg "TT Builtin"*/
...
static struct platform_driver sg_mdrvinfinityfb_driver  =
{
    ...
    .remove  = mdrvinfinityfb_remove,  /* it free(mem), release framebuffer, free irq etc. */

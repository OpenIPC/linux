...
void SwitchSubBank(int subbank)
{
    ...
    if ((subbank>=0)&&(subbank<=2))
    {
        ...
        GOPDBG( "[GOP]%s: Switch to Subbank %d\n",__func__,subbank);
    }
    else
        GOPDBG( "[GOP]%s: Wrong Subbank %d\n",__func__,subbank);
}
...
void GOP_Set_OutFormat(int screen, int byuvoutput)
{
    ...
    GOPDBG("[GOP]%s: screen:%d, byuvoutput:%d\n",__func__, screen, byuvoutput);
...
void GOP_Set_Constant_Alpha(int screen, int bEn, int alpha)
{
    ...
    GOPDBG("[GOP]%s: screen:%d, bEn:%d, alpha:%x\n",__func__, screen, bEn, alpha);
...
void GOP_Set_Color_Key(int screen, int bEn, int u8R, int u8G, int u8B)
{
    ...
    GOPDBG("[GOP]%s: screen:%d, bEn:%d, (R=%x, G=%x, B=%x)\n",__func__, screen, bEn, u8R, u8G, u8B);
...
void GOP_Set_Palette_RIU(int screen, int u8Index, int u8A, int u8R, int u8G, int u8B)
{
    U32 GOP_Reg_Base = 0;
...
void GOP_Set_Outputmode(int Screen, int Panel_Width, int Panel_Height,U32 FB_Start_Addr, U32 Pixel_Format, U16 Bytes_Per_Pixel)
{
    U32 FB_add    = FB_Start_Addr;
    U16 FB_WIDTH  = Panel_Width;
    U16 FB_HEIGHT = Panel_Height;
...
void GOP_Set_Enable_GWIN(int screen, unsigned char bEn)
{
    unsigned long GOP_Reg_Base = 0;
...
void GOP_Get_Enable_GWIN(int screen, unsigned char *bEn)
{
    ...
    GOPDBG( "[GOP]%s: Get enable_GOP = [%d]\n",__func__,enGOP);

}

void GOP_Get_Alpha(int screen, int *alpha_type, int *alpha_val)
{
    unsigned long GOP_Reg_Base = 0;
    ...
    *alpha_val = (unsigned char)INREGMSK16(GOP_Reg_Base+REG_GOP_0A, 0x00FF);
    ...
    GOPDBG( "[GOP]%s: Alpha_type = [%s], Alpha_value = [%d]\n",__func__,PARSING_ALPHA_TYPE(*alpha_type),*alpha_val);

}

void GOP_Get_Color_Key(int screen, int *bEn, int *u8R, int *u8G, int *u8B)
{

#include "mdrv_gop0_inv_color.h"

void GOP0_invColor_init(void)
{
    unsigned long GOP0_invColor_Reg_Base = 0;
    int output_YUV_mode=0x0002;
    int complementary_color_mode=0x0008;

    // 1. GOP Set inverse color init settings

    GOP0_invColor_Reg_Base = mdrv_BASE_REG_GOP0_invColor;

    // set gop output mode YUV
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BASE_SETTINGS, output_YUV_mode, GOP0_invColor_gop_output_mode);

    // set inverse color mode to complementary color mode
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BASE_SETTINGS, complementary_color_mode, GOP0_invColor_color_mode);
...
void GOP0_invColor_Enable(unsigned char bEn)
{
    ...
    GOP0_invColor_DBG( "[GOP0_invColor]%s: Set enable_GOP = [%d]\n",__func__,bEn);

    GOP0_invColor_Reg_Base = mdrv_BASE_REG_GOP0_invColor;

    // enable/disable inverse color
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BASE_SETTINGS, bEn, GOP0_invColor_enable_mask);
...
void GOP0_invColor_Set_UpdateMode(unsigned char bMode)
{
    ...
    if(bMode)
    {
        OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BASE_SETTINGS, GOP0_invColor_update_mode, GOP0_invColor_update_mode);
    }
    else
    {
        OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BASE_SETTINGS, 0, GOP0_invColor_update_mode);
    }
...
void GOP0_invColor_Set_Y_Threshold(int AE_Y_Thres)
{
    ...
    GOP0_invColor_DBG ("[GOP0_invColor]%s: Set Y Threshold  = [%d]\n",__func__,AE_Y_Thres);

    // set AE_Y_Thres
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BASE_SETTINGS, (AE_Y_Thres<<8),GOP0_invColor_AE_Y_Threshold_mask);
...
void GOP0_invColor_Set_AE_Config(int AE_Blk_Width, int AE_Blk_Height, int AE_x_res)
{
    ...
    GOP0_invColor_DBG ("[GOP0_invColor]%s: reg AE_Blk_Width  = [%d]\n",__func__,AE_Blk_Width-1);
    GOP0_invColor_DBG ("[GOP0_invColor]%s: reg AE_Blk_Height = [%d]\n",__func__,AE_Blk_Height-1);
    GOP0_invColor_DBG ("[GOP0_invColor]%s: reg AE_Blk_x_num  = [%d]\n",__func__,AE_Blk_x_num-1);

    // set AE_Blk_Width
    OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BLK_WIDTH, AE_Blk_Width-1); // reg_val=real_size-1

    // set AE_Blk_Height
    OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BLK_HEIGHT, AE_Blk_Height-1); // reg_val=real_size-1

    // set AE_Blk_x_num
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_AE_X_BLK_NUM, (AE_Blk_x_num-1)<<8,GOP0_invColor_AE_X_BLK_NUM_mask); // reg_val=real_size-1
...
void GOP0_invColor_Set_AE_Config_Scaling(int AE_Blk_Width, int AE_Blk_Height, int AE_x_blk_num)
{
    ...
    GOP0_invColor_DBG ("[GOP0_invColor]%s: reg AE_Blk_Width  = [%d]\n",__func__,AE_Blk_Width-1);
    GOP0_invColor_DBG ("[GOP0_invColor]%s: reg AE_Blk_Height = [%d]\n",__func__,AE_Blk_Height-1);
    GOP0_invColor_DBG ("[GOP0_invColor]%s: reg AE_x_blk_num  = [%d]\n",__func__,AE_x_blk_num-1);
    ...
    // set AE_Blk_x_num
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_AE_X_BLK_NUM, (AE_x_blk_num-1)<<8,GOP0_invColor_AE_X_BLK_NUM_mask); // reg_val=real_size-1
...
void GOP0_invColor_Set_Crop_Config(int crop_x_cor, int crop_y_cor, int AE_Blk_Width, int AE_Blk_Height, int AE_x_res)
{
    ...
    GOP0_invColor_DBG ("[GOP0_invColor]%s: x_offset   = [%d]\n",__func__,x_offset);
    GOP0_invColor_DBG ("[GOP0_invColor]%s: y_offset   = [%d]\n",__func__,y_offset);
    GOP0_invColor_DBG ("[GOP0_invColor]%s: idx_offset = [%d]\n",__func__,invTable_idx_offset);

    // set x_offset
    OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_X_PIX_OFFSET, x_offset);

    // set y_offset
    OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_Y_PIX_OFFSET, y_offset);

    // set invTable_idx_offset
    OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_BLK_INDEX_OFFSET, invTable_idx_offset);

}

int GOP0_invColor_Engine_Update_Done(void)
{
    unsigned long GOP0_invColor_Reg_Update = 0;
    unsigned char bDone=0;

    GOP0_invColor_Reg_Update=mdrv_BASE_REG_GOP0_invColor_AE_Update_Done;

    if(INREGMSK16(GOP0_invColor_Reg_Update+GOP0_invColor_REG_AE_Update_Done, GOP0_invColor_AE_Update_Done_mask)!=0)
    {
        bDone=1;
        //if get ready, clear it after read
        OUTREGMSK16(GOP0_invColor_Reg_Update+GOP0_invColor_REG_AE_Update_Done, 0 , GOP0_invColor_AE_Update_Done_mask);
    }
    else
    {
        bDone=0;
...
int GOP0_invColor_CPU_Upate_InvTable(int AE_Blk_Width, int AE_Blk_Height, int AE_x_res, int AE_y_res)
{
    ...
    {
        GOP0_invColor_DBG ("[GOP0_invColor]%s: Error! blk_num too big!!! x_num=%d, y_num=%d\n",__func__,blk_origin_num_x,blk_origin_num_y);
        return FALSE;
    }
    ...
    while(!GOP0_invColor_Engine_Update_Done())
    {
        mdelay(30);
        if(retry_count>10)
        {
            GOP0_invColor_DBG ("[GOP0_invColor]%s: Error! wait no ack\n",__func__);
            return FALSE;
        }
        retry_count++;
    }
    ...
    {
        //set index
        OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_SETTINGS, temp_idx,GOP0_invColor_SRAM_ADDR_mask);

        //set read enable
        OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_SETTINGS, GOP0_invColor_SRAM_READ_enable ,GOP0_invColor_SRAM_READ_enable);

        //read inverse table
        temp_data1 = INREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_READ_DATA1);
        temp_data2 = INREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_READ_DATA2);
    ...
    for(temp_idx=0;temp_idx<reg_index_used;temp_idx++)
    {
        ...
        //write data
        OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_WRITE_DATA1, temp_data1);
        OUTREG16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_WRITE_DATA2, temp_data2);

        //set write enable
        OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_SETTINGS, GOP0_invColor_SRAM_WRITE_enable ,GOP0_invColor_SRAM_WRITE_enable);
    }
    ...
    OUTREGMSK16(GOP0_invColor_Reg_Base+GOP0_invColor_REG_SRAM_SETTINGS, GOP0_invColor_SRAM_CPU_UPDATE_DONE ,GOP0_invColor_SRAM_CPU_UPDATE_DONE);
...
{
    unsigned long GOP0_invColor_Reg_Base = 0;
    unsigned long GOP0_invColor_DebugMode_Reg_Base = 0;
    int temp_idx=0;
    int reg_index_used=0;
    int temp_data1=0;
    int temp_data2=0;
    ...
    GOP0_invColor_Reg_Base = mdrv_BASE_REG_GOP0_invColor;
    GOP0_invColor_DebugMode_Reg_Base = GET_REG16_ADDR_GOP0_invColor(BASE_REG_RIU_PA_GOP0_invColor, (0x122500/2));

    //open debug mode
    OUTREG16(GOP0_invColor_DebugMode_Reg_Base+BK_REG_GOP0_invColor(0x10), 0x7);

    //open CPU update mode
    GOP0_invColor_Set_UpdateMode(1);

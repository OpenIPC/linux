...
static int  ms_set_type_irq(struct irq_data *data, unsigned int flow_type)
{
    ...
    ms_irq = data->hwirq - GIC_HWIRQ_MS_START;
    ms_fiq = data->hwirq - GIC_HWIRQ_MS_START - GIC_SPI_MS_IRQ_NR;
...
static void ms_handle_cascade_pm_irq(unsigned int irq, struct irq_desc *desc)
{
    ...
    spin_lock(&infinity_irq_controller_lock);
    {
        for(i=0;i<PMSLEEP_IRQ_NR;i++)
        {
            ...
            {
                cascade_irq = i + (GIC_HWIRQ_MS_START+GIC_SPI_MS_IRQ_NR+GIC_SPI_MS_FIQ_NR+PMSLEEP_FIQ_NR);
                break;
            }
        }
...
static int __init infinity_init_irqchip(struct device_node *np, struct device_node *interrupt_parent)
{
    ...
    virq_base = irq_alloc_descs(-1, 0, PMSLEEP_IRQ_END, 0);
    BUG_ON(virq_base < 0);
...
IRQCHIP_DECLARE(infinity3, "mstar,pmsleep-intr", infinity_init_irqchip);

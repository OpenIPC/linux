...
S32 halPadGetVal(U32 padID, U32* mode)
{
    ...
    {
    case PAD_PM_GPIO4:
        if( INREG16(PMSLEEP_BASE_ADDR<<9 + REG_GPIO_PM_LOCK<<2) == 0xBABE )
        {
            return TRUE;
        }
        else
    ...
    case PAD_PM_IRIN:
        if( INREG16(PMSLEEP_BASE_ADDR<<9 + REG_IR_IS_GPIO<<2)&BIT4 == BIT4 )
        {
            mode = PINMUX_FOR_GPIO_MODE;
        }
        else
    ...
    default:
        for(i=0; i<sizeof(padmux_table)/sizeof(struct stPadmux); i++)
        {
            ...
            {
                if( (INREG16( (padmux_table[i].base)<<9 + (padmux_table[i].offset)<<2)&(padmux_table[i].mask)) == padmux_table[i].val )
                {
...
S32 halPadSetVal(U32 padID, U32 mode)
{
    ...
    case PAD_PM_GPIO4:
        ...
        {
            OUTREG16( PMSLEEP_BASE_ADDR<<9 + REG_GPIO_PM_LOCK<<2, 0xBABE);
        }
        return TRUE;
    ...
    case PAD_PM_IRIN:
        if(mode == PINMUX_FOR_GPIO_MODE)
        {
            U16Reg_val = INREG16( PMSLEEP_BASE_ADDR<<9 + REG_IR_IS_GPIO)<<2 );
            U16Reg_val |= BIT4;
            OUTREG16( PMSLEEP_BASE_ADDR<<9 + REG_IR_IS_GPIO<<2, U16Reg_val);
        }
        return TRUE;
    ...
    default:
        for(i=0; i<sizeof(padmux_table)/sizeof(struct stPadmux); i++)
        {
            if(padID == padmux_table[i].padID)
            {
                ...
                {
                    U16Reg_val = INREG16( (padmux_table[i].base)<<9 + (padmux_table[i].offset)<<2 );
                    U16Reg_val &= ~(padmux_table[i].mask);
                    U16Reg_val |= padmux_table[i].val;

                    OUTREG16( (padmux_table[i].base)<<9 + (padmux_table[padID+i].offset)<<2, U16Reg_val);
                    fgModeIsFind = 1;

                    //避免次优先级被修改，如TTL_LCK要单独使能，TTL其它口可以配置做其他功能使用
                    break;
                }
                else
                {
                    //如果高优先级功能复用该pin脚取消高优先级复用功能，否则不改变高优先级功能原有配置
                    U16Reg_val = INREG16( (padmux_table[i].base)<<9 + (padmux_table[i].offset)<<2 );
                    if(U16Reg_val&padmux_table[i].mask == padmux_table[i].val)
                    {
                        U16Reg_val &= (~padmux_table[i].mask);
                        OUTREG16( (padmux_table[i].base)<<9 + (padmux_table[i].offset)<<2, U16Reg_val);
                    }
                }
            }
        }

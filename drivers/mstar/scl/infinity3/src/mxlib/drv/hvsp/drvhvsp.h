...
INTERFACE void Drv_HVSP_PriMaskBufferReset(void);
INTERFACE void Drv_HVSP_Exit(unsigned char bCloseISR);
INTERFACE void Drv_HVSP_PriMaskBufferReset(void);
...
INTERFACE EN_HVSP_IP_MUX_TYPE Drv_HVSP_GetInputSrcMux(void);
INTERFACE MS_BOOL Drv_HVSP_SetRegisterForce(MS_U32 u32Reg, MS_U8 u8Val, MS_U8 u8Msk);
INTERFACE void Drv_HVSP_SetSCIQHSRAM(EN_HVSP_ID_TYPE enHVSP_ID);
INTERFACE void Drv_HVSP_SetSCIQVSRAM(EN_HVSP_ID_TYPE enHVSP_ID);
...
INTERFACE void Drv_HVSP_SCLIQ(EN_HVSP_ID_TYPE enID,EN_DRV_HVSP_IQ_TYPE enIQ);
INTERFACE void Drv_HVSP_SetPrv2CropOnOff(unsigned char bEn);
INTERFACE void Drv_HVSP_GetSCLInts(ST_HVSP_SCINTS_TYPE *sthvspints);
INTERFACE void Drv_HVSP_SetOSDConfig(EN_HVSP_ID_TYPE enID, ST_DRV_HVSP_OSD_CONFIG *stOSdCfg);
INTERFACE void Drv_HVSP_SetPriMaskConfig(ST_HVSP_PRIMASK_CONFIG *stCfg);
INTERFACE unsigned short Drv_HVSP_PriMaskGetSRAM(unsigned char u8idx, unsigned char u8idy);
INTERFACE void Drv_HVSP_SetPriMaskTrigger(EN_HVSP_PRIMASK_TYPE enType);
...
INTERFACE void Drv_HVSP_Open(EN_HVSP_ID_TYPE HVSP_IP);
INTERFACE MS_BOOL Drv_HVSP_SetCMDQTrigType(ST_HVSP_CMD_TRIG_CONFIG *stCmdTrigCfg);
INTERFACE void Drv_HVSP_SetCMDQTrigTypeByRIU(ST_HVSP_CMD_TRIG_CONFIG *stCmdTrigCfg);
...
INTERFACE void * Drv_HVSP_GetWaitQueueHead(void);
INTERFACE MS_BOOL Drv_HVSP_GetCMDQDoneStatus(EN_HVSP_POLL_ID_TYPE enPollId);

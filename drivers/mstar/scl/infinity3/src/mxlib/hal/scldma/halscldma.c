...
void Hal_SCLDMA_SetReqLen(EN_SCLDMA_CLIENT_TYPE enSCLDMA_ID, MS_U8 u8value)
{
    ...
    else if(enSCLDMA_ID == E_SCLDMA_2_FRM2_W)
    {
        W2BYTEMSK(REG_SCL_DMA1_61_L, ((MS_U16)u8value<<8), 0xFF00);
    }
...
void Hal_SCLDMA_SetReqTh(EN_SCLDMA_CLIENT_TYPE enSCLDMA_ID, MS_U8 u8value)
{
    ...
    {
        W2BYTEMSK(REG_SCL_DMA1_61_L, ((MS_U16)u8value), 0x00FF);
    }
    else if(enSCLDMA_ID == E_SCLDMA_2_IMI_W)
    {
...
void Hal_SCLDMA_SetPriThd(EN_SCLDMA_CLIENT_TYPE enSCLDMA_ID, MS_U8 u8value)
{
    ...
    {
        W2BYTEMSK(REG_SCL_DMA1_60_L, ((MS_U16)u8value<<8), 0xFF00);
    }
    else if(enSCLDMA_ID == E_SCLDMA_2_IMI_W)
    {
...
void Hal_SCLDMA_HKForceAuto(void)
{
    ...
    W2BYTEMSK(REG_SCL0_03_L, BIT11|BIT10|BIT9|BIT8|BIT2|BIT1|BIT0, BIT11|BIT10|BIT9|BIT8|BIT2|BIT1|BIT0);
}
void Hal_SCLDMA_SC3DbHWAuto(MS_BOOL bEn)
{
    W2BYTEMSK(REG_SCL0_06_L, bEn ? BIT4 :0, BIT4);
}
void Hal_SCLDMA_DbHWAuto(MS_BOOL bEn)
{
    W2BYTEMSK(REG_SCL0_06_L, bEn ? BIT3 :0, BIT3);
}
void Hal_SCLDMA_HKHWAutoMd(MS_BOOL bDbmode)
{
    W2BYTEMSK(REG_SCL0_06_L, bDbmode ? BIT6 : 0, BIT6);
}
void Hal_SCLDMA_HKHWAuto(MS_BOOL bEn)
{
    W2BYTEMSK(REG_SCL0_06_L, bEn ? BIT5 :0, BIT5);
}
void Hal_SCLDMA_HWInit(void)
{
    ...
    Hal_SCLDMA_SetReqLen(E_SCLDMA_2_FRM2_W,0x10);
    Hal_SCLDMA_SetReqTh(E_SCLDMA_2_FRM2_W,0x12);
...
void Hal_SCLDMA_SetSC2HandshakeForce(EN_SCLDMA_RW_MODE_TYPE enRWMode, MS_BOOL bEn)
{
    ...
    else if(enRWMode == E_SCLDMA_FRM2_W)
    {
        W2BYTEMSK(REG_SCL0_02_L, bEn ? 0 : BIT2, BIT2);
    }
}
...
{
    if((R2BYTE(REG_SCL0_02_L)&0x7) == 0x7 && bEn==0)
    {
        W2BYTEMSK(REG_SCL0_02_L, BIT8, BIT8);
    }
    else if((R2BYTE(REG_SCL0_02_L)&0xF) != 0x7 && bEn)
    {
        W2BYTEMSK(REG_SCL0_02_L, 0, BIT8);
    }
}
...
void Hal_SCLDMA_SetCheckFrmEndSignal(EN_SCLDMA_CLIENT_TYPE enSCLDMA_ID, MS_BOOL bEn)
{
    ...
    {
        W2BYTEMSK(REG_SCL_DMA2_50_L, bEn ? BIT14 : 0, BIT14);
    }
    else if(enSCLDMA_ID == E_SCLDMA_2_IMI_W)
    {
...
void Hal_SCLDMA_SetSC2DMAEn(EN_SCLDMA_RW_MODE_TYPE enRWMode, MS_BOOL bEn)
{
    ...
    {
        W2BYTEMSK(REG_SCL_DMA1_04_L, bEn ? BIT15 : 0, BIT15);
    }
    else if(enRWMode == E_SCLDMA_IMI_W)
    {
...
MS_U16 Hal_SCLDMA_GetOutputHsize(EN_SCLDMA_CLIENT_TYPE enClientType)
{
    ...
    switch(enClientType)
        ...
        case E_SCLDMA_2_FRM2_W:
            u16def = R2BYTE(REG_SCL_DMA1_62_L);
            break;

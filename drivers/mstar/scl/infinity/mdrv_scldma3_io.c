...
#define MDRV_MS_SCLDMA_NAME            "mscldma3"
#define MAX_FILE_HANDLE_SUPPRT         64
#define MDRV_NAME_SCLDMA               "mscldma3"
#define MDRV_MAJOR_SCLDMA              0xea
#define MDRV_MINOR_SCLDMA              0x06
...
int mdrv_ms_scldma3_open(struct inode *inode, struct file *filp);
int mdrv_ms_scldma3_release(struct inode *inode, struct file *filp);
long mdrv_ms_scldma3_ioctl(struct file *filp, unsigned int u32Cmd, unsigned long u32Arg);
static int mdrv_ms_scldma3_probe(struct platform_device *pdev);
static int mdrv_ms_scldma3_remove(struct platform_device *pdev);
static int mdrv_ms_scldma3_suspend(struct platform_device *dev, pm_message_t state);
static int mdrv_ms_scldma3_resume(struct platform_device *dev);
static unsigned int mdrv_ms_scldma3_poll(struct file *filp, struct poll_table_struct *wait);
...
static ST_DEV_SCLDMA _dev_ms_scldma3 =
{
    ...
    .fops =
    {
        .open = mdrv_ms_scldma3_open,
        .release = mdrv_ms_scldma3_release,
        .unlocked_ioctl = mdrv_ms_scldma3_ioctl,
        .poll = mdrv_ms_scldma3_poll,
    }
};

static struct class * m_scldma3_class = NULL;
static char * scldma3_classname = "m_scldma3_class";


static const struct of_device_id ms_scldma3_of_match_table[] =
{
    { .compatible = "mstar,scldma3" },
    {}
};

static struct platform_driver st_ms_scldma3_driver =
{
    .probe      = mdrv_ms_scldma3_probe,
    .remove     = mdrv_ms_scldma3_remove,
    .suspend    = mdrv_ms_scldma3_suspend,
    .resume     = mdrv_ms_scldma3_resume,
    .driver =
    {
        .name   = MDRV_NAME_SCLDMA,
        .owner  = THIS_MODULE,
        .of_match_table = of_match_ptr(ms_scldma3_of_match_table),
    },
};

static u64 ms_scldma3_dma_mask = 0xffffffffUL;

static struct platform_device st_ms_scldma3_device =
{
    .name = "mscldma3",
    .id = 0,
    .dev =
    {
        .dma_mask = &ms_scldma3_dma_mask,
...
static ssize_t check_frmR_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t n)
{
	if(NULL!=buf)
	{
        ...
        if((int)*str == 48)  //input 0  echo 0 >ptgen_call
        {
            ...
            MDrv_SCLDMA_ResetTrigCountByClient(E_MDRV_SCLDMA_ID_3,E_MDRV_SCLDMA_MEM_FRM,1);
        }
        else if((int)*str == 50) //input 2
        {
            SCL_ERR( "dma count FORCE CLOSE OPEN %d\n",(int)*str);
            MDrv_SCLDMA_SetForceCloseDMAClient(E_MDRV_SCLDMA_ID_3,E_MDRV_SCLDMA_MEM_FRM,1,1);
        }
        else if((int)*str == 51) //input 3
        {
            SCL_ERR( "dma count FORCE CLOSE OVER %d\n",(int)*str);
            MDrv_SCLDMA_SetForceCloseDMAClient(E_MDRV_SCLDMA_ID_3,E_MDRV_SCLDMA_MEM_FRM,1,0);
        }
        return n;
    }
    return 0;
...
static DEVICE_ATTR(ckfrmR,0600, check_frmR_show, check_frmR_store);
...
static ssize_t check_frmW_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t n)
{
	if(NULL!=buf)
	{
        ...
        {
            SCL_ERR( "dma count reset %d\n",(int)*str);
            MDrv_SCLDMA_ResetTrigCountByClient(E_MDRV_SCLDMA_ID_3,E_MDRV_SCLDMA_MEM_FRM,0);
        }
        else if((int)*str == 50) //input 2
        {
            SCL_ERR( "dma count FORCE CLOSE OPEN %d\n",(int)*str);
            MDrv_SCLDMA_SetForceCloseDMAClient(E_MDRV_SCLDMA_ID_3,E_MDRV_SCLDMA_MEM_FRM,0,1);
        }
        else if((int)*str == 51) //input 3
        {
            SCL_ERR( "dma count FORCE CLOSE OVER %d\n",(int)*str);
            MDrv_SCLDMA_SetForceCloseDMAClient(E_MDRV_SCLDMA_ID_3,E_MDRV_SCLDMA_MEM_FRM,0,0);
        }
        return n;
    }
    return 0;
...
static int _ms_scldma3_multiinstSet(EN_MDRV_MULTI_INST_CMD_TYPE enType, void *stCfg ,void *privatedata)
{
    EN_MDRV_MULTI_INST_STATUS_TYPE enMultiInstRet;
    int ret = 0;
    enMultiInstRet = MDrv_MultiInst_Entry_FlashData(E_MDRV_MULTI_INST_ENTRY_ID_SCLDMA3,privatedata ,enType,stCfg);
...
int _mdrv_ms_scldma3_io_get_information_config(struct file *filp, unsigned long arg)
{
...
int _mdrv_ms_scldma3_io_set_in_buffer_config(struct file *filp, unsigned long arg)
{
    ...
    {
        if(!MDrv_SCLDMA_SetDMAReadClientConfig(E_MDRV_SCLDMA_ID_3,  &stDMABufferCfg))
        {
            ret = -EFAULT;
        }
        else
...
int _mdrv_ms_scldma3_io_set_in_trigger_config(struct file *filp, unsigned long arg)
{
    ...
    stDrvTrigCfg.stclk =& (_dev_ms_scldma3.stclk);
    if(_ms_scldma3_multiinstSet(E_MDRV_MULTI_INST_CMD_SCLDMA_IN_TRIGGER_CONFIG, (void *)&stDrvTrigCfg, filp->private_data))
    {
        ret = -EFAULT;
    }
    else
    {
        if(!MDrv_SCLDMA_SetDMAReadClientTrigger(E_MDRV_SCLDMA_ID_3,  &stDrvTrigCfg))
        {
            ret = -EFAULT;
        }
        else
...
int _mdrv_ms_scldma3_io_set_out_buffer_config(struct file *filp, unsigned long arg)
{
    ST_IOCTL_SCLDMA_BUFFER_CONFIG stIODMABufferCfg;
    ST_MDRV_SCLDMA_BUFFER_CONFIG stDMABufferCfg;
    int ret = 0;
    ...
    if(_ms_scldma3_multiinstSet(E_MDRV_MULTI_INST_CMD_SCLDMA_OUT_BUFFER_CONFIG, (void *)&stDMABufferCfg, filp->private_data))
    {
        ret = -EFAULT;
    }
    else
    {
        if(!MDrv_SCLDMA_SetDMAWriteClientConfig(E_MDRV_SCLDMA_ID_3,  &stDMABufferCfg))
        {
            ret = -EFAULT;
        }
        else
...
int _mdrv_ms_scldma3_io_set_out_trigger_config(struct file *filp, unsigned long arg)
{
    ST_MDRV_SCLDMA_TRIGGER_CONFIG stDrvTrigCfg;
    ST_IOCTL_SCLDMA_TRIGGER_CONFIG stIOTrigCfg;
    int ret = 0;
    ...
    stDrvTrigCfg.stclk=&(_dev_ms_scldma3.stclk);
    if(_ms_scldma3_multiinstSet(E_MDRV_MULTI_INST_CMD_SCLDMA_OUT_TRIGGER_CONFIG, (void *)&stDrvTrigCfg, filp->private_data))
    {
        ret = -EFAULT;
    }
    else
    {
        if(!MDrv_SCLDMA_SetDMAWriteClientTrigger(E_MDRV_SCLDMA_ID_3,  &stDrvTrigCfg))
        {
            ret = -EFAULT;
        }
        else
...
int _mdrv_ms_scldma3_io_get_in_active_buffer_config(struct file *filp, unsigned long arg)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if(!MDrv_SCLDMA_GetDMAReadBufferActiveIdx(E_MDRV_SCLDMA_ID_3,  &stActiveCfg))
                {
                    ret = -EFAULT;
                }
                else
...
int _mdrv_ms_scldma3_io_get_out_active_buffer_config(struct file *filp, unsigned long arg)
{
    ST_MDRV_SCLDMA_ACTIVE_BUFFER_CONFIG stActiveCfg;
    ST_IOCTL_SCLDMA_ACTIVE_BUFFER_CONFIG stIOActiveCfg;
    EN_MDRV_MULTI_INST_STATUS_TYPE enMultiInstRet;
    ...
    else
    {
        ...
        else
        {
            ...
            {
                if(!MDrv_SCLDMA_GetDMAWriteBufferAcitveIdx(E_MDRV_SCLDMA_ID_3,  &stActiveCfg))
                {
                    ret = -EFAULT;
                }
                else
...
int _mdrv_ms_scldma3_io_buffer_queue_handle_config(struct file *filp, unsigned long arg)
{
    ...
    {
        if(!MDrv_SCLDMA_BufferQueueHandle(E_MDRV_SCLDMA_ID_3,  &stBufferQCfg))
        {
            ret = 0;
        }
        else
...
int _mdrv_ms_scldma3_io_get_private_id_config(struct file *filp, unsigned long arg)
{
    ST_IOCTL_SCLDMA_PRIVATE_ID_CONFIG stCfg;

    if(!MDrv_MultiInst_Entry_GetPirvateId(E_MDRV_MULTI_INST_ENTRY_ID_SCLDMA3, filp->private_data, &stCfg.s32Id))
    {
        return -EFAULT;
    }
...
int _mdrv_ms_scldma3_io_set_lock_config(struct file *filp, unsigned long arg)
{
...
int _mdrv_ms_scldma3_io_set_unlock_config(struct file *filp, unsigned long arg)
{
    ST_IOCTL_SCLDMA_LOCK_CONFIG stCfg;
    ST_MDRV_MULTI_INST_LOCK_CONFIG stMultiInstLockCfg;
    ST_MDRV_SCLDMA_VERSIONCHK_CONFIG stVersion;
...
int _mdrv_ms_scldma3_io_get_version(struct file *filp, unsigned long arg)
{
    int ret = 0;
...
long mdrv_ms_scldma3_ioctl(struct file *filp, unsigned int u32Cmd, unsigned long u32Arg)
{
    ...
    if(_dev_ms_scldma3.refCnt <= 0)
    {
        SCL_ERR( "[SCLDMA3] SCLDMA3IO_IOCTL refCnt =%d!!! \n", _dev_ms_scldma3.refCnt);
        return -EFAULT;
    }
    /* check u32Cmd valid */
    if(IOCTL_SCLDMA_MAGIC == _IOC_TYPE(u32Cmd))
    {
        ...
        {
            SCL_ERR( "[SCLDMA3] IOCtl NR Error!!! (Cmd=%x)\n",u32Cmd);
            return -ENOTTY;
        }
    }
    else
    {
        SCL_ERR( "[SCLDMA3] IOCtl MAGIC Error!!! (Cmd=%x)\n",u32Cmd);
        return -ENOTTY;
    }
    ...
    SCL_DBG(SCL_DBG_LV_IOCTL()&EN_DBGMG_IOCTLEVEL_SC3, "[SCLDMA1] IOCTL_NUM:: == %s == \n", (CMD_PARSING(u32Cmd)));

    switch(u32Cmd)
    {
    case IOCTL_SCLDMA_SET_IN_BUFFER_CONFIG:
        retval = _mdrv_ms_scldma3_io_set_in_buffer_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_SET_IN_TRIGGER_CONFIG:
        retval = _mdrv_ms_scldma3_io_set_in_trigger_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_SET_OUT_BUFFER_CONFIG:
        retval = _mdrv_ms_scldma3_io_set_out_buffer_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_SET_OUT_TRIGGER_CONFIG:
        retval = _mdrv_ms_scldma3_io_set_out_trigger_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_GET_IN_ACTIVE_BUFFER_CONFIG:
        retval = _mdrv_ms_scldma3_io_get_in_active_buffer_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_GET_OUT_ACTIVE_BUFFER_CONFIG:
        retval = _mdrv_ms_scldma3_io_get_out_active_buffer_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_GET_PRIVATE_ID_CONFIG:
        retval = _mdrv_ms_scldma3_io_get_private_id_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_SET_LOCK_CONFIG:
        retval = _mdrv_ms_scldma3_io_set_lock_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_SET_UNLOCK_CONFIG:
        retval = _mdrv_ms_scldma3_io_set_unlock_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_BUFFER_QUEUE_HANDLE_CONFIG:
        retval = _mdrv_ms_scldma3_io_buffer_queue_handle_config(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_GET_VERSION_CONFIG:
        retval = _mdrv_ms_scldma3_io_get_version(filp, u32Arg);
        break;

    case IOCTL_SCLDMA_GET_INFORMATION_CONFIG:
        retval = _mdrv_ms_scldma3_io_get_information_config(filp, u32Arg);
        break;

    default:  /* redundant, as cmd was checked against MAXNR */
        SCL_ERR( "[SCLDMA3] ERROR IOCtl number %x\n ",u32Cmd);
        retval = -ENOTTY;
        break;
    }
...
static unsigned int mdrv_ms_scldma3_poll(struct file *filp, struct poll_table_struct *pstPollQueue)
{
    ST_MDRV_SCLDMA_BUFFER_DONE_CONFIG stDoneCfg;
    unsigned char bFRMR_Done = 0;
    unsigned char bFRMW_Done = 0;
    unsigned int ret = 0;
    EN_MDRV_MULTI_INST_STATUS_TYPE enMultiInstRet;
    wait_queue_head_t *pWaitQueueHead = NULL;

    enMultiInstRet = MDrv_MultiInst_Etnry_IsFree(E_MDRV_MULTI_INST_ENTRY_ID_SCLDMA3, filp->private_data);
    SCL_DBG(SCL_DBG_LV_IOCTL()&EN_DBGMG_IOCTLEVEL_SC3, "[SCLDMA3] %s ret=%x\n",__FUNCTION__,ret);
    if(enMultiInstRet == E_MDRV_MULTI_INST_STATUS_SUCCESS)
    {
        ...
        {
            bFRMW_Done = stDoneCfg.bDone ? 1: 0;
        }
        else
        {
            bFRMW_Done = 2;
...
static int mdrv_ms_scldma3_probe(struct platform_device *pdev)
{
    ...
    //module
    if(_dev_ms_scldma3.s32Major)
    {
        dev = MKDEV(_dev_ms_scldma3.s32Major, _dev_ms_scldma3.s32Minor);
        s32Ret = register_chrdev_region(dev, MDRV_MS_SCLDMA_DEVICE_COUNT, MDRV_MS_SCLDMA_NAME);
    }
    else
    {
        s32Ret = alloc_chrdev_region(&dev, _dev_ms_scldma3.s32Minor, MDRV_MS_SCLDMA_DEVICE_COUNT, MDRV_MS_SCLDMA_NAME);
        _dev_ms_scldma3.s32Major = MAJOR(dev);
    }

    if (0 > s32Ret)
    {
        SCL_ERR( "[SCLDMA3] Unable to get major %d\n", _dev_ms_scldma3.s32Major);
        return s32Ret;
    }

    cdev_init(&_dev_ms_scldma3.cdev, &_dev_ms_scldma3.fops);
    if (0 != (s32Ret= cdev_add(&_dev_ms_scldma3.cdev, dev, MDRV_MS_SCLDMA_DEVICE_COUNT)))
    {
        SCL_ERR( "[SCLDMA3] Unable add a character device\n");
        unregister_chrdev_region(dev, MDRV_MS_SCLDMA_DEVICE_COUNT);
        return s32Ret;
    }

    m_scldma3_class = msys_get_sysfs_class();
    if(!m_scldma3_class)
    {
        m_scldma3_class = class_create(THIS_MODULE, scldma3_classname);
    }
    if(IS_ERR(m_scldma3_class))
    {
        printk(KERN_WARNING"Failed at class_create().Please exec [mknod] before operate the device/n");
    }
    else
    {
        _dev_ms_scldma3.devicenode =device_create(m_scldma3_class, NULL, dev,NULL, "mscldma3");
        _dev_ms_scldma3.devicenode->dma_mask=&ms_scldma3_dma_mask;
        _dev_ms_scldma3.devicenode->coherent_dma_mask=ms_scldma3_dma_mask;
    }
    ...
    st_ms_scldma3_device.dev.of_node = pdev->dev.of_node;
    _dev_ms_scldma3.stclk.idclk = of_clk_get(st_ms_scldma3_device.dev.of_node,0);
    _dev_ms_scldma3.stclk.fclk1 = of_clk_get(st_ms_scldma3_device.dev.of_node,1);
    _dev_ms_scldma3.stclk.fclk2 = of_clk_get(st_ms_scldma3_device.dev.of_node,2);
    _dev_ms_scldma3.stclk.odclk = of_clk_get(st_ms_scldma3_device.dev.of_node,3);
    if (device_create_file(_dev_ms_scldma3.devicenode, &dev_attr_ckfrmW)!= 0)
    {
      dev_err(_dev_ms_scldma3.devicenode,
       "Failed to create ckfrm sysfs files\n");
    }
    if (device_create_file(_dev_ms_scldma3.devicenode, &dev_attr_ckfrmR)!= 0)
    {
      dev_err(_dev_ms_scldma3.devicenode,
       "Failed to create cksnp sysfs files\n");
    }
    MDrv_MultiInst_Entry_Init_Variable(E_MDRV_MULTI_INST_ENTRY_ID_SCLDMA3);

    MDrv_MultiInst_Lock_Init(E_MDRV_MULTI_INST_LOCK_ID_SC_3);
    gbProbeAlready |= EN_DBG_SCLDMA3_CONFIG;
    return 0;
}
...
static int mdrv_ms_scldma3_remove(struct platform_device *pdev)
{
    ...
    unregister_chrdev_region(MKDEV(_dev_ms_scldma3.s32Major, _dev_ms_scldma3.s32Minor), MDRV_MS_SCLDMA_DEVICE_COUNT);
    return 0;
}
#else
static int mdrv_ms_scldma3_probe(struct platform_device *pdev)
{
    ...
    if (device_create_file(&pdev->dev, &dev_attr_ckfrmW)!= 0)
    {
      dev_err(&pdev->dev,
       "Failed to create ckfrm sysfs files\n");
    }
    if (device_create_file(&pdev->dev, &dev_attr_ckfrmR)!= 0)
    {
      dev_err(&pdev->dev,
       "Failed to create cksnp sysfs files\n");
    }
...
static int mdrv_ms_scldma3_suspend(struct platform_device *dev, pm_message_t state)
{
    ...
    if(MDrv_SCLDMA_Suspend(E_MDRV_SCLDMA_ID_3))
    {
        MDrv_SCLDMA_ClkClose(&(_dev_ms_scldma3.stclk));
        ret = 0;
    }
...
static int mdrv_ms_scldma3_resume(struct platform_device *dev)
{
    ...
    if(MDrv_SCLDMA_Resume(E_MDRV_SCLDMA_ID_3))
    {
...
int mdrv_ms_scldma3_open(struct inode *inode, struct file *filp)
{
    ...
    SCL_ASSERT(_dev_ms_scldma3.refCnt>=0);

    if(filp->private_data == NULL)
    {
        if(MDrv_MultiInst_Entry_Alloc(E_MDRV_MULTI_INST_ENTRY_ID_SCLDMA3, &filp->private_data) == 0)
        {
            ret =  -EFAULT;
        }
...
int mdrv_ms_scldma3_release(struct inode *inode, struct file *filp)
{
    ...
    MDrv_MultiInst_Entry_Free(E_MDRV_MULTI_INST_ENTRY_ID_SCLDMA3, filp->private_data);

    MDrv_MultiInst_Lock_Free(E_MDRV_MULTI_INST_LOCK_ID_SC_3, NULL);

    _dev_ms_scldma3.refCnt--;
    SCL_ASSERT(_dev_ms_scldma3.refCnt>=0);
    if(_dev_ms_scldma3.refCnt==0)
    {
        MDrv_SCLDMA_Release(E_MDRV_SCLDMA_ID_3,&(_dev_ms_scldma3.stclk));
    }
    //free_irq(INT_IRQ_SCLDMA3W, MDrv_SCLDMA3W_isr);
    return 0;
}
...
//-------------------------------------------------------------------------------------------------
#if CONFIG_OF
int _mdrv_ms_scldma3_init(void)
{
    int ret = 0;
    SCL_DBG(SCL_DBG_LV_MDRV_IO(), "[SCLDMA3] %s:%d\n",__FUNCTION__,__LINE__);
    ret = platform_driver_register(&st_ms_scldma3_driver);
    if (!ret)
    {
        SCL_DBG(SCL_DBG_LV_MDRV_IO(), "[SCLDMA3] platform_driver_register success\n");
        if(gbProbeAlready&EN_DBG_SCLDMA3_CONFIG)
        {
            SCL_DBG(SCL_DBG_LV_MDRV_IO(), "[SCLDMA3] Probe success\n");
        }
        else
        {
            SCL_ERR( "[SCLDMA3] Probe Fail\n");
        }
    ...
    else
    {
        ...
        platform_driver_unregister(&st_ms_scldma3_driver);
    }


    return ret;
}
void _mdrv_ms_scldma3_exit(void)
{
...
int _mdrv_ms_scldma3_init(void)
{
    ...
    {
        device_create(m_scldma3_class, NULL, dev,NULL, "mscldma3");
    }

    /* initial the whole SCLDMA3 Driver */
    ret = platform_driver_register(&st_ms_scldma3_driver);

    if (!ret)
    {
        ret = platform_device_register(&st_ms_scldma3_device);
...
#endif
module_init(_mdrv_ms_scldma3_init);
module_exit(_mdrv_ms_scldma3_exit);

MODULE_AUTHOR("MSTAR");
MODULE_DESCRIPTION("ms scldma3 ioctrl driver");
MODULE_LICENSE("GPL");

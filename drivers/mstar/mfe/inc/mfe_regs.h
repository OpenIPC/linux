...
#define ADDRESS_LO_BITS     (16)
#define ADDRESS_HI_BITS     (MFE_MIAW-5-16)
#define OUTBUFF_LO_BITS     (16)
#define OUTBUFF_HI_BITS     (MFE_MIAW-16)
#define IMIBUFF_LO_BITS     (16)
#define IMIBUFF_HI_BITS     (MFE_MIAW-16)     // low-bandwidth imi buffer is using 8-byte unit.

#define PIC_AVGQP_LO_BITS   (16)
#define PIC_AVGQP_HI_BITS   (24-16)
#define BIT_COUNT_LO_BITS   (16)
#define BIT_COUNT_HI_BITS   (24-16)
...
typedef struct _mfe_regs_ {
    ...
    union {
        struct {
        ushort reg01_g_pic_width:12;    // picture width
        ushort reg01_g_ver_minor_idx:4;
        };
        ushort reg01;
    };
    union {
        struct {
        ushort reg02_g_pic_height:12;   // picture height
        ushort reg02_g_ver_major_idx:4;
        };
        ushort reg02;
    };
    union {
    ...
    union {
        struct {
        ushort reg06_g_cur_y_addr_lo:ADDRESS_LO_BITS;   // current luma base address
        };
        ushort reg06;
    };
    union {
        struct {
        ushort reg07_g_cur_y_addr_hi:ADDRESS_HI_BITS;   // current luma base address
        };
        ushort reg07;
    };
    union {
        struct {
        ushort reg08_g_cur_c_addr_lo:ADDRESS_LO_BITS;   // current chroma base address
        };
        ushort reg08;
    };
    union {
        struct {
        ushort reg09_g_cur_c_addr_hi:ADDRESS_HI_BITS;   // current chroma base address
        };
        ushort reg09;
    };
    union {
        struct {
        ushort reg0a_g_ref_y_addr0_lo:ADDRESS_LO_BITS;  // reference luma base address0
        };
        ushort reg0a;
    };
    union {
        struct {
        ushort reg0b_g_ref_y_addr0_hi:ADDRESS_HI_BITS;  // reference luma base address0
        };
        ushort reg0b;
    };
    union {
        struct {
        ushort reg0c_g_ref_y_addr1_lo:ADDRESS_LO_BITS;  // reference luma base address1
        };
        ushort reg0c;
    };
    union {
    struct {
        ushort reg0d_g_ref_y_addr1_hi:ADDRESS_HI_BITS;  // reference luma base address0
        };
        ushort reg0d;
    };
    union {
        struct {
        ushort reg0e_g_ref_c_addr0_lo:ADDRESS_LO_BITS;  // reference chroma base address0
        };
        ushort reg0e;
    };
    union {
        struct {
        ushort reg0f_g_ref_c_addr0_hi:ADDRESS_HI_BITS;  // reference chroma base address0
        };
        ushort reg0f;
    };
    union {
        struct {
        ushort reg10_g_ref_c_addr1_lo:ADDRESS_LO_BITS;  // reference chroma base address1
        };
        ushort reg10;
    };
    union {
        struct {
        ushort reg11_g_ref_c_addr1_hi:ADDRESS_HI_BITS;  // reference chroma base address1
        };
        ushort reg11;
    };
    union {
        struct {
        ushort reg12_g_rec_y_addr_lo:ADDRESS_LO_BITS;   // reconstructed luma base address
        };
        ushort reg12;
    };
    union {
        struct {
        ushort reg13_g_rec_y_addr_hi:ADDRESS_HI_BITS;   // reconstructed luma base address
        };
        ushort reg13;
    };
    union {
        struct {
        ushort reg14_g_rec_c_addr_lo:ADDRESS_LO_BITS;   // reconstructed chroma base address
        };
        ushort reg14;
    };
    union {
        struct {
        ushort reg15_g_rec_c_addr_hi:ADDRESS_HI_BITS;   // reconstructed chroma base address
        };
        ushort reg15;
    };
    union {
    ...
    union {
        ...
        ushort reg1a_reserved:1;
        ushort reg1a_g_mb_pitch_en:1;
        };
        ushort reg1a;
    };
    union {
    ...
    union {
        struct {
        ushort reg28_s_mbr_last_frm_avg_qp_lo:PIC_AVGQP_LO_BITS;    // last frame average qp (status register)
        };
        ushort reg28;
    };
    union {
        struct {
        ushort reg29_s_mbr_last_frm_avg_qp_hi:PIC_AVGQP_HI_BITS;    // last frame average qp (status register)
    ...
    union {
        ...
        ushort reg2c_g_mb_pitch:8;          // mb pitch (x-direction)
        };
        ushort reg2c;
    };
    // TXIP control & debug
    ...
    union {
        struct {
        ushort reg35_s_ecdb_mbx:9;          // ecdb mbx
        };
        ushort reg35;
    };
    union {
        struct {
        ushort reg36_s_ecdb_mby:9;          // ecdb mby
        };
        ushort reg36;
    };
    // MDC
    ...
    union {
        struct {
        ushort reg3c_s_bspobuf_lo:OUTBUFF_LO_BITS;  // bsp obuf start address
        };
        ushort reg3c;
    };
    union {
        struct {
        ushort reg3d_s_bspobuf_hi:OUTBUFF_HI_BITS;  // bsp obuf address high
        ushort reg3d_reserved:(16-OUTBUFF_HI_BITS-2);
    ...
    union {
        struct {
        ushort reg40_s_mvobuf_saddr_lo:OUTBUFF_LO_BITS; // mv obuf start address
        };
        ushort reg40;
    };
    union {
        struct {
        ushort reg41_s_mvobuf_saddr_hi:OUTBUFF_HI_BITS; // mv obuf start address
        };
        ushort reg41;
    };
    union {
        struct {
        ushort reg42_s_bsp_bit_cnt_lo:BIT_COUNT_LO_BITS;    // encoded bit count (one frame)
        };
        ushort reg42;
    };
    union {
        struct {
        ushort reg43_s_bsp_bit_cnt_hi:BIT_COUNT_HI_BITS;    // encoded bit count (one frame)
        };
        ushort reg43;
    };
    union {
        struct {
        ushort reg44_s_bspobuf_wptr_lo:OUTBUFF_LO_BITS; // bspobuf write pointer (8 byte unit)
        };
        ushort reg44;
    };
    union {
        struct {
        ushort reg45_s_bspobuf_wptr_hi:OUTBUFF_HI_BITS; // bspobuf write pointer (8 byte unit)
        };
        ushort reg45;
    };
    // FDC
    ...
    union {
        struct {
        ushort reg4c_s_gn_saddr_lo:OUTBUFF_LO_BITS; // gn base adr low
        };
        ushort reg4c;
    };
    union {
        struct {
        ushort reg4d_s_gn_saddr_hi:OUTBUFF_HI_BITS; // gn base adr high
#if (16-OUTBUFF_HI_BITS-1>0)
        ushort reg4d_reserved:16-OUTBUFF_HI_BITS-1;
#endif
    ...
    union {
        struct {
        ushort reg4e_s_gn_mvibuf_saddr_lo:OUTBUFF_LO_BITS;               // mv ibuf start address low
        };
        ushort reg4e;
    };
    union {
        struct {
        ushort reg4f_s_gn_mvibuf_saddr_hi:OUTBUFF_HI_BITS; // mv ibuf start address high
        };
        ushort reg4f;
    };
    union {
    ...
    union {
        struct {
        ushort reg58_s_marb_ubound_0_lo:OUTBUFF_LO_BITS;    // MIU protect for MPEG4 BSP obuf
        };
        ushort reg58;
    };
    union {
        struct {
        ushort reg59_s_marb_ubound_0_hi:OUTBUFF_HI_BITS;    // MIU protect for MPEG4 BSP obuf
        ushort reg59_reserved:(16-OUTBUFF_HI_BITS);
        };
        ushort reg59;
    };
    union {
        struct {
        ushort reg5a_s_marb_lbound_0_lo:OUTBUFF_LO_BITS;    // MIU protect for MPEG4 BSP obuf
        };
        ushort reg5a;
    };
    union {
        struct {
        ushort reg5b_s_marb_lbound_0_hi:OUTBUFF_HI_BITS;    // MIU protect for MPEG4 BSP obuf
    ...
    union {
        struct {
        ushort reg5c_s_marb_ubound_1_lo:OUTBUFF_LO_BITS;    // MIU Upper bound protect for MPEG4 MC obuf  rec
        };
        ushort reg5c;
    };
    union {
        struct {
        ushort reg5d_s_marb_ubound_1_hi:OUTBUFF_HI_BITS;    // MIU Upper bound protect for MPEG4 MC obuf rec
        };
        ushort reg5d;
    };
    union {
        struct {
        ushort reg5e_s_marb_lbound_1_lo:OUTBUFF_LO_BITS;    // MIU Lower bound protect for MPEG4 MC obuf
        };
        ushort reg5e;
    };
    union {
        struct {
        ushort reg5f_s_marb_lbound_1_hi:OUTBUFF_HI_BITS;    // MIU Lower bound protect for MPEG4 MC obuf
    ...
    union {
        struct {
        ushort reg60_s_marb_ubound_2_lo:OUTBUFF_LO_BITS;    // MIU protect for MPEG4 MV obuf
        };
        ushort reg60;
    };
    union {
        struct {
        ushort reg61_s_marb_ubound_2_hi:OUTBUFF_HI_BITS;    // MIU protect for MPEG4 MV obuf
        };
        ushort reg61;
    };
    union {
        struct {
        ushort reg62_s_marb_lbound_2_lo:OUTBUFF_LO_BITS;    // MIU protect for MPEG4 MV obuf
        };
        ushort reg62;
    };
    union {
        struct {
        ushort reg63_s_marb_lbound_2_hi:OUTBUFF_HI_BITS;    // MIU protect for MPEG4 MV obuf
    ...
    union {
        struct {
        ushort reg64_s_marb_ubound_3_lo:OUTBUFF_LO_BITS;    // MIU protect for MPEG4 GN
        };
        ushort reg64;
    };
    union {
        struct {
        ushort reg65_s_marb_ubound_3_hi:OUTBUFF_HI_BITS;    // MIU protect for MPEG4 GN
        };
        ushort reg65;
    };
    union {
        struct {
        ushort reg66_s_marb_lbound_3_lo:OUTBUFF_LO_BITS;    // MIU protect for MPEG4 GN
        };
        ushort reg66;
    };
    union {
        struct {
        ushort reg67_s_marb_lbound_3_hi:OUTBUFF_HI_BITS;    // MIU protect for MPEG4 GN
    ...
    union {
        struct {
        ushort reg6b_s_marb_imi_saddr_lo:IMIBUFF_LO_BITS;   // imi base address for low bandwidth mode (in 8-byte unit)
        };
        ushort reg6b;
    };
    union {
        struct {
        ushort reg6c_s_marb_imi_saddr_hi:IMIBUFF_HI_BITS;
#if ((16-IMIBUFF_HI_BITS-1-2)>0)
        ushort reg6c_reserved:16-IMIBUFF_HI_BITS-1-2;
#endif
    ...
    union {
        struct {
        ushort reg71_g_debug_trig_mbx:9;        // debug trigger mbx
        };
        ushort reg71;
    };
    union {
        struct {
        ushort reg72_g_debug_trig_mby:9;        // debug trigger mby
        };
        ushort reg72;
    };
    union {

...
int eMMC_mmc_Init(struct mmc* mmc)
{
	...
	mmc->has_init = 1;
	mmc->capacity = g_eMMCDrv.u32_SEC_COUNT << 9;
...
U32 eMMC_Init_Device(void) 
{
	...
	eMMC_PlatformInit();
	// init eMMC device
	u8_eMMC_Identify_retry = 0;
EMMC_IDENTIFY:	//20130515
	u32_err = eMMC_Identify();  
	if(u32_err)
	{
		eMMC_printf("eMMC Err: Identify fail, %X\r\n", u32_err);
		u8_eMMC_Identify_retry ++;
		if (u8_eMMC_Identify_retry < 3)
			goto EMMC_IDENTIFY;
		else
			goto LABEL_INIT_END; 
	}
	...
	eMMC_printf("eMMC_FCIE_EnableDDRMode\r\n");
	if(eMMC_ST_SUCCESS != eMMC_FCIE_EnableDDRMode())
	{
		eMMC_printf("eMMC_ST_SUCCESS != eMMC_FCIE_EnableDDRMode()\r\n");
	    #if IF_DETECT_eMMC_DDR_TIMING

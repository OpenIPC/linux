...
U32 eMMC_FCIE_Reset(void)
{
	...
	{
		u32_err = eMMC_ST_ERR_FCIE_NO_RIU;
		eMMC_printf("eMMC Err: preset CIFC fail\r\n");
...
)
{
	...
	//check power cut
	#if (defined(eMMC_DRV_CEDRIC_LINUX)&&eMMC_DRV_CEDRIC_LINUX)
    while ((((REG_FCIE(REG_RESET)) & BIT3) == 0) || (((REG_FCIE(REG_BATTERY)) & BIT7) == BIT7)) 
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR,1,"Stop for powercut or reset\r\n");
	...
	// wait event
	//eMMC_printf("eMMC_FCIE_SendCmd %X, %X, %X, %X, %X", u16_Mode, u16_Ctrl, u32_Arg, u8_CmdIdx, u8_RspByteCnt);
	//eMMC_printf("M");
...
U32 eMMC_CMD25_MIU(U32 u32_eMMCBlkAddr, U8 *pu8_DataBuf, U16 u16_BlkCnt)
{
	...
	{			 
		if(reliable_test_sleep>0)
		 ...
		 temp = *(volatile unsigned int *)(0x1f000000+(0x0700*2 + 0x10*2)*2);
		 temp &= ~BIT4;
		 *(volatile unsigned int *)(0x1f000000+(0x0700*2 + 0x010*2)*2) =temp; 
		 temp = *(volatile unsigned int *)(0x1f000000+(0x0700*2 + 0x0F*2)*2);
		 temp &= ~BIT4;
		 *(volatile unsigned int *)(0x1f000000+(0x0700*2 + 0x0F*2)*2) =temp; 
		 *(volatile unsigned int *)(0x1f000000+(0x0700*2 + 0x12*2)*2) = 0xBABE;
	}		
    #endif 
  //eMMC_printf("G");
	u32_err = eMMC_FCIE_WaitEvents(FCIE_MIE_EVENT, 

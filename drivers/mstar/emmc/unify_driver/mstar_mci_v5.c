...
//    #endif
    strcpy(pMStarHost_st->name,MSTAR_MCI_NAME);
    ...
    s32_ret = request_irq(fcie_irq, eMMC_FCIE_IRQ, IRQF_SHARED, DRIVER_NAME, pMStarHost_st);
    if (s32_ret)
    {
...
static s32 __exit mstar_mci_remove(struct platform_device *pDev_st)
{
    ...
    #if defined(ENABLE_eMMC_INTERRUPT_MODE) && ENABLE_eMMC_INTERRUPT_MODE
    free_irq(fcie_irq, pMStarHost_st);
    #endif

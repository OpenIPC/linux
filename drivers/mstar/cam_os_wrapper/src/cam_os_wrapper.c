...
static s32 _CamOsVsscanf(char* szBuf, char* szFmt, va_list tArgp)
{
    ...
    for (pFmt = szFmt; *pFmt; pFmt++)
    {
        if (*pFmt == '%')
        {
            ...
            {
                CamOsDebug("_CamOsVsscanf error: unsupported format (\%%s)\n", pFmt);
            }
        }
    }
    ...
    *pThread = (void*) - 1;
    do
    {
        ...
        if (szName && strlen(szName))
            strncpy(tTaskArgs.TaskName, szName, sizeof(tTaskArgs.TaskName));
        else
    ...
    if(ptAttrb != NULL)
    {
        ...
        pthread_setname_np(tThreadHandle, ptAttrb->szName);
    }
    else
    {
                               ...
                               u64 *lpPhysAddr)
{
    ...
    *ppUserPtr = MsAllocateNonCacheMemExt(nSize, 12);
    ...
    if(nAllocSucc == TRUE)
    {
        ...
        *pMiuAddr = HalUtilPHY2MIUAddr((u32)*lpPhysAddr);
    }
    else
    {
        *ppUserPtr = 0;
    ...
    MSYS_DMEM_INFO *ptDmemSearch = NULL;
    ...
    *lpPhysAddr = 0;
    ...
    do
    {
        ...
        CAM_OS_LIST_FOR_EACH_SAFE(ptPos, ptQ, &_gtMemList.tList)
        {
            ...
            ptDmemSearch = ptTmp->pMemifoPtr;
            if(ptTmp->pPtr && ptDmemSearch && 0 == strcmp(szName, ptDmemSearch->name))
            {
        ...
        snprintf(ptDmem->name, sizeof(ptDmem->name), "%s", szName);
        ptDmem->length = nSize;
...
CamOsRet_e CamOsDirectMemStat(void)
{
    ...
    CAM_OS_LIST_FOR_EACH_SAFE(ptPos, ptQ, &_gtMemList.tList)
    {
        ...
        ptDmemSearch = ptTmp->pMemifoPtr;
        if(ptTmp->pPtr && ptDmemSearch)
        {
            printk(KERN_WARNING "%s memory allocated %p %s\n", __FUNCTION__, ptTmp->pPtr, ptDmemSearch->name);
        }
    }
    mutex_unlock(&_gtMemLock);
#endif
...
u64 CamOsDirectMemPhysToMiu(u64 nAddr)
{
#ifdef CAM_OS_RTK
    return HalUtilPHY2MIUAddr(nAddr);
#elif defined(CAM_OS_LINUX_USER)
    s32 nMsysFd = -1;
    u32 nMiuAddr = 0;
    MSYS_ADDR_TRANSLATION_INFO tAddrInfo;
    ...
    do
    {
        ...
        tAddrInfo.addr = nAddr;
        if(ioctl(nMsysFd, IOCTL_MSYS_PHYS_TO_MIU, &tAddrInfo))
        {
            fprintf(stderr, "%s IOCTL_MSYS_PHYS_TO_MIU Failed!!\n", __FUNCTION__);
            break;
        }
        nMiuAddr = (u32)tAddrInfo.addr;
    }
    while(0);
    ...
    return nMiuAddr;
#elif defined(CAM_OS_LINUX_KERNEL)
    return Chip_Phys_to_MIU(nAddr);
#endif
}

u64 CamOsDirectMemMiuToPhys(u64 nAddr)
{
#ifdef CAM_OS_RTK
    return HalUtilMIU2PHYAddr(nAddr);
#elif defined(CAM_OS_LINUX_USER)
    s32 nMsysFd = -1;
    u32 nPhysAddr = 0;
    MSYS_ADDR_TRANSLATION_INFO tAddrInfo;
...
CamOsMemSize_e CamOsPhysMemSize(void)
{
    ...
    do
    {
        if(0 > (nMemFd = open("/dev/mem", O_RDWR | O_SYNC)))
    {
            CamOsDebug("%s open /dev/mem failed!!\n", __FUNCTION__);
            break;
        }
    ...
    pRegs = ioremap(RIU_BASE_ADDR + RIU_MEM_SIZE_OFFSET, 4);
...
u32 CamOsChipId(void)
{
    ...
    pRegs = ioremap(RIU_BASE_ADDR + RIU_CHIP_ID_OFFSET, 4);

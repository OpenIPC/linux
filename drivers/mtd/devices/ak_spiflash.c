...
#ifdef SPANSION_FLS_BA24_SET
#define	OPCODE_BRWR		0xc5//0x17	/* Bank register write */
#endif
...
#define SFLAG_COM_BIT24          	  (1<<15)   //bit24λ��ǡ�
};
...
#ifdef SPANSION_FLS_BA24_SET
static int spansion_fls_ba24_set(struct ak_spiflash *flash, u8 BA24Value)
{
  int ret;
  uint32_t regval;	
...
//  return ;
  ...
  flash->command[1] = BA24Value&0x01;
...
#ifdef SPANSION_FLS_BA24_SET
	  if ((offset >= 0x1000000)&&((flash->info.flags & SFLAG_COM_BIT24) == 0))
	  {
		flash->info.flags = flash->info.flags | SFLAG_COM_BIT24;
		spansion_fls_ba24_set(flash, 1);
	  }
	  else if((offset < 0x1000000)&&((flash->info.flags & SFLAG_COM_BIT24) != 0))
	  {
		flash->info.flags = flash->info.flags & (~SFLAG_COM_BIT24) ;
		spansion_fls_ba24_set(flash, 0);
	  }
#endif
...
static int ak_spiflash_cfg_quad_mode(struct ak_spiflash *flash)
{
	...
	if((flash->bus_width & FLASH_BUS_WIDTH_4WIRE) && 
		...
		if (flash->info.flags & (SFLAG_QUAD_WRITE|SFLAG_QUAD_IO_WRITE|
			...
			if((flash->info.flags&SFLAG_QUAD_NO_QE) != SFLAG_QUAD_NO_QE){
			    quad_mode_enable(flash); // cdh: test KH 1 WIRE for pin7 is not spi_hold but reset
             }
        }
	}
...
#ifdef SPANSION_FLS_BA24_SET
	  if ((from >= 0x1000000)&&((flash->info.flags & SFLAG_COM_BIT24) == 0))
	  {
		flash->info.flags = flash->info.flags | SFLAG_COM_BIT24;
		spansion_fls_ba24_set(flash, 1);
	  }
	  else if((from < 0x1000000)&&((flash->info.flags & SFLAG_COM_BIT24) != 0))
	  {
		flash->info.flags = flash->info.flags & (~SFLAG_COM_BIT24) ;
		spansion_fls_ba24_set(flash, 0);
	  }
...
#ifdef SPANSION_FLS_BA24_SET
	  if ((to >= 0x1000000)&&((flash->info.flags & SFLAG_COM_BIT24) == 0))
	  {
		flash->info.flags = flash->info.flags | SFLAG_COM_BIT24;
		spansion_fls_ba24_set(flash, 1);
	  }
	  else if((to < 0x1000000)&&((flash->info.flags & SFLAG_COM_BIT24) != 0))
	  {
		flash->info.flags = flash->info.flags & (~SFLAG_COM_BIT24) ;
		spansion_fls_ba24_set(flash, 0);
	  }
...
static struct flash_info __devinitdata ak_spiflash_supportlist [] = {
	...
	{ "mx25l25645g", 0xc22019, 0, 64 * 1024, 512, SFLAG_COM_BIT24|SFLAG_SECT_4K | SFLAG_DUAL_READ | SFLAG_QUAD_READ | SFLAG_DUAL_IO_READ | SFLAG_QUAD_IO_WRITE},
	...
	{ "w25q256", 0xef4019, 0, 64 * 1024, 512, SFLAG_COM_BIT24|SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_COM_STATUS3|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE ,},
	...
	{ "gd25q256", 0xc84019, 0, 64 * 1024, 512, SFLAG_COM_BIT24|SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE ,},
...
//	{ "xt25f128a", 0x207018, 0, 64 * 1024, 256, SFLAG_QUAD_NO_QE|SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE ,},
    ...
    { "EN25QH128A",  0x1c7018, 0, 64 * 1024, 256, SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE,},
   ...
   { "ZB25VQ128",  0x5e4018, 0, 64 * 1024, 256, SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE,},
	...
	/* GM25Q */
	{ "GM25Q128AIQ",  0x1c4018, 0, 64 * 1024, 256, SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE,},
	{ "GM25Q128AIM",  0x1c7018, 0, 64 * 1024, 256, SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE,},

    /*XT25F64B*/
    { "XT25F64B",  0x0b4017, 0, 64 * 1024, 128, SFLAG_SECT_4K|SFLAG_COM_STATUS2|SFLAG_DUAL_READ|SFLAG_QUAD_READ|SFLAG_DUAL_IO_READ|SFLAG_QUAD_IO_READ|SFLAG_QUAD_WRITE,},            
};
...
static int ak_mount_partitions(struct spi_device *spi)
{
	...
	ret = partition_get_partition_table(&part_tab, PART_SPIFLASH);
	if (ret == FHA_FAIL) {
...
static int ak_spiflash_init_stat_reg(struct ak_spiflash *flash)
{
	...
	*/
	if(0xc84017 == info->jedec_id)
	{
		...
		if (0x53==id_ver_c)
	 	{
		 	info->jedec_id = info->jedec_id << 8;
		  	info->jedec_id |= id_ver_c;
	  	}
	}

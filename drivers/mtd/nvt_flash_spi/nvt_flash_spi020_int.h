...
typedef enum {
	FCTRL_OP_NORMAL,                //< normal
	FCTRL_OP_READ_STATUS,           //< read flash status
	FCTRL_OP_POLLING_STATUS,        //< polling flash status until ready

	FCTRL_OP_MODE_COUNT,
	ENUM_DUMMY4WORD(FCTRL_OP_MODE)
} FCTRL_OP_MODE;
...
typedef enum {
	...
	FCTRL_CS_COUNT,
	ENUM_DUMMY4WORD(FCTRL_CS_ENUM)
} FCTRL_CS_ENUM;

typedef enum {
	FCTRL_IO_SERIAL,                //< serial SPI
	FCTRL_IO_DUAL,                  //< SPI dual (only data phase is dual)
	FCTRL_IO_QUAD,                  //< SPI quad (only data phase is quad)
	FCTRL_IO_DUAL_IO,               //< SPI dual IO mode(dual from address phase)
	FCTRL_IO_QUAD_IO,               //< SPI quad IO mode (quad from address phase)

	FCTRL_IO_MODE_COUNT,
	ENUM_DUMMY4WORD(FCTRL_IO_MODE)
} FCTRL_IO_MODE;
...
typedef enum {
	...
	FCTRL_XDR,
	ENUM_DUMMY4WORD(FCTRL_XDR_ENUM)
} FCTRL_XDR_ENUM;
...
typedef enum {
	...
	ENUM_DUMMY4WORD(FIFO_DIR_ENUM)
} FIFO_DIR_ENUM;

typedef struct _NAND_PIN {
	CHAR     *cName;
} NAND_PIN_INFO, *PNAND_PIN_INFO;

typedef struct _NAND_FREE_BLOCK_QUEUE_ITEM {
	struct _NAND_FREE_BLOCK_QUEUE_ITEM *p_forw;
	struct _NAND_FREE_BLOCK_QUEUE_ITEM *p_back;
	UINT16  physicalBlockNum;
} NAND_FREE_BLOCK_QUEUE_ITEM;

typedef struct {
	FCTRL_CS_ENUM cs_sel;           ///< which chip select
	FCTRL_OP_MODE ctrl_mode;        ///< flash controller mode selection
	FCTRL_IO_MODE io_mode;          ///< io mode selection
	FCTRL_XDR_ENUM xdr_sel;         ///< select SDR/DDR

	UINT32 command;                 ///< controller operation command

	UINT32 flash_addr;              ///< flash address value
	UINT32 flash_addr_count;        ///< how many bytes in address cycle (unit: byte)

	UINT32 conti_mode;              ///< conti mode value
	UINT32 is_conti_mode;           ///< if continous mode exist

	UINT32 dummy_cyles;             ///< dummy clock cycles (unit: clock)

	UINT32 data_len;                ///< data length in data phase (unit: byte)
	FIFO_DIR_ENUM data_dir;         ///< read/write data
	UINT32 data_buf;                ///< data buffer DRAM address (virtual address)

	UINT8 *p_status;                ///< pointer used to return flash status (if flash read is enabled)

} FTSPI_TRANS_T, * PFTSPI_TRANS;

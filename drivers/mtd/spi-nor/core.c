...
int macronix_quad_enable(struct spi_nor *nor)
{
	...
	ret = spi_nor_read_sr(nor, &regval);
	if (ret)
		return ret;

	if (regval & SR_QUAD_EN_MX)
		return 0;
	...
	regval = regval | SR_QUAD_EN_MX;
	spi_nor_write_sr(nor, &regval, 1);

	if (spi_nor_wait_till_ready(nor))
		return 1;

	spi_nor_read_sr(nor, &regval);
	if (!(regval > 0 && (regval & SR_QUAD_EN_MX))) {
...
int gd_quad_enable(struct spi_nor *nor)
{
	u8 sr_cr[2] = {0};
	int ret;
	u16 buf;
	...
	spi_nor_read_cr(nor, &regval);
	buf = ((regval & 0xff) | CR_QUAD_EN_SPAN) << 8;
	...
	spi_nor_read_cr(nor, &regval);
	regval = (regval & 0xff) | CR_QUAD_EN_SPAN;

	ret = nor->controller_ops->write_reg(nor, SPINOR_OP_WRCR, &regval, 1);
	if (ret < 0) {
	...
	/* read back and check it */
	spi_nor_read_cr(nor, &regval);
	if (!(regval > 0 && (regval & CR_QUAD_EN_SPAN))) {
 ...
 * Return: 0 on success, -errno otherwise.
 */
int spansion_quad_enable(struct spi_nor *nor)
{
	...
	sr_cr[1] = (regval & 0xff) | CR_QUAD_EN_SPAN;
	spi_nor_read_sr(nor, &regval);
	sr_cr[0] = regval & 0xff;
...
int xtx_quad_enable(struct spi_nor *nor)
{
	u8 ret;
	u8 regval_h = 0;
	u8 regval_l = 0;
	...
	/* read SR high 8bit*/
	ret = spi_nor_read_cr(nor, &regval_h);
	if (ret)
		return ret;

	if (regval_h & SR_QUAD_EN_XTX)
		return 0;
	...
	/* read SR low 8bit*/
	ret = spi_nor_read_sr(nor, &regval_l);
	if (ret)
		return ret;

	/* write SR */
	nor->bouncebuf[0] = regval_l;
	nor->bouncebuf[1] = regval_h;
	nor->controller_ops->write_reg(nor, SPINOR_OP_WRSR, nor->bouncebuf, 2);

	if (spi_nor_wait_till_ready(nor))
		return 1;

	spi_nor_read_cr(nor, &regval_l);
	if (!(regval_l > 0 && (regval_l & SR_QUAD_EN_XTX))) {
...
int puya_quad_enable(struct spi_nor *nor)
{
	int ret;
	u8 regval = 0;
...
int nm_quad_enable(struct spi_nor *nor)
{
	int ret;
	u8 regval = 0;

	spi_nor_read_cr(nor, &regval);
	if (regval & CR_QUAD_EN_NM)
		return 0;

	regval = ((regval & 0xff) | CR_QUAD_EN_NM);
	spi_nor_write_enable(nor);
...
static void spi_lock_update_address(struct spi_nor *nor, const struct flash_info *info)
{
	...
	switch (JEDEC_MFR(info)) {
		case SNOR_MFR_MACRONIX:
			if (chipsize == _2M) {
				if ((nor->level != lock_level_max) && (nor->level != 1))
...
static unsigned char bp_to_level(struct spi_nor *nor,
...
static void get_spi_lock_info(struct spi_nor *nor, const struct flash_info *info)
{
	unsigned int chipsize;
	...
	switch (JEDEC_MFR(info)) {
		...
		case SNOR_MFR_SPANSION:
		   ...
		   nor->level = bp_to_level(nor, info, BP_NUM_3);
		   break;
	   case SNOR_MFR_WINBOND:
		   ...
		   if (chipsize <= _16M)
			   ...
			   nor->level = bp_to_level(nor, info, BP_NUM_4);
		   break;
	   case SNOR_MFR_MACRONIX:
		   /* BP bit convert to lock level */
		   if (chipsize <= _8M)
...
static int spi_nor_clear_dtr_mode(struct spi_nor *nor,const struct flash_info *info)
{
	...
	if (JEDEC_MFR(info) == SNOR_MFR_MACRONIX){
		...
		if (cval & CR_DUMMY_CYCLE){
			...
			ret = nor->controller_ops->write_reg(nor, SPINOR_OP_WRSR, nor->bouncebuf, 2);
		}
	}
				 ...
				 const struct spi_nor_hwcaps *hwcaps)
{
...
#ifdef CONFIG_ARCH_XMEDIA
	err = spi_nor_clear_dtr_mode(nor, nor->info);
	if (err) {
	...
	if (!(hwcaps->mask & (SNOR_HWCAPS_READ_1_1_4 | SNOR_HWCAPS_READ_1_4_4))) {
		err = spi_nor_reset_pin_enable(nor, nor->info);
		if (err) {
...
#ifdef CONFIG_ARCH_XMEDIA
	if (nor->info->fixups->post_bfpt) {
		nor->info->fixups->post_bfpt(nor, NULL, NULL, nor->params);
	} else
#endif
	{
...
void spi_nor_driver_shutdown(struct spi_nor *nor)
{
	...
	if (nor->addr_width == 4) {
		...
		info = spi_nor_read_id(nor);
		spi_nor_set_4byte_addr_mode(nor, 0);
	}
	return;
}
...
#ifdef CONFIG_PM
/******************************************************************************/
int spi_nor_driver_suspend(struct spi_nor *nor, pm_message_t state)
{
	return spi_nor_wait_till_ready(nor);
}

/******************************************************************************/
int spi_nor_driver_resume(struct spi_nor *nor)
{
	int ret;
	const struct flash_info *info = NULL;
	...
	if (nor->addr_width == 4 && JEDEC_MFR(info) != SNOR_MFR_SPANSION)
		spi_nor_set_4byte_addr_mode(nor, 1);

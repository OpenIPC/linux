...
/******************************************************************************/
static void fmc_spi_nor_switch_spi_type(struct fmc_host *host)
{
	unsigned int reg;
...
/******************************************************************************/
static void fmc_spi_nor_init(struct fmc_host *host)
{
	unsigned int reg;

	/* switch the flash type to spi nor */
	fmc_spi_nor_switch_spi_type(host);
...
/******************************************************************************/
static int fmc_spi_nor_prep(struct spi_nor *nor)
{
...
/******************************************************************************/
static void fmc_spi_nor_unprep(struct spi_nor *nor)
{
	struct fmc_priv *priv = nor->priv;
	struct fmc_host *host = priv->host;
...
/******************************************************************************/
static int fmc_spi_nor_op_reg(struct spi_nor *nor,
...
/******************************************************************************/
static int fmc_spi_nor_read_reg(struct spi_nor *nor, u8 opcode, u8 *buf,
		unsigned int len)
{
	struct fmc_priv *priv = nor->priv;
	struct fmc_host *host = priv->host;
	...
	ret = fmc_spi_nor_op_reg(nor, opcode, len, FMC_OP_READ_DATA_EN);
	if (ret)
		return ret;
...
/******************************************************************************/
static int fmc_spi_nor_write_reg(struct spi_nor *nor, u8 opcode,
				const u8 *buf, unsigned int len)
{
	...
	return fmc_spi_nor_op_reg(nor, opcode, len, FMC_OP_WRITE_DATA_EN);
}

/******************************************************************************/
static int fmc_spi_nor_dma_transfer(struct spi_nor *nor, loff_t start_off,
...
static ssize_t fmc_spi_nor_read(struct spi_nor *nor, loff_t from, size_t len,
		u_char *read_buf)
{
	struct fmc_priv *priv = nor->priv;
	struct fmc_host *host = priv->host;
	...
	for (offset = 0; offset < len; offset += host->dma_len) {
		...
		ret = fmc_spi_nor_dma_transfer(nor,
...
static ssize_t fmc_spi_nor_write(struct spi_nor *nor, loff_t to,
...
static const struct spi_nor_controller_ops fmc_controller_ops = {
	.prepare = fmc_spi_nor_prep,
	.unprepare = fmc_spi_nor_unprep,
	.read_reg = fmc_spi_nor_read_reg,
	.write_reg = fmc_spi_nor_write_reg,
	.read = fmc_spi_nor_read,
	.write = fmc_spi_nor_write,
};

/**
 * Get spi flash device information and register it as a mtd device.
 */
static int fmc_spi_nor_register(struct device_node *np,
				struct fmc_host *host)
{
	...
	mtd = &nor->mtd;
	mtd->name = np->name;
    /* kernel 5.10 already include parse partitions */
...
static void fmc_spi_nor_unregister_all(struct fmc_host *host)
{
	int i;
...
static int fmc_spi_nor_register_all(struct fmc_host *host)
{
	struct device *dev = host->dev;
	...
	for_each_available_child_of_node(dev->of_node, np) {
		...
		ret = fmc_spi_nor_register(np, host);
		if (ret)
			goto fail;
...
fail:
	fmc_spi_nor_unregister_all(host);
	return ret;
}

/******************************************************************************/
static int fmc_spi_nor_probe(struct platform_device *pdev)
{
	...
	fmc_spi_nor_init(host);
	ret = fmc_spi_nor_register_all(host);
	if (ret)
...
/******************************************************************************/
static int fmc_spi_nor_remove(struct platform_device *pdev)
{
...
/******************************************************************************/
static void fmc_spi_nor_driver_shutdown(struct platform_device *pdev)
{
	int i;
	struct fmc_host *host = platform_get_drvdata(pdev);
...
#ifdef CONFIG_PM
/******************************************************************************/
static int fmc_spi_nor_driver_suspend(struct platform_device *pdev,
		pm_message_t state)
{
	int i;
	struct fmc_host *host = platform_get_drvdata(pdev);
	...
	fmc_spi_nor_switch_spi_type(host);
	for (i = 0; i < host->num_chip; i++)
		spi_nor_driver_suspend(host->nor[i], state);
...
/******************************************************************************/
static int fmc_spi_nor_driver_resume(struct platform_device *pdev)
{
	int i;
	struct fmc_host *host = platform_get_drvdata(pdev);
	...
	fmc_spi_nor_switch_spi_type(host);
	for (i = 0; i < host->num_chip; i++)
		spi_nor_driver_resume(host->nor[i]);
...
/******************************************************************************/
static const struct of_device_id fmc_spi_nor_dt_ids[] = {
	{ .compatible = "xmedia,fmc-spi-nor"},
	{ /* sentinel */ }
};
MODULE_DEVICE_TABLE(of, fmc_spi_nor_dt_ids);

/******************************************************************************/
static struct platform_driver fmc_spi_nor_driver = {
	.driver = {
		.name	= "xmedia-sfc",
		.of_match_table = fmc_spi_nor_dt_ids,
	},
	.probe		= fmc_spi_nor_probe,
	.remove		= fmc_spi_nor_remove,
	.shutdown	= fmc_spi_nor_driver_shutdown,
#ifdef CONFIG_PM
	.suspend	= fmc_spi_nor_driver_suspend,
	.resume		= fmc_spi_nor_driver_resume,
#endif
};
module_platform_driver(fmc_spi_nor_driver);

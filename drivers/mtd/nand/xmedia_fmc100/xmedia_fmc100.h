...
#ifndef __XMEDIA_FMC100_H__
#define __XMEDIA_FMC100_H__
...
#define XMEDIA_FMC_SPI_NAND_SUPPORT_READ (SPI_IF_READ_STD \
                    | SPI_IF_READ_FAST \
                    | SPI_IF_READ_DUAL \
...
#define XMEDIA_FMC_SPI_NAND_SUPPORT_WRITE    (SPI_IF_WRITE_STD | SPI_IF_WRITE_QUAD)

#define XMEDIA_FMC_SPI_NAND_SUPPORT_MAX_DUMMY    8
...
#undef XMEDIA_FMC100_SPI_NAND_SUPPORT_REG_READ
/* #define XMEDIA_FMC100_SPI_NAND_SUPPORT_REG_READ */

#undef XMEDIA_FMC100_SPI_NAND_SUPPORT_REG_WRITE
/* #define XMEDIA_FMC100_SPI_NAND_SUPPORT_REG_WRITE */

#ifdef CONFIG_XMEDIA_NAND_ECC_STATUS_REPORT
#define XMEDIA_FMC100_ECC_ERR_NUM0_BUF0      0xc0
...
#define XMEDIA_FMC100_ADDR_CYCLE_MASK        0x2
...
struct spi_drv {
	int (*wait_ready)(struct xmedia_fmc_spi *spi);
	int (*write_enable)(struct xmedia_fmc_spi *spi);
	int (*qe_enable)(struct xmedia_fmc_spi *spi);
	int (*bus_prepare)(struct xmedia_fmc_spi *spi, int op);
	int (*entry_4addr)(struct xmedia_fmc_spi *spi, int en);
};
...
extern char spi_nand_feature_op(struct xmedia_fmc_spi *spi, u_char op, u_char addr,
				  u_char *val);

struct xmedia_fmc_host {
	struct mtd_info *mtd;
	struct nand_chip *chip;
	struct xmedia_fmc_spi spi[CONFIG_SPI_NAND_MAX_CHIP_NUM];
	...
	int  need_rr_data;
#define XMEDIA_FMC100_READ_RETRY_DATA_LEN         128
	char rr_data[XMEDIA_FMC100_READ_RETRY_DATA_LEN];
	struct read_retry_t *read_retry;

	int version;

	/* BOOTROM read two bytes to detect the bad block flag */
#define XMEDIA_FMC_BAD_BLOCK_POS     0
	unsigned char *bbm; /* nand bad block mark */
	unsigned short *epm;    /* nand empty page mark */

	unsigned int uc_er;

	void (*send_cmd_write)(struct xmedia_fmc_host *host);
	void (*send_cmd_status)(struct xmedia_fmc_host *host);
	void (*send_cmd_read)(struct xmedia_fmc_host *host);
	void (*send_cmd_erase)(struct xmedia_fmc_host *host);
	void (*send_cmd_readid)(struct xmedia_fmc_host *host);
	void (*send_cmd_reset)(struct xmedia_fmc_host *host);
#ifdef CONFIG_PM
...
extern void xmedia_fmc_spi_nand_ids_register(void);

extern void xmedia_fmc_set_nand_system_clock(struct spi_op *op, int clk_en);
...
void xmedia_fmc100_spi_nand_config(struct xmedia_fmc_host *host);
#endif

#endif /* End of __XMEDIA_FMC100_H__ */

...
#include "xmedia_fmc100_spi_general.c"
 ...
 ******************************************************************************/
struct spi_nand_info xmedia_fmc_spi_nand_flash_table[] = {
...
static void xmedia_fmc100_spi_nand_search_rw(struct spi_nand_info *spiinfo,
...
static void xmedia_fmc100_spi_nand_get_erase(const struct spi_nand_info *spiinfo,
...
static void xmedia_fmc100_map_spi_op(struct xmedia_fmc_spi *spi)
{
	unsigned char ix;
...
static void xmedia_fmc100_spi_ids_probe(struct mtd_info *mtd,
				   struct spi_nand_info *spi_dev)
{
	...
	xmedia_fmc100_spi_nand_search_rw(spi_dev, spi->read,
				    XMEDIA_FMC_SPI_NAND_SUPPORT_READ,
				    XMEDIA_FMC_SPI_NAND_SUPPORT_MAX_DUMMY, RW_OP_READ);
	FMC_PR(BT_DBG, "\t||-Save spi->read op cmd:%#x\n", spi->read->cmd);

	xmedia_fmc100_spi_nand_search_rw(spi_dev, spi->write,
				    XMEDIA_FMC_SPI_NAND_SUPPORT_WRITE,
				    XMEDIA_FMC_SPI_NAND_SUPPORT_MAX_DUMMY, RW_OP_WRITE);
	FMC_PR(BT_DBG, "\t||-Save spi->write op cmd:%#x\n", spi->write->cmd);

	xmedia_fmc100_spi_nand_get_erase(spi_dev, spi->erase);
	FMC_PR(BT_DBG, "\t||-Save spi->erase op cmd:%#x\n", spi->erase->cmd);

	xmedia_fmc100_map_spi_op(spi);
		...
		unsigned char *id)
{
	...
	struct spi_nand_info *spi_dev = xmedia_fmc_spi_nand_flash_table;
	...
	for (; spi_dev->id_len; spi_dev++) {
		if (!access_ok(id, spi_dev->id_len)) {
		...
		xmedia_fmc100_spi_ids_probe(mtd, spi_dev);
...
void xmedia_fmc_spi_nand_ids_register(void)
{

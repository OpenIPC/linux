...
static void ms_main_irq_mask(struct irq_data *d)
{
    ...
    else if (ms_irq >= 0 && ms_irq < GIC_SPI_MS_IRQ_NR)
    {
        ...
        {
            SETREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_54 + ((ms_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                            (1 << ((ms_irq - INT_IRQ_BANK2_BASE) % 16)));
            INREG16(BASE_REG_MAILBOX_PA); // read a register make ensure the previous write command was compeleted
        }
    }
    else
...
static void ms_main_irq_unmask(struct irq_data *d)
{
    ...
    else if (ms_irq >= 0 && ms_irq < GIC_SPI_MS_IRQ_NR)
    {
        ...
        {
            CLRREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_54 + ((ms_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                            (1 << ((ms_irq - INT_IRQ_BANK2_BASE) % 16)));
            INREG16(BASE_REG_MAILBOX_PA); // read a register make ensure the previous write command was compeleted
        }
    }
...
static int ms_main_irq_set_type(struct irq_data *data, unsigned int flow_type)
{
    ...
    else if (ms_irq >= 0 && ms_irq < GIC_SPI_MS_IRQ_NR)
    {
        ...
        {
            if (flow_type & IRQ_TYPE_LEVEL_LOW)
                SETREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_58 + ((ms_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                                (1 << ((ms_irq - INT_IRQ_BANK2_BASE) % 16)));
            else
                CLRREG16_BIT_OP((BASE_REG_INTRCTL2_PA + REG_ID_58 + ((ms_irq - INT_IRQ_BANK2_BASE) / 16) * 4),
                                (1 << ((ms_irq - INT_IRQ_BANK2_BASE) % 16)));
        }
    }
    else
...
static int ms_irqchip_suspend(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            ms_main_irq_priv.irq_mask[i] = INREG16(BASE_REG_INTRCTL2_PA + REG_ID_54 + ((i - INT_IRQ_BANK2_BASE) << 2));
...
static void ms_irqchip_resume(void)
{
    ...
    for (i = 0; i < num; i++)
    {
        ...
        {
            OUTREG16(BASE_REG_INTRCTL2_PA + REG_ID_54 + ((i - INT_IRQ_BANK2_BASE) << 2), ms_main_irq_priv.irq_mask[i]);
            OUTREG16(BASE_REG_INTRCTL2_PA + REG_ID_58 + ((i - INT_IRQ_BANK2_BASE) << 2),
                     ms_main_irq_priv.irq_polarity[i]);
        }
    }

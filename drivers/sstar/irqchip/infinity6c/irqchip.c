...
#endif
struct irq_domain *ss_irq_domain;
...
static int __init ms_init_main_intc(struct device_node *np, struct device_node *interrupt_parent)
{
    ...
    ss_irq_domain =
        irq_domain_add_hierarchy(parent_domain, 0, GIC_SPI_ARM_INTERNAL_NR + GIC_SPI_MS_IRQ_NR + GIC_SPI_MS_FIQ_NR, np,
                                 &ms_main_intc_domain_ops, NULL);

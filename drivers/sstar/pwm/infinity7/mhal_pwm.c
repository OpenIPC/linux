...
int MHal_GroupEnable(struct sstar_pwm_chip *sstar_chip, U8 u8GroupId, U8 u8Val)
{
...
//+++[Only4I6e]
void MHal_PWM_AllGrpEnable(struct sstar_pwm_chip *sstar_chip)
{
    U8 i = 0;
    MHal_PWM_SetClock();
    for (i = 0; i < PWM_GROUP_NUM; i++)
    {
        ...
        MHal_GroupEnable(sstar_chip, i, 1);
...
void MHal_PWMInit(struct sstar_pwm_chip *sstar_chip, U8 u8Id)
{
    ...
    reset = INREG16(U32PwmAddr + u16REG_SW_RESET) & (BIT0 << (u8Id == 11 ? 14 : u8Id));
...
void MHal_PWMSetConfig(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U32 duty, U32 period)
{
    ...
    if (duty == 0)
    {
        if (_pwmEnSatus[u8Id])
        {
            SETREG16(U32PwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 14 : u8Id));
        }
    }
    else
    {
        if (_pwmEnSatus[u8Id])
        {
            CLRREG16(U32PwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 14 : u8Id));
        }
    }
...
void MHal_PWMSetDuty(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U32 u32Val)
{
    ...
    {
        if (0 == u32Val)
            OUTREGMSK16(U32PwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 14 : u8Id),
    ...
    if (_pwmEnSatus[u8Id])
    {
        U32 reset = INREG16(U32PwmAddr + u16REG_SW_RESET) & (BIT0 << (u8Id == 11 ? 14 : u8Id));
        if (u32Val && reset)
            CLRREG16(U32PwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 14 : u8Id));
    }
}
...
void MHal_PWMGetPolarity(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U8 *pu8Val)
{
    ...
    *pu8Val        = (INREG16(sstar_chip->base + (u32PwmOffs) + u16REG_PWM_CTRL) & (0x1 << POLARITY_BIT)) ? 1 : 0;
}
...
void MHal_PWMEnable(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U8 u8Val)
{
    ...
    if (CamOsChipRevision() == 0x01)
    {
        if (u8Val)
        {
            if (u32DutyL || u32DutyH)
            {
                CLRREG16(U32PwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 10 : u8Id));
            }
            else
            {
                SETREG16(U32PwmAddr + u16REG_SW_RESET, BIT0 << (u8Id == 11 ? 10 : u8Id));
            }
        }
        else
        {
    ...
    else if (CamOsChipRevision() >= 0x02)
    {
        if (u8Val)
        {
            if (u32DutyL || u32DutyH)
...
int MHal_PWMSetEndToReg(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U8 u8DutyArgNum)
{
    ...
    for (arg_no = 0; arg_no <= u8DutyArgNum; arg_no++)
    {
        if (0 == arg_no)
        {
            OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_DUTY_L,
                     (_pwmDuty[u8Id][arg_no] & 0xFFFF));
            OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_DUTY_H,
                     ((_pwmDuty[u8Id][arg_no] >> 16) & 0x0003));
        }
        else
        {
            OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + (arg_no << 3) + 28,
                     (_pwmDuty[u8Id][arg_no] & 0xFFFF));
        }
    }
    return 1;
...
int MHal_PWMSetBeginToReg(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U8 u8ShftArgNum)
{
    ...
    for (arg_no = 0; arg_no <= u8ShftArgNum; arg_no++)
    {
        if (0 == arg_no)
        {
            OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_SHIFT_L,
                     (_pwmShft[u8Id][arg_no] & 0xFFFF));
            OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_SHIFT_H,
                     ((_pwmShft[u8Id][arg_no] >> 16) & 0x0003));
        }
        else
        {
            OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + (arg_no << 3) + 24,
                     (_pwmShft[u8Id][arg_no] & 0xFFFF));
        }
    }
    return 1;
...
{
    if (u8Val)
        SETREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_CTRL,
                 (0x1 << POLARITY_BIT));
    else
        CLRREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_CTRL,
                 (0x1 << POLARITY_BIT));
    return 1;
}
#ifdef CONFIG_PWM_NEW
void MHal_PWMSetPeriodEx(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U32 u32Val)
{
    ...
    MHal_PWM_SetSyncFlag(0);
    pr_err("reg=0x%08llX clk=%d, period=0x%x\n",
           (U64)(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_PERIOD_L),
           (U32)(clk_get_rate(sstar_chip->clk)), u32Period);
}
...
void MHal_PWMSetPeriodEx(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U32 u32Val)
{
    ...
    u32Div = INREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40))
                     + u16REG_PWM_DIV); // workaround
...
{
    OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_PERIOD_L,
             (u32Period & 0xFFFF));
    OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_PERIOD_H,
             ((u32Period >> 16) & 0x3));
}
...
int MHal_PWMDiv(struct sstar_pwm_chip *sstar_chip, U8 u8Id, U8 u8Val)
{
    if (PWM_NUM <= u8Id)
        return 0;
    OUTREG16(sstar_chip->base + ((u8Id < 4) ? (u8Id * 0x80) : (u8Id * 0x40 + 4 * 0x40)) + u16REG_PWM_DIV, u8Val);
...
int MHal_PWMGroupInfo(struct sstar_pwm_chip *sstar_chip, char *buf_start, char *buf_end)
{
    ...
    for (i = 0; i < PWM_NUM; i++)
    {
        ...
        // rest status
        u32ResetStatus = (INREG16(U32PwmAddr + u16REG_SW_RESET) >> (i == 11 ? 14 : i)) & BIT0;

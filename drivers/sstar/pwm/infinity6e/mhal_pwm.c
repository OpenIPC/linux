...
//+++[Only4I6e]
static int DrvPm_PWMSet(struct mstar_pwm_chip *ms_chip,U8 u8Id)
{
    if((ms_chip->pad_ctrl[u8Id] >= PAD_PM_GPIO0) && (ms_chip->pad_ctrl[u8Id] <= PAD_PM_GPIO7))
    {
        //printk("[NOTICE]ms_chip->pad_ctrl[%d]=%d 1<< %d \n",u8Id,ms_chip->pad_ctrl[u8Id],ms_chip->pad_ctrl[u8Id]-2);
        SETREG16(PADDEFALUT + PMPADTOP + PAD_EXT_MODE, BIT0<<(ms_chip->pad_ctrl[u8Id]-2));
    }
    return 0;
}
...
void DrvPWMInit(struct mstar_pwm_chip *ms_chip, U8 u8Id)
{
    ...
    if (!u8Id) {
        ...
        printk(KERN_DEBUG "[NOTICE]Each grp bit0 must be enabled!\r\n");
    }
    if (PWM_NUM <= u8Id)
        return;
    DrvPm_PWMSet(ms_chip,u8Id);
...
void DrvPWMSetDuty(struct mstar_pwm_chip *ms_chip, U8 u8Id, U32 u32Val)
{
    ...
    {
        if (_pwmDutyeq0[u8Id])
            OUTREGMSK16(U32PwmAddr + u16REG_SW_RESET, BIT0<<((u8Id==10)?0:u8Id),BIT0<<((u8Id==10)?0:u8Id));
    }
    if(u32Val == 0)
    {
        u32Duty = 0;
...
{
    U32 u32PwmOffs = ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40));
...
int DrvPWMGroupEnable(struct mstar_pwm_chip* ms_chip, U8 u8GroupId, U8 u8Val)
{
    ...
    if(u32JoinMask & (BIT0<<(PWM_NUM-1))){
         if(ms_chip->group_data != NULL){
            if(u8Val){
                CLRREG16(((U32)ms_chip->group_data) + u16REG_SW_RESET, BIT0);
            }else{
                SETREG16(((U32)ms_chip->group_data) + u16REG_SW_RESET, BIT0);
            }
        }
    }
    return 1;
...
int DrvPWMSetEndToReg(struct mstar_pwm_chip *ms_chip, U8 u8Id, U8 u8DutyArgNum)
{
	...
	{
		if (0 == arg_no) {
			OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_DUTY_L, (_pwmDuty[u8Id][arg_no]&0xFFFF));
			OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_DUTY_H, ((_pwmDuty[u8Id][arg_no]>> 16)&0x0003));
		}
		else {
			OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + (arg_no << 3) + 28, (_pwmDuty[u8Id][arg_no]&0xFFFF));
		}
	}
	return 1;
}
...
int DrvPWMSetBeginToReg(struct mstar_pwm_chip *ms_chip, U8 u8Id, U8 u8ShftArgNum)
{
    ...
    {
        if (0 == arg_no) {
            OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_SHIFT_L, (_pwmShft[u8Id][arg_no]&0xFFFF));
            OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_SHIFT_H, ((_pwmShft[u8Id][arg_no]>> 16)&0x0003));
        }
        else {
            OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + (arg_no << 3) + 24, (_pwmShft[u8Id][arg_no]&0xFFFF));
        }
    }
    return 1;
}
...
{
    if (u8Val)
        SETREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_CTRL, (0x1<<POLARITY_BIT));
    else
        CLRREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_CTRL, (0x1<<POLARITY_BIT));
    return 1;
}
#ifdef CONFIG_PWM_NEW
void DrvPWMSetPeriodEx(struct mstar_pwm_chip *ms_chip, U8 u8Id, U32 u32Val)
{
    ...
    MDEV_PWM_SetSyncFlag(0);
        pr_err("reg=0x%08X clk=%d, period=0x%x\n", (U32)(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_PERIOD_L), (U32)(clk_get_rate(ms_chip->clk)), u32Period);
}
...
void DrvPWMSetPeriodEx(struct mstar_pwm_chip *ms_chip, U8 u8Id, U32 u32Val)
{
    ...
    u32Div = INREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_DIV); // workaround
...
{
    OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_PERIOD_L, (u32Period&0xFFFF));
    OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_PERIOD_H, ((u32Period>>16)&0x3));
}
...
int DrvPWMDiv(struct mstar_pwm_chip *ms_chip, U8 u8Id, U8 u8Val)
{
    if (PWM_NUM <= u8Id)
        return 0;
    OUTREG16(ms_chip->base + ((u8Id < 4) ? (u8Id*0x80) : (u8Id*0x40 + 4*0x40)) + u16REG_PWM_DIV, u8Val);
...
int DrvPWMGroupInfo(struct mstar_pwm_chip *ms_chip, char* buf_start, char* buf_end)
{
    ...
    for (i = 0; i < PWM_NUM; i++)
    {
        DrvPWMGetGrpAddr(ms_chip,&U32PwmAddr,&u32PwmOffs,i);
...
#endif
        u32Shft0 = INREG16(U32PwmAddr + u32PwmOffs + u16REG_PWM_SHIFT_L) | ((INREG16(U32PwmAddr + u32PwmOffs + u16REG_PWM_SHIFT_H) & 0x3) << 16);
...
#endif
        u32Duty0 = INREG16(U32PwmAddr + u32PwmOffs + u16REG_PWM_DUTY_L) | ((INREG16(U32PwmAddr + u32PwmOffs + u16REG_PWM_DUTY_H) & 0x3) << 16);
        u32Duty0 -=u32Shft0;

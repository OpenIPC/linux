...
static pwmPadTbl_t padTbl_0[] =
{
    ...
    { PAD_SD1_IO0,  PINMUX_FOR_PWM0_MODE_5},
    { PAD_PM_GPIO0, PINMUX_FOR_PM_PWM0_MODE_1},
...
static pwmPadTbl_t padTbl_1[] =
{
    ...
    { PAD_SD1_IO1, PINMUX_FOR_PWM1_MODE_5},
    { PAD_PM_GPIO1, PINMUX_FOR_PM_PWM1_MODE_1},
    { PAD_UNKNOWN, BASE_REG_NULL},
};

static pwmPadTbl_t padTbl_2[] =
{
    { PAD_GPIO14, PINMUX_FOR_PWM2_MODE_1},
    { PAD_FUART_CTS, PINMUX_FOR_PWM2_MODE_2},
    { PAD_SD1_IO0, PINMUX_FOR_PWM2_MODE_3},
    { PAD_GPIO2, PINMUX_FOR_PWM2_MODE_4},
    { PAD_SD1_IO2,  PINMUX_FOR_PWM2_MODE_5},
    { PAD_PM_GPIO2, PINMUX_FOR_PM_PWM2_MODE_1},
    { PAD_PM_GPIO9, PINMUX_FOR_PM_PWM2_MODE_2},
    { PAD_UNKNOWN, BASE_REG_NULL},
};

static pwmPadTbl_t padTbl_3[] =
{
    { PAD_GPIO15,  PINMUX_FOR_PWM3_MODE_1},
    { PAD_FUART_RTS, PINMUX_FOR_PWM3_MODE_2},
    { PAD_SD1_IO1, PINMUX_FOR_PWM3_MODE_3},
    { PAD_GPIO3, PINMUX_FOR_PWM3_MODE_4},
    { PAD_SD1_IO3, PINMUX_FOR_PWM3_MODE_5},
    { PAD_PM_GPIO3, PINMUX_FOR_PM_PWM3_MODE_1},
    { PAD_PM_GPIO7, PINMUX_FOR_PM_PWM3_MODE_2},
    { PAD_UNKNOWN, BASE_REG_NULL},
};

static pwmPadTbl_t padTbl_4[] =
{
    { PAD_SD1_IO2, PINMUX_FOR_PWM4_MODE_1},
    { PAD_SPI0_CZ, PINMUX_FOR_PWM4_MODE_2},
    { PAD_GPIO4, PINMUX_FOR_PWM4_MODE_3},
    { PAD_SD1_IO4,  PINMUX_FOR_PWM4_MODE_4},
    { PAD_PM_LED0, PINMUX_FOR_PM_PWM4_MODE},
    { PAD_UNKNOWN, BASE_REG_NULL},
};

static pwmPadTbl_t padTbl_5[] =
{
    { PAD_SD1_IO3, PINMUX_FOR_PWM5_MODE_1},
    { PAD_SPI0_CK,  PINMUX_FOR_PWM5_MODE_2},
    { PAD_GPIO5, PINMUX_FOR_PWM5_MODE_3},
    { PAD_SD1_IO5,  PINMUX_FOR_PWM5_MODE_4},
...
static pwmPadTbl_t padTbl_8[] =
{
    ...
    { PAD_GPIO8, PINMUX_FOR_PWM8_MODE_2},
    { PAD_SD1_IO8, PINMUX_FOR_PWM8_MODE_3},
    { PAD_SR_IO14,  PINMUX_FOR_PWM8_MODE_4},
    { PAD_PM_GPIO9, PINMUX_FOR_PM_PWM2_MODE_2},
    { PAD_UNKNOWN, BASE_REG_NULL},
};
...
static bool _pwmDutyeq0[PWM_NUM] = { 0 };
...
#if 0
static int totalRounds_Stop[PWM_NUM]={0};
#endif
...
void MDEV_PWM_MemToReg(struct mstar_pwm_chip* ms_chip, U8 u8Id)
{
    ...
    for(idx=0;idx<4;idx++)
    {
        ...
        if(pwmId<PWM_NUM)
        {
			...
			DrvPWMDiv(ms_chip, pwmId, _pwmDiv[pwmId]);
#endif
...
//            printk("Period 0x%x ", _pwmPeriod[u8Id]);
//            printk("Freq %d Hz", _pwmFreq[u8Id]);
//            printk("Duty 0x%x ", _pwmDuty[u8Id]);
//            printk("Div 0x%x \r\n", _pwmDiv[u8Id]);
        }
    }
}
void MDev_PWM_isr_round_Act(void *dev,U16 u16Id)
{
    ...
    if(isSync) //no new data
    {
        ...
        for(i = 0; i < 4 ;i++){
                   if(_pwmFreq[(group_round_id * PWM_PER_GROUP) + i]){

                        totalRounds[(group_round_id * PWM_PER_GROUP) + i] += INREG16(ms_chip->base + u32Reg) & 0xFFFF;
                        }
        }
...
void DrvPWMSetConfig(struct mstar_pwm_chip *ms_chip, U8 u8Id, U32 duty,U32 period)
{
    ...
    if(u8Id >= PWM_NUM)
        return;
	_pwmDutyeq0[u8Id] = (duty==0)?TRUE:FALSE;
...
void DrvPWMSetDuty(struct mstar_pwm_chip *ms_chip, U8 u8Id, U32 u32Val)
{
	...
	_pwmDutyeq0[u8Id] = (u32Val==0)?TRUE:FALSE;
    if (_pwmEnSatus[u8Id])
    {
        if (_pwmDutyeq0[u8Id])
...
int DrvPWMGroupGetRoundNum(struct mstar_pwm_chip* ms_chip, U8 u8PwmId, U16* u16Val)
{
    ...
    if (!DrvPWMGroupIsEnable(ms_chip, u8PwmId/4, &enable_status))
    {
        printk("[%s][%d] unable to get enable status of group %d\n", __FUNCTION__, __LINE__, u8PwmId/4);
    }

    if((u32GroupRound)&&(enable_status)){
...
int DrvPWMGroupSetRound(struct mstar_pwm_chip* ms_chip, U8 u8GroupId, U16 u16Val)
{
    ...
    //isEmptyRnd=0;
    //timeStart[u8GroupId]=getCurNs(); //new start round ns
    return 1;
}
#if 0
int DrvPWMGroupShowStopRound(struct mstar_pwm_chip *ms_chip, char* buf_start, char* buf_end)
{
    char *str = buf_start;
    char *end = buf_end;
    int i;
    ...
    for (i = 0; i < PWM_NUM; i++)
    {
        str += scnprintf(str, end - str, "PWM%d\t Round = \t%d\n", i, totalRounds_Stop[i]);
    }
    return (str - buf_start);
}

void DrvPWMGroupGetStopRound(U8 u8PwmId)
{
    printk("totalRounds_stop: %d",  totalRounds_Stop[u8PwmId]);
}
#endif
...
int DrvPWMGroupStop(struct mstar_pwm_chip *ms_chip, U8 u8GroupId, U8 u8Val)
{
    ...
    for (idx = (u8GroupId * PWM_PER_GROUP); idx < ((u8GroupId+1) * PWM_PER_GROUP); idx++)
    {
        if ((u8Val) && (idx < PWM_NUM))
        {
            //Keep round
            #if 0
            DrvPWMGroupGetRoundNum(ms_chip, idx, &round_set);
            totalRounds_Stop[idx]=round_set;
            #endif
...
int DrvPWMSetEnd(struct mstar_pwm_chip *ms_chip, U8 u8Id, U8 u8DutyId, U32 u32Val)
{
    ...
    if(u32Val < (0xFFFFFFFF / pwmclk))
    {
        u32Duty= (pwmclk * u32Val) / (u16Div * common);

        if((((pwmclk * u32Val) % (u16Div * common)) > (u16Div * common / 2)) || (u32Duty == 0))
        {
            u32Duty++;
        }
        _pwmDuty_ns[u8Id] = (u32Duty * u16Div * common) / pwmclk;
    ...
    {
        u32Duty= (u32Val / u16Div) * pwmclk / common;
        u32Duty++;
...
int DrvPWMSetBegin(struct mstar_pwm_chip *ms_chip, U8 u8Id, U8 u8ShftId, U32 u32Val)
{
    ...
    if(u32Val < (0xFFFFFFFF / pwmclk))
    {
        u32Shift= (pwmclk * u32Val) / (u16Div * common);

        if((((pwmclk * u32Val) % (u16Div * common)) > (u16Div * common / 2)) || (u32Shift == 0))
        {
            u32Shift++;
        }
    }
    ...
    {
        u32Shift= (u32Val / u16Div) * pwmclk / common;
        u32Shift++;
    }
...
#ifdef CONFIG_PWM_NEW
void DrvPWMSetPeriodEx(struct mstar_pwm_chip *ms_chip, U8 u8Id, U32 period)
{
    U8 i;
    U16 u16Div = 0;
    U32 common = 0;
  ...
  //matt
    _pwmDiv[u8Id] = u16Div;
    _pwmPeriod[u8Id] = u32Period;
    _pwmFreq[u8Id]=1000000000/period;
    _pwmStopFreq[u8Id]=1000000000/period;
    isSync=0;
...
/*Get pwm's round number*/
//int DrvPWMGetRoundNum(int pwmID)
//{
//    int i;
//    U32 u32GroupRound;
//    int PerRound=0;
...
//    for (i = 0; i < PWM_NUM; i++)
//    {
//        u32GroupRound = INREG16(ms_chip->base + (((pwmID/4) << 0x7) + 0x40)) & 0xFFFF;
//        PerRound = (timeStop-timeStart[pwmID/4])*_pwmFreq[pwmID];
//        PerRound /= 1000000000;
//        PerRound = (PerRound > u32GroupRound) ? u32GroupRound : PerRound;
//    }
//    return PerRound;
//}
...
int DrvPWMGroupShowRoundNum(struct mstar_pwm_chip *ms_chip, char* buf_start, char* buf_end)
{
    ...
    for (i = 0; i < PWM_NUM; i++)
    {
        u32GroupRound = INREG16(ms_chip->base + (((i>>2) << 0x7) + 0x40)) & 0xFFFF;
        timeVal = (timeStop-timeStart[i/4])*_pwmFreq[i];
        timeVal /= 1000000000;
        timeVal = (timeVal > u32GroupRound) ? u32GroupRound : timeVal;
        DrvPWMGroupIsEnable(ms_chip, i/4, &enable_status);

...
BOOL MDrv_SPINAND_Init(SPINAND_FLASH_INFO_t *tSpinandInfo)
{
    ...
    if (tSpinandInfo->au8_ID[0] == MID_FORESEE)
    {
...
//    HAL_SPINAND_PLANE_HANDLER(u32_PageIdx);
    ...
    if (u32Ret == ERR_SPINAND_SUCCESS)
    {
        if((pSpiNandDrv->tSpinandInfo.au8_ID[0] == 0xA1) &&
           (pSpiNandDrv->tSpinandInfo.au8_ID[1] == 0xF1))
        {
            if ((u8Status & ECC_STATUS_3BIT) == ECC_STATUS_3BIT_ERR)
            {
                u32Ret = ERR_SPINAND_ECC_ERROR;
                printk("ecc error P: 0x%lx\r\n", u32_PageIdx);
            }
            else if ((u8Status & ECC_STATUS_3BIT) != ECC_STATUS_3BIT_PASS)
            {

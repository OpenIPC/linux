...
static struct nand_bbt_descr spi_nand_bbt_descr = {.options = NAND_BBT_2BIT | NAND_BBT_LASTBLOCK | NAND_BBT_VERSION
...
static struct nand_bbt_descr spi_nand_bbt_main_descr = {.options = NAND_BBT_LASTBLOCK | NAND_BBT_CREATE | NAND_BBT_WRITE
...
static struct nand_bbt_descr spi_nand_bbt_mirror_descr = {.options = NAND_BBT_LASTBLOCK | NAND_BBT_CREATE
...
u16 spi_nand_read_word(struct mtd_info *mtd)
{
    ...
    spi_nand_debug("");
    u16_word = ((u16)gtSpiNandDrv.pu8_sparebuf[gtSpiNandDrv.u32_column]
                | ((u16)gtSpiNandDrv.pu8_sparebuf[gtSpiNandDrv.u32_column + 1] << 8));
...
int spi_nand_write_page(struct mtd_info *mtd, struct nand_chip *chip, uint32_t offset, int data_len, const uint8_t *buf,
...
int spi_nand_ecc_read_page_raw(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
    U32 ret;
    U8 *u8_DmaBuf = buf;
...
int spi_nand_ecc_read_page(struct mtd_info *mtd, struct nand_chip *chip, uint8_t *buf, int oob_required, int page)
{
    U32 ret;
    U8 *u8_DmaBuf = buf;
...
static void _addSstarPartition(struct mtd_info *mtd, SPI_NAND_PARTITION_INFO_t *pPartInfo)
{
    ...
    while (pRecord - pPartInfo->records < pPartInfo->u16_PartCnt)
    {
        ...
        if (pRecord - pPartInfo->records >= nMaxRec)
            break;
        ...
        switch (nPartTypeNoFlag)
        {
            ...
            case UNFD_PART_MISC:
                ...
                break;
            default:
                if (nPartTypeNoFlag >= UNFD_PART_CUST0 && nPartTypeNoFlag <= UNFD_PART_CUSTf)
                {
        ...
        printk("%s:%llX, %llX\n", partition_info[NumOfPart].name, partition_info[NumOfPart].offset,
    ...
    if (pRecord->u16_PartType == 0xC000)
    {
        ...
        partition_info[NumOfPart].offset = (U32)(pRecord->u16_StartBlk * u32_BlkSize);
        partition_info[NumOfPart].size =
            (U32)((gtSpiNandDrv.tSpinandInfo.u16_BlkCnt - pRecord->u16_StartBlk) * u32_BlkSize);
        partition_info[NumOfPart].mask_flags = 0;
        printk("%s:%llX, %llX\n", partition_info[NumOfPart].name, partition_info[NumOfPart].offset,
               partition_info[NumOfPart].size);
        NumOfPart++;
...
int _ms_mtd_param_init(struct mtd_info *mtd, struct nand_chip *chip, int *maf_id, int *dev_id,
                       const struct nand_flash_dev *type)
{
    ...
    chip->chipsize = (uint64_t)pNandDrv->tSpinandInfo.u16_BlkCnt * (uint64_t)pNandDrv->tSpinandInfo.u16_BlkPageCnt
                     * (uint64_t)pNandDrv->tSpinandInfo.u16_PageByteCnt;
...
static int mstar_spinand_probe(struct platform_device *pdev)
{
    ...
    {
        u32_ret = MDrv_SPINAND_Read(u8_i * gtSpiNandDrv.tSpinandInfo.u16_BlkPageCnt, gtSpiNandDrv.pu8_pagebuf,
                                    gtSpiNandDrv.pu8_sparebuf);
        if (u32_ret == ERR_SPINAND_SUCCESS)
        {
            if (memcmp((const void *)gtSpiNandDrv.pu8_pagebuf, SPINAND_FLASH_INFO_TAG, 16) == 0)
            {
                ...
                u32_ret = MDrv_SPINAND_Read(u8_i * gtSpiNandDrv.tSpinandInfo.u16_BlkPageCnt + 1,

...
int generate_aj_packet(char **buf, int dataLen, int packetid)
{
    ...
    //      "+String.format("0x%02X,0x%02X,0x%02X,0x%02X",pkt[4],pkt[5],pkt[6],pkt[7]));
    ...
    ;
    // System.out.println("dataLen="+dataLen+", "+String.format("0x%02X,0x%02X,0x%02X,0x%02X, 0x%08X,
    // 0x%02X,0x%02X,0x%02X,0x%02X",pkt[4],pkt[5],pkt[6],pkt[7],ckm,pkt[dataLen+4],pkt[dataLen+5],pkt[dataLen+6],pkt[dataLen+7]));
    ...
    while ((option = getopt(argc, argv, "p:s:c:f")) != -1)
    {
        switch (option)
        {
            ...
            case 'p':
                ...
                break;
            case 'c':
                count = atoi(optarg); // test packet count
                break;

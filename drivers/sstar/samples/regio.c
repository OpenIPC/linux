...
int main(int argc, char *argv[])
{
    ...
    if (((unsigned long)(reg_map = mmap(NULL, reg_map_info.size, PROT_READ | PROT_WRITE, MAP_SHARED, mem_fd,
                                        reg_map_info.addr)))
    ...
    if (0 == strncmp("-r", argv[1], 4))
    {
        ...
        for (i = 0; i < rlen; i += 8)
        {
            printf("%08X:%04X %04X %04X %04X %04X %04X %04X %04X\n",
                   (unsigned int)(reg_map_info.addr + roffset + i * sizeof(int)), (unsigned short)(*(p + 0)),
                   (unsigned short)(*(p + 1)), (unsigned short)(*(p + 2)), (unsigned short)(*(p + 3)),
                   (unsigned short)(*(p + 4)), (unsigned short)(*(p + 5)), (unsigned short)(*(p + 6)),
                   (unsigned short)(*(p + 7)));

...
#define REGW(base, idx, val)                                                        \
    do                                                                              \
    {                                                                               \
        HAL_MSG(HAL_MSG_DBG, "write 0x%08X = 0x%04X\n", ((uint)base + (idx)), val); \
        ms_writel(val, ((uint)base + (idx)));                                       \
    } while (0)
#endif
...
void warp_hal_set_image_point(warp_hal_handle* handle, HAL_WARP_CONFIG* config)
{
    ...
    handle->reg_bank.dsua_uvg  = (s32)Chip_MIU_to_Phys(output_data->data[HAL_WARP_IMAGE_PLANE_UV]);
    ; // physical address

    REGW(handle->base_addr, WARP_DLUA_YRGB_REG_ADDR,
         handle->reg_bank.dlua_yrgb);                                          // 0x10, RGBA or Y base address of input
    REGW(handle->base_addr, WARP_DLUA_UV_REG_ADDR, handle->reg_bank.dlua_uvg); // 0x14, UV base address of input
    REGW(handle->base_addr, WARP_DSUA_YRGB_REG_ADDR,
         handle->reg_bank.dsua_yrgb);                                          // 0x20, RGBA or Y base address of output
...
void warp_hal_set_output_tile(warp_hal_handle* handle, u16 tile_w, u16 tile_h)
{
    ...
    REGW(handle->base_addr, WARP_OCS_REG_ADDR,
         handle->reg_bank.ocs.overlay); // 0x30, Output tile scanning block height and width
}
/*******************************************************************************************************************
 * warp_hal_set_disp
...
void warp_hal_set_pers_matirx(warp_hal_handle* handle, HAL_WARP_CONFIG* config)
{
    ...
    REGW(handle->base_addr, WARP_PDC_C00_REGS_ADDR,
         handle->reg_bank.c00); // 0x48, perspective transform coefficient(0x48~0x68)

...
static u32 warp_lxdrv_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    poll_wait(filp, &file_data->wait_queue,
              wait); // add file_data to wait list, then block until HW finish (isr_post_process)
...
static s32 warp_lxdrv_probe(struct platform_device *pdev)
{
    ...
    dev = device_create(g_warp_drv.class, NULL, MKDEV(g_warp_drv.major, g_warp_drv.minor_star + g_warp_drv.reg_count),
                        dev_data, WARP_LXDRV_NAME);
...
static struct platform_driver warp_lxdrv_driver = {.probe   = warp_lxdrv_probe,

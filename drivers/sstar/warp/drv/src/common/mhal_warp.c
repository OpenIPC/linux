...
static void dump_config(MHAL_WARP_CONFIG *config)
{
    ...
    DEV_MSG(DEV_MSG_DBG, ".disp_table = {%d, %d, %d, 0x%p},\n", config->disp_table.resolution,
            config->disp_table.format, config->disp_table.size, (void *)(MS_U32)config->disp_table.table);
    DEV_MSG(DEV_MSG_DBG, ".bb_table = {%d, 0x%p},\n", config->bb_table.size, (void *)(MS_U32)config->bb_table.table);
    DEV_MSG(DEV_MSG_DBG, ".coeff = {%d, %d, %d, %d, %d, %d, %d, %d, %d},\n", config->coeff[0], config->coeff[1],
            config->coeff[2], config->coeff[3], config->coeff[4], config->coeff[5], config->coeff[6], config->coeff[7],
            config->coeff[8]);
    DEV_MSG(DEV_MSG_DBG, ".input_image = {%d, %d, %d},\n", config->input_image.format, config->input_image.width,
            config->input_image.height);
    DEV_MSG(DEV_MSG_DBG, ".input_data = {%d, 0x%p, 0x%p},\n", config->input_data.num_planes,
            (void *)(MS_U32)config->input_data.data[0], (void *)(MS_U32)config->input_data.data[1]);
    DEV_MSG(DEV_MSG_DBG, ".output_image = {%d, %d, %d},\n", config->output_image.format, config->output_image.width,
            config->output_image.height);
    DEV_MSG(DEV_MSG_DBG, ".output_data = {%d, 0x%p, 0x%p},\n", config->output_data.num_planes,
            (void *)(MS_U32)config->output_data.data[0], (void *)(MS_U32)config->output_data.data[1]);
...
MS_S32 MHAL_WARP_CreateDevice(MS_U32 id, MHAL_WARP_DEV_HANDLE *device)
{
    ...
    ret = warp_dev_create(device, BANK_CWVA_WARP, BANK_CEVA_SYS0, BANK_CEVA_BUS0, BANK_CEVA_BUS0 + 0x100,
                          BANK_CEVA_BUS1, BANK_CEVA_BUS1 + 0x100);
    if (ret != 0)
    {
...
MS_S32 MHAL_WARP_Trigger(MHAL_WARP_INST_HANDLE instance, MHAL_WARP_CONFIG *config, MHAL_WARP_CALLBACK callback,
                         void *user_data)
{
    mhal_warp_instance *  inst_handle = (mhal_warp_instance *)instance;

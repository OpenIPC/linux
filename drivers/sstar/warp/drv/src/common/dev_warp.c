...
static WARP_DEV_PROC_STATE_E warp_dev_check_config(MHAL_WARP_CONFIG *config)
{
    ...
    if (NULL == config || NULL == input_data || NULL == output_data || NULL == disp_table || NULL == bound_box)
    {
    ...
    if (WARP_MIN_IMAGE_WIDTH > input_image->width || WARP_MIN_IMAGE_HEIGHT > input_image->height
        || WARP_MAX_IMAGE_WIDTH < input_image->width || WARP_MAX_IMAGE_HEIGHT < input_image->height)
    {
        return WARP_DEV_PROC_STATE_PARAM_ERROR;
    }
    if (WARP_MIN_IMAGE_WIDTH > output_image->width || WARP_MIN_IMAGE_HEIGHT > output_image->height
        || WARP_MAX_IMAGE_WIDTH < output_image->width || WARP_MAX_IMAGE_HEIGHT < output_image->height)
    {
        return WARP_DEV_PROC_STATE_PARAM_ERROR;
    }
 ...
 *   none
 */
static void warp_dev_add_request(warp_dev_handle *handle, MHAL_WARP_CONFIG *config, WARP_DEV_POST_CALLBACK callback,
                                 void *user_data)
{
 ...
 *   WARP_DEV_PROC_STATE_E
 */
WARP_DEV_PROC_STATE_E warp_dev_trigger(MHAL_WARP_DEV_HANDLE device, MHAL_WARP_CONFIG *config,
                                       WARP_DEV_POST_CALLBACK callback, void *user_data)
{
...
WARP_DEV_STATE warp_dev_isr_proc(MHAL_WARP_DEV_HANDLE device)
{
    ...
    DEV_MSG(DEV_MSG_WRN, "Warp proc %lld us\n",
            (long long)gT_end.tv_sec * 1000000 + (long long)gT_end.tv_nsec / 1000 - (long long)gT_start.tv_sec * 1000000
                - (long long)gT_start.tv_nsec / 1000);
#endif
    ...
    if (handle->hal_handle.reg_bank.sta.fields.axierr) // we should use HAL function, not check register directly
    {
        DEV_MSG(DEV_MSG_ERR, "AXI ERROR: 0x%X (0x%X), status = 0x%X, err = 0x%X\n", irq_ceva, handle->ceva_irq,
                irq_status, handle->hal_handle.reg_bank.sta.fields.axierr);
 ...
 *   none
 */
s32 warp_dev_create(MHAL_WARP_DEV_HANDLE *device, phys_addr_t base_warp, phys_addr_t base_sys,
                    phys_addr_t base_axi2miu0, phys_addr_t base_axi2miu1, phys_addr_t base_axi2miu2,

...
int camdriver_gpio_set_config(struct gpio_chip *chip, unsigned offset, unsigned long config)
{
    if ((config & (0x00FF)) == PIN_CONFIG_INPUT_DEBOUNCE)
    {
        return MDrv_GPIO_Set_Debounce(offset, (config >> 8) * 1000);
    }
    else if ((config & (0x00FF)) == PIN_CONFIG_PERSIST_STATE)
    {
        // avoid gpio export failure
        return 0;
    }
    else
    {
        printk("This pin config is not supported\n");
        return -EINVAL;
    }
}
...
static struct gpio_chip camdriver_gpio_chip = {
    ...
    .set_config       = camdriver_gpio_set_config,
    .base             = 0,
};

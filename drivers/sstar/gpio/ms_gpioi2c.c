...
static int ms_gpioi2c_probe(struct platform_device *pdev)
{
    ...
    if (0 != of_property_read_u32(pdev->dev.of_node, "sda-pad", &sda))
        return -EINVAL;

    if (0 != of_property_read_u32(pdev->dev.of_node, "scl-pad", &scl))
        return -EINVAL;
    ...
    printk("[gpioi2c] sda-pad=%d, scl-pad=%d\n", sda, scl);
...
static const struct of_device_id ms_gpioi2c_of_match_table[] = {{.compatible = "sstar,infinity-gpioi2c"}, {}};

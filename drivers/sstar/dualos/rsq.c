...
void *init_rsq_sndr(void *rqbuf, unsigned int size, unsigned int oobsize, char *name, int datatype)
{
    ...
    rvr->size = size - (unsigned int)((unsigned long)rvr - (unsigned long)rqbuf);
    if (rvr->size < (SIZEOF_RESERVIOR + oobsize))
        return NULL;
    rvr->me       = (u64)(unsigned long)rvr;
    rvr->slotno   = 0;
    rvr->headno   = 0;
    rvr->dropcnt  = 0;
    rvr->dropfrms = 0;
    ...
    prsq            = &(rvr->rsq);
    prsq->woff      = 0;
    prsq->sndr_sbuf = (u64)(unsigned long)((unsigned char *)rvr + SIZEOF_RESERVIOR);
    prsq->sndr_end  = (u64)(unsigned long)((unsigned char *)rvr + rvr->size);
    prsq->sndr_oob  = prsq->sndr_end - oobsize;
    /* init write and read pointer */
    prsq->sndr_wp = prsq->sndr_sbuf;
    prsq->sndr_rp = prsq->sndr_sbuf;

    prsq->wrno = prsq->rdno = rvr->slotno;
    prsq->head = prsq->tail = 0;
    return rvr;
}
...
void init_rsq_rcvr(reservoir_t *rvr)
{
    ...
    prsq       = &(rvr->rsq);
    prsq->woff = (u64)(unsigned long)rvr - rvr->me;
...
void reset_rsq(reservoir_t *rvr)
{
    ...
    slot      = (slot_t *)(unsigned long)prsq->sndr_wp;
    slot->tag = 0;
    rvr->resetcnt++;
}
...
unsigned int check_rsqslot(reservoir_t *rvr, unsigned int expect)
{
    ...
    if (!rvr)
        return 0;
    Chip_Inv_Cache_Range((void *)rvr, sizeof(reservoir_t));
    prsq = &(rvr->rsq);

    rp  = (unsigned char *)(unsigned long)prsq->sndr_rp;
    ptr = (unsigned char *)(unsigned long)prsq->sndr_wp;
...
unsigned int gc_rsqslot(reservoir_t *rvr, unsigned int expect)
{
    ...
    if ((ptr <= rp) && (prsq->rdno != prsq->wrno) && ((rp - ptr) < nws))
    {
        ...
        do
        {
            slot_t *s = (slot_t *)(unsigned long)(prsq->sndr_rp);

            p = rp + SIZEOF_SLOTHEADER + slot->bs;
            if (p >= (unsigned char *)(unsigned long)prsq->sndr_oob)
            {
                /* |--R---------------WdOddddp-----|  */
                /* data in oob, wrap to head of buffer*/
                pns = (unsigned char *)(unsigned long)prsq->sndr_sbuf;
            }
            else
            {
                /* |--R--------Wddddp-R-O----------|  */
            ...
            prsq->sndr_rp = (u64)(unsigned long)pns;
            rp            = pns;
            slot          = (slot_t *)rp;
            /* for check only */
            if (slot->tag != RQ_TAG)
            {
                CamOsPrintf(KERN_ERR "DO DROP 0x%x mark %d to 0x%x %s\n", (unsigned long)s, s->mark,
                            (unsigned long)prsq->sndr_rp, slot->tag == RQ_TAG ? "OK" : "BAD");
            }

            df += (slot->no - s->no);
            prsq->rdno = rvr->headno = slot->no; /* oldest no. in rvr */
    ...
    Chip_Flush_Cache_Range((void *)rvr, sizeof(reservoir_t));
...
void *begin_rsqslot(reservoir_t *rvr, unsigned int expect)
{
    ...
    slot = (slot_t *)(unsigned long)(prsq->sndr_wp);
    Chip_Inv_Cache_Range((void *)slot, sizeof(slot_t));
    slot->rese = (u64)(unsigned long)rvr;
    Chip_Flush_Cache_Range((void *)slot, sizeof(slot_t));
    /* |-------Wssssssss-R--O----------|  */
    /* sbuf   ptr        ^ oob        end */
...
unsigned int end_rsqslot(void *wp, unsigned int ws, unsigned int mark, unsigned int ts)
{
    ...
    Chip_Inv_Cache_Range((void *)slot, sizeof(slot_t));
    rvr = (reservoir_t *)(unsigned long)slot->rese;
    Chip_Inv_Cache_Range((void *)rvr, sizeof(reservoir_t));
    prsq = &(rvr->rsq);

    /* for debug/verify only */
    if ((void *)slot != (void *)(unsigned long)prsq->sndr_wp)
    {
        /* wp is invalid */
    ...
    if (             /* this is made sure by begin_rsqslot() */
        ...
        (ptr > (unsigned char *)(unsigned long)prsq->sndr_end))
    {
        /* |-----------WdddRddp-O----------|  */
        /* |----------------WdddOdddddddddddp */
        /* overwrite!! */
    ...
    if (ptr >= (unsigned char *)(unsigned long)prsq->sndr_oob)
    {
        /* |--R---------------WdOddddp-----|  */
        /* data in oob, wrap to head of buffer*/
        prsq->sndr_wp = prsq->sndr_sbuf;
    ...
    {
        /* |--R--------Wddddp-R-O----------|  */
        prsq->sndr_wp = (u64)(unsigned long)ptr;
    }
...
unsigned int rcvr_de_rsqslot(reservoir_t *rvr, void *buf, unsigned int size, slot_t *out)
{
    ...
    memcpy(out, (unsigned char *)(unsigned long)prsq->rcvr_rp, sizeof(slot_t));
    rp = (unsigned char *)(unsigned long)prsq->rcvr_rp + out->sc;
    ...
    if (buf && size > out->dc)
    {
        ...
        rp += out->bs;
        prsq->rcvr_rp =
            (rp >= (unsigned char *)(unsigned long)prsq->rcvr_oob) ? prsq->rcvr_sbuf : (u64)(unsigned long)rp;
...
unsigned int sndr_de_rsqslot(reservoir_t *rvr, void *buf, unsigned int size, slot_t *out)
{
    ...
    memcpy(out, (unsigned char *)(unsigned long)prsq->sndr_rp, sizeof(slot_t));
    rp = (unsigned char *)(unsigned long)prsq->sndr_rp + out->sc;
    ...
    if (buf && size > out->dc)
    {
        ...
        rp += out->bs;
        prsq->sndr_rp =
            (rp >= (unsigned char *)(unsigned long)prsq->sndr_oob) ? prsq->sndr_sbuf : (u64)(unsigned long)rp;

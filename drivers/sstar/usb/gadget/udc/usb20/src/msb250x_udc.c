...
static irqreturn_t msb250x_udc_link_isr(struct msb250x_udc *dev)
{
    ...
    u16 tx_cur = ms_readw(GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
#endif
    ...
    if (1 == usb_st->bReset)
    {
...
#ifndef CONFIG_USB_FPGA_VERIFICATION
        // TX-current adjust to 105%
        // tx-swing:[6:4], de-emphasis:[9:7] => [4] set 1, [9] set 1
        ms_writew(0x0210 | (tx_cur & 0xFC0F), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
#endif
        ...
        if (1 == pst_power->bHSMode)
        {
            ...
            if (!tx_swing && !dem_cur)
            {
                ms_writew(0x0230 | (tx_cur & 0xFC0F), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
            }
            else
            {
                ms_writew(tx_cur, GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16)); // write back
            }
#endif
        }
        else
...
#ifndef CONFIG_USB_FPGA_VERIFICATION
            ms_writew(0x0030 | (tx_cur & 0xFC0F), GET_REG16_ADDR(UTMI_BASE_ADDR, 0x16));
#endif
        }
...
void tx_swing_set(u8 value)
{
    ...
    {
        printk(KERN_INFO "unsupport tx swing value\n");
    }
}
...
void cm_current_set(u8 value)
{
    ...
    {
        printk(KERN_INFO "unsupport cm current value\n");
    }
}
...
void de_emphasis_current_set(u8 value)
{
    ...
    {
        printk(KERN_INFO "unsupport de_emphasis_current value\n");
    }
}

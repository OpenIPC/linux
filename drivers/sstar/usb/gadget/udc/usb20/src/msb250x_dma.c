...
int msb250x_dma_setup_control(struct usb_ep* _ep, struct msb250x_request* req, u32 bytes)
{
    s8 ch   = 0;
    u8 csr1 = 0;
    u8 csr2 = 0;
    ...
    ms_writeb(csr2, ((usb_endpoint_dir_out(_ep->desc)) ? MSB250X_OTG0_EP_RXCSR2_REG(ep_num)
                                                       : MSB250X_OTG0_EP_TXCSR2_REG(ep_num)));
#if 0
...
#else
    dma_addr = req->req.dma + req->req.actual;
    ms_writelw((dma_addr & 0xFFFFFFFF), MSB250X_OTG0_DMA_ADDR(ch));
    if ((dma_addr >> 31) & 0x1)
        ms_writeb((ms_readb(USBC_BASE_ADDR) | 0x10), USBC_BASE_ADDR);
    else
        ms_writeb((ms_readb(USBC_BASE_ADDR) & 0xEF), USBC_BASE_ADDR);
    dma_extend_addr = (dma_addr >> 28) & 0xF0;
    ms_writeb(((ms_readb(MSB250X_USBC_CTRH_REG) & (~(0xF0))) | dma_extend_addr), MSB250X_USBC_CTRH_REG);
...
void msb250x_dma_isr_handler(u8 ch, struct msb250x_udc* dev)
{
    ...
    dma_extend_addr = (ms_readb(MSB250X_USBC_CTRH_REG) & 0xF0);
    ...
    if (req)
    {
        ...
        if (req->req.actual == req->req.length || 0 < (bytesdone % ep_maxpacket))
        {
            is_last = 1;
        }
#if 0

...
int __msb250x_ep_queue(struct usb_ep *_ep, struct usb_request *_req)
{
    struct msb250x_request* req = to_msb250x_req(_req);
    struct msb250x_ep* ep = to_msb250x_ep(_ep);
    struct msb250x_udc* dev = NULL;
...
int msb250x_ep_queue(struct usb_ep *_ep, struct usb_request *_req, gfp_t gfp_flags)
{
    ...
    ret = __msb250x_ep_queue(_ep, _req);

    if (ret == 0 && _req && _req->zero && _req->length && (_req->length % _ep->maxpacket == 0))
    {
        zlp_req = msb250x_ep_alloc_request(_ep, gfp_flags);
        zlp_req->length = 0;
        zlp_req->complete = msb250x_zlp_complete;
        zlp_req->buf = dev->zlp_buf;
        ret = __msb250x_ep_queue(_ep, zlp_req);
    }

			   ...
			   struct device *alloc_devs[])
{
	...
	*nplanes = 1;
	...
	if (opts->bulk_streaming_ep)
		video->uvc_num_requests = UVC_BULK_NUM_REQUESTS;
	else
		video->uvc_num_requests = UVC_ISOC_NUM_REQUESTS;
		    ...
		    enum v4l2_buf_type type, struct mutex *lock)
{
	...
	if (cdev->gadget->sg_supported && !opts->bulk_streaming_ep) {
	...
	queue->queue.timestamp_flags =
		V4L2_BUF_FLAG_TIMESTAMP_COPY | V4L2_BUF_FLAG_TSTAMP_SRC_EOF;
	queue->queue.dev = dev;
...
void uvcg_complete_sg(struct uvc_video_queue *queue)
{
	...
	list_for_each_entry (buf, &queue->irqqueue, queue) {
		if (buf->state == UVC_BUF_STATE_DONE) {
			uvcg_queue_next_buffer(queue, buf);
			return;
		}
	}
}

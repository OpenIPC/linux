...
void usb_bc_enable(struct usb_hcd *hcd, bool enable)
{
    ...
    {
        printk("BC enable \n");
        writeb(readb((void *)(hcd->utmi_base + (0x1 * 2 - 1))) | 0x40,
               (void *)(hcd->utmi_base + (0x1 * 2 - 1))); // IREF_PDN=1¡¦b1. (utmi+0x01[6] )
        writeb(readb((void *)(hcd->bc_base + (0x3 * 2 - 1))) | 0x40,
               (void *)(hcd->bc_base + (0x3 * 2 - 1))); // [6]= reg_host_bc_en
        writeb(readb((void *)(hcd->bc_base + (0xc * 2))) | 0x40,
               (void *)(hcd->bc_base + (0xc * 2))); // [6]= reg_into_host_bc_sw_tri
        writew(0x0000, (void *)(hcd->bc_base));     // [15:0] = bc_ctl_ov_en
        writeb(readb((void *)(hcd->bc_base + (0xa * 2))) | 0x80,
               (void *)(hcd->bc_base + (0xa * 2))); // [7]=reg_bc_switch_en
    ...
    else
    {
        // disable BC
        printk("BC disable \n");
        writeb(readb((void *)(hcd->bc_base + (0xc * 2))) & (u8)(~0x40),
               (void *)(hcd->bc_base + (0xc * 2))); // [6]= reg_into_host_bc_sw_tri
        writeb(readb((void *)(hcd->bc_base + (0x3 * 2 - 1))) & (u8)(~0x40),
               (void *)(hcd->bc_base + (0x3 * 2 - 1))); // [6]= reg_host_bc_en
        writeb(readb((void *)(hcd->utmi_base + (0x1 * 2 - 1))) & (u8)(~0x40),
               (void *)(hcd->utmi_base + (0x1 * 2 - 1))); // IREF_PDN=1¡¦b1. (utmi+0x01[6] )
...
void usb_power_saving_enable(struct usb_hcd *hcd, bool enable)
{
    if (enable)
    {
        ...
        if (hcd->utmi_base != 0)
        {
            //([0]: power down override, [1]:Termination override, [6]:15Kohm pull low for dp, [7] :15Kohm pull low for
            // dm)
            ...
            writeb(readb((void *)(hcd->utmi_base + 0x1 * 2 - 1)) | (BIT2 | BIT3 | BIT6),
                   (void *)(hcd->utmi_base + 0x1 * 2 - 1));
        }
    ...
    else
    {
        ...
        if (hcd->utmi_base != 0)
        {
            ...
            writeb(readb((void *)(hcd->utmi_base + 0x52 * 2)) & (u8)(~(BIT5 | BIT4 | BIT3 | BIT2 | BIT1 | BIT0)),
                   (void *)(hcd->utmi_base + 0x52 * 2));
            //([2]:TED power down, [3]Preamp power down, [6]TX power down)
            writeb(readb((void *)(hcd->utmi_base + 0x1 * 2 - 1)) & (u8)(~(BIT2 | BIT3 | BIT6)),
                   (void *)(hcd->utmi_base + 0x1 * 2 - 1));
        }
...
void usb30mac_sram_power_saving(struct usb_hcd *hcd, bool enable)
{
    ...
    if (enable)
    {
        ...
        writew(readw((void *)USB3_MAC_SRAM_CTRL_ADDR(hcd)) | (u16)USB3_MAC_SRAM_CTRL_BIT(hcd),
               (void *)USB3_MAC_SRAM_CTRL_ADDR(hcd));
    }
    else
    {
...
void usb20mac_sram_power_saving(struct usb_hcd *hcd, bool enable)
{
    ...
    if (enable)
    {
        ...
        writew(readw((void *)USB_MAC_SRAM_CTRL_ADDR(hcd)) | (u16)USB_MAC_SRAM_CTRL_BIT(hcd),
               (void *)USB_MAC_SRAM_CTRL_ADDR(hcd));
    }
    else
    {
    ...
    // USB_MAC_SRAM_CTRL_BIT(hcd));
}
EXPORT_SYMBOL_GPL(usb20mac_sram_power_saving);
#endif
void dma_ehci_enable_over4GB(struct usb_hcd *hcd, dma_addr_t dma)
{
    u8 dma_extend_addr;

    if ((dma >> 31) & 0x1)
        writeb((readb((void *)(hcd->usbc_base)) | 0x10), (void *)hcd->usbc_base);
    else
        writeb((readb((void *)(hcd->usbc_base)) & 0xEF), (void *)hcd->usbc_base);
    dma_extend_addr = (dma >> 28) & 0xF0;
    writeb(((readb((void *)(hcd->usbc_base + 0x1 * 2 - 1)) & (~(0xF0))) | dma_extend_addr),
           (void *)(hcd->usbc_base + 0x1 * 2 - 1));
}
EXPORT_SYMBOL_GPL(dma_ehci_enable_over4GB);

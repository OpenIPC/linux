...
static ssize_t _MDrv_NOE_LRO_PROC_Write_Auto_Tlb(struct file *file, const char __user *buffer, size_t count,
                                                 loff_t *data);
...
static const LRO_PROC_SET lro_proc_set[] = {[E_AGG_CNT]      = _MDrv_NOE_LRO_PROC_Set_Agg_Cnt,
...
static const char lro_proc_name[E_LRO_PROC_MAX][16] = {NOE_PROC_LRO_RX_RING1, NOE_PROC_LRO_RX_RING2,
                                                       NOE_PROC_LRO_RX_RING3, NOE_PROC_HW_LRO_STATS,
                                                       NOE_PROC_HW_LRO_AUTO_TLB};

static const struct file_operations lro_file_fops[E_LRO_PROC_MAX] = {
    [E_LRO_PROC_RING1] = {.owner   = THIS_MODULE,
                          .open    = _MDrv_NOE_LRO_PROC_Open_Rx_Ring1,
                          .read    = seq_read,
                          .llseek  = seq_lseek,
                          .release = single_release},
    [E_LRO_PROC_RING2] = {.owner   = THIS_MODULE,
                          .open    = _MDrv_NOE_LRO_PROC_Open_Rx_Ring2,
                          .read    = seq_read,
                          .llseek  = seq_lseek,
                          .release = single_release},
    [E_LRO_PROC_RING3] = {.owner   = THIS_MODULE,
                          .open    = _MDrv_NOE_LRO_PROC_Open_Rx_Ring3,
                          .read    = seq_read,
                          .llseek  = seq_lseek,
                          .release = single_release},
    [E_LRO_PROC_STATS] = {.owner   = THIS_MODULE,
                          .open    = _MDrv_NOE_LRO_PROC_Open_Stats,
                          .read    = seq_read,
                          .llseek  = seq_lseek,
                          .write   = _MDrv_NOE_LRO_PROC_Write_Stats,
                          .release = single_release},
    [E_LRO_PROC_ATTLB] = {.owner   = THIS_MODULE,
...
void _MDrv_NOE_LRO_PROC_Dump_Auto_Tlb(struct seq_file *seq, unsigned int index)
{
    ...
    if (pdma_lro_auto_tlb.auto_tlb_info8.IPV4)
    {
        seq_printf(seq, "SIP = 0x%x:0x%x:0x%x:0x%x (IPv4)\n", pdma_lro_auto_tlb.auto_tlb_info4.SIP3,
                   pdma_lro_auto_tlb.auto_tlb_info3.SIP2, pdma_lro_auto_tlb.auto_tlb_info2.SIP1,
    ...
    seq_printf(seq, "TPUT = %d | FREQ = %d\n", pdma_lro_auto_tlb.auto_tlb_info7.DW_LEN,
               pdma_lro_auto_tlb.auto_tlb_info6.CNT);
                                                 ...
                                                 loff_t *data)
{
    ...
    if (lro_proc_set[x] && (ARRAY_SIZE(lro_proc_set) > x))
    {
        (*lro_proc_set[x])(y);
    }
...
static int _MDrv_NOE_LRO_PROC_Show_Auto_Tlb(struct seq_file *seq, void *offset)
{
    ...
    for (i = E_NOE_RING_NO1; i <= E_NOE_RING_NO3; i++)
    {
        ...
        seq_printf(seq, "Ring[%d]: MAX_AGG_CNT=%d, AGG_TIME=%d, AGE_TIME=%d, Threshold=%d\n", i, info.agg_cnt,
                   info.agg_time, info.age_time, info.threshold);
    }
...
static int _MDrv_NOE_LRO_PROC_Show_Stats(struct seq_file *seq, void *offset)
{
    ...
    for (i = 0; i <= MAX_HW_LRO_AGGR; i++)
    {
        seq_printf(seq, " %d :      %d        %d        %d        %d\n", i, lro_agg_num_cnt[0][i],
                   ...
                   lro_agg_num_cnt[0][i] + lro_agg_num_cnt[1][i] + lro_agg_num_cnt[2][i]);
    }

    /* Total agg count */
    seq_puts(seq, "Total agg:   RING1 | RING2 | RING3 | Total\n");
    seq_printf(seq, "                %d      %d      %d      %d\n", lro_tot_agg_cnt[0], lro_tot_agg_cnt[1],
               lro_tot_agg_cnt[2], lro_tot_agg_cnt[0] + lro_tot_agg_cnt[1] + lro_tot_agg_cnt[2]);

    /* Total flush count */
    seq_puts(seq, "Total flush:   RING1 | RING2 | RING3 | Total\n");
    seq_printf(seq, "                %d      %d      %d      %d\n", lro_flush_cnt_tot[0], lro_flush_cnt_tot[1],
               lro_flush_cnt_tot[2], lro_flush_cnt_tot[0] + lro_flush_cnt_tot[1] + lro_flush_cnt_tot[2]);
    ...
    seq_printf(seq, "                %d      %d      %d      %d\n",
               ...
               (lro_flush_cnt_tot[0] + lro_flush_cnt_tot[1] + lro_flush_cnt_tot[2])
                   ? ((lro_tot_agg_cnt[0] + lro_tot_agg_cnt[1] + lro_tot_agg_cnt[2])
                      / (lro_flush_cnt_tot[0] + lro_flush_cnt_tot[1] + lro_flush_cnt_tot[2]))
                   : 0);
    ...
    for (i = 0; i < 15; i++)
    {
        seq_printf(seq, "%d~%d: %d      %d      %d      %d\n", i * 5000, (i + 1) * 5000, lro_agg_size_cnt[0][i],
                   lro_agg_size_cnt[1][i], lro_agg_size_cnt[2][i],
                   lro_agg_size_cnt[0][i] + lro_agg_size_cnt[1][i] + lro_agg_size_cnt[2][i]);
    }
    ...
    if (ei_local->features & FE_HW_LRO_DBG)
    {
        ...
        seq_printf(seq, "AGG timeout:      %d      %d      %d      %d\n", lro_flush_cnt_agg[0], lro_flush_cnt_agg[1],
                   lro_flush_cnt_agg[2], (lro_flush_cnt_agg[0] + lro_flush_cnt_agg[1] + lro_flush_cnt_agg[2]));
        seq_printf(seq, "AGE timeout:      %d      %d      %d      %d\n", lro_flush_cnt_age[0], lro_flush_cnt_age[1],
                   lro_flush_cnt_age[2], (lro_flush_cnt_age[0] + lro_flush_cnt_age[1] + lro_flush_cnt_age[2]));
        seq_printf(seq, "Not in-sequence:  %d      %d      %d      %d\n", lro_flush_cnt_unseq[0],
                   lro_flush_cnt_unseq[1], lro_flush_cnt_unseq[2],
                   (lro_flush_cnt_unseq[0] + lro_flush_cnt_unseq[1] + lro_flush_cnt_unseq[2]));
        seq_printf(seq, "Timestamp:        %d      %d      %d      %d\n", lro_flush_cnt_timestamp[0],
                   lro_flush_cnt_timestamp[1], lro_flush_cnt_timestamp[2],
                   (lro_flush_cnt_timestamp[0] + lro_flush_cnt_timestamp[1] + lro_flush_cnt_timestamp[2]));
        seq_printf(seq, "No LRO rule:      %d      %d      %d      %d\n", lro_flush_cnt_norule[0],
                   lro_flush_cnt_norule[1], lro_flush_cnt_norule[2],
                   (lro_flush_cnt_norule[0] + lro_flush_cnt_norule[1] + lro_flush_cnt_norule[2]));
    }
...
static int _MDrv_NOE_LRO_PROC_Read_Ring(struct seq_file *seq, void *offset, struct PDMA_rxdesc *rx_ring_p)
{
    ...
    {
        seq_printf(seq, "%4d: %08x %08x %08x %08x\n", i, *(int *)&rx_ring_p[i].rxd_info1,
                   *(int *)&rx_ring_p[i].rxd_info2, *(int *)&rx_ring_p[i].rxd_info3, *(int *)&rx_ring_p[i].rxd_info4);
    }

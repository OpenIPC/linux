...
#else
#define IS_TRAFFIC_RECEIVED_OFFLOAD(rx_skb, napi) \
    likely(((rx_skb)->ip_summed == CHECKSUM_UNNECESSARY) && ((napi) != NULL))
...
static const char *const _mdrv_noe_clk_source_name[] = {
    ...
    "sgmii_cdr_ref", "sgmii_cdr_fb", "trgpll", "sgmipll", "eth1pll", "eth2pll"};
...
static int MDrv_NOE_TX_Poll(struct napi_struct *napi, int budget)
{
    struct END_DEVICE *ei_local = container_of(napi, struct END_DEVICE, napi_tx);
    struct net_device *netdev   = ei_local->netdev;
...
static irqreturn_t MDrv_NOE_Interrupt(int irq, void *dev_id)
{
    ...
    MHal_NOE_Get_Intr_Status((ei_local->features & FE_QDMA) ? (E_NOE_DMA_PACKET | E_NOE_DMA_QUEUE) : E_NOE_DMA_PACKET,
                             &recv, &transmit);
    ei_local->ei_xmit_housekeeping(dev, NUM_TX_MAX_PROCESS);
    MHal_NOE_Clear_Intr_Status((ei_local->features & FE_QDMA) ? (E_NOE_DMA_PACKET | E_NOE_DMA_QUEUE)
                                                              : E_NOE_DMA_PACKET);
    ...
    if (ei_local->features & FE_IRQ_SEPARATE)
    {
        if (ei_local->features & FE_INT_NAPI)
        {
            if (ei_local->features & FE_INT_NAPI_TX_RX)
            {
                ...
                err1 =
                    request_irq(ei_local->irq1, MDrv_NOE_NAPI_Separate_TX_Interrupt, IRQF_TRIGGER_HIGH, "eth_tx", dev);
                err2 =
                    request_irq(ei_local->irq2, MDrv_NOE_NAPI_Separate_RX_Interrupt, IRQF_TRIGGER_HIGH, "eth_rx", dev);
            }
            else if (ei_local->features & FE_INT_NAPI_RX_ONLY)
            {
                NOE_MSG_DBG("RX NAPI\n");
...
static ssize_t noe_mdio_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    {
        if (reg % 8 == 0)
            str += scnprintf(str, end - str, "%02d: ", reg);

...
bool _MDrv_NOE_QDMA_Alloc_Txd(struct net_device *dev)
{
    ...
    NOE_MSG_DBG("[QUE][TX_DESC] num=%d vir=0x%p phy_txd_pool=0x%p  len=%x\n", NUM_TX_DESC, ei_local->txd_pool,
                (void *)ei_local->phy_txd_pool, QTXD_LEN);
...
bool _MDrv_NOE_QDMA_SFQ_Init(struct net_device *dev)
{
    ...
    int                 i = 0, j = 0;
    int                 vq_num[8] = {
        VQ_NUM0, VQ_NUM1, VQ_NUM2, VQ_NUM3, VQ_NUM4, VQ_NUM5, VQ_NUM6, VQ_NUM7,
    };

    for (i = 0; i < 8; i++)
    {
        sfq[i] = dma_alloc_coherent(&ei_local->qdma_pdev->dev, vq_num[i] * sizeof(struct SFQ_table), &sfq_phy[i],
                                    GFP_KERNEL);
        if (unlikely(!sfq[i]))
        {
...
static int _MDrv_NOE_QDMA_SFQ_Parse_Net_Layer(struct sk_buff *skb)
{
    ...
    if ((sfq_parse_result.eth_type == htons(ETH_P_IP))
        || (sfq_parse_result.eth_type == htons(ETH_P_PPP_SES) && sfq_parse_result.ppp_tag == htons(PPP_IP)))
    {
        ...
        else
        {
            sfq_prot = 1;
             ...
             || (sfq_parse_result.eth_type == htons(ETH_P_PPP_SES) && sfq_parse_result.ppp_tag == htons(PPP_IPV6)))
    {
        ...
        if (ip6h_sfq->nexthdr == NEXTHDR_TCP)
        {
            sfq_prot = 4; /* IPV6_5T */
#if (sfq_debug)
            if (ntohl(sfq_parse_result.ip6h.saddr.s6_addr32[3]) == 8)
                ack_packt = 1;
            else
                ack_packt = 0;
#endif
...
static int MDrv_NOE_QDMA_Eth_Send(struct END_DEVICE *ei_local, struct net_device *dev, struct sk_buff *skb, int gmac_no,
                                  int ring_no)
{
...
static int MDrv_NOE_QDMA_Eth_Tso_Send(struct END_DEVICE *ei_local, struct net_device *dev, struct sk_buff *skb,
                                      int gmac_no, int ring_no)
{
    ...
    Chip_Flush_Cache_Range((unsigned long)skb->data, len);
    ...
    for (i = 0; i < nr_frags; i++)
    {
        ...
        for (frag_txd_num = frag_txd_num; frag_txd_num > 0; frag_txd_num--)
        {
            ...
            cpu_ptr->txd_info1.SDP =
                dma_map_page(&ei_local->qdma_pdev->dev, frag->page.p, frag->page_offset + offset, size, DMA_TO_DEVICE);
    ...
    if (skb_shinfo(skb)->gso_segs > 1)
    {
        ...
        if ((iph->version == 4) && (iph->protocol == IPPROTO_TCP))
        {
            ...
            // dma_sync_single_for_device(&ei_local->qdma_pdev->dev, virt_to_phys(th), sizeof(struct tcphdr),
            // DMA_TO_DEVICE);
...
int MDrv_NOE_QDMA_Init_Rx(struct net_device *dev)
{
    ...
    for (i = 0; i < NUM_QRX_DESC; i++)
    {
        ...
        ei_local->qrx_ring[i].rxd_info1.PDP0 =
            dma_map_single(&ei_local->qdma_pdev->dev, ei_local->netrx0_skbuf[i]->data, MAX_RX_LENGTH + NET_IP_ALIGN,
                           PCI_DMA_FROMDEVICE);

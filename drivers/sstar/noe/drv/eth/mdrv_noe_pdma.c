...
int MDrv_NOE_PDMA_Init_Rx(struct net_device *dev)
{
    ...
    skb_size =
        SKB_DATA_ALIGN(MAX_RX_LENGTH + NET_IP_ALIGN + NET_SKB_PAD) + SKB_DATA_ALIGN(sizeof(struct skb_shared_info));
    ...
    for (i = 0; i < NUM_RX_DESC; i++)
    {
        ...
        ei_local->rx_ring[0][i].rxd_info1.PDP0      = dma_map_single(
                 dev->dev.parent, ei_local->netrx_skb_data[0][i] + NET_SKB_PAD, MAX_RX_LENGTH, DMA_FROM_DEVICE);
        ...
        NOE_MSG_DUMP("va=0x%p ,pa =0x%x \n", (ei_local->netrx_skb_data[0][i] + NET_SKB_PAD),
...
int MDrv_NOE_PDMA_Init_Tx(struct net_device *dev)
{
    ...
    ei_local->free_idx     = 0;
    ei_local->tx_ring0     = dma_alloc_coherent(dev->dev.parent, NUM_TX_DESC * sizeof(struct PDMA_txdesc),
                                                &ei_local->phy_tx_ring0, GFP_ATOMIC | __GFP_ZERO);
...
void MDrv_NOE_PDMA_Deinit_Rx(struct net_device *dev)
{
    ...
    //     dma_free_coherent(dev->dev.parent, NUM_RX_DESC * sizeof(struct PDMA_rxdesc), ei_local->rx_ring[0],
...
void MDrv_NOE_PDMA_Deinit_Tx(struct net_device *dev)
{
    ...
    if (ei_local->tx_ring0)
        dma_free_coherent(dev->dev.parent, NUM_TX_DESC * sizeof(struct PDMA_txdesc), ei_local->tx_ring0,
...
static int _MDrv_NOE_PDMA_Fill_Txd(struct net_device *dev, unsigned long *tx_cpu_owner_idx, struct sk_buff *skb,
                                   int gmac_no)
{
    struct END_DEVICE *     ei_local = netdev_priv(dev);
...
static int _MDrv_NOE_PDMA_Tso_Fill_Data(struct END_DEVICE *ei_local, unsigned int frag_offset, unsigned int frag_size,
                                        unsigned long *tx_cpu_owner_idx, unsigned int nr_frags, int gmac_no)
{
...
static int _MDrv_NOE_PDMA_Tso_Fill_Frag(struct net_device *netdev, struct sk_buff *skb, unsigned long *tx_cpu_owner_idx,
                                        int gmac_no)
{
            ...
            if (ei_local->skb_txd_num % 2 == 0)
            {
                *tx_cpu_owner_idx = (*tx_cpu_owner_idx + 1) % NUM_TX_DESC;
                ...
                tx_ring->txd_info1.SDP0 =
                    dma_map_page(netdev->dev.parent, frag->page.p, frag_offset, size, DMA_TO_DEVICE);
...
err_dma:
    ...
    for (i = 0; i < unmap_idx; i++)
    {
        ...
        while (frag_txd_num > 0)
        {
            ...
            {
                j = (j + 1) % NUM_TX_DESC;
                dma_unmap_page(netdev->dev.parent, ei_local->tx_ring0[j].txd_info1.SDP0,
                               ei_local->tx_ring0[j].txd_info2.SDL0, DMA_TO_DEVICE);
                /* reinit txd */
            ...
            {
                dma_unmap_page(netdev->dev.parent, ei_local->tx_ring0[j].txd_info3.SDP1,
                               ei_local->tx_ring0[j].txd_info2.SDL1, DMA_TO_DEVICE);
                /* reinit txd */
...
static int _MDrv_NOE_PDMA_Tso_Fill_Txd(struct net_device *dev, unsigned long *tx_cpu_owner_idx, struct sk_buff *skb,
                                       int gmac_no)
{
    struct END_DEVICE * ei_local = netdev_priv(dev);
    ...
    if (skb_shinfo(skb)->gso_segs > 1)
    {
        ...
        if ((iph->version == 4) && (iph->protocol == IPPROTO_TCP))
        {
            ...
            Chip_Flush_Cache_Range((unsigned long)th, sizeof(struct tcphdr));
        }
...
static inline int MDrv_NOE_PDMA_Eth_Send(struct net_device *dev, struct sk_buff *skb, int gmac_no,
...
int MDrv_NOE_PDMA_Start_Xmit(struct sk_buff *skb, struct net_device *dev, int gmac_no)
{
    ...
    if ((ei_local->skb_free[tx_cpu_owner_idx_next] == 0) && (ei_local->skb_free[tx_cpu_owner_idx] == 0))
    {

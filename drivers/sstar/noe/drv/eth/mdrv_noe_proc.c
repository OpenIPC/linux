...
static ssize_t _MDrv_NOE_PROC_Write_Lan_Ip_Setting(struct file *file, const char __user *buffer, size_t count,
                                                   loff_t *data);
...
static const struct seq_operations _MDrv_NOE_PROC_Set_Skb_free_Ops = {.start = _MDrv_NOE_PROC_Start_Skb_Free,
                                                                      .next  = _MDrv_NOE_PROC_Get_Next_Skb_Free,
                                                                      .stop  = _MDrv_NOE_PROC_Stop_Skb_Free,
                                                                      .show  = _MDrv_NOE_PROC_Show_Skb_Free};

static const struct seq_operations _MDrv_NOE_PROC_Set_Tso_Txd_Num_Ops = {.start = _MDrv_NOE_PROC_Start_Tso_Txd_Num,
                                                                         .next  = _MDrv_NOE_PROC_Get_Next_Tso_Txd_Num,
                                                                         .stop  = _MDrv_NOE_PROC_Stop_Tso_Txd_Num,
                                                                         .show  = _MDrv_NOE_PROC_Show_Tso_Txd_Num};

static const struct seq_operations _MDrv_NOE_PROC_Set_Tso_Len_Ops = {.start = _MDrv_NOE_PROC_Start_Tso_Len,
...
static const char noe_proc_name[E_NOE_PROC_MAX][16] = {
    [E_NOE_PROC_GMAC1] = {NOE_PROC_GMAC},       [E_NOE_PROC_GMAC2] = {NOE_PROC_GMAC2},
    [E_NOE_PROC_SKB_FREE] = {NOE_PROC_SKBFREE}, [E_NOE_PROC_TX_RING] = {NOE_PROC_TX_RING},
    [E_NOE_PROC_RX_RING] = {NOE_PROC_RX_RING},  [E_NOE_PROC_NUM_OF_TXD] = {NOE_PROC_NUM_OF_TXD},
    [E_NOE_PROC_TSO_LEN] = {NOE_PROC_TSO_LEN},  [E_NOE_PROC_LRO_STATS] = {NOE_PROC_LRO_STATS},
    [E_NOE_PROC_SNMP] = {NOE_PROC_SNMP},        [E_NOE_PROC_ESW_CNT] = {NOE_PROC_ESW_CNT},
    [E_NOE_PROC_ETH_CNT] = {NOE_PROC_ETH_CNT},  [E_NOE_PROC_SCHE] = {NOE_PROC_SCHE},
    [E_NOE_PROC_INT_DBG] = {NOE_PROC_INT_DBG},  [E_NOE_PROC_SET_LAN_IP] = {NOE_PROC_SET_LAN_IP},
    [E_NOE_PROC_PIN_MUX] = {NOE_PROC_PIN_MUX},  [E_NOE_PROC_LOG_CTRL] = {NOE_PROC_LOG_CTRL},
    [E_NOE_PROC_QSCH] = {NOE_PROC_QSCH},
};

static const struct file_operations noe_proc_fops[E_NOE_PROC_MAX] = {
    [E_NOE_PROC_GMAC1]      = {.owner   = THIS_MODULE,
                          .open    = _MDrv_NOE_PROC_Open_Gmac,
                          .read    = seq_read,
                          .llseek  = seq_lseek,
                          .write   = _MDrv_NOE_PROC_Set_Gmac1_Phyid,
                          .release = single_release},
    [E_NOE_PROC_GMAC2]      = {.owner  = THIS_MODULE,
                          .open   = _MDrv_NOE_PROC_Open_Gmac2,
                          .read   = seq_read,
                          .llseek = seq_lseek,
                          .write  = _MDrv_NOE_PROC_Set_Gmac2_Phyid},
    [E_NOE_PROC_SKB_FREE]   = {.owner   = THIS_MODULE,
                             .open    = _MDrv_NOE_PROC_Open_Skb_Free,
                             .read    = seq_read,
                             .llseek  = seq_lseek,
                             .release = seq_release},
    [E_NOE_PROC_TX_RING]    = {.owner   = THIS_MODULE,
                            .open    = _MDrv_NOE_PROC_Open_Tx_Ring,
                            .read    = seq_read,
                            .llseek  = seq_lseek,
                            .release = single_release},
    [E_NOE_PROC_RX_RING]    = {.owner   = THIS_MODULE,
                            .open    = _MDrv_NOE_PROC_Open_Rx_Ring,
                            .read    = seq_read,
                            .llseek  = seq_lseek,
                            .release = single_release},
    [E_NOE_PROC_NUM_OF_TXD] = {.owner   = THIS_MODULE,
                               .open    = _MDrv_NOE_PROC_Open_Tso_Txd_Num,
                               .read    = seq_read,
                               .llseek  = seq_lseek,
                               .write   = _MDrv_NOE_PROC_Write_Txd_Num,
                               .release = seq_release},
    [E_NOE_PROC_TSO_LEN]    = {.owner   = THIS_MODULE,
                            .open    = _MDrv_NOE_PROC_Open_Tso_Len,
                            .read    = seq_read,
                            .llseek  = seq_lseek,
                            .write   = _MDrv_NOE_PROC_Write_Tso_Len,
                            .release = seq_release},
    [E_NOE_PROC_LRO_STATS]  = {.owner   = THIS_MODULE,
                              .open    = _MDrv_NOE_PROC_Open_Lro_Stats,
                              .read    = seq_read,
                              .llseek  = seq_lseek,
                              .write   = _MDrv_NOE_PROC_Write_Lro_Stats,
                              .release = single_release},
    [E_NOE_PROC_SNMP]       = {.owner   = THIS_MODULE,
                         .open    = _MDrv_NOE_PROC_Open_Snmp,
                         .read    = seq_read,
                         .llseek  = seq_lseek,
                         .release = single_release},
    [E_NOE_PROC_ESW_CNT]    = {.owner   = THIS_MODULE,
                            .open    = _MDrv_NOE_PROC_Open_Switch_Count,
                            .read    = seq_read,
                            .llseek  = seq_lseek,
                            .release = single_release},
    [E_NOE_PROC_ETH_CNT]    = {.owner   = THIS_MODULE,
                            .open    = _MDrv_NOE_PROC_Open_Eth_Count,
                            .read    = seq_read,
                            .llseek  = seq_lseek,
                            .release = single_release},
    [E_NOE_PROC_SCHE]       = {.owner   = THIS_MODULE,
                         .open    = _MDrv_NOE_PROC_Open_Schedule,
                         .read    = seq_read,
                         .write   = _MDrv_NOE_PROC_Write_Schedule,
                         .llseek  = seq_lseek,
                         .release = single_release},
    [E_NOE_PROC_INT_DBG]    = {.owner = THIS_MODULE,
                            .open  = _MDrv_NOE_PROC_Open_Intr_Dbg,
                            .read  = seq_read,
                            .write = _MDrv_NOE_PROC_Write_Intr_Dbg},
    [E_NOE_PROC_SET_LAN_IP] = {.owner = THIS_MODULE,
    ...
    [E_NOE_PROC_QSCH] = {.owner   = THIS_MODULE,
...
int _MDrv_NOE_PROC_Read_Rx_Ring(struct seq_file *seq, void *v)
{
    ...
    {
        seq_printf(seq, "%8d: %08x %08x %08x %08x\n", i, *(int *)&ei_local->rx_ring[j][i].rxd_info1,
                   *(int *)&ei_local->rx_ring[j][i].rxd_info2, *(int *)&ei_local->rx_ring[j][i].rxd_info3,
...
int _MDrv_NOE_PROC_Show_Tx_Ring(struct seq_file *seq, void *v)
{
    ...
    if (ei_local->features & FE_QDMA_TX)
    {
        ...
        {
            seq_printf(seq, "%4d:[%08x] %08x %08x %08x %08x \n", i, (ei_local->phy_txd_pool + i * QTXD_LEN),
                       *(int *)&ei_local->txd_pool[i].txd_info1, *(int *)&ei_local->txd_pool[i].txd_info2,
                       *(int *)&ei_local->txd_pool[i].txd_info3, *(int *)&ei_local->txd_pool[i].txd_info4);
        }
    }
    else
    {
        for (i = 0; i < NUM_TX_DESC; i++)
        {
            ...
            seq_printf(seq, "%4d: %08x %08x %08x %08x\n", i, *(int *)&tx_ring->txd_info1, *(int *)&tx_ring->txd_info2,
                       *(int *)&tx_ring->txd_info3, *(int *)&tx_ring->txd_info4);
        }
    }
...
static int _MDrv_NOE_PROC_Show_QoS(struct seq_file *seq, void *v)
{
    ...
    if (ei_local->features & FE_QDMA)
    {
        ...
        for (queue = 0; queue < NUM_PQ; queue++)
        {
            ...
            seq_printf(seq, "HW_RESV: %d; SW_RESV: %d; SCH: %d; Weighting: %d\n", qdma_pq.hw_resv, qdma_pq.sw_resv,
                       qdma_pq.sch, qdma_pq.weight);
        ...
        for (i = 0; i < NUM_PQ; i += 8)
        {
            seq_printf(seq, "(%2d,%2d)(%2d,%2d)(%2d,%2d)(%2d,%2d)(%2d,%2d)(%2d,%2d)(%2d,%2d)(%2d,%2d)\n", i,
                       M2Q_table[i], i + 1, M2Q_table[i + 1], i + 2, M2Q_table[i + 2], i + 3, M2Q_table[i + 3], i + 4,
                       M2Q_table[i + 4], i + 5, M2Q_table[i + 5], i + 6, M2Q_table[i + 6], i + 7, M2Q_table[i + 7]);
        }

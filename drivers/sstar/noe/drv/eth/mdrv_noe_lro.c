...
int MDrv_NOE_LRO_Init(struct net_device *dev)
{
    ...
    {
        // ei_local->rx_ring[i] = dma_alloc_coherent(dev->dev.parent, NUM_LRO_RX_DESC * sizeof(struct PDMA_rxdesc),
        // &ei_local->phy_rx_ring[i], GFP_ATOMIC | __GFP_ZERO);
        ...
        for (j = 0; j < NUM_LRO_RX_DESC; j++)
        {
            ...
            ei_local->rx_ring[i][j].rxd_info1.PDP0      = dma_map_single(
                     dev->dev.parent, ei_local->netrx_skb_data[i][j] + NET_SKB_PAD, MAX_LRO_RX_LENGTH, DMA_FROM_DEVICE);
        ...
        NOE_MSG_DUMP("\nphy_rx_ring[%d] = 0x%08x, rx_ring[%d] = 0x%p, NUM_LRO_RX_DESC=%d\n", i,
                     (unsigned int)ei_local->phy_rx_ring[i], i, ei_local->rx_ring[i], NUM_LRO_RX_DESC);
    }
...
void MDrv_NOE_LRO_Deinit(struct net_device *dev)
{
    ...
    {
        /* free RX Ring */
        // dma_free_coherent(dev->dev.parent, NUM_LRO_RX_DESC * sizeof(struct PDMA_rxdesc), ei_local->rx_ring[i],
...
static inline void _MDrv_NOE_LRO_Init_Rx_Desc(struct END_DEVICE *ei_local, struct PDMA_rxdesc *rx_ring,
                                              unsigned int rx_ring_no, dma_addr_t dma_addr)
{
    if (rx_ring_no != 0)
    {
        /* lro ring */
...
int MDrv_NOE_LRO_Recv(struct net_device *dev, struct napi_struct *napi, int budget)
{
    ...
    for (;;)
    {
        ...
        else
        {
            ...
            skb_size                     = SKB_DATA_ALIGN(MAX_LRO_RX_LENGTH + NET_IP_ALIGN + NET_SKB_PAD)
                       + SKB_DATA_ALIGN(sizeof(struct skb_shared_info));
        ...
        dma_addr = dma_map_single_attrs(dev->dev.parent, new_data + NET_SKB_PAD, map_size, DMA_FROM_DEVICE,
                                        &attrs); // edie.chen--4.9 not support
#else

...
enum foe_cpu_reason
{
    ...
    HIT_BIND_WITH_VLAN_VIOLATION =
        0x12, /* Hit bind and VLAN replacement violation (Ingress 1(0) VLAN layers and egress 4(3 or 4) VLAN layers) */
...
#define IS_MAGIC_TAG_PROTECT_VALID_CB(skb) \
    ((FOE_TAG_PROTECT_CB0(skb) == TAG_PROTECT) && (FOE_TAG_PROTECT_CB0(skb) == FOE_TAG_PROTECT_CB1(skb)))

#define IS_IF_PCIE_WLAN_HEAD(skb)                                                              \
    ((FOE_MAGIC_TAG_HEAD(skb) == FOE_MAGIC_PCI) || (FOE_MAGIC_TAG_HEAD(skb) == FOE_MAGIC_WLAN) \
     || (FOE_MAGIC_TAG_HEAD(skb) == FOE_MAGIC_GE))
#define IS_IF_PCIE_WLAN_TAIL(skb) \
    ((FOE_MAGIC_TAG_TAIL(skb) == FOE_MAGIC_PCI) || (FOE_MAGIC_TAG_TAIL(skb) == FOE_MAGIC_WLAN))
...
#define magic_tag_set_zero(skb)                                                                       \
    {                                                                                                 \
        if ((FOE_MAGIC_TAG_HEAD(skb) == FOE_MAGIC_PCI) || (FOE_MAGIC_TAG_HEAD(skb) == FOE_MAGIC_WLAN) \
            || (FOE_MAGIC_TAG_HEAD(skb) == FOE_MAGIC_GE))                                             \
        {                                                                                             \
            if (IS_SPACE_AVAILABLE_HEAD(skb))                                                         \
                FOE_MAGIC_TAG_HEAD(skb) = 0;                                                          \
        }                                                                                             \
        if ((FOE_MAGIC_TAG_TAIL(skb) == FOE_MAGIC_PCI) || (FOE_MAGIC_TAG_TAIL(skb) == FOE_MAGIC_WLAN) \

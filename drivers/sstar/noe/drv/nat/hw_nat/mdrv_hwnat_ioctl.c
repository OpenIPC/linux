    ...
    switch (cmd)
    {
        ...
        case HW_NAT_MCAST_INS:
            ...
            if (copy_from_user(opt5_k, opt5, sizeof(struct hwnat_mcast_args)))
                HWNAT_MSG_ERR("copy_from_user fail\n");
            MDrv_HWNAT_Mcast_Insert_Entry(opt5_k->mc_vid, opt5_k->dst_mac, opt5_k->mc_px_en, opt5_k->mc_px_qos_en,
                                          opt5_k->mc_qos_qid);
            kfree(opt5_k);
            break;
        case HW_NAT_MCAST_DEL:
            ...
            if (copy_from_user(opt5_k, opt5, sizeof(struct hwnat_mcast_args)))
                HWNAT_MSG_ERR("copy_from_user fail\n");
            MDrv_HWNAT_Mcast_Delete_Entry(opt5->mc_vid, opt5->dst_mac, opt5->mc_px_en, opt5->mc_px_qos_en,
                                          opt5->mc_qos_qid);
            kfree(opt5_k);
            break;

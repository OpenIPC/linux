...
static int _MDrv_HWNAT_Is_BrLan_Subnet(struct sk_buff *skb)
{
    ...
    if ((eth_type == htons(ETH_P_8021Q)) || (((eth_type)&0x00FF) == htons(ETH_P_8021Q))
        || hwnat_vlan_tx_tag_present(skb))
    {
...
int32_t MDrv_HWNAT_Handle_Rx(struct sk_buff *skb)
{
    ...
    else if (FOE_AI(skb) == HIT_BIND_FORCE_TO_CPU)
    {
        ...
        /* handle the incoming packet which came back from PPE */
             ...
             && (FOE_AI(skb) != HIT_BIND_KEEPALIVE_UC_OLD_HDR) && (FOE_AI(skb) != HIT_BIND_KEEPALIVE_MC_NEW_HDR)
...
int32_t _MDrv_HWNAT_Is_Special_Tag(uint16_t eth_type)
{
     ...
     */
    if ((eth_type & 0x00FF) == htons(ETH_P_8021Q))
    { /* Ralink Special Tag: 0x81xx */
...
int32_t _MDrv_HWNAT_Parse_Pkt_Layer(struct sk_buff *skb)
{
    ...
    if (_MDrv_HWNAT_Is_802_1q(ppe_parse_result.eth_type) || _MDrv_HWNAT_Is_Special_Tag(ppe_parse_result.eth_type)
        || _MDrv_HWNAT_Is_Hw_Vlan_Tx(skb))
    {
        if (_hwnat_info.features & HW_NAT_VLAN_TX)
        {
        ...
        if (ntohs(vh->h_vlan_encapsulated_proto) == ETH_P_PPP_SES)
        {
            ...
            if (_MDrv_HWNAT_Get_Pppoe_Sid(skb, ppe_parse_result.vlan1_gap, &ppe_parse_result.pppoe_sid,
                 ...
                 || _MDrv_HWNAT_Is_Special_Tag(vh->h_vlan_encapsulated_proto))
        {
            ...
            {
                ppe_parse_result.pppoe_gap = 8;
                if (_MDrv_HWNAT_Get_Pppoe_Sid(skb, (ppe_parse_result.vlan1_gap + ppe_parse_result.vlan2_gap),
            ...
            else if (_MDrv_HWNAT_Is_802_1q(vh->h_vlan_encapsulated_proto))
            {
                /* VLAN + VLAN + VLAN */
                ppe_parse_result.vlan_layer++;
                vh = (struct vlan_hdr *)(skb->data + ETH_HLEN + ppe_parse_result.vlan1_gap + VLAN_HLEN);
    ...
    skb_set_network_header(skb, ETH_HLEN + ppe_parse_result.vlan1_gap + ppe_parse_result.vlan2_gap
                                    + ppe_parse_result.pppoe_gap);

    /* set layer4 start addr */
    if ((ppe_parse_result.eth_type == htons(ETH_P_IP))
        || (ppe_parse_result.eth_type == htons(ETH_P_PPP_SES) && (ppe_parse_result.ppp_tag == htons(PPP_IP))))
    {
        ...
        if (iph->protocol == IPPROTO_TCP)
        {
            skb_set_transport_header(skb, ETH_HLEN + ppe_parse_result.vlan1_gap + ppe_parse_result.vlan2_gap
        ...
        else if (iph->protocol == IPPROTO_IPV6)
        {
            ...
            {
                skb_set_transport_header(skb, ETH_HLEN + ppe_parse_result.vlan1_gap + ppe_parse_result.vlan2_gap
                                                  + ppe_parse_result.pppoe_gap + (sizeof(struct ipv6hdr)));
        ...
        else
        {
            ...
            return 1;
             ...
             || (ppe_parse_result.eth_type == htons(ETH_P_PPP_SES) && ppe_parse_result.ppp_tag == htons(PPP_IPV6)))
    {
    ...
    if (_hwnat_info.debug_level >= 6)
    {
        ...
        NAT_PRINT("DMAC:%02X:%02X:%02X:%02X:%02X:%02X\n", ppe_parse_result.dmac[0], ppe_parse_result.dmac[1],
                  ppe_parse_result.dmac[2], ppe_parse_result.dmac[3], ppe_parse_result.dmac[4],
                  ppe_parse_result.dmac[5]);
        NAT_PRINT("SMAC:%02X:%02X:%02X:%02X:%02X:%02X\n", ppe_parse_result.smac[0], ppe_parse_result.smac[1],
                  ppe_parse_result.smac[2], ppe_parse_result.smac[3], ppe_parse_result.smac[4],
                  ppe_parse_result.smac[5]);
        ...
        NAT_PRINT("PKT_TYPE=%s\n", ppe_parse_result.pkt_type == 0   ? "IPV4_HNAT"
                                   : ppe_parse_result.pkt_type == 1 ? "IPV4_HNAPT"
                                   : ppe_parse_result.pkt_type == 3 ? "IPV4_DSLITE"
                                   : ppe_parse_result.pkt_type == 5 ? "IPV6_ROUTE"
                                   : ppe_parse_result.pkt_type == 7 ? "IPV6_6RD"
                                                                    : "Unknown");
...
static int32_t _MDrv_HWNAT_Fill_L3(struct sk_buff *skb, struct foe_entry *entry)
{
    ...
    /* IPv6 or IPv6 over PPPoE */
             ...
             && (ppe_parse_result.eth_type == htons(ETH_P_IPV6)
                 || (ppe_parse_result.eth_type == htons(ETH_P_PPP_SES) && ppe_parse_result.ppp_tag == htons(PPP_IPV6))))
    {
        if (ppe_parse_result.pkt_type == IPV6_3T_ROUTE || ppe_parse_result.pkt_type == IPV6_5T_ROUTE)
        {
            ...
            if (entry->bfib1.pkt_type == IPV6_6RD)
            {
                ...
                entry->ipv6_3t_route.iblk2.dscp =
                    (ppe_parse_result.ip6h.priority << 4 | (ppe_parse_result.ip6h.flow_lbl[0] >> 4));
                if (_hwnat_info.features & PPE_MIB)
                {
                    entry->ipv4_dslite.iblk2.mibf = 1;
                }
                ...
                else
                {
                    ...
                    if (_hwnat_info.features & HW_NAT_ARCH_WIFI_WDMA)
                    {
                        if (gmac_no == 3)
                        {
                            ...
                            entry->ipv4_hnapt.vlan2_winfo  = ((FOE_RX_ID(skb) & 0x03) << 14)
                                                            | ((FOE_WC_ID(skb) & 0xff) << 6) | (FOE_BSS_ID(skb) & 0x3f);
                        }
                        else
                        {
            ...
            else
            {
                if (gmac_no == 3)
                {
                    ...
                    entry->ipv4_hnapt.vlan2_winfo =
                        ((FOE_RX_ID(skb) & 0x03) << 14) | ((FOE_WC_ID(skb) & 0xff) << 6) | (FOE_BSS_ID(skb) & 0x3f);
                }
                else
                {
            ...
            if (_hwnat_info.features & HW_NAT_QDMA)
            {
                if (_hwnat_info.features & HW_NAT_QDMATX_QDMARX)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if ((_hwnat_info.features & WAN_TO_WLAN_SUPPORT_QOS)
                                && (MHal_HWNAT_Get_QoS_Status() == TRUE))
...
{
    if ((strncmp(skb->dev->name, "rai", 3) == 0) || (strncmp(skb->dev->name, "apclii", 6) == 0)
        || (strncmp(skb->dev->name, "wdsi", 4) == 0) || (strncmp(skb->dev->name, "wlan", 4) == 0))
        return 1;
    else
        return 0;
}
...
static MS_BOOL _MDrv_HWNAT_Reach_Unbind_Rate(struct sk_buff *skb, struct foe_entry *entry)
{
    ...
    {
        if (IS_MAGIC_TAG_PROTECT_VALID(skb) && (FOE_AI(skb) == HIT_UNBIND_RATE_REACH) && (FOE_ALG(skb) == 0)
            && (_MDrv_HWNAT_Get_Done_Bit(skb, entry) == 0))
        {
...
int32_t MDrv_HWNAT_Handle_Tx(struct sk_buff *skb, int gmac_no)
{
    ...
    else if (IS_MAGIC_TAG_PROTECT_VALID(skb) && (FOE_AI(skb) == HIT_BIND_PACKET_SAMPLING))
    {
        ...
        return 0;
             ...
             && (FOE_AI(skb) == HIT_BIND_KEEPALIVE_MC_NEW_HDR || (FOE_AI(skb) == HIT_BIND_KEEPALIVE_DUP_OLD_HDR)))
    {
    ...
    else if (IS_MAGIC_TAG_PROTECT_VALID(skb) && (FOE_AI(skb) == HIT_UNBIND_RATE_REACH) && (FOE_ALG(skb) == 1))
    {
        if (_hwnat_info.debug_level >= 3)
    ...
    for (hash_index = 0; hash_index < FOE_4TB_SIZ; hash_index++)
    {
        ...
        if (entry->bfib1.state == BIND)
        {
            ...
            */
            ...
            {
                if ((entry->ipv4_hnapt.dmac_hi[3] != mac0) || (entry->ipv4_hnapt.dmac_hi[2] != mac1)
                    || (entry->ipv4_hnapt.dmac_hi[1] != mac2) || (entry->ipv4_hnapt.dmac_hi[0] != mac3)
                    || (entry->ipv4_hnapt.dmac_lo[1] != mac4) || (entry->ipv4_hnapt.dmac_lo[0] != mac5))
                {
                    ...
                    NAT_PRINT("old mac= %x:%x:%x:%x:%x:%x, dip=%x\n", entry->ipv4_hnapt.dmac_hi[3],
                              entry->ipv4_hnapt.dmac_hi[2], entry->ipv4_hnapt.dmac_hi[1], entry->ipv4_hnapt.dmac_hi[0],
                              entry->ipv4_hnapt.dmac_lo[1], entry->ipv4_hnapt.dmac_lo[0], ntohl(dip));

...
{
    u16 u16Temp = 0, otp_value = 0;
    u16 u16Otp_5A = 0, u16Otp_5D = 0;
    // int portId = 0;
    ...
    u16Otp_5A = INREG16(BASE_REG_OTP2_PA + REG_ID_5A);
    u16Otp_5D = INREG16(BASE_REG_OTP2_PA + REG_ID_5D);

    pr_err("%s ,u16Otp_5A = 0x%X , u16Otp_5D = 0x%X\n", __func__, u16Otp_5A, u16Otp_5D);

    if ((port_base == IO_ADDRESS(BASE_REG_SATA0_SNPS_PORT0_PA)) && (u16Otp_5A & BIT14))
    {
        printk("%s ,u16Otp_58 = 0x%X , u16Otp_59 = 0x%X ,u16Otp_5A = 0x%X \n", __func__,
               INREG16(BASE_REG_OTP2_PA + REG_ID_58), INREG16(BASE_REG_OTP2_PA + REG_ID_59),
               INREG16(BASE_REG_OTP2_PA + REG_ID_5A));
    }

    if ((port_base == IO_ADDRESS(BASE_REG_SATA1_SNPS_PORT0_PA)) && (u16Otp_5D & BIT13))
    {
        printk("%s ,u16Otp_5A = 0x%X, u16Otp_5B = 0x%X, u16Otp_5C = 0x%X, u16Otp_5D = 0x%X \n", __func__,
               INREG16(BASE_REG_OTP2_PA + REG_ID_5A), INREG16(BASE_REG_OTP2_PA + REG_ID_5B),
               INREG16(BASE_REG_OTP2_PA + REG_ID_5C), INREG16(BASE_REG_OTP2_PA + REG_ID_5D));
    }
    ...
    if ((port_base == IO_ADDRESS(BASE_REG_SATA0_SNPS_PORT0_PA)) && (u16Otp_5A & BIT14))
    {
        ...
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_58)) & 0x1F;

        u16Temp = INREG16(GHC_PHY_D + REG_ID_2A);
        u16Temp &= ~(0x0F80);
        u16Temp |= (otp_value << 7); // read bit[4:0] to bit[11:7]
        ...
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_58) >> 5) & 0x1F;

        u16Temp = INREG16(GHC_PHY_D + REG_ID_3C);
        u16Temp &= ~(0x01F0);
        u16Temp |= (otp_value << 4); // read bit[9:5] to bit[8:4]
        ...
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_58) >> 10) & 0x3F;

        u16Temp = INREG16(GHC_PHY_D + REG_ID_50);
        u16Temp &= ~(0x0FC00);
        u16Temp |= otp_value << 10; // read bit[15:10] to bit[15:10]
        OUTREG16((GHC_PHY_D + REG_ID_50), u16Temp);
        printk("SATA0_TX_R50_IBIAS trim = 0x%04X\n", otp_value);

        // SATA_RXPLL_3G
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_59) & 0x03FF);

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_55);
        u16Temp &= ~(0x03FF);
        u16Temp |= otp_value; // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_55), u16Temp);
        printk("SATA0_RXPLL_3G trim = 0x%04X\n", otp_value);

        // SATA_RXPLL_1P5G
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_5A) & 0x0F) << 6 | (INREG16(BASE_REG_OTP2_PA + REG_ID_59) >> 10);

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_56);
        u16Temp &= ~(0x03FF);
        u16Temp |= (otp_value & 0x03FF); // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_56), u16Temp);

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_57);
        u16Temp &= ~(0x03FF);
        u16Temp |= (otp_value & 0x03FF); // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_57), u16Temp);
        printk("SATA0_RXPLL_1P5G trim = 0x%04X\n", otp_value);

        // SATA_TXPLL
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_5A) & 0x03FF0) >> 4;

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_45);
        u16Temp &= ~(0x03FF);
        u16Temp |= (otp_value & 0x03FF); // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_45), u16Temp);

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_46);
        u16Temp &= ~(0x03FF);
        u16Temp |= (otp_value & 0x03FF); // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_46), u16Temp);

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_47);
        u16Temp &= ~(0x03FF);
        u16Temp |= (otp_value & 0x03FF); // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_47), u16Temp);

        printk("SATA0_TXPLL trim = 0x%04X\n", otp_value);
    }
    else if ((port_base == IO_ADDRESS(BASE_REG_SATA1_SNPS_PORT0_PA)) && (u16Otp_5D & BIT13))
    {
#if 0
        printk("SATA0_TX_R50 trim = 0x%04X\n", (INREG16(GHC_PHY_D + REG_ID_2A) >> 7) & 0x1F);
        printk("SATA0_RX_R50 trim = 0x%04X\n", (INREG16(GHC_PHY_D + REG_ID_3C) >> 4) & 0x1F);
        ...
        otp_value = ((INREG16(BASE_REG_OTP2_PA + REG_ID_5B) & 0x0F) << 1)
                    | ((INREG16(BASE_REG_OTP2_PA + REG_ID_5A) & 0x08000) >> 15);

        u16Temp = INREG16(GHC_PHY_D + REG_ID_2A);
        u16Temp &= ~(0x0F80);
        u16Temp |= (otp_value << 7); // read bit[3:0] | read bit[15] to bit[11:7]
        OUTREG16((GHC_PHY_D + REG_ID_2A), u16Temp);
        printk("SATA1_TX_R50 trim=0x%04X\n", otp_value);
        ...
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_5B) >> 4) & 0x1F;

        u16Temp = INREG16(GHC_PHY_D + REG_ID_3C);
        u16Temp &= ~(0x01F0);
        u16Temp |= (otp_value << 4); // read bit[8:4] to bit[8:4]
        OUTREG16((GHC_PHY_D + REG_ID_3C), u16Temp);
        printk("SATA1_RX_R50 trim=0x%04X\n", otp_value);
        ...
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_5B) >> 9) & 0x3F;

        u16Temp = INREG16(GHC_PHY_D + REG_ID_50);
        u16Temp &= ~(0x0FC00);
        u16Temp |= otp_value << 10; // read bit[15:10] to bit[15:10]
        OUTREG16((GHC_PHY_D + REG_ID_50), u16Temp);
        printk("SATA1_TX_R50_IBIAS trim = 0x%04X\n", otp_value);

        // SATA_RXPLL_3G
        otp_value =
            ((INREG16(BASE_REG_OTP2_PA + REG_ID_5C) & 0x1FF) << 1) | ((INREG16(BASE_REG_OTP2_PA + REG_ID_5B)) >> 15);

        u16Temp = INREG16(GHC_PHY2_ANA + REG_ID_55);
        u16Temp &= ~(0x03FF);
        u16Temp |= otp_value; // read bit[8:0] | read bit[15] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_55), u16Temp);
        printk("SATA1_RXPLL_3G trim = 0x%04X\n", otp_value);

        // SATA_RXPLL_1P5G
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_5D) & 0x07) << 7 | (INREG16(BASE_REG_OTP2_PA + REG_ID_5C) >> 9);
        ...
        u16Temp |= (otp_value & 0x03FF); // read bit[9:0] to bit[9:0]
        OUTREG16((GHC_PHY2_ANA + REG_ID_57), u16Temp);
        printk("SATA1_RXPLL_1P5G trim = 0x%04X\n", otp_value);

        // SATA_TXPLL
        otp_value = (INREG16(BASE_REG_OTP2_PA + REG_ID_5A) & 0x1FF8) >> 3;
        ...
        printk("SATA1_TXPLL trim = 0x%04X\n", otp_value);
    }
    else
    {
        pr_err("%s ,sata use default trim\n", __func__);
...
int _MHal_SATA_Power_On(int portId)
{
    ...
    {
        dev_node = of_find_compatible_node(NULL, NULL, SSTAR_SATA1_DTS_NAME);
        // printk("%s , i am sata2 , config_of 6+%d\n", __func__, __LINE__);
    }

...
extern int g_hd_link1[4];
// unsigned int ssc_mode1                     = 0;
// u32          u32_sata1_gen1_gen2_vpp[2]    = {0xFF};
// u32          u32_sata1_gen3_de_emphasis[3] = {0xFF};
...
struct _sstar_phy_adjust phy_adj1;
static unsigned int      sstar_ahci_fill_sg1(struct ata_queued_cmd *qc, void *cmd_tbl)
{
...
static enum ata_completion_errors sstar_ahci_qc_prep1(struct ata_queued_cmd *qc)
{
    ...
    if (qc->flags & ATA_QCFLAG_DMAMAP)
        n_elem = sstar_ahci_fill_sg1(qc, cmd_tbl);
...
void sstar_ahci_sw_activity1(struct ata_link *link)
{
...
unsigned int sstar_ahci_qc_issue1(struct ata_queued_cmd *qc)
{
    struct ata_port *      ap        = qc->ap;
    void __iomem *         port_mmio = ahci_port_base(ap);
    struct ahci_port_priv *pp        = ap->private_data;
    ...
    sstar_ahci_sw_activity1(qc->dev->link);

    return 0;
}
EXPORT_SYMBOL_GPL(sstar_ahci_qc_issue1);

struct ata_port_operations sstar_ahci_platform_ops1 = {
    .inherits  = &ahci_ops,
    .qc_prep   = sstar_ahci_qc_prep1,
    .qc_issue  = sstar_ahci_qc_issue1,
...
int __ss_sata_get_phy_mode1(struct sstar_ahci_priv *priv)
{
    ...
    printk("[SATA1] phy_mode =%d , tx_ssc_mode = 0x%X\n", phy_mode, ssc_mode);
...
static ssize_t sata_link_store1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    return 0;
}

static ssize_t sata_link_show1(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *       str     = buf;
    int          i       = 0;
    const int    nGen    = 4;
    ...
    str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "link1 : 0, G1, G2,G3 \n");

    for (i = 0; i < nGen; ++i)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "link1[%d] = %d ,", i, g_hd_link1[i]);
    }
...
DEVICE_ATTR(sata_dump1, 0644, sata_link_show1, sata_link_store1);

static ssize_t sata_de_emphasis_store1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int nArg   = 0;
    int db     = 0;
    int enable = 0;
    ...
    phy_adj1.dmhs.enable = enable;
    phy_adj1.dmhs.level  = level;
    phy_adj1.dmhs.db     = db;

    hal_sata_set_de_emphasis(1, &phy_adj1);
    phy_adj1.new_de_emphasis = 1;
out:
    return count;
}

static ssize_t sata_de_emphasis_show1(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *       str     = buf;
    unsigned int nStrLen = PAGE_SIZE;

    if (phy_adj1.new_de_emphasis == 0)
    {
    ...
    str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "Gen3 de_emphasis now is %s \n",
                         (phy_adj1.dmhs.enable == 1) ? ("enable") : ("disable"));

    if (phy_adj1.dmhs.enable == 1)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "current run on %s mode\n",
                             (phy_adj1.dmhs.db == 0) ? ("Gen3 -1.5db")
                                                     : ((phy_adj1.dmhs.db == 1) ? ("Gen3 -2.5db") : ("Gen3 -3.5db")));
        //        "Gen3 -1.5db"
        //        "Gen3 -2.5db"
        //        "Gen3 -3.5db"
    }
    else
    {
        str +=
            CamOsSnprintf(str, nStrLen - (int)(str - buf), "current run on %s mode\n",
                          (phy_adj1.test_gen == 3) ? ("Gen3 Vppdiff")
                                                   : ((phy_adj1.test_gen == 2) ? ("Gen2 Vppdiff") : ("Gen1 Vppdiff")));
    }

    if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_0p4)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 0.4\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_0p5)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 0.5\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_0p6)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 0.6\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_0p7)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 0.7\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_0p8)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 0.8\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_0p9)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 0.9\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_1p0)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now level is 1.0\n");
    }
    else if (phy_adj1.dmhs.level == DE_EMPHASIS_LEVEL_1p1)
    {
...
DEVICE_ATTR(de_emphasis1, 0644, sata_de_emphasis_show1, sata_de_emphasis_store1);

static ssize_t cts_pattern_store1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int nArg    = 0;
    int pattern = 0;
    ...
    phy_adj1.cts_pattern     = pattern;
    phy_adj1.new_cts_pattern = 1;

    hal_sata_set_cts_pattern(1, &phy_adj1);

    pr_err("\n\nnow cts patern is \n");

    if (phy_adj1.cts_pattern == 0)
    {
        pr_err("HFTP\n");
    }
    else if (phy_adj1.cts_pattern == 1)
    {
        pr_err("MFTP\n");
    }
    else if (phy_adj1.cts_pattern == 2)
    {
        pr_err("LFTP\n");
    }
    else if (phy_adj1.cts_pattern == 3)
    {
        pr_err("LBP\n");
    }
    else
...
static ssize_t cts_pattern_show1(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *       str     = buf;
    unsigned int nStrLen = PAGE_SIZE;

    if (phy_adj1.new_cts_pattern == 0)
    {
    ...
    if (phy_adj1.cts_pattern == 0)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now paatern is HFTP\n");
    }
    else if (phy_adj1.cts_pattern == 1)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now paatern is MFTP\n");
    }
    else if (phy_adj1.cts_pattern == 2)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now paatern is LFTP\n");
    }
    else if (phy_adj1.cts_pattern == 3)
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now paatern is LBP\n");
    }
    else
    {
        str += CamOsSnprintf(str, nStrLen - (int)(str - buf), "now paatern is SSOP\n");
    }

    str +=
        CamOsSnprintf(str, nStrLen - (int)(str - buf), "gen now is %s \n",
                      (phy_adj1.test_gen == 3) ? ("6.0Gbps") : ((phy_adj1.test_gen == 2) ? ("3.0Gbps") : ("1.5Gbps")));
...
DEVICE_ATTR(cts_pattern1, 0644, cts_pattern_show1, cts_pattern_store1);

static ssize_t test_gen_store1(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int nArg     = 0;
    int test_gen = 0;
    ...
    phy_adj1.test_gen = test_gen;
    hal_sata_set_testGen(1, phy_adj1.test_gen);
out:
    return count;
}

static ssize_t test_gen_show1(struct device *dev, struct device_attribute *attr, char *buf)
{
    char *       str     = buf;
    unsigned int nStrLen = PAGE_SIZE;
    ...
    str +=
        CamOsSnprintf(str, nStrLen - (int)(str - buf), "test_gen now is %s \n",
                      (phy_adj1.test_gen == 2) ? ("6.0Gbps") : ((phy_adj1.test_gen == 1) ? ("3.0Gbps") : ("1.5Gbps")));
...
DEVICE_ATTR(test_gen1, 0644, test_gen_show1, test_gen_store1);
...
static int ahci_sstar_probe1(struct platform_device *pdev)
{
    ...
    memset(&phy_adj1, 0, sizeof(phy_adj1));
    ...
    __ss_sata_get_phy_mode1(priv);
    // priv->phy_mode = phy_mode;
    // priv->ssc_mode = ssc_mode1;
    ...
    ss_sata_applySetting(1, hpriv);
    // pr_info("[%s] , line = %d \n", __func__, __LINE__);

    // ss_sata_applyDmhs(1, u32_sata1_gen1_gen2_vpp, u32_sata1_gen3_de_emphasis, &phy_adj1);
    ss_sata_applyDmhs(1, priv, &phy_adj1);
    ...
    if (alloc_chrdev_region(&priv->dev_no, 0, 1, "sstar_sata1") == 0)
    {
        int Major           = MAJOR(priv->dev_no);
        priv->attribute_dev = MKDEV(Major, 0);
        priv->sysfs_dev     = device_create(msys_get_sysfs_class(), NULL, priv->attribute_dev, NULL, "sstar_sata1");
    } /*replace device_create_file -> CamDeviceCreateFile*/
    device_create_file(priv->sysfs_dev, &dev_attr_sata_dump1);

    device_create_file(priv->sysfs_dev, &dev_attr_de_emphasis1);
    device_create_file(priv->sysfs_dev, &dev_attr_cts_pattern1);
    device_create_file(priv->sysfs_dev, &dev_attr_test_gen1);
...
static int sstar_ahci_remove1(struct platform_device *pdev)
{
    ...
    device_remove_file(priv->sysfs_dev, &dev_attr_sata_dump1);
    device_remove_file(priv->sysfs_dev, &dev_attr_de_emphasis1);
    device_remove_file(priv->sysfs_dev, &dev_attr_cts_pattern1);
    device_remove_file(priv->sysfs_dev, &dev_attr_test_gen1);
...
MODULE_DESCRIPTION("SSTAR AHCI SATA platform driver1");

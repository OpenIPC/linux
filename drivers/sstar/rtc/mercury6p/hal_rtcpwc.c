...
{
    u16 u16Check = ISO_ACK_RETRY_TIME * 10;
    u16 reg      = 0;
    ...
    do
    {
        ...
        CamOsUsSleep(RTC_CHECK_STATUS_DELAY_TIME_US / 10);
        u16Check--;
    } while (u16Check);
    return 0;
}
...
void hal_rtc_set_base_time(struct hal_rtcpwc_t *hal, u32 seconds)
{
    ...
    RTC_DBG("Set RTC Base Time L=%x\r\n", RTC_READ(RTCPWC_DIG2RTC_WRDATA_L));
    RTC_DBG("Set RTC Base Time H=%x\r\n", RTC_READ(RTCPWC_DIG2RTC_WRDATA_H));

...
@return It reports the status of the operation.
*/
void hal_rtc_set_base_time(struct hal_rtcpwc_t *hal, unsigned long seconds)
{
    u16 reg;
...
int hal_rtc_read_time(struct hal_rtcpwc_t *hal, unsigned long *second)
{
    u16 reg           = 0;
    u32 run_sec       = 0;
    u16 counterH      = 0;
...
static void hal_rtc_set_alarm_time(struct hal_rtcpwc_t *hal, unsigned long seconds)
{
    u16 reg;
...
void hal_rtc_get_alarm(struct hal_rtcpwc_t *hal, unsigned long *seconds)
{
    u16 reg;
    u32 base;
    u32 alarm;
...
void hal_rtc_set_alarm(struct hal_rtcpwc_t *hal, unsigned long seconds)
{
    // set alarm time

...
void hal_rtc_set_sw2(struct hal_rtcpwc_t *hal, u16 val)
{
    u16 reg;
    ...
    RTC_WRITE(RTCPWC_DIG2RTC_WRDATA_L, (u16)val);
    hal_rtc_iso_ctrl(hal);
    mdelay(2);
...
void hal_rtc_set_sw3(struct hal_rtcpwc_t *hal, u16 val)
{
    u16 reg;

    reg = RTC_READ(RTCPWC_DIG2RTC_BASE_WR);
    RTC_WRITE(RTCPWC_DIG2RTC_BASE_WR, reg | RTCPWC_DIG2RTC_SW3_WD);
...
u16 hal_rtc_get_sw2(struct hal_rtcpwc_t *hal)
{
    return RTC_READ(RTCPWC_SW2_RDDATA);
}
u16 hal_rtc_get_sw3(struct hal_rtcpwc_t *hal)
{
    return RTC_READ(RTCPWC_SW3_RDDATA);
}

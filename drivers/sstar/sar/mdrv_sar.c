...
void        ms_sar_set_vol(int ch, int ref_vol);
...
static long ms_sar_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd)
    {
        ...
        case IOCTL_SAR_SET_CHANNEL_READ_VALUE:
            ...
            ms_sar_set_vol(channel, ref_voltage);
...
void ms_sar_set_vol(int ch, int ref_vol)
{
    if (ref_vol == 1)
    {
        ...
        {
            case 0:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, BIT0, BIT0);
                break;
            case 1:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, BIT1, BIT1);
                break;
            case 2:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, BIT2, BIT2);
                break;
            case 3:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, BIT3, BIT3);
                break;
            default:
                printk(KERN_ERR "error channel,support SAR0,SAR1,SAR2,SAR3\n");
                break;
    ...
    else if (ref_vol == 0)
    {
        ...
        {
            case 0:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, 0, BIT0);
                break;
            case 1:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, 0, BIT1);
                break;
            case 2:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, 0, BIT2);
                break;
            case 3:
                HAL_SAR_Write2ByteMask(REG_SAR_CH_REF_V_SEL, 0, BIT3);
                break;
            default:
                printk(KERN_ERR "error channel,support SAR0,SAR1,SAR2,SAR3\n");
                break;
    ...
    {
        printk(KERN_ERR "error voltage,support 0,1\n");
    }
}
EXPORT_SYMBOL(ms_sar_set_vol);
...
static ssize_t channel_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t n)
{
    if (NULL != buf)
    {
        ...
        if (len)
        {
            ...
            ms_sar_set_vol(channel & 3, ref_voltage);
            return n;
        }
        return -EINVAL;
    }
...
static ssize_t voltage_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t n)
{
    if (NULL != buf)
    {
        size_t      len;
        ...
        if (len)
        {
            ref_voltage = simple_strtoul(buf, NULL, 10);
            ms_sar_set_vol(channel & 3, ref_voltage);
            return n;
        }
        return -EINVAL;
...
static ssize_t voltage_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    ...
    if (ref_voltage)
    {
        str += scnprintf(str, end - str, "%d -> 3.3V\n", ref_voltage);
    }
    else
    {
        str += scnprintf(str, end - str, "%d -> 2.0V\n", ref_voltage);
    }
    ms_sar_set_vol(channel & 3, ref_voltage);
    return (str - buf);
}
DEVICE_ATTR(voltage, 0644, voltage_show, voltage_store);
...
static int infinity_sar_probe(struct platform_device *pdev)
{
    ...
    m_u8Init = 0;
    channel  = 0;
    retval   = of_property_read_u32(pdev->dev.of_node, "sar_ref_voltage", &ref_voltage);
    if (retval < 0)
    {
        printk("%s:%d of_property_read_u32() failed\n", __func__, __LINE__);
        return retval;
    }
    ...
    if (sar->msysdev)
    {
        ...
        device_create_file(sar->msysdev, &dev_attr_voltage);
    }
    return 0;
}

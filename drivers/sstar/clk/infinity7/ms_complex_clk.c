...
static unsigned long ms_cpuclk_recalc_rate(struct clk_hw *clk_hw, unsigned long parent_rate)
{
     ...
     * Ref_clk = CPU_CLK * 2 / 24
     */
    rate = (div64_u64(432000000llu * 524288, lpf_value) * 2 / post_div * 24 / 2);
...
static int ms_cpuclk_set_rate(struct clk_hw *clk_hw, unsigned long rate, unsigned long parent_rate)
{
    ...
    lpf_value = (U32)(div64_u64(432000000llu * 524288, (rate * 2 / 24) * post_div / 2));
...
static int ms_venpll_enable(struct clk_hw *hw)
{
    ...
    OUTREG8(BASE_REG_VENPLL_PA + REG_ID_16, 0xbc);  // turn on d2s & vco_en
    return 0;
}

static void ms_venpll_disable(struct clk_hw *hw)
{
    CLRREG16(BASE_REG_VENPLL_PA + REG_ID_0A, BIT14);
    OUTREG8(BASE_REG_VENPLL_PA + REG_ID_16, 0x0);   // turn off d2s & vco_en
...
static unsigned long ms_venpll_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
    ...
    val                      = INREG16(BASE_REG_VENPLL_PA + REG_ID_12);
    val                      = val | INREG8(BASE_REG_VENPLL_PA + REG_ID_13) << 16;
    rate_in_mhz              = 432 * 6 * 524288 / val;
...
static int ms_venpll_set_rate(struct clk_hw *clk_hw, unsigned long rate, unsigned long parent_rate)
{
    ...
    if ((rate == 708000000) || (rate == 672000000) || (rate == 636000000) || (rate == 576000000) || (rate == 504000000)
        || (rate == 456000000) || (rate == 408000000) || (rate == 348000000))
    {
        unsigned int rate_in_mhz = rate / 1000000;
        // reg val = 432 * 24 * 2^19/ (rate * div)
        unsigned int val = 432 * 6 * 524288 / rate_in_mhz;
        // high 16bit trigger update, so set low 16bit first
        OUTREG16(BASE_REG_VENPLL_PA + REG_ID_12, val & 0xffff);
        OUTREG8(BASE_REG_VENPLL_PA + REG_ID_13, (val >> 16) & 0xff);

        udelay(200);
        SETREG16(BASE_REG_VENPLL_PA + REG_ID_0A, BIT14);
        udelay(10);
    }
    else
    {
...
static int ms_fuart0_synth_out_set_rate(struct clk_hw *clk_hw, unsigned long rate, unsigned long parent_rate)
{
        ...
        || (rate == 164571000))
    {
        for (i = 0; i < sizeof(fuart0_synth_out_tbl) / sizeof(struct fuart_clk_tbl); i++)
        {
            ...
            {
                OUTREG16(BASE_REG_CLKGEN_PA + REG_ID_35, fuart0_synth_out_tbl[i].val);
            }
        }
    }
    else

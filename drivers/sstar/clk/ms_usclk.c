...
static ssize_t set_rate_show(struct device *dev, struct device_attribute *attr, char *buf)
{
    struct usclk_data *pdata = dev_get_drvdata(dev);
...
static ssize_t set_rate_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
    int                ret = 0;
    unsigned long      rate;
    struct usclk_data *pdata = dev_get_drvdata(dev);
...
static const struct attribute *usclk_attrs[] = {&dev_attr_enable.attr, &dev_attr_rate.attr, NULL};
...
static int usclk_setup(void)
{
    ...
    for (i = 0; i < g_clock_count; i++)
    {
        ...
        g_dev[i] = device_create(g_clk_class, NULL, MKDEV(0, 0), NULL, of_clk_get_parent_name(np, i));

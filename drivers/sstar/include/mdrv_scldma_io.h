 ...
 * sysfs R/W mode: R/W
 *
 * sysfs Usage & Description: R:check trig off count and dma line count ,W: if 0 reset scldma status ,if echo 1 all
 * reset
 *
...
#define IOCTL_SCLDMA_BUFFER_QUEUE_HANDLE_CONFIG_NR \
    (12)                         ///<  The IOCTL NR definition, IOCTL_SCLDMA_BUFFER_QUEUE_HANDLE_CONFIG_NR
...
/**
 * Used to set parameters of in buffer configurate ,buffer address and number,dma mode,color mode,resolution, use
 * ST_IOCTL_SCLDMA_BUFFER_CONFIG.
 */
...
/**
 * Used to set parameters of out buffer configurate,buffer address and number ,dma mode,color mode,resolution, use
 * ST_IOCTL_SCLDMA_BUFFER_CONFIG.
 */
...
/**
 * Used to set Rpoint(by OMX) and get Wpoint with warning flag,it's like buffer control in DMA_R device, use
 * ST_IOCTL_SCLDMA_ACTIVE_BUFFER_CONFIG.
 */
#define IOCTL_SCLDMA_GET_IN_ACTIVE_BUFFER_CONFIG _IO(IOCTL_SCLDMA_MAGIC, IOCTL_SCLDMA_GET_IN_ACTIVE_BUFFER_CONFIG_NR)

/**
 * Used to set Rpoint(by OMX) and get Wpoint with warning flag,it's like buffer control in DMA_W device, use
 * ST_IOCTL_SCLDMA_ACTIVE_BUFFER_CONFIG.
 */

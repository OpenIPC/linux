...
typedef struct
{
    ...
    unsigned char
        u8nlm_luma_adap_gain_lut[VIP_NLM_LUMAGAIN_NUM]; ///<  reg_nlm_luma_adap_gain_lut0-63,adjust register squence
    unsigned char u8nlm_post_luma_adap_gain_lut[VIP_NLM_POSTLUMA_NUM]; ///<  reg_nlm_post_luma_adap_gain_lut0-15,adjust
                                                                       ///<  register squence
    unsigned char
        u8nlm_dist_weight_7x7_lut[VIP_NLM_DISTWEIGHT_NUM]; ///<  reg_nlm_dist_weight_7x7_lut0-8,adjust register squence
...
typedef struct
{
    ...
    unsigned char u8low_diff_thrd_and_adaptive_gain_step
        [VIP_PEAKING_BAND_TERM_NUM];                    ///<  alternation(reg_band1_adaptive_gain_step,
                                                        ///<  reg_band1_low_diff_thrd) 1-8
...
typedef struct
{
    ...
    unsigned short
        u16FullRange[E_VIP_FCC_Y_DIS_NUM]; ///<  reg_fcc_fr_win_cr_down ~reg_fcc_fr_win_cb_up full range 16 bit
...
typedef struct
{
    ...
    unsigned short
        u16FullRange[E_VIP_FCC_Y_DIS_NUM]; ///<  reg_fcc_fr_win9_cr_down ~reg_fcc_fr_win9_cb_up full range 16 bit
...
{
    unsigned int                    VerChk_Version; ///< VerChk version
    EN_VIP_IOCTL_VTRACK_ENABLE_TYPE EnType;         ///< en
    unsigned char                   u8framerate;    ///< rate
    // VerChk_Version & VerChk_Size must be the latest 2 parameter and
    // the order can't be changed
    unsigned int VerChk_Size; ///< VerChk Size
} __attribute__((__packed__)) ST_IOCTL_VIP_VTRACK_ONOFF_CONFIG;
/**
...
typedef struct
{
    ...
    unsigned char  u8SetKey[VIP_VTRACK_KEY_SETTING_LENGTH]; ///< key
    unsigned char  u8SetUserDef[VIP_VTRACK_SETTING_LENGTH]; ///< userdef
    unsigned char  u8OperatorID;                            ///< operator
    unsigned short u16Timecode;                             ///< timecode
    // VerChk_Version & VerChk_Size must be the latest 2 parameter and
    // the order can't be changed
    unsigned int VerChk_Size; ///< VerChk Size
} __attribute__((__packed__)) ST_IOCTL_VIP_VTRACK_CONFIG;

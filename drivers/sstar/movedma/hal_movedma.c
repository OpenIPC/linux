...
static void *            m_u32MdmaTxDoneCBParm = NULL;
...
static irqreturn_t HalMoveDma_ISR(int irq, void *priv)
{
    ...
    if (intsrc & MOVEDMA_INT_MOVE0_DONE)
    {
        ...
        {
            (*m_pfMdmaTxDoneCBFunc)(m_u32MdmaTxDoneCBParm);
        }
    }
    else
    {
...
HalMoveDmaErr_e HalMoveDma_MoveData(HalMoveDmaParam_t *ptMoveDmaParam)
{
    ...
    m_u32MdmaTxDoneCBParm = ptMoveDmaParam->CallBackParm;
    ...
    if (ptMoveDmaParam->u32Mode == HAL_MOVEDMA_MSPI)
    {
        if (ptMoveDmaParam->pstMspist)
        {
            ...
            {
                g_ptKeMoveDma->reg_spi_device_select =
                    ptMoveDmaParam->pstMspist->u32DeviceSelect; // 0 select mspi0 , 1 select mspi1
            }
            else
            {
    ...
    g_ptKeMoveDma->reg_dma_move0_src_miu_sel =
        (ptMoveDmaParam->u32SrcMiuSel) ? (REG_DMA_MOVE0_SEL_MIU1) : (REG_DMA_MOVE0_SEL_MIU0);
    g_ptKeMoveDma->reg_dma_move0_dst_miu_sel =
        (ptMoveDmaParam->u32DstMiuSel) ? (REG_DMA_MOVE0_SEL_MIU1) : (REG_DMA_MOVE0_SEL_MIU0);

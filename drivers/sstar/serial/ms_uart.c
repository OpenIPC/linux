...
#if UART_DEBUG
#define UART_DBG(fmt, arg...) printk(KERN_ERR fmt, ##arg)
#else
...
static void ms_uart_set_termios(struct uart_port *p, struct ktermios *pTermios_st, struct ktermios *pOld_st)
{
    ...
    p->read_status_mask = UART_LSR_OE | UART_LSR_THRE | UART_LSR_DR;
    if (pTermios_st->c_iflag & INPCK)
    {
        p->read_status_mask |= UART_LSR_FE | UART_LSR_PE;
    }
    if (pTermios_st->c_iflag & (IGNBRK | BRKINT | PARMRK))
    {
        p->read_status_mask |= UART_LSR_BI;
    }
    ...
    if (pTermios_st->c_iflag & IGNPAR)
    {
        p->ignore_status_mask |= UART_LSR_PE | UART_LSR_FE;
    }
    if (pTermios_st->c_iflag & IGNBRK)
    {
        p->ignore_status_mask |= UART_LSR_BI;
        /*
        ...
        if (pTermios_st->c_iflag & IGNPAR)
        {
            p->ignore_status_mask |= UART_LSR_OE;
        }
    }
    /*
     * ignore all characters if CREAD is not set
     */
    if ((pTermios_st->c_cflag & CREAD) == 0)
    {
        p->ignore_status_mask |= UART_LSR_DR;
    }

...
void MHal_EMAC_Power_On_Clk(void* hal, struct device *dev )
{
    ...
    */
    ...
    if (PHY_INTERFACE_MODE_RMII == pHal->phy_mode)
    {
        ...
        // workaround for padmux conflict
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_PADTOP, 0xc0, 0x00);
        MHal_EMAC_WritReg8(EMAC_RIU_REG_BASE, REG_BANK_PADTOP, 0xc1, 0x00);
    }
...
static void _MHal_EMAC_albany_clk_on(void* hal)
{
    ...
    //bit8: enable 10M HW control LED1
    //bit9: enable invert function for LED1
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY0, 0xfd, 0x03);

    //set scale factor of LMS & LPF
    MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x10, 0x46); //1515 08

    if(CamOsChipRevision() == 0x1)
    {
        //set scale factor of LMS & LPF
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x10, 0x53); //1515 08
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x11, 0x00);

        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x16, 0x00); //1515 0b
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x17, 0x00);

        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x18, 0x00); //1515 0c
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x19, 0x10);

        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x1A, 0x00); //1515 0d
        MHal_EMAC_WritReg8(pHal->phyRIU, REG_BANK_ALBANY1, 0x1B, 0x01);
    }
#if 0

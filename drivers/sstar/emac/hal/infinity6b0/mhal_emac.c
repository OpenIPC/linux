...
#ifdef CONFIG_EMAC_DPHY_REINIT
static int _MHal_EMAC_DPHY_REINIT(void* hal);
static int _MHal_EMAC_DPHY_SPEED_UPDATE(void* hal);
#endif
//-------------------------------------------------------------------------------------------------
//  Local variables
//-------------------------------------------------------------------------------------------------
...
void MHal_EMAC_update_speed_duplex(void* hal, int speed, int duplex )
{
    ...
    printk("[%s] speed: %d , duplex: %d\n",__FUNCTION__, speed, duplex);
    ...
    if( ((old_speed != speed)||(old_duplex != duplex)) && (pHal->phy_mode!=PHY_INTERFACE_MODE_RMII) )
    {
        _MHal_EMAC_DPHY_SPEED_UPDATE(hal);
    }
    old_speed = speed;
    old_duplex = duplex;
#endif

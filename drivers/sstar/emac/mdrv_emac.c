...
//  EMAC Function
//-------------------------------------------------------------------------------------------------
static u8   MDev_EMAC_ClkDisable(struct emac_handle *hemac);
static u8   MDev_EMAC_ClkEnable(struct emac_handle *hemac);
...
static void _MDev_EMAC_Swing(void *hal, int param, u8 isAbsolute);
...
static void _MDev_EMAC_ClkEnable(struct emac_handle *hemac, int en)
{
    int         nclk;
    int         i;
    ...
    for (i = 0; i < nclk; i++)
    {
        ...
        if ((en) && !__clk_is_enabled(emac_clks))
        {
...
#endif
    _MDev_EMAC_ClkEnable(hemac, 0);
    MHal_EMAC_Power_Off_Clk(hemac->hal, NULL);
    hemac->bEmacClkOn = 0;
    return 1;
}
...
#endif
    _MDev_EMAC_ClkEnable(hemac, 1);
    MHal_EMAC_Power_On_Clk(hemac->hal, NULL);
    hemac->bEmacClkOn = 1;
    return 1;
}
...
#endif
        printk(KERN_INFO "[%s] EMAC Link Up \n", __FUNCTION__);
    }
    else
    {
        // unsigned long flags;
        ...
        printk(KERN_INFO "[%s] EMAC Link Down \n", __FUNCTION__);
    }
    spin_unlock_irqrestore(&hemac->mutexNetIf, flag1);
...
static void _MDev_EMAC_Timing_adjust(void *hal, u8 rise, u8 delay, u8 duty, u8 phase)
{
    u32 val;
    u8  base_offset = 0;

    MHal_EMAC_read_phy(hal, 0, 0x177, &val); // Bank: phy2_77
    if (rise)
    {
        base_offset = 8;
    }
    ...
    MHal_EMAC_write_phy(hal, 0, 0x177, val);
}
...
static void _MDev_EMAC_Swing(void *hal, int param, u8 isAbsolute)
{
    ...
    {
        // enable
        MHal_EMAC_read_phy(hal, 0, 0x02e, &val); // Bank: phy0_2e
        val = val | 0x1;
        MHal_EMAC_write_phy(hal, 0, 0x02e, val);
    }
    // swing
    MHal_EMAC_read_phy(hal, 0, 0x142, &val); // Bank: phy2_42
    ...
    MHal_EMAC_write_phy(hal, 0, 0x142, val);
    MHal_EMAC_read_phy(hal, 0, 0x142, &val); // Bank: phy2_42
    tmp = ((val >> 8) & 0x1f);
    printk("swing : 0x%02x \n", tmp);
}
...
static ssize_t turndrv_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...

    {
        ...
        if (2 == sscanf(buf, "%s %d", cmd, &param))
        {
            if (!strncmp(cmd, "swing_100_st", strlen("swing_100_st")))
            {
                sscanf(buf, "%s %x", cmd, &param);
                _MDev_EMAC_Swing(hemac->hal, param, 1);
            }
            else if (!strncmp(cmd, "swing_100", strlen("swing_100")))
            {
                _MDev_EMAC_Swing(hemac->hal, param, 0);
            }
            else if (!strncmp(cmd, "rx_imp", strlen("rx_imp")))
            {
                u32 val;
                int tmp;
                ...
                // enable
                MHal_EMAC_read_phy(hemac->hal, 0, 0x130, &val); // Bank: phy2_30
                val = val | 0x4;
                MHal_EMAC_write_phy(hemac->hal, 0, 0x130, val);

                MHal_EMAC_read_phy(hemac->hal, 0, 0x134, &val); // Bank: phy2_34
                val = val | 0x8000;
                MHal_EMAC_write_phy(hemac->hal, 0, 0x134, val);
                ...
                MHal_EMAC_read_phy(hemac->hal, 0, 0x130, &val); // Bank: phy2_30[10:7]
                val = val & ~(0xf << 7);
                val = val | tmp << 7;
                MHal_EMAC_write_phy(hemac->hal, 0, 0x130, val);

                MHal_EMAC_read_phy(hemac->hal, 0, 0x141, &val); // Bank: phy2_41[7:4]
                val = val & ~(0xf << 4);
                val = val | tmp << 4;
                MHal_EMAC_write_phy(hemac->hal, 0, 0x141, val);
            }
        }
    }
    {
        ...
        if (strcmp(argv[0], "timing") == 0)
        {
            ...
            _MDev_EMAC_Timing_adjust(hemac->hal, rise, delay, duty, phase);
        }
    }

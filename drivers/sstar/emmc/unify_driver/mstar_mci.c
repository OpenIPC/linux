...
static void mstar_mci_pre_dma_read(struct mstar_mci_host *pSstarHost_st)
{
    ...
    {
        REG_FCIE_W(FCIE_TOGGLE_CNT,
                   (g_eMMCDrv.u32_DrvFlag & DRV_FLAG_SPEED_HS200) ? TOGGLE_CNT_512_CLK_R : TOGGLE_CNT_256_CLK_R);
...
static U32 mstar_mci_dma_write(struct mstar_mci_host *pSstarHost_st)
{
    ...
    {
        REG_FCIE_W(FCIE_TOGGLE_CNT,
                   (g_eMMCDrv.u32_DrvFlag & DRV_FLAG_SPEED_HS200) ? TOGGLE_CNT_512_CLK_W : TOGGLE_CNT_256_CLK_W);
        REG_FCIE_SETBIT(FCIE_MACRO_REDNT, BIT_MACRO_DIR);
    }
    pCmd_st        = pSstarHost_st->cmd;
    pData_st       = pCmd_st->data;
    ...
    for (i = 0; i < pData_st->sg_len; i++)
    {
        ...
        do
        {
            ...
            split_len = dmalen > g_eMMCDrv.BurstWriteLen_t.u16_BestBrustLen ? g_eMMCDrv.BurstWriteLen_t.u16_BestBrustLen
                                                                            : dmalen;
            split_len = split_len == g_eMMCDrv.BurstWriteLen_t.u16_WorstBrustLen ? split_len >> 1 : split_len;
#else
            split_len = dmalen;
#endif

            if (split_unit_len && (!split_up_len || dmalen <= split_up_len))
                split_len = dmalen > split_unit_len ? split_unit_len : dmalen;
            ...
            if (eMMC_FCIE_WaitEvents(FCIE_MIE_EVENT, BIT_CARD_DMA_END, eMMC_GENERIC_WAIT_TIME) != eMMC_ST_SUCCESS)
            {
...
static void mstar_mci_completed_command(struct mstar_mci_host *pSstarHost_st)
{
    ...
    if ((u16_st & BIT_SD_FCIE_ERR_FLAGS)                   /*|| mstar_SD_CardChange()*/
        || (g_eMMCDrv.u32_DrvFlag & DRV_FLAG_ERROR_RETRY)) // || ((mmc_resp_type(pCmd_st)==MMC_RSP_R1) &&
                                                           // (pCmd_st->resp[0]&eMMC_ERR_R1_NEED_RETRY)))
    {
        ...
        {
#if defined(eMMC_RSP_FROM_RAM) && eMMC_RSP_FROM_RAM
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: ST:%Xh, CMD:%u, retry: %u, flag: %Xh, R1 err: %Xh\n",
        ...
        if (pCmd_st->resp[0] & eMMC_ERR_R1_31_0)
        {
            ...
            if (pCmd_st->opcode == 12)
                ...
                eMMC_debug(0, 0, "eMMC Warn: CMD%u R1 error: %Xh, arg %08x, blocks %08x\n", pCmd_st->opcode,
                           pCmd_st->resp[0], pCmd_st->arg, pCmd_st->data->blocks);
        }
    }
    ...
    if (err)
    {
        ...
        if (pData_st->flags & MMC_DATA_WRITE)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: w, cmd.%u arg.%Xh, ST: %Xh or TO\n", pCmd_st->opcode,
                       pCmd_st->arg, REG_FCIE(FCIE_SD_STATUS));
        }
        else if (pData_st->flags & MMC_DATA_READ)
        {
...
// ------------------------------------
        ...
        // ------------------------------------
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: cmd.%u arg.%Xh St: %Xh, retry %u \n", pCmd_st->opcode,
                   pCmd_st->arg, REG_FCIE(FCIE_SD_STATUS), u8_retry_cmd);
...
static ssize_t fcie_pwrsvr_gpio_trigger_store(struct device *dev, struct device_attribute *attr, const char *buf,
                                              size_t count)
{
...
DEVICE_ATTR(fcie_pwrsvr_gpio_trigger, S_IRUSR | S_IWUSR, fcie_pwrsvr_gpio_trigger_show, fcie_pwrsvr_gpio_trigger_store);
...
static ssize_t fcie_pwrsvr_gpio_bit_store(struct device *dev, struct device_attribute *attr, const char *buf,
                                          size_t count)
{
...
DEVICE_ATTR(fcie_pwrsvr_gpio_bit, S_IRUSR | S_IWUSR, fcie_pwrsvr_gpio_bit_show, fcie_pwrsvr_gpio_bit_store);
...
static ssize_t fcie_pwrsvr_gpio_addr_store(struct device *dev, struct device_attribute *attr, const char *buf,
                                           size_t count)
{
...
DEVICE_ATTR(fcie_pwrsvr_gpio_addr, S_IRUSR | S_IWUSR, fcie_pwrsvr_gpio_addr_show, fcie_pwrsvr_gpio_addr_store);
...
static ssize_t fcie_pwrsvr_gpio_enable_store(struct device *dev, struct device_attribute *attr, const char *buf,
                                             size_t count)
{
...
DEVICE_ATTR(fcie_pwrsvr_gpio_enable, S_IRUSR | S_IWUSR, fcie_pwrsvr_gpio_enable_show, fcie_pwrsvr_gpio_enable_store);
...
static ssize_t emmc_sanitize_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t count)
{
...
DEVICE_ATTR(emmc_sanitize, S_IRUSR | S_IWUSR, emmc_sanitize_show, emmc_sanitize_store);

static struct attribute *mstar_mci_attr[] = {
    &dev_attr_fcie_pwrsvr_gpio_enable.attr,  &dev_attr_fcie_pwrsvr_gpio_addr.attr, &dev_attr_fcie_pwrsvr_gpio_bit.attr,
    &dev_attr_fcie_pwrsvr_gpio_trigger.attr, &dev_attr_emmc_sanitize.attr,         NULL,
};

...
U32 eMMC_FCIE_PollingEvents(U32 u32_RegAddr, U16 u16_Events, U32 u32_MicroSec)
{
    ...
    if (u32_i == u32_MicroSec)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: %u us, Reg.%04Xh: %04Xh, but wanted: %04Xh\n",
                   u32_MicroSec * u32_DelayX, (u32_RegAddr - FCIE_MIE_EVENT) >> REG_OFFSET_SHIFT_BITS, u16_val,
                   u16_Events);
...
U32       eMMC_FCIE_ErrHandler_Retry(void)
{
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL_WARNING, 0, "eMMC Warn: slow clk to %u.%uMHz, %Xh\n", g_eMMCDrv.u32_ClkKHz / 1000,
               (g_eMMCDrv.u32_ClkKHz % 1000) / 100, g_eMMCDrv.u16_ClkRegVal);
...
U32 eMMC_ExtCSD_Config(void)
{
    ...
    g_eMMCDrv.GP_Part[0].u32_PartSize =
...
U32 eMMC_CMD35_CMD36(U32 u32_eMMCBlkAddr, U8 u8_CmdIdx)
{
    ...
    { // check status
        ...
        if (u16_reg & (BIT_SD_RSP_TIMEOUT | BIT_SD_RSP_CRC_ERR))
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD%u Reg.12: %04Xh \n", u8_CmdIdx, u16_reg);
        ...
        { // CMD3 ok, do things here
            ...
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD%u check R1 error: %Xh\n", u8_CmdIdx, u32_err);
                return u32_err;
            }
            eMMC_KEEP_RSP(g_eMMCDrv.au8_Rsp, u8_CmdIdx);
        }
...
U32 eMMC_CMD38(void)
{
    ...
    else
    {
        ...
        if (u16_reg & (BIT_SD_RSP_TIMEOUT | BIT_SD_RSP_CRC_ERR))
        {
            ...
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD38 Reg.12: %04Xh\n", u16_reg);
            return u32_err;
        }
        else
        { // CMD38 ok, do things here
            ...
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD38 check R1 error: %Xh\n", u32_err);
                return u32_err;
            }
    ...
    { // check status
        ...
        { // CMD13 ok, do things here
            ...
            if (eMMC_ST_SUCCESS != u32_err)
            {
                ...
                eMMC_FCIE_ErrHandler_Stop();
#else
                eMMC_debug(0, 1, "eMMC: CMD13 check R1 error: %Xh, should not retry\n", u32_err);
#endif
            }
...
// check FCIE
    ...
    if (IF_TEST_RETRY(33) || eMMC_ST_SUCCESS != u32_err || (u16_reg & BIT_SD_FCIE_ERR_FLAGS))
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD17 Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD17 Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n", u16_reg,
...
LABEL_SEND_CMD:
    ...
    if (IF_TEST_RETRY(28) || eMMC_ST_SUCCESS != u32_err || (u16_reg & BIT_SD_FCIE_ERR_FLAGS))
    {
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD17 CIFD Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
                   u16_reg, u32_err, u8_retry_fcie, u32_arg);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
...
U32 eMMC_CMD12(U16 u16_RCA)
{
    ...
    { // check status
        ...
        if (IF_TEST_RETRY(3) || (u16_reg & (BIT_SD_RSP_TIMEOUT | BIT_SD_RSP_CRC_ERR)))
        {
            ...
            {
                u8_retry_fcie++;
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD12 check Reg.12: %Xh, %Xh, retry: %u\n", u16_reg,
                           u32_err, u8_retry_fcie);
                eMMC_FCIE_ErrHandler_ReInit();
                // eMMC_FCIE_ErrHandler_Retry();
                // goto LABEL_SEND_CMD;
            ...
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD12 check Reg.12: %Xh, %Xh, retry: %u\n", u16_reg,
                       u32_err, u8_retry_fcie);
            eMMC_FCIE_ErrHandler_Stop();
        }
        else
...
// check FCIE
    ...
    u32_err = eMMC_FCIE_WaitEvents(FCIE_MIE_EVENT, BIT_MIU_LAST_DONE | BIT_CARD_DMA_END,
                                   TIME_WAIT_n_BLK_END * (1 + (u16_BlkCnt >> 11)));
...
LABEL_RETRY:
    ...
    if (u32_ChkSum != g_eMMCDrv.BurstWriteLen_t.u32_ChkSum || 0 == g_eMMCDrv.BurstWriteLen_t.u16_BestBrustLen)
    {
...
LABEL_SEND_CMD:
    ...
    if (IF_TEST_RETRY(13) || (u16_reg & (BIT_SD_RSP_TIMEOUT | BIT_SD_RSP_CRC_ERR)))
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD25 Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
                       u16_reg, u32_err, u8_retry_fcie, u32_arg);
            eMMC_CMD12_NoCheck(g_eMMCDrv.u16_RCA);
            eMMC_FCIE_ErrHandler_ReInit();
            eMMC_FCIE_ErrHandler_Retry();
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD25 Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n", u16_reg,
                   u32_err, u8_retry_fcie, u32_arg);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
...
LABEL_SEND_CMD:
    ...
    if (IF_TEST_RETRY(34) || (u16_reg & (BIT_SD_RSP_TIMEOUT | BIT_SD_RSP_CRC_ERR)))
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD24 Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
                       u16_reg, u32_err, u8_retry_fcie, u32_arg);
            eMMC_FCIE_ErrHandler_ReInit();
            eMMC_FCIE_ErrHandler_Retry();
            eMMC_DMA_UNMAP_address(u32_DataMapAddr, eMMC_SECTOR_512BYTE, 0);
        ...
        u32_err = eMMC_ST_ERR_CMD25;
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD24 Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n", u16_reg,
                   u32_err, u8_retry_fcie, u32_arg);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
    ...
    if (IF_TEST_RETRY(11) || eMMC_ST_SUCCESS != u32_err || (u16_reg & (BIT_SD_W_FAIL | BIT_SD_W_CRC_ERR)))
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Warn: CMD24 %sReg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD24 %sReg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
                   (eMMC_ST_SUCCESS != u32_err) ? "TO " : "", u16_reg, u32_err, u8_retry_fcie, u32_arg);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
...
LABEL_SEND_CMD:
    ...
    if (IF_TEST_RETRY(23) || eMMC_ST_SUCCESS != u32_err || (u16_reg & (BIT_SD_W_FAIL | BIT_SD_W_CRC_ERR)))
    {
        ...
        {
            u8_retry_fcie++;
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: CMD24 TO Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
                       u16_reg, u32_err, u8_retry_fcie, u32_arg);
            eMMC_FCIE_ErrHandler_ReInit();
            eMMC_FCIE_ErrHandler_Retry();
            goto LABEL_SEND_CMD;
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: CMD24 CIFD Reg.12: %04Xh, Err: %Xh, Retry: %u, Arg: %Xh\n",
                   u16_reg, u32_err, u8_retry_fcie, u32_arg);
        eMMC_FCIE_ErrHandler_Stop();
        goto LABEL_END;
    }
...
U32 eMMC_GetR1(void)
{
    return (g_eMMCDrv.au8_Rsp[1] << 24) | (g_eMMCDrv.au8_Rsp[2] << 16) | (g_eMMCDrv.au8_Rsp[3] << 8)
           | g_eMMCDrv.au8_Rsp[4];
}

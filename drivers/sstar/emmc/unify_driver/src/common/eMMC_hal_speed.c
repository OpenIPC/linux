...
#if 0
    if(DRV_FLAG_SPEED_HS200 == eMMC_SPEED_MODE(emmc_ip))
    {
        u32_ErrSpeed = eMMC_SetBusSpeed(emmc_ip, eMMC_SPEED_HIGH);
    }
    else if(DRV_FLAG_SPEED_HS400 == eMMC_SPEED_MODE(emmc_ip))
    {
        u32_ErrSpeed = eMMC_SetBusSpeed(emmc_ip, eMMC_SPEED_HIGH);
        u32_ErrWidth = eMMC_SetBusWidth(emmc_ip, g_eMMCDrv[emmc_ip].u16_of_buswidth, 0);
    }
    else
#else
    if (DRV_FLAG_SPEED_HS200 == eMMC_SPEED_MODE(emmc_ip) || DRV_FLAG_SPEED_HS400 == eMMC_SPEED_MODE(emmc_ip))
    {
...
void eMMC_DumpDDR48TTable(eMMC_IP_EmType emmc_ip)
{
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "\n  eMMC DDR Timing Table: Cnt:%u CurIdx:%u \n",
               g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt, g_eMMCDrv[emmc_ip].TimingTable_t.u8_CurSetIdx);

    for (u16_i = 0; u16_i < g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt; u16_i++)
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "    Set:%u: clk:%02Xh, DQS:%02Xh, Cell:%02Xh \n", u16_i,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_Clk,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].Param.u8_DQS,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].Param.u8_Cell);
#endif
}

void eMMC_FCIE_SetDDR48TimingReg(U8 u8_DQS, U8 u8_DelaySel)
{
#if USE_SPEED_DRIVER // ifndef IP_FCIE_VERSION_5
...
void eMMC_DumpATopTable(void)
{
#if USE_SPEED_DRIVER
    U16            u16_i;
    eMMC_IP_EmType emmc_ip = gu8_emmcPADOrderSlot[0];

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "\n  eMMC ATop Timing Table: Ver:%Xh Cnt:%u CurIdx:%u \n",
               g_eMMCDrv[emmc_ip].TimingTable_t.u32_VerNo, g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt,
               g_eMMCDrv[emmc_ip].TimingTable_t.u8_CurSetIdx);
    ...
    for (u16_i = 0; u16_i < g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt; u16_i++)
    {
        ...
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "      Clk: %04Xh, Reg2Ch: %02Xh, Skew4: %02Xh, Result: %08Xh, Idx: %u \n",
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_Clk,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_Reg2Ch,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_Skew4,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u32_ScanResult,
                   g_eMMCDrv[emmc_ip].TimingTable_Ext_t.Set.u8_Skew4Idx);

#if defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "      Skew2: %02Xh, RXDLL: %Xh, RXDLLCnt: %02Xh \n",
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_Skew2,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_Cell,
                   g_eMMCDrv[emmc_ip].TimingTable_t.Set[u16_i].u8_CellCnt);
#endif
    }
#endif
}
...
void eMMC_FCIE_SetATopTimingReg(U8 u8_SetIdx)
{
    ...
    if (g_eMMCDrv[emmc_ip].u8_PadType == FCIE_eMMC_DDR)
    {
        REG_FCIE_CLRBIT(reg_emmcpll_0x6c, BIT_DQS_MODE_MASK | BIT_DQS_DELAY_CELL_MASK);

        REG_FCIE_SETBIT(reg_emmcpll_0x6c,
                        g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_SetIdx].u8_Skew4 << BIT_DQS_MODE_SHIFT);
        REG_FCIE_SETBIT(reg_emmcpll_0x6c,
                        g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_SetIdx].u8_Cell << BIT_DQS_DELAY_CELL_SHIFT);
    }

#if (defined(ENABLE_eMMC_HS200) && ENABLE_eMMC_HS200) || (defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400)
    else // HS400,HS200
    {
        if (g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_SetIdx].u8_Reg2Ch)
        {
        ...
        REG_FCIE_SETBIT(reg_emmcpll_0x03, g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_SetIdx].u8_Skew4 << 12);

#if defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400
        eMMC_FCIE_SetDelayLatch(g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_SetIdx].u8_Cell);

        REG_FCIE_CLRBIT(reg_emmcpll_0x03, BIT_SKEW2_MASK);
        REG_FCIE_SETBIT(reg_emmcpll_0x03, g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_SetIdx].u8_Skew2 << 4);
#endif
    }
#endif
...
void eMMC_FCIE_Apply_Reg(U8 u8_SetIdx)
{
    ...
    for (i = 0; i < g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt; i++)
    {
        if (g_eMMCDrv[emmc_ip]
                .TimingTable_G_t.RegSet[u8_SetIdx * g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt + i]
                .u16_OpCode
            == REG_OP_W)
        {
            REG_FCIE_W(
                RIU_BASE
                    + g_eMMCDrv[emmc_ip]
                          .TimingTable_G_t.RegSet[u8_SetIdx * g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt + i]
                          .u32_RegAddress, // address = (bank address+ register offset) << 2
                g_eMMCDrv[emmc_ip]
                    .TimingTable_G_t.RegSet[u8_SetIdx * g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt + i]
                    .u16_RegValue);
        }
        else if (g_eMMCDrv[emmc_ip]
                     .TimingTable_G_t.RegSet[u8_SetIdx * g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt + i]
                     .u16_OpCode
                 == REG_OP_CLRBIT)
        {
...
void eMMC_DumpGeneralTable(void)
{
    ...
    for (i = 0; i < g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_SetCnt; i++)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "==========Set 0x%X==========\n", i);

        for (j = 0; j < g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt; j++)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL, 0, "0x%X 0x%08X 0x%04X 0x%04X\n",
                       g_eMMCDrv[emmc_ip]
                           .TimingTable_G_t.RegSet[i * g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt + j]
                           .u16_OpCode,
                       g_eMMCDrv[emmc_ip]
                           .TimingTable_G_t.RegSet[i * g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_RegisterCnt + j]
                           .u32_RegAddress,
...
U32 eMMC_FCIE_ChooseSpeedMode(void)
{
...
#if defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400
    if (g_eMMCDrv[emmc_ip].u8_ECSD196_DevType & eMMC_DEVTYPE_HS400_1_8V)
    {
        u32_err = eMMC_CMD13(emmc_ip, g_eMMCDrv[emmc_ip].u16_RCA);
        if (eMMC_ST_SUCCESS != u32_err)
            return u32_err;
        if (eMMC_GetR1(emmc_ip) & eMMC_R1_DEVICE_IS_LOCKED)
        {
        ...
        if (eMMC_ST_SUCCESS != eMMC_FCIE_EnableFastMode(FCIE_eMMC_HS400))
        {
            ...
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: eMMC_FCIE_DetectHS400Timing fail: %Xh\n", u32_err);
    ...
    if (g_eMMCDrv[emmc_ip].u8_ECSD196_DevType & eMMC_DEVTYPE_HS200_1_8V)
    {
        u32_err = eMMC_CMD13(emmc_ip, g_eMMCDrv[emmc_ip].u16_RCA);
        if (eMMC_ST_SUCCESS != u32_err)
            return u32_err;
...
void eMMC_FCIE_ApplyTimingSet(eMMC_IP_EmType emmc_ip, U8 u8_Idx)
{
    ...
    REG_FCIE_CLRBIT(FCIE_SD_MODE(emmc_ip), BIT_SD_CLK_EN);

#if !(defined(ENABLE_eMMC_ATOP) && ENABLE_eMMC_ATOP)
    // DDR48
    eMMC_clock_setting(emmc_ip, g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_Idx].u8_Clk);
    eMMC_FCIE_SetDDR48TimingReg(g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_Idx].Param.u8_DQS,
                                g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_Idx].Param.u8_Cell);
    g_eMMCDrv[emmc_ip].TimingTable_t.u8_CurSetIdx = u8_Idx;
#else
    // HS400 or HS200 or DDR52
    if (g_eMMCDrv[emmc_ip].u8_PadType != FCIE_eMMC_HS400 || g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_SetCnt == 0)
    {
        eMMC_clock_setting(emmc_ip, g_eMMCDrv[emmc_ip].TimingTable_t.Set[u8_Idx].u8_Clk);
    ...
    // apply ext table
    {
        eMMC_clock_setting(emmc_ip, g_eMMCDrv[emmc_ip].TimingTable_G_t.u32_Clk);
        eMMC_FCIE_Apply_Reg(u8_Idx);
        g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_CurSetIdx = u8_Idx;
    }
...
#if USE_SPEED_DRIVER
#if !(defined(ENABLE_eMMC_ATOP) && ENABLE_eMMC_ATOP)
    eMMC_DumpDDR48TTable(emmc_ip);
#else
    eMMC_DumpATopTable();
    if (g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_SetCnt)
...
U32 eMMC_LoadTimingTable(eMMC_IP_EmType emmc_ip, U8 u8_PadType)
{
    ...
    u32_err = eMMC_CMD18(emmc_ip, u32_eMMCBlkAddr, gau8_eMMC_SectorBuf, 1);
    if (eMMC_ST_SUCCESS != u32_err)
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: load 1 Table fail, %Xh\n", u32_err);

        u32_err = eMMC_CMD18(emmc_ip, u32_eMMCBlkAddr + 1, gau8_eMMC_SectorBuf, 1);
        if (eMMC_ST_SUCCESS != u32_err)
        {
    ...
    {
        u32_err = eMMC_CMD18(emmc_ip, eMMC_HS400EXTTABLE_BLK_0, gau8_eMMC_SectorBuf, 1);
        if (eMMC_ST_SUCCESS != u32_err)
        {
            eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC WARN: load 1st Gen_TTable fail, %Xh\n", u32_err);

            u32_err = eMMC_CMD18(emmc_ip, eMMC_HS400EXTTABLE_BLK_1, gau8_eMMC_SectorBuf, 1);
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Warn: load 2nd Gen_TTables fail, %Xh\n", u32_err);
                g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_SetCnt  = 0;
                g_eMMCDrv[emmc_ip].TimingTable_G_t.u32_ChkSum = g_eMMCDrv[emmc_ip].TimingTable_G_t.u32_VerNo = 0;
                goto LABEL_END_OF_NO_GEN_TABLE;
            }
        }
        memcpy((U8*)&g_eMMCDrv[emmc_ip].TimingTable_G_t, gau8_eMMC_SectorBuf,
               sizeof(g_eMMCDrv[emmc_ip].TimingTable_G_t));

        u32_ChkSum = eMMC_ChkSum((U8*)&g_eMMCDrv[emmc_ip].TimingTable_G_t.u32_VerNo,
                                 (sizeof(g_eMMCDrv[emmc_ip].TimingTable_G_t) - sizeof(U32) /*checksum*/));
...
LABEL_END_OF_NO_TABLE:
    g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt = 0;
#if defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400
    g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_SetCnt  = 0;
    g_eMMCDrv[emmc_ip].TimingTable_G_t.u32_ChkSum = g_eMMCDrv[emmc_ip].TimingTable_G_t.u32_VerNo = 0;
#endif
...
U32 eMMC_FCIE_EnableFastMode_Ex(eMMC_IP_EmType emmc_ip, U8 u8_PadType)
{
    ...
    switch (u8_PadType)
    {
        case FCIE_eMMC_DDR:
            ...
            u32_err = eMMC_SetBusWidth(emmc_ip, g_eMMCDrv[emmc_ip].u16_of_buswidth, 1);
            if (eMMC_ST_SUCCESS != u32_err)
            {
            ...
            if (0 == g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt)
#if defined(ENABLE_eMMC_ATOP) && ENABLE_eMMC_ATOP
                eMMC_clock_setting(emmc_ip, gau8_eMMCPLLSel_52[0]);
#else
...
#if (defined(ENABLE_eMMC_HS200) && ENABLE_eMMC_HS200) || (defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400)
        case FCIE_eMMC_HS200:
            u32_err = eMMC_SetBusWidth(emmc_ip, g_eMMCDrv[emmc_ip].u16_of_buswidth, 0); // disable DDR
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: HS200 disable DDR fail: %Xh\n", u32_err);
                return u32_err;
            }
            u32_err = eMMC_SetBusSpeed(emmc_ip, eMMC_SPEED_HS200);
            if (eMMC_ST_SUCCESS != u32_err)
            {
            ...
            if (0 == g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt)
                eMMC_clock_setting(emmc_ip, gau8_eMMCPLLSel_200[0]);
...
#if defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400
        case FCIE_eMMC_HS400:
            u32_err = eMMC_SetBusWidth(emmc_ip, g_eMMCDrv[emmc_ip].u16_of_buswidth, 1); // enable DDR
            if (eMMC_ST_SUCCESS != u32_err)
            {
                eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: HS400 enable DDR fail: %Xh\n", u32_err);
                return u32_err;
            }
            u32_err = eMMC_SetBusSpeed(emmc_ip, eMMC_SPEED_HS400);
            if (eMMC_ST_SUCCESS != u32_err)
            {
        ...
        case FCIE_eMMC_HS400_5_1:
            u32_err = eMMC_SetBusWidth(emmc_ip, g_eMMCDrv[emmc_ip].u16_of_buswidth, 2); // enable DDR
            if (eMMC_ST_SUCCESS != u32_err)
            {
    ...
    eMMC_pads_switch(emmc_ip, gu8_emmcPADOrderSlot[emmc_ip], u8_PadType);
#if defined(ENABLE_eMMC_HS400) && ENABLE_eMMC_HS400
    if (g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_SetCnt && u8_PadType == FCIE_eMMC_HS400)
        eMMC_FCIE_ApplyTimingSet(0, g_eMMCDrv[emmc_ip].TimingTable_G_t.u8_CurSetIdx);
    else
#endif
        if (g_eMMCDrv[emmc_ip].TimingTable_t.u8_SetCnt)
        eMMC_FCIE_ApplyTimingSet(0, eMMC_TIMING_SET_MAX);
...
U32 eMMC_FCIE_EnableFastMode(eMMC_IP_EmType emmc_ip, U8 u8_PadType)
{
    ...
    if (u8_PadType == g_eMMCDrv[emmc_ip].u8_PadType)
        return eMMC_ST_SUCCESS;

    u32_err = eMMC_LoadTimingTable(emmc_ip, u8_PadType);
    if (eMMC_ST_SUCCESS != u32_err)
    {
...
#if defined(ENABLE_eMMC_ATOP) && ENABLE_eMMC_ATOP
    {
        ...
        eMMC_clock_setting(emmc_ip, g_eMMCDrv[emmc_ip].TimingTable_t.Set[0].u8_Clk);
        // --------------------------------
        eMMC_clock_setting(emmc_ip, u16_OldClkRegVal);
    }
#endif
#endif
    u32_err = eMMC_FCIE_EnableFastMode_Ex(emmc_ip, u8_PadType);
    if (eMMC_ST_SUCCESS != u32_err)
    {

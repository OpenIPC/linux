...
U32                                                             eMMC_CheckAlignPack(U8 u8_AlignByteCnt)
{
    ...
    if ((uintptr_t) & (g_eMMCTestAlignPack_t.u8_0) & (u8_AlignByteCnt - 1))
    {
        eMMC_debug(eMMC_DEBUG_LEVEL_ERROR, 1, "eMMC Err: not aliged. expect %Xh but %lXh \n", u8_AlignByteCnt,
                   (uintptr_t) & (g_eMMCTestAlignPack_t.u8_0));
        eMMC_die("\n");
...
U32 eMMC_ComapreData(U8 *pu8_Buf0, U8 *pu8_Buf1, U32 u32_ByteCnt)
{
    ...
    if (u32_i != u32_ByteCnt) //&& 0==(g_eMMCDrv[emmc_ip].u32_DrvFlag&DRV_FLAG_TUNING_TTABLE))
    {
...
U32 eMMC_PrintDeviceInfo(eMMC_IP_EmType emmc_ip)
{
    U32 u32_err;
    ...
    // ------------------------------
    u32_err = eMMC_CMD8(emmc_ip, au8_buf);
    if (eMMC_ST_SUCCESS != u32_err)
        return u32_err;
    ...
    switch (g_eMMCDrv[emmc_ip].u8_ECSD192_Ver)
    {
        case 7:
        ...
        case 5:
            ...
            break;
        default:
            eMMC_debug(eMMC_DEBUG_LEVEL, 0, "eMMC V.x, ECSD[192]:%u\n", g_eMMCDrv[emmc_ip].u8_ECSD192_Ver);
    }
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "Support: %s %s %s %s\n",
               (g_eMMCDrv[emmc_ip].u8_ECSD196_DevType & BIT5) ? "HS200 1.2V" : "",
               (g_eMMCDrv[emmc_ip].u8_ECSD196_DevType & BIT4) ? "HS200 1.8V" : "",
               (g_eMMCDrv[emmc_ip].u8_ECSD196_DevType & BIT3) ? "DDR52 1.2V" : "",
               (g_eMMCDrv[emmc_ip].u8_ECSD196_DevType & BIT2) ? "DDR52 1.8/3V" : "");

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "SEC_COUNT: %Xh\n", g_eMMCDrv[emmc_ip].u32_SEC_COUNT);
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "Capacity: %u MB\n", g_eMMCDrv[emmc_ip].u32_SEC_COUNT >> 11);
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "eMMC BUS_WIDTH: %Xh\n", g_eMMCDrv[emmc_ip].u8_BUS_WIDTH);

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "TRIM: %Xh\n", (g_eMMCDrv[emmc_ip].u32_eMMCFlag & eMMC_FLAG_TRIM) ? 1 : 0);

    if (g_eMMCDrv[emmc_ip].u32_eMMCFlag & (eMMC_FLAG_HPI_CMD12 | eMMC_FLAG_HPI_CMD13))
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "HPI: CMD%u \n",
                   (g_eMMCDrv[emmc_ip].u32_eMMCFlag & eMMC_FLAG_HPI_CMD12) ? 12 : 13);
    else
        eMMC_debug(eMMC_DEBUG_LEVEL, 0, "HPI: 0 \n");

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "Reliable Write BlkCnt: %Xh\n", g_eMMCDrv[emmc_ip].u16_ReliableWBlkCnt);

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "Power Off Notification: ECSD[34]:%Xh, Short: %u ms, Long: %u ms\n",
               g_eMMCDrv[emmc_ip].u8_ECSD34_PwrOffCtrl, g_eMMCDrv[emmc_ip].u8_ECSD248_CMD6TO * 10,
               g_eMMCDrv[emmc_ip].u8_ECSD247_PwrOffLongTO * 10);
...
// ------------------------------
    ...
    u32_err = eMMC_CMD9(g_eMMCDrv[emmc_ip].u16_RCA);
    if(eMMC_ST_SUCCESS != u32_err)
        return u32_err;
#endif
    ...
    switch (g_eMMCDrv[emmc_ip].u8_Tran_Speed)
    {
        ...
        case 0x32:
            ...
            break;
        default:
            eMMC_debug(eMMC_DEBUG_LEVEL, 0, "normal TRAN_SPEED: %Xh\n", g_eMMCDrv[emmc_ip].u8_Tran_Speed);
    }
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "TAAC: %Xh, NSAC: %Xh, R2W_FACTOR: %Xh\n", g_eMMCDrv[emmc_ip].u8_TAAC,
               g_eMMCDrv[emmc_ip].u8_NSAC, g_eMMCDrv[emmc_ip].u8_R2W_FACTOR);
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "-------------------------------\n\n");

    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "RCA: %Xh\n", g_eMMCDrv[emmc_ip].u16_RCA);
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "CID:\n");
    eMMC_dump_mem(&g_eMMCDrv[emmc_ip].au8_CID[1], eMMC_MAX_RSP_BYTE_CNT - 1);
    eMMC_debug(eMMC_DEBUG_LEVEL, 0, "-------------------------------\n\n");

#if 0
    u32_err = eMMC_CMD3_CMD7(g_eMMCDrv[emmc_ip].u16_RCA, 7);
        if(eMMC_ST_SUCCESS != u32_err)
            return u32_err;
#endif
    return u32_err;
...
void eMMC_dump_WR_Count(void)
{
    ...
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "TotalRBlk: %llu, TotalWBlk: %llu \n", g_eMMCDrv[emmc_ip].u64_CNT_TotalRBlk,
               g_eMMCDrv[emmc_ip].u64_CNT_TotalWBlk);
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "MinRBlk: %u, MinWBlk: %u \n", g_eMMCDrv[emmc_ip].u32_CNT_MinRBlk,
               g_eMMCDrv[emmc_ip].u32_CNT_MinWBlk);
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "MaxRBlk: %u, MaxWBlk: %u \n", g_eMMCDrv[emmc_ip].u32_CNT_MaxRBlk,
               g_eMMCDrv[emmc_ip].u32_CNT_MaxWBlk);
    ...
    eMMC_dump_mem_32(g_eMMCDrv[emmc_ip].au32_CNT_MinRBlk, 0x200);
    eMMC_debug(eMMC_DEBUG_LEVEL, 1, "au32_CNT_MinWBlk: \n");
    eMMC_dump_mem_32(g_eMMCDrv[emmc_ip].au32_CNT_MinWBlk, 0x200);

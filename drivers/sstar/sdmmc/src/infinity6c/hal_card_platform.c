...
#define REG_CLK_EIGHT_PHASE (0x4e)
...
extern U32_T gu32_EnEightPhase[3];
extern U32_T gu32_TXEightPhase[3];
extern U32_T gu32_RXEightPhase[3];
#endif
extern U8_T gu8_Enable_C2[3];
...
void Hal_CARD_SetClock(IpOrder eIP, U32_T u32ClkFromIPSet)
{
    ...
    if (eIpSel == IP_SD)
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_SD),
                        BIT13_T | BIT12_T | BIT11_T | BIT10_T | BIT09_T | BIT08_T);
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_EIGHT_PHASE), BIT02_T | BIT01_T | BIT00_T);
...
#if (SUPPORT_SET_SD_CLK_PHASE)
                if (gu32_EnClkPhase[eIpSel])
                {
                    if (gu32_EnEightPhase[eIpSel])
                        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_SD),
                    ...
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_SD),
                                    gu32_TXClkPhase[eIpSel] << 9 | gu32_RXClkPhase[eIpSel] << 11);
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_EIGHT_PHASE),
                                    gu32_EnEightPhase[eIpSel] << 0); // eight phase enable
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_EIGHT_PHASE),
                                    gu32_TXEightPhase[eIpSel] << 1 | gu32_RXEightPhase[eIpSel] << 2);
                }
#endif
                break;
    ...
    else if (eIpSel == IP_SDIO)
    {
        ...
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_FCIE),
                        BIT13_T | BIT12_T | BIT11_T | BIT10_T | BIT09_T | BIT08_T);
        CARD_REG_CLRBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_EIGHT_PHASE), BIT08_T | BIT09_T | BIT10_T);
        ...
        switch (u32ClkFromIPSet)
        {
            case CLK2_F: // 48000KHz
                ...
                {
                    if (gu32_EnEightPhase[eIpSel])
                        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_FCIE),
                                        BIT05_T | BIT03_T); // 10 384M
                    else
                        CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_FCIE),
                                        BIT05_T | BIT04_T | BIT02_T); // 13 216M
                    ...
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_IP_FCIE),
                                    gu32_TXClkPhase[eIpSel] << 9 | gu32_RXClkPhase[eIpSel] << 11);
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_EIGHT_PHASE),
                                    gu32_EnEightPhase[eIpSel] << 8); // eight phase enable
                    CARD_REG_SETBIT(GET_CARD_REG_ADDR(A_CLKGEN_BANK, REG_CLK_EIGHT_PHASE),
                                    gu32_TXEightPhase[eIpSel] << 9 | gu32_RXEightPhase[eIpSel] << 10);
                }
#endif
                break;

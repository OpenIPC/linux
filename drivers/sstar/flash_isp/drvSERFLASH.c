...
#define CUSTOMER_NAME          '0'      //  0: Sstar
...
//  Global Functions
//-------------------------------------------------------------------------------------------------
#ifdef CONFIG_FLASH_ISP_READFUNC_MAP
...
MS_BOOL        MDrv_MAP_Read(MS_U32 u32FlashAddr, MS_U32 u32FlashSize, MS_U8 *user_buffer)
{
    ...
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, printk("%s(0x%08X, %d, %p)\n", __FUNCTION__, (unsigned int)u32FlashAddr,
                                                   (int)u32FlashSize, user_buffer));
...
const SERFLASH_Info *MDrv_SERFLASH_GetInfo(void)
{
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_INFO,
                    printk("MDrv_SERFLASH_GetInfo()\n"
                           ...
                           "\tu32SecNum      = %d\n"
                           "\tu32SecSize     = %d\n",
                           (int)(_SERFLASHInfo.u32AccessWidth), (int)(_SERFLASHInfo.u32TotalSize),
                           (int)(_SERFLASHInfo.u32SecNum), (int)(_SERFLASHInfo.u32SecSize)));
...
{
    _u8SERFLASHDbgLevel = E_SERFLASH_DBGLV_ERR; // init debug level first         //SERFLASH_DBGLV_DEBUG
...
MS_BOOL MDrv_SERFLASH_AddressErase(MS_U32 u32StartAddr, MS_U32 u32EraseSize, MS_BOOL bWait)
{
    ...
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, printk("%s(0x%08x, 0x%08x, %d)\n", __FUNCTION__, (unsigned int)u32StartAddr,
                                                   (unsigned int)u32EraseSize, (int)bWait));
...
MS_BOOL MDrv_SERFLASH_WriteProtect_Disable_Range_Set(MS_U32 u32DisableLowerBound, MS_U32 u32DisableSize)
{
    ...
    if (u32DisableLowerBound > u32FlashIndexMax || u32DisableUpperBound > u32FlashIndexMax
        || u32DisableLowerBound > u32DisableUpperBound)
    {
        DEBUG_SER_FLASH(
            E_SERFLASH_DBGLV_INFO,
            printk(" = FALSE, u32DisableLowerBound(0x%08X), u32DisableUpperBound(0x%08X), u32FlashIndexMax(0x%08X)\n",
    ...
    if (u32DisableUpperBound != u32FlashIndexMax && u32DisableLowerBound != 0)
    {
        u32DisableUpperBound = u32FlashIndexMax;
    }

    // Step 2. decide u32EnableLowerBound & u32EnableUpperBound
    if (u32DisableUpperBound > (u32FlashIndexMax - _SERFLASHInfo.u32SecSize) && u32DisableLowerBound == 0)
    {
        // i.e. no protect
...
#define FSP_WRITE_SIZE	4
        ...
        for(Index = 0; Index < u32FlashSize; )
        {
            u32ProgData = (*(user_buffer + Index))|(*(user_buffer + Index + 1)<<8)|(*(user_buffer + Index +
2)<<16)|(*(user_buffer + Index + 3)<<24); HAL_SERFLASH_ProgramFlashByFSP(u32FlashAddr+Index, u32ProgData); Index +=
FSP_WRITE_SIZE;
        }
...
MS_BOOL MDrv_FLASH_Write(MS_U32 u32FlashAddr, MS_U32 u32FlashSize, MS_U8 *user_buffer)
{
    ...
    // (unsigned int)u32FlashSize, user_buffer));
...
{
    // DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, printk("%s(0x%x, 0x%x)\n", __FUNCTION__, (unsigned int)DisableLowerBound,
    // (unsigned int)DisableSize));
...
MS_BOOL MDrv_FLASH_AddressErase(MS_U32 u32StartAddr, MS_U32 u32EraseSize, MS_BOOL bWait)
{
    ...
    // int)u32StartAddr, (unsigned int)u32Size, (int)bWait));

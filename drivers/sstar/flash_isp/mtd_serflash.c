...
extern int parse_mtd_partitions(struct mtd_info *master, const char *const *types, struct mtd_partition **pparts,
...
static int serflash_erase(struct mtd_info *mtd, struct erase_info *instr)
{
     ...
     * struct erase_info not longger include mem state
     */
...
#ifndef CONFIG_FLASH_ISP_AS_STORAGE
...
#else
static const struct mtd_partition serflash_partition_info[] = {
    {.name = "DATA", .offset = 0x00000000, .size = 0x01000000, .mask_flags = 0},
};
#endif
    ...
    if (flash->mtd.numeraseregions)
        for (i = 0; i < flash->mtd.numeraseregions; i++)
            printk(KERN_WARNING
                   ...
                   ".numblocks = %d }\n",
                   i, (u32)flash->mtd.eraseregions[i].offset, (unsigned int)flash->mtd.eraseregions[i].erasesize,
                   flash->mtd.eraseregions[i].erasesize / 1024, (unsigned int)flash->mtd.eraseregions[i].numblocks);
        ...
        if (mxp_init_nor_flash() >= 0)
        {
            ...
            for (i = 0; i < nr_parts; i++)
            {
                ...
                if (rec.type & MXP_PART_TYPE_MTD)
                {
                    ...
                    parts[j].types      = NULL; // nor-flash bootup bringup
                    parts[j].of_node    = NULL; // nor-flash bootup bringup
                    j++;
                }
            }
        ...
        if (nr_parts > 0)
        {
            for (i = 0; i < nr_parts; i++)
            {
                printk(KERN_WARNING
                       ...
                       "{.name = %s, .offset = 0x%.8x, "
                       ".size = 0x%.8x (%uKiB) }\n",
                       i, parts[i].name, (unsigned int)(parts[i].offset), (unsigned int)(parts[i].size),

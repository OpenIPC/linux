...
//  - The clock of DMA & Read via XIU operations are determined by only REG_ISP_CLK_SRC; other operations by
//  REG_ISP_CLK_SRC only
...
#define WRITE_BYTE(_reg, _val)                         \
    {                                                  \
        (*((volatile MS_U8 *)(_reg))) = (MS_U8)(_val); \
    }
#define WRITE_WORD(_reg, _val)                           \
    {                                                    \
        (*((volatile MS_U16 *)(_reg))) = (MS_U16)(_val); \
    }
#define WRITE_LONG(_reg, _val)                           \
    {                                                    \
        (*((volatile MS_U32 *)(_reg))) = (MS_U32)(_val); \
    }
#define WRITE_WORD_MASK(_reg, _val, _mask)                                                                         \
    {                                                                                                              \
        (*((volatile MS_U16 *)(_reg))) = ((*((volatile MS_U16 *)(_reg))) & ~(_mask)) | ((MS_U16)(_val) & (_mask)); \
    }
//#define WRITE_WORD_MASK(_reg, _val, _mask)  {*((volatile MS_U16*)(_reg)) = (*((volatile MS_U16*)(_reg))) & ~(_mask)) |
//((MS_U16)(_val) & (_mask); }

#define SPI_BDMA_QUIRK (CamOsChipRevision() & 0x1)

// XIU_ADDR
// #define SFSH_XIU_REG32(addr)                (*((volatile MS_U32 *)(_hal_isp.u32XiuBaseAddr + ((addr)<<2))))

//#define SFSH_XIU_READ32(addr)               (*((volatile MS_U32 *)(_hal_isp.u32XiuBaseAddr + ((addr)<<2)))) // TODO:
// check AEON 32 byte access order issue
//
...
//#define PM_GPIO_WRITE_MASK(addr, val, mask)     WRITE_WORD_MASK(_hal_isp.u32PMGPIOBaseAddr+ ((addr)<<2), (val),
//(mask))
...
#define SER_FLASH_TIME(_stamp, _msec)              \
    {                                              \
        (_stamp) = MsOS_GetSystemTime() + (_msec); \
    }
...
#define SER_FLASH_TIME(_stamp)          (ktime_get_ts64(&_stamp))
...
//  Debug Functions
//-------------------------------------------------------------------------------------------------
// BDMA_Result MDrv_BDMA_CopyHnd(MS_PHYADDR u32SrcAddr, MS_PHYADDR u32DstAddr, MS_U32 u32Len, BDMA_CpyType eCpyType,
// MS_U8 u8OpCfg)
//{
//    return -1;
//}

static MS_BOOL _Hal_GetMsTime(struct timespec64 tPreTime, MS_U32 u32Fac)
{
    MS_U32            u32NsTime = 0;
    struct timespec64 time_st;
    ktime_get_ts64(&time_st);
    u32NsTime = ((time_st.tv_sec - tPreTime.tv_sec) * 1000) + ((time_st.tv_nsec - tPreTime.tv_nsec) / NSEC_PER_MSEC);
    if (u32NsTime > u32Fac)
        return TRUE;
    return FALSE;
}
...
static void _HAL_SERFLASH_ActiveFlash_Set_HW_WP(MS_BOOL bEnable)
{
    ...
    else
    {
        ...
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_ALERT,
                        printk("msFlash_ActiveFlash_Set_HW_WP() is not defined in this system\n"));
...
void HAL_SERFLASH_SetGPIO(MS_BOOL bSwitch)
{
    ...
    //		WRITE_BYTE( u32PmGPIObase + ((REG_PM_GPIO_SPICZ_OEN)<<2), (READ_BYTE(u32PmGPIObase +
    //((REG_PM_GPIO_SPICZ_OEN)<<2))|(BIT(0)))); 		WRITE_BYTE( u32PmGPIObase + ((REG_PM_GPIO_SPICK_OEN)<<2),
    //(READ_BYTE(u32PmGPIObase + ((REG_PM_GPIO_SPICK_OEN)<<2))|(BIT(0)))); 		WRITE_BYTE( u32PmGPIObase +
    //((REG_PM_GPIO_SPIDI_OEN)<<2), (READ_BYTE(u32PmGPIObase + ((REG_PM_GPIO_SPIDI_OEN)<<2))|(BIT(0))));
    // WRITE_BYTE( u32PmGPIObase + ((REG_PM_GPIO_SPIDO_OEN)<<2), (READ_BYTE(u32PmGPIObase +
    // ((REG_PM_GPIO_SPIDO_OEN)<<2))|(BIT(0))));
    //	}
    //	else
    //	{
    ...
    if (HAL_SERFLASH_ReadID(u8FlashId, sizeof(u8FlashId)) == TRUE)
    {
        ...
        for (u32Index = 0; _hal_SERFLASH_table[u32Index].u8MID != 0; u32Index++)
        {
                ...
                && (_hal_SERFLASH_table[u32Index].u8DID1 == u8FlashId[2]))
            {
                ...
                if (_hal_SERFLASH.u16FlashType == FLASH_IC_MX25L6405D
                    || _hal_SERFLASH.u16FlashType == FLASH_IC_MX25L12805D)
                {
                ...
                DEBUG_SER_FLASH(E_SERFLASH_DBGLV_INFO,
                                printk("[FSP] Flash is detected (0x%04X, 0x%02X, 0x%02X, 0x%02X) ver1.1\n",
                                       _hal_SERFLASH.u16FlashType, _hal_SERFLASH.u8MID, _hal_SERFLASH.u8DID0,
        ...
        if (bDetect != TRUE)
        {
            ...
            DEBUG_SER_FLASH(
                ...
                printk("[FSP] Unknown flash type (0x%02X, 0x%02X, 0x%02X) and use default flash type 0x%04X\n",
                       _hal_SERFLASH.u8MID, _hal_SERFLASH.u8DID0, _hal_SERFLASH.u8DID1, _hal_SERFLASH.u16FlashType));
#else
            DEBUG_SER_FLASH(
                E_SERFLASH_DBGLV_INFO,
...
MS_BOOL HAL_SERFLASH_AddressToBlock(MS_U32 u32FlashAddr, MS_U32 *pu32BlockIndex)
{
    ...
    else
    {
        ...
        for (u32NextAddr = 0, *pu32BlockIndex = 0; *pu32BlockIndex < NUMBER_OF_SERFLASH_SECTORS; (*pu32BlockIndex)++)
        {
            ...
            {
                u32NextAddr += _hal_SERFLASH.pSpecialBlocks
                                   ->au32SizeList[*pu32BlockIndex - _hal_SERFLASH.pSpecialBlocks->u16Start];
            }
...
{
    if (_hal_SERFLASH.pSpecialBlocks == NULL || u32BlockIndex <= _hal_SERFLASH.pSpecialBlocks->u16Start)
    {
    ...
    else
    {
        ...
        *pu32FlashAddr = _hal_SERFLASH.pSpecialBlocks->u16Start * SERFLASH_SECTOR_SIZE;
        ...
        for (u32Index = _hal_SERFLASH.pSpecialBlocks->u16Start;
             u32Index < u32BlockIndex && u32Index <= _hal_SERFLASH.pSpecialBlocks->u16End; u32Index++)
        {
            *pu32FlashAddr +=
                _hal_SERFLASH.pSpecialBlocks->au32SizeList[u32Index - _hal_SERFLASH.pSpecialBlocks->u16Start];
        }
...
HAL_SERFLASH_Write_return:
    ...
    HAL_FSP_Exit();
#elif defined(CONFIG_FSP_WRITE_BDMA) || defined(CONFIG_FSP_WRITE_RIUOP_BRUST)
    HAL_FSP_Entry();
...
MS_BOOL HAL_SERFLASH_ReadBdma(MS_U32 u32Offset, MS_U32 u32Size, MS_U8 *pu8Data)
{
    ...
    u32AlignedSize = ((u32Size + u32Delta) + 0xF) & (~0xF);
    {
        ...
        tBdmaParam.ePathSel = HAL_BDMA_SPI_TO_MIU0; //(pstDmaCfg->phyaddr < ARM_MIU1_BASE_ADDR) ? (HAL_BDMA_MEM_TO_MIU0)
...
MS_BOOL HAL_SERFLASH_Read(MS_U32 u32Addr, MS_U32 u32Size, MS_U8 *pu8Data)
{
    ...
    Ret = TRUE;
#elif defined(CONFIG_FSP_READ_RIUOP) || defined(CONFIG_FSP_READ_BDMA)
    {
        MS_U8  u8EAR_min   = u32Addr >> 24;
        MS_U8  u8EAR_max   = (u32Addr + u32Size) >> 24;
        MS_U32 u32ReadSize = 0;
        // printk("u8EAR_min  %d u8EAR_max  %d\n",u8EAR_min,u8EAR_max);
        while (u8EAR_min <= u8EAR_max)
        {
            u32ReadSize = ((u8EAR_min + 1) << 24) - u32Addr;
            u32ReadSize = (u32ReadSize > u32Size) ? u32Size : u32ReadSize;
            if (u8EAR_min != _u8RegEAR)
            {
                HAL_FSP_WriteExtAddrReg(u8EAR_min);
            }
            if (u32ReadSize)
            {
                if (SPI_BDMA_QUIRK)
                {
                    Ret = HAL_FSP_Read(u32Addr, u32ReadSize, pu8Data);
                }
                else
                {
                    Ret = HAL_SERFLASH_ReadBdma(u32Addr, u32ReadSize, pu8Data);
                }
            }
            u32Addr += u32ReadSize;
            u32Size -= u32ReadSize;
...
EN_WP_AREA_EXISTED_RTN HAL_SERFLASH_WP_Area_Existed(MS_U32 u32UpperBound, MS_U32 u32LowerBound,
                                                    MS_U8 *pu8BlockProtectBits)
{
    ...
    for (u8Index = 0, bEndOfTable = FALSE, bPartialBoundFitted = FALSE; FALSE == bEndOfTable; u8Index++)
    {
        ...
        if (0xFFFFFFFF == pWriteProtectTable->u32LowerBound && 0xFFFFFFFF == pWriteProtectTable->u32UpperBound)
        {
            bEndOfTable = TRUE;
        }

        if (pWriteProtectTable->u32LowerBound == u32LowerBound && pWriteProtectTable->u32UpperBound == u32UpperBound)
        {
            *pu8BlockProtectBits = pWriteProtectTable->u8BlockProtectBits;

            return WP_AREA_EXACTLY_AVAILABLE;
        }
        else if (u32LowerBound <= pWriteProtectTable->u32LowerBound
                 && pWriteProtectTable->u32UpperBound <= u32UpperBound)
        {
            //
            // u32PartialFittedUpperBound & u32PartialFittedLowerBound would be initialized first time when
            // bPartialBoundFitted == FALSE (init value)
            // 1. first match:  FALSE == bPartialBoundFitted
            // 2. better match: (pWriteProtectTable->u32UpperBound - pWriteProtectTable->u32LowerBound) >
            // (u32PartialFittedUpperBound - u32PartialFittedLowerBound)
            //

            if (FALSE == bPartialBoundFitted
                || (pWriteProtectTable->u32UpperBound - pWriteProtectTable->u32LowerBound)
                       > (u32PartialFittedUpperBound - u32PartialFittedLowerBound))
            {
...
MS_BOOL HAL_SERFLASH_WriteProtect_Area(MS_BOOL bEnableAllArea, MS_U8 u8BlockProtectBits)
{
    ...
    if (TRUE == bEnableAllArea)
    {
        ...
        ISP_WRITE(REG_ISP_SPI_WDATA,
                  SF_SR_SRWD | SERFLASH_WRSR_BLK_PROTECT); // SF_SR_SRWD: SRWD Status Register Write Protect
    }
    else
    {
        if (_hal_SERFLASH.u8MID == MID_ATMEL)
        {
            ...
            // programming sector protection
            {
                ...
                for (i = 0;
                     0xFFFFFFFF != _hal_SERFLASH.pWriteProtectTable[i].u32LowerBound
                     && 0xFFFFFFFF
                            != _hal_SERFLASH.pWriteProtectTable[i].u32UpperBound; // the end of write protect table
                     i++)
                {
                    // if found, write
                    if (u8BlockProtectBits == _hal_SERFLASH.pWriteProtectTable[i].u8BlockProtectBits)
                    {
                        DEBUG_SER_FLASH(
                            ...
                            printk("u8BlockProtectBits = 0x%X, u32LowerBound = 0x%X, u32UpperBound = 0x%X\n",
                        ...
                        for (u32FlashAddr = 0; u32FlashAddr < _hal_SERFLASH.u32FlashSize;
                             u32FlashAddr += _hal_SERFLASH.u32SecSize)
                        {
                            if (_hal_SERFLASH.pWriteProtectTable[i].u32LowerBound
                                    <= (u32FlashAddr + _hal_SERFLASH.u32SecSize - 1)
...
MS_BOOL HAL_SPI_EnterIBPM(void)
{
    ...
    {
        bRet = TRUE;
        DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, printk("MXIC Security Register 0x%02X\n", ISP_READ(REG_ISP_SPI_RDATA)));
    }
...
void HAL_FSP_WriteBufs(MS_U8 u8Idx, MS_U8 u8Data)
{
    switch (u8Idx)
    {
        ...
        case 9:
            ...
            break;
        case 10:
            FSP_WRITE_MASK(REG_FSP_WDB10, REG_FSP_WDB10_DATA(u8Data), REG_FSP_WDB10_MASK);
            break;
        case 11:
            FSP_WRITE_MASK(REG_FSP_WDB11, REG_FSP_WDB11_DATA(u8Data), REG_FSP_WDB11_MASK);
            break;
        case 12:
            FSP_WRITE_MASK(REG_FSP_WDB12, REG_FSP_WDB12_DATA(u8Data), REG_FSP_WDB12_MASK);
            break;
        case 13:
            FSP_WRITE_MASK(REG_FSP_WDB13, REG_FSP_WDB13_DATA(u8Data), REG_FSP_WDB13_MASK);
            break;
        case 14:
            FSP_WRITE_MASK(REG_FSP_WDB14, REG_FSP_WDB14_DATA(u8Data), REG_FSP_WDB14_MASK);
            break;
        case 15:
            FSP_WRITE_MASK(REG_FSP_WDB15, REG_FSP_WDB15_DATA(u8Data), REG_FSP_WDB15_MASK);
            break;
        case 16:
            FSP_WRITE_MASK(REG_FSP_WDB16, REG_FSP_WDB16_DATA(u8Data), REG_FSP_WDB16_MASK);
            break;
...
void HAL_FSP_Entry(void)
{
    ...
    HAL_SERFLASH_SelectReadMode(
        E_FAST_MODE); // write operation decide in here ;  read operation will apply gReadMode before the Real action.
#if 0
...
MS_BOOL HAL_FSP_BlockErase(MS_U32 u32StartBlock, MS_U32 u32EndBlock, MS_BOOL bWait)
{
    ...
    MS_U32  u32FlashAddr = 0;
    DEBUG_SER_FLASH(E_SERFLASH_DBGLV_DEBUG, printk("%s(0x%08x, 0x%08x, %d)\n", __FUNCTION__,
                                                   (unsigned int)u32StartBlock, (unsigned int)u32EndBlock, (int)bWait));
...
void _HAL_BDMA_INIT(U32 u32DataSize)
{
    ...
    if (!msys_request_dmem(&mem_info))
    {
        ...
        printk("[Ser flash] phys=0x%08x, virt=0x%08x, bus=0x%08x len:0x%lX\n", (unsigned int)pu8BDMA_phys,
               (unsigned int)pu8BDMA_virt, (unsigned int)pu8BDMA_bus, u32BdmaSize);
    }
    else
    {
    ...
    // printk("[Ser flash] BDMA vaddr=0x%x,paddr=0x%x bus=0x%x len=0x%x\n", (unsigned int)pu8BDMA_virt, (unsigned
    // int)pu8BDMA_phys, (unsigned int)pu8BDMA_bus , (unsigned int)mem_info.length);
}
/*
...
#ifdef CONFIG_FSP_WRITE_RIUOP_BRUST
MS_BOOL _HAL_PP_RIUOP_BURST(MS_U32 u32Src_off, MS_U32 u32Dst_off, MS_U32 u32Len)
{
    MS_U32  u32offs  = u32Src_off;
    MS_BOOL bRet     = TRUE;
    MS_U8   u8Status = 0;
    MS_U32  u32I, u32J;
    MS_U32  u32quotient;
    ...
    // printk("PP 0x%x len:%d\r\n",(unsigned int)u32Dst_off, (unsigned int)u32Len);
#if 1
    ...
    /* We can't get what time the CS is pull high with hardware contorl.
     Use software control output a level waveform */
    QSPI_WRITE(REG_SPI_BURST_WRITE, (REG_SPI_ENABLE_BURST | 0x2)); // SW control pull high CS
    udelay(5);
    ...
    QSPI_WRITE(REG_SPI_BURST_WRITE, (REG_SPI_ENABLE_BURST)); // SW control pull down CS
    FSP_WRITE_MASK(REG_FSP_TRIGGER, REG_FSP_FIRE, REG_FSP_TRIGGER_MASK);
    bRet &= _HAL_FSP_WaitDone();
    ...
    u32quotient  = (u32Len / REG_FSP_MAX_WRITEDATA_SIZE);
    u32remainder = (u32Len % REG_FSP_MAX_WRITEDATA_SIZE);
    // printk("quotient:%lu, rem:%lu\r\n",u32quotient, u32remainder);
    if (u32remainder)
    {
        u32quotient++;
        u32Size = u32remainder;
    ...
    for (u32I = 0; u32I < u32quotient; u32I++)
    {
        for (u32J = 0; u32J < u32Size; u32J++)
        {
            HAL_FSP_WriteBufs(u32J, *(MS_U8 *)(u32offs + u32J));
        }
        u32offs += u32Size;
        u32Size = REG_FSP_MAX_WRITEDATA_SIZE;
        FSP_WRITE_MASK(REG_FSP_WBF_SIZE, REG_FSP_WBF_SIZE0(u32J), REG_FSP_WBF_SIZE0_MASK);
    ...
    // printk("CS up\r\n");
    QSPI_WRITE(REG_SPI_BURST_WRITE, REG_SPI_DISABLE_BURST); // Disable SW control
    ...
    CompareMemory(u32Src_off, u32Dst_off+BASE_FLASH_OFFSET, u32Len);*/

    return bRet;
}
#endif

#if defined(CONFIG_FSP_WRITE_BDMA)
static MS_BOOL _HAL_PP_BDMAToFSP(MS_U32 u32Src_off, MS_U32 u32Dst_off, MS_U32 u32Len)
{
    ...
    WRITE_WORD((_hal_isp.u32BdmaBaseAddr + (0x3 << 2)), 0x8000); // set channel0 to MIU

    // Set start address
    WRITE_WORD((_hal_isp.u32BdmaBaseAddr + (0x4 << 2)), (pu8BDMA_bus & 0x0000FFFF));
    WRITE_WORD((_hal_isp.u32BdmaBaseAddr + (0x5 << 2)), (pu8BDMA_bus >> 16) & 0x0FFF);
    // Set end address
...
#if defined(CONFIG_FSP_WRITE_RIUOP_BRUST) || defined(CONFIG_FSP_WRITE_BDMA)
...
#ifdef CONFIG_FSP_WRITE_RIUOP_BRUST
        bRet = _HAL_PP_RIUOP_BURST((MS_U32)u8Buf, u32Addr, u16WriteBytes);
#else

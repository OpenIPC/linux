...
void mdrv_ive_drv_isr_post_proc(struct work_struct *wq)
{
    ...
    if (mdrv_ive_clock_en(dev_data)) {
        IVE_MSG(IVE_MSG_ERR, "clock enable failed!\n");
    }
#endif
    // distory the previous task before start the next request
    ive_drv_distory_process(&dev_data->drv_handle);
...
void mdrv_ive_drv_isr_pre_proc(ive_dev_data  *dev_data)
{
    ...
    IVE_MSG(IVE_MSG_DBG, "file_data:0x%p, post porcess 0x%p\n", file_data, &file_data->wait_queue);

    // set ready and wake up waiting thread/process
    wake_up(&file_data->wait_queue);
}
...
irqreturn_t  mdrv_ive_drv_isr(int irq, void* data)
{
    ...
    switch(state) {
        case IVE_DRV_STATE_DONE:
            mdrv_ive_drv_isr_pre_proc(dev_data);
 ...
 *
 */
void mdrv_ive_drv_reset_statue(ive_file_data *file_data)
{
    ...
    // distoray the previous task list from the request list
    previous_file_data = ive_drv_distory_process(handle);
    if (previous_file_data != file_data)
        previous_file_data = IVE_FILE_STATE_READY;

    // start the next tast list if it existed
...
static unsigned int mdrv_ive_drv_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    wait_event_timeout(file_data->wait_queue, file_data->state == IVE_FILE_STATE_DONE, msecs_to_jiffies(5000));
    ...
    switch(file_data->state)
    {
        ...
        default:
            IVE_MSG(IVE_MSG_ERR, "Polling fail, start reset status. filp:0x%p\n", filp);
            mdrv_ive_drv_reset_statue(file_data);
            break;
    }

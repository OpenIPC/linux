...
static int mdrv_ive_drv_probe(struct platform_device *pdev)
{
    ...
    dev = device_create(g_ive_drv.class, NULL, MKDEV(g_ive_drv.major, g_ive_drv.minor_star + g_ive_drv.reg_count),
                        dev_data, MDRV_IVE_NAME "%d", g_ive_drv.minor_star + g_ive_drv.reg_count);
    if (IS_ERR(dev))
    {
...
static struct platform_driver mdrv_ive_driver = {.probe   = mdrv_ive_drv_probe,

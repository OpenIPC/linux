...
#define HAL_MIU1_BASE MSTAR_MIU1_BUS_BASE
...
#else
#define REGR(base, idx) ms_readw(((uint)base + (idx)*4))
#define REGW(base, idx, val)                                                          \
    do                                                                                \
    {                                                                                 \
        IVE_MSG(IVE_MSG_DBG, "write 0x%08X = 0x%04X\n", ((uint)base + (idx)*4), val); \
        ms_writew(val, ((uint)base + (idx)*4));                                       \
    } while (0)
#endif
 ...
 *   none
 */
void ive_hal_dummy_set(ive_hal_handle *handle, int dummy_vaule)
{
    // dummy[0]  0: ive_busy disable.
    //           1: ive_busy enable.
    handle->reg_bank0.dummy = dummy_vaule;
...
void ive_hal_set_images(ive_hal_handle *handle, ive_ioc_image *input, ive_ioc_image *output)
{
    ...
    IVE_MSG(IVE_MSG_DBG, "Ori -input : address[0] = 0x%llx\n", (u64)input->address[0]);
    IVE_MSG(IVE_MSG_DBG, "Ori -input : address[1] = 0x%llx\n", (u64)input->address[1]);
    IVE_MSG(IVE_MSG_DBG, "Ori -input : address[2] = 0x%llx\n", (u64)input->address[2]);

#if defined(USE_MIU_DIRECT)
    input_addr0  = (u64)(input->address[0]);
    input_addr1  = (u64)(input->address[1]);
    input_addr2  = (u64)(input->address[2]);
    output_addr0 = (u64)(output->address[0]);
    output_addr1 = (u64)(output->address[1]);
    output_addr2 = (u64)(output->address[2]);
#else
    ...
    IVE_MSG(IVE_MSG_DBG, "Final - input_addr0 = %llx\n", input_addr0);
    IVE_MSG(IVE_MSG_DBG, "Final - input_addr1 = %llx\n", input_addr1);
    IVE_MSG(IVE_MSG_DBG, "Final - input_addr2 = %llx\n", input_addr2);
    IVE_MSG(IVE_MSG_DBG, "Final - output_addr0 = %llx\n", output_addr0);
    IVE_MSG(IVE_MSG_DBG, "Final - output_addr1 = %llx\n", output_addr1);
    IVE_MSG(IVE_MSG_DBG, "Final - output_addr2 = %llx\n", output_addr2);
...
void ive_hal_set_coeff_map(ive_hal_handle *handle, u64 map_addr)
{
    ...
    handle->reg_bank1.reg_ive_src1_addr_35_32 = ADDR_HIGH_EXT(miu_addr);
...
void ive_hal_set_coeff_ncc(ive_hal_handle *handle, u64 output_addr)
{
    ...
    handle->reg_bank1.reg_ive_dst1_addr_35_32 = ADDR_HIGH_EXT(miu_addr);
...
void ive_hal_set_coeff_matrix_transform(ive_hal_handle *handle, ive_ioc_coeff_matrix_transform *coeff)
{
    ...
    {
#if defined(CONFIG_CHIP_INFINITYI6E) // Only used for I6E
        case IVE_IOC_MODE_MATRIX_TRANSFORM_C1:
            handle->reg_bank1.infmt  = 13;
            handle->reg_bank1.outfmt = 13;

            handle->reg_bank1.mask0  = coeff->s32MatrixArray[0] & 0xFF;
            handle->reg_bank1.mask1  = (coeff->s32MatrixArray[0] & 0xFF00) >> 8;
        ...
        case IVE_IOC_MODE_MATRIX_TRANSFORM_C2:
            ...
            handle->reg_bank1.mask0  = coeff->s32MatrixArray[0] & 0xFF;
            handle->reg_bank1.mask1  = (coeff->s32MatrixArray[0] & 0xFF00) >> 8;
            handle->reg_bank1.mask2  = coeff->s32MatrixArray[1] & 0xFF;
            handle->reg_bank1.mask3  = (coeff->s32MatrixArray[1] & 0xFF00) >> 8;
            handle->reg_bank1.mask4  = 0;
            handle->reg_bank1.mask5  = 0;
            handle->reg_bank1.mask6  = coeff->s32MatrixArray[2] & 0xFF;
            handle->reg_bank1.mask7  = (coeff->s32MatrixArray[2] & 0xFF00) >> 8;
            handle->reg_bank1.mask8  = coeff->s32MatrixArray[3] & 0xFF;
            handle->reg_bank1.mask9  = (coeff->s32MatrixArray[3] & 0xFF00) >> 8;
            handle->reg_bank1.mask10 = 0;
            handle->reg_bank1.mask11 = 0;
            handle->reg_bank1.mask12 = 0;
            handle->reg_bank1.mask13 = 0;
        ...
        case IVE_IOC_MODE_MATRIX_TRANSFORM_C3:
            ...
            handle->reg_bank1.mask2  = coeff->s32MatrixArray[1] & 0xFF;
            handle->reg_bank1.mask3  = (coeff->s32MatrixArray[1] & 0xFF00) >> 8;
            handle->reg_bank1.mask4  = coeff->s32MatrixArray[2] & 0xFF;
            handle->reg_bank1.mask5  = (coeff->s32MatrixArray[2] & 0xFF00) >> 8;
            handle->reg_bank1.mask6  = coeff->s32MatrixArray[3] & 0xFF;
            handle->reg_bank1.mask7  = (coeff->s32MatrixArray[3] & 0xFF00) >> 8;
            handle->reg_bank1.mask8  = coeff->s32MatrixArray[4] & 0xFF;
            handle->reg_bank1.mask9  = (coeff->s32MatrixArray[4] & 0xFF00) >> 8;
            handle->reg_bank1.mask10 = coeff->s32MatrixArray[5] & 0xFF;
            handle->reg_bank1.mask11 = (coeff->s32MatrixArray[5] & 0xFF00) >> 8;
            handle->reg_bank1.mask12 = coeff->s32MatrixArray[6] & 0xFF;
            handle->reg_bank1.mask13 = (coeff->s32MatrixArray[6] & 0xFF00) >> 8;
            handle->reg_bank1.mask14 = coeff->s32MatrixArray[7] & 0xFF;
            handle->reg_bank1.mask15 = (coeff->s32MatrixArray[7] & 0xFF00) >> 8;
            handle->reg_bank1.mask16 = coeff->s32MatrixArray[8] & 0xFF;
            handle->reg_bank1.mask17 = (coeff->s32MatrixArray[8] & 0xFF00) >> 8;
            break;
#else
        case IVE_IOC_MODE_MATRIX_TRANSFORM_C1:
            handle->reg_bank1.infmt  = 13;
...
#if !defined(CONFIG_CHIP_INFINITYI6E) // Only used for M6 and the chip after that
    REGW(handle->base_addr1, 0x23, handle->reg_bank1.reg23);
    REGW(handle->base_addr1, 0x24, handle->reg_bank1.reg24);
    REGW(handle->base_addr1, 0x25, handle->reg_bank1.reg25);
...
/*******************************************************************************************************************
 * ive_hal_set_coeff_alpha_blending
 ...
 *   none
 */
void ive_hal_set_coeff_alpha_blending(ive_hal_handle *handle)
{

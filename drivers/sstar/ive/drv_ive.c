...
static void ive_drv_sync_image(ive_drv_handle *handle, ive_ioc_image *image, enum dma_data_direction direct)
{
    ...
    IVE_MSG(IVE_MSG_DBG, "sync buffer %llx, but nothing to do for MIU buffer\n", (u64)image->address[0]);
#else
    ...
    for (i = 0; i < 3; i++)
    {
        if (image->address[i] != NULL)
        {
            ...
            IVE_MSG(IVE_MSG_DBG, "sync buffer %p, format %x, size %d * %d * %d = %d\n", image->address[i],
                    image->format, image->stride[i], image->height, byte_per_pixel, size);
        }
    }
#endif
}
...
IVE_DRV_STATE ive_drv_isr_handler(int irq, ive_drv_handle *handle)
{
    ...
    if (ive_hal_get_irq_check(&handle->hal_handle, IVE_HAL_IRQ_MASK_FRAME_DONE))
    {
        ...
        // disble ive_busy signal
        ive_hal_dummy_set(&handle->hal_handle, 0x0);
...
static void ive_drv_start(ive_drv_handle *handle, ive_ioc_config *config)
{
    ...
    switch (config->op_type)
    {
        ...
        case IVE_IOC_OP_TYPE_MAP:
            ive_hal_set_coeff_map(&handle->hal_handle, (u64)config->coeff_map.map);
            break;
        ...
        case IVE_IOC_OP_TYPE_NCC:
            ive_hal_set_coeff_ncc(&handle->hal_handle, (u64)config->coeff_ncc);
            break;
        ...
        case IVE_IOC_OP_TYPE_ALPHA_BLENDING:
            ive_hal_set_coeff_alpha_blending(&handle->hal_handle);
            break;
    ...
    ive_hal_dummy_set(&handle->hal_handle, 0x1);
...
IVE_IOC_ERROR ive_drv_process(ive_drv_handle *handle, ive_file_data *file_data)
{
    ...
    IVE_MSG(IVE_MSG_DBG, "process: %llx ; %llx ; %llx\n", (u64)file_data->ioc_config.input.address[0],
            (u64)file_data->ioc_config.input.address[1], (u64)file_data->ioc_config.input.address[2]);
...
ive_file_data *ive_drv_post_process(ive_drv_handle *handle)
{
    ...
    else
    {
        IVE_MSG(IVE_MSG_DBG, "process: 0x%llx, 0x%llx, 0x%llx\n", (u64)next_file_data->ioc_config.input.address[0],
                (u64)next_file_data->ioc_config.input.address[1], (u64)next_file_data->ioc_config.output.address[2]);

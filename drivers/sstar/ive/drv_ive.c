...
static ive_file_data* drv_ive_get_request(ive_drv_handle *handle)
{
    ...
    if (list_data != NULL) {
        ...
        IVE_MSG(IVE_MSG_DBG, "list is null, get: 0x%p\n", list_data);
    }
...
ive_file_data* ive_drv_pre_process(ive_drv_handle *handle)
{
    ...
    if (previous_file_data != NULL) {
        ...
        IVE_MSG(IVE_MSG_DBG, "previous_file_data = IVE_FILE_STATE_DONE\n");
    }
 ...
 *   ive_file_data *
 */
ive_file_data* ive_drv_distory_process(ive_drv_handle *handle)
{

...
static ssize_t                  _ceva_linkdrv_proc_read(struct file *, char __user *, size_t, loff_t *);
static int                      _ceva_linkdrv_proc_read_break(struct ceva_linkdrv *link);
static struct ceva_linkdrv_pfd *_get_linkdrv_pfd(struct ceva_linkdrv *link, pid_t pid);
static struct ceva_linkdrv_pfd *_create_linkdrv_pfd(struct ceva_linkdrv *link, pid_t pid);
...
static struct ceva_linkdrv_pfd *_get_linkdrv_pfd(struct ceva_linkdrv *link, pid_t pid)
{
    ...
    {
        i = (struct ceva_linkdrv_pfd *)list_entry(pos, struct ceva_linkdrv_pfd, list_node);
        if (!i)
        {
...
static struct ceva_linkdrv_pfd *_create_linkdrv_pfd(struct ceva_linkdrv *link, pid_t pid)
{
    ...
    pfd->events = proc_create_data(CEVA_LINKDRV_PROC_EVENTS_NAME,         /* name */
 ...
 */
long ceva_linkdrv_ioctl(struct ceva_linkdrv *link, unsigned int cmd, unsigned long arg)
{
    long                 ret;
    void *               buf;
    ceva_event_t         event;
    ...
    switch (cmd)
    {
        case IOC_CEVADRV_BYPASS_REQ:
            ...
            memcpy(buf + CEVADEV_BYPASS__OUT_BUF_OFFSET, buf + CEVADEV_BYPASS__IN_BUF_OFFSET, CEVADEV_BYPASS__BUF_LEN);
...
#if defined(CEVA_LINKDRV_DEBUG)
            ret = copy_to_user(cbuf, &link->debug.data, sizeof(struct ceva_linkdrv_debug_info_s));
#else
            ret = -EINVAL;
#endif
            break;
...
static int _ceva_linkdrv_proc_open(struct inode *inode, struct file *filp)
{
    ...
    XM6_MSG(XM6_MSG_ERR, "%d/%s was opened (by p%d/t%d), open descriptor value: %d\n", pid,
            CEVA_LINKDRV_PROC_EVENTS_NAME, pid, tid, pfd->events_ref_count);
    ...
    if (pfd->events_ref_count == 1)
    {
        ...
        if (ret)
        {
            ...
            XM6_MSG(XM6_MSG_ERR, "unable to cancel the read operation on %d/%s (p%d/t%d)\n", pid,
                    CEVA_LINKDRV_PROC_EVENTS_NAME, pid, tid);
            return ret;
        }
        /*
 ...
 */
static ssize_t _ceva_linkdrv_proc_read(struct file *filp, char __user *buf, size_t count, loff_t *ppos)
{
    struct ceva_linkdrv_pfd *pfd  = PDE_DATA(filp->f_path.dentry->d_inode);
    struct ceva_linkdrv *    link = pfd->owner;
    unsigned long            flags;
 ...
 * @return zero for success, nonzero for failure
 */
int ceva_linkdrv_broadcast_events(struct ceva_linkdrv *link, ceva_event_t *events, int sz)
{
    struct list_head *       pos;
    struct ceva_linkdrv_pfd *i;
    int                      avail, ret;
    ...
    list_for_each(pos, &link->pfd_list)
    {
        ...
        if (!ret)
        {
            ...
            XM6_MSG(XM6_MSG_WRN,
                    "unable to insert more data to descriptor %s's kfifo, "
...
static int _ceva_linkdrv_proc_read_break(struct ceva_linkdrv *link)
{
     ...
     */
    for (j = 0; (j < CANCEL_READ_MAX_RELAX_TRIES && !list_empty(&link->pfd_list)); j++)
    {
        msleep(0);
    }
...
static int _ceva_link_debug_proc_write_cb(struct file *file, const char __user *buffer, unsigned long count, void *data)
{
...
static int _ceva_link_debug_proc_init(struct ceva_linkdrv *link)
{
    ceva_linkdrv_proc_entry_debug = create_proc_read_entry(CEVA_LINKDRV_PROC_DEBUG,               /* name */
                                                           S_IWUSR | S_IRUSR | S_IRGRP | S_IROTH, /* mode */
    ...
    ceva_linkdrv_proc_entry_debug->write_proc = _ceva_link_debug_proc_write_cb;

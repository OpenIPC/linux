 ...
 */
static struct vm_operations_struct xm6_drv_dma_vma_ops = {.open = xm6_drv_dma_vma_open, .close = xm6_drv_dma_vma_close};
...
static struct platform_driver xm6_drv_driver = {.probe  = xm6_drv_probe,
...
static void dump_XM6VerInfo(struct Version_Info *pVerInfo)
{
    XM6_MSG(XM6_MSG_DBG, "Version_Info CEVA_VInfo Main.Second.Sub : %d.%d.%d \n", pVerInfo->CEVA_VInfo.Main,
            pVerInfo->CEVA_VInfo.Second, pVerInfo->CEVA_VInfo.Sub);
...
static int xm6_drv_init_dsp_buffer(struct xm6_dev_data *dev_data, struct boot_config_t *boot_config)
{
    ...
    XM6_MSG(XM6_MSG_DBG, "share memory: miu: 0x%llx, phys: 0x%x, virt: %p\n", boot_config->share_mem.miu,
            (int)dev_data->share_mem.phys, dev_data->share_mem.virt);

    // Reset share_mem
    memset(dev_data->share_mem.virt, 0, dev_data->share_mem.size);

    // The sahre memory is seperated into 2 part, share (DMA) memory and hprintf buffer, both are shared between DSP and
    // host.

    // Assign hprintf buffer, we put hprintf buffer first because this is also used for boot copier and it must
    // page-alignment
...
static int xm6_drv_dsp_boot_up(struct xm6_dev_data *dev_data, struct boot_config_t *boot_config)
{
    ...
    if (0)
    {
        ...
        ret = dsp_log_init(dev_data->boot_config.dsp_log_addr, dev_data->boot_config.dsp_log_size);
        if (unlikely(ret != 0))
        {
...
static int xm6_drv_setup_cdev(struct platform_device *plat_dev, struct xm6_dev_data *dev_data)
{
    ...
    device = device_create(xm6_drv_data.class, /* class */
                           NULL,               /* no parent device */
                           dev_no, dev_data, CEVA_PCIDEV_DEVICE_NAME "%d", dev_count);

    if (IS_ERR(device))
    {
        ret = PTR_ERR(device);
        XM6_MSG(XM6_MSG_ERR, "setup_cdev: Error %d while trying to create %s%d\n", ret, CEVA_PCIDEV_DEVICE_NAME,
                dev_count);
        cdev_del(&dev_data->cdev);
    }
...
static inline int probe_get_hw_prop(struct platform_device *plat_dev, struct xm6_dev_data *dev_data)
{
    ...
    XM6_MSG(XM6_MSG_DBG, "irq id: %d, hw addr: %x, %x %x, %x, %x, %x, %x ,%x \n", dev_data->hw_irq_id,
            dev_data->hw_addr_sys, dev_data->hw_addr_axi2miu0, dev_data->hw_addr_axi2miu1, dev_data->hw_addr_axi2miu2,
            dev_data->hw_addr_axi2miu3, dev_data->hw_addr_cpm, dev_data->hw_addr_cevatopctl,
            dev_data->hw_addr_cevapllpower);
    return 0;
}
...
static int probe_init_irq(struct platform_device *plat_dev, struct xm6_dev_data *dev_data)
{
    int ret = request_irq(dev_data->hw_irq_id, ceva_linkdrv_interrupt, 0, DRIVER_NAME, dev_data);
    if (ret != 0)
    {
...
static int xm6_drv_probe(struct platform_device *plat_dev)
{
    ...
    dsp_ceva_hal_init(&dev_data->ceva_hal, dev_data->hw_addr_sys, dev_data->hw_addr_axi2miu0,
                      dev_data->hw_addr_axi2miu1, dev_data->hw_addr_axi2miu2, dev_data->hw_addr_axi2miu3);

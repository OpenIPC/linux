...
static ssize_t protected_mem_db_read(struct file *file, char __user *buf, size_t count, loff_t *ppos);
static ssize_t protected_mem_db_write(struct file *file, const char __user *buf, size_t count, loff_t *ppos);
static long    protected_mem_db_ioctl(struct file *file, unsigned int cmd, unsigned long arg);
 ...
 */
long protected_mem_db_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    struct protected_mem_db_data *dev;
    pid_t                         pid;
    int                           ret = 0;

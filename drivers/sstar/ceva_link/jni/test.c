...
static int do_stat(int argc, char *argv[])
{
    ...
    LOG("debug info retrieved:\n"
        ...
        debug.data.bypass_interrupts_success, debug.data.bypass_interrupts_failed, debug.data.events_recieved,
        debug.data.fifo_full, debug.data.generated_interrupts, debug.data.generate_interrupt_failures);
    return 0;
}
...
static int do_help(int argc, char *argv[])
{
    ...
    for (i = 0; i < sizeof(commands) / sizeof(exec_cmd_t); i++)
    {
        ...
        {
            LOG("  %s\t%s", commands[i].argv[j].name, commands[i].argv[j].description);
        }
    }
    return 0;
}

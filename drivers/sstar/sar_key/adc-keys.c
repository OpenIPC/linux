...
static bool ms_RtcPwr_ISOCTL_EX(void)
{
    ...
    u16 reg     = 0;
    // Input ISO ctrl sequence ,  3'b000(S0) -> 3'b001(S1)  -> 3'b011(S2)  -> 3'b111(S3)  -> 3'b101(S4)  -> 3'b001(S5)
    // -> 3'b000(S0) Following notes is from MV2 The switch of state needs delay, 1ms at least according to designer,
    ...
    // [from designer Belon.Chen] wait 2 ms is must since read/write base/counter/SW0/SW1 is valid after iso state
    // complete
    mdelay(2);
    return TRUE;
}

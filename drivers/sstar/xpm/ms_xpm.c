...
static long xpm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
    ...
    if (_IOC_TYPE(cmd) != XPM_IOCTL_MAGIC)
        return -ENOTTY;
    if (_IOC_NR(cmd) > IOCTL_XPM_MAXNR)
        return -ENOTTY;
    switch (cmd)
    {
        case IOCTL_XPM_REGISTER_SOURCE:
        {
            ...
            if (copy_from_user((void *)_name, (void __user *)arg,
                               (XPM_NAME_SIZE > strlen((char *)arg)) ? (strlen((char *)arg) + 1) : XPM_NAME_SIZE - 1))
            {
                BUG();
            }
            if (strlen(_name) > 0)
...
static unsigned int xpm_poll(struct file *filp, struct poll_table_struct *wait)
{
    ...
    else if ((st == (XPM_STATE_WAKEUPED | XPM_STATE_SUSPEND_ABORT))
...
ssize_t xpm_store(struct kobject *kobj, struct kobj_attribute *attr, const char *buf, size_t n)
{
    ...
    if ((st != (long)XPM_STATE_SUSPENDING) && (st != (long)XPM_STATE_WAKEUPED)
        && (st != (long)(XPM_STATE_WAKEUPED | XPM_STATE_SUSPEND_ABORT)))
    {

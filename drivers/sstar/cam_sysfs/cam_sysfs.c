...
void CamSysfsDeleteLink(struct kobject *kobj, struct kobject *target, const char *name)
{
    return sysfs_delete_link(kobj, target, name);
}
                               ...
                               ...)
{
    ...
    va_start(vargs, fmt);
    dev_name = kvasprintf_const(GFP_KERNEL, fmt, vargs);
    if (!dev_name)
        return NULL;

    dev = device_create(class, parent, devt, drvdata, dev_name);

    kfree_const(dev_name);
    va_end(vargs);

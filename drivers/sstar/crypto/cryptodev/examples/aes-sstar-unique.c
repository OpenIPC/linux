...
void sstar_dump_data(void* addr, int size)
{
    char* out_buf = (char*)addr;
    int   i;
    for (i = 0; i < size; i++)
    {
        AES_SSTAR_DBG("%02x ", out_buf[i]);
        if (i % 16 == 15)
            AES_SSTAR_DBG("\n");
    }
    AES_SSTAR_DBG("\n");
...
int aes_ctx_init(struct cryptodev_ctx* ctx, int cfd, const uint8_t* key, unsigned int key_size)
{
    ...
    AES_SSTAR_DBG("Got %s with driver %s\n", siop.cipher_info.cra_name, siop.cipher_info.cra_driver_name);
    if (!(siop.flags & SIOP_FLAG_KERNEL_DRIVER_ONLY))
    {
        AES_SSTAR_DBG("Note: This is not an accelerated cipher\n");
    }
...
static int test_aes(int cfd, uint8_t* key1, char* cipher)
{
    ...
    aes_ctx_init(&ctx, cfd, key1, KEY_SIZE);

    AES_SSTAR_DBG("key:\n");
    sstar_dump_data(key1, AES_BLOCK_SIZE);
    ...
    memset(plaintext1, 0x5a, AES_BLOCK_SIZE);
    memset(iv1, 0x0, sizeof(iv1));

    AES_SSTAR_DBG("plaintext:\n");
    sstar_dump_data(plaintext1, AES_BLOCK_SIZE);

    aes_encrypt(&ctx, iv1, plaintext1, cipher, AES_BLOCK_SIZE);
    AES_SSTAR_DBG("Encrypted :\n");
    sstar_dump_data(cipher, AES_BLOCK_SIZE);

    aes_decrypt(&ctx, iv1, cipher, decrypttext1, AES_BLOCK_SIZE);
    AES_SSTAR_DBG("Decrypted:\n");
    sstar_dump_data(decrypttext1, AES_BLOCK_SIZE);
...
int test_aes_hwkey(int cfd, int hwkey)
{
    char __attribute__((aligned(64))) ciphertext1[AES_BLOCK_SIZE] = {0};
    char __attribute__((aligned(64))) ciphertext2[AES_BLOCK_SIZE] = {0};

    uint8_t key1[KEY_SIZE] = {'S', 'S', 't', 'a', 'r', 'U', 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

    key1[6] = hwkey;
    /* Run the test itself */
    if (test_aes(cfd, key1, ciphertext1) != 0)
        return 1;

    key1[7] = 0xff;

    if (test_aes(cfd, key1, ciphertext2) != 0)
        return 1;

    if (memcmp(ciphertext1, ciphertext2, AES_BLOCK_SIZE) != 0)
    {
        fprintf(stderr, "AES Test hw_key(%d) - [failed] !!! \n", hwkey);
        return -1;
    }
    else
        fprintf(stderr, "AES Test hw_key(%d) - [passed] \n", hwkey);
}

static int test_aes_swkey(int cfd)
{
    char                 plaintext1_raw[AES_BLOCK_SIZE + 63], *plaintext1;
    ...
    if (memcmp(plaintext2, ciphertext2, AES_BLOCK_SIZE) != 0)
    {
        ...
        fprintf(stderr, "FAIL: Decrypted data are different from the input data.\n");
        AES_SSTAR_DBG("plaintext:");
        sstar_dump_data(plaintext2, AES_BLOCK_SIZE);
        AES_SSTAR_DBG("ciphertext:");
        sstar_dump_data(ciphertext2, AES_BLOCK_SIZE);
        return 1;
    }

    aes_ctx_deinit(&ctx);

    fprintf(stderr, "AES Test sw_key    - [passed] \n");

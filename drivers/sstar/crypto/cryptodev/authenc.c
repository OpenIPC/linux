 ...
 */
static int get_userbuf_tls(struct csession *ses, struct kernel_crypt_auth_op *kcaop, struct scatterlist **dst_sg)
{
    int                   pagecount = 0;
    ...
    if (ses->alignmask)
    {
        if (!IS_ALIGNED((unsigned long)caop->dst, ses->alignmask + 1))
            dwarning(2, "careful - source address %p is not %d byte aligned", caop->dst, ses->alignmask + 1);
    }
    ...
    rc = __get_userbuf(caop->dst, kcaop->dst_len, 1, pagecount, ses->pages, ses->sg, kcaop->task, kcaop->mm);
    if (unlikely(rc))
    {
 ...
 * returns error.
 */
static int get_userbuf_srtp(struct csession *ses, struct kernel_crypt_auth_op *kcaop, struct scatterlist **auth_sg,
                            struct scatterlist **dst_sg)
{
    ...
    if (ses->alignmask)
    {
        ...
        if (!IS_ALIGNED((unsigned long)caop->auth_src, ses->alignmask + 1))
            dwarning(2, "careful - source address %p is not %d byte aligned", caop->auth_src, ses->alignmask + 1);
    }
    ...
    rc = __get_userbuf(caop->auth_src, caop->auth_len, 1, auth_pagecount, ses->pages, ses->sg, kcaop->task, kcaop->mm);
    if (unlikely(rc))
    {
        derr(1, "failed to get user pages for data input");
        return -EINVAL;
...
static int fill_kcaop_from_caop(struct kernel_crypt_auth_op *kcaop, struct fcrypt *fcr)
{
    ...
    if (caop->iv)
    {
        ...
        if (unlikely(ret))
        {
            derr(1, "error copying IV (%d bytes), copy_from_user returned %d for address %p", kcaop->ivlen, ret,
                 caop->iv);
            ret = -EFAULT;
            goto out_unlock;
        }
...
static int fill_caop_from_kcaop(struct kernel_crypt_auth_op *kcaop, struct fcrypt *fcr)
{
    ...
    {
        ret = copy_to_user(kcaop->caop.iv, kcaop->iv, kcaop->ivlen);
        if (unlikely(ret))
        {
...
int kcaop_from_user(struct kernel_crypt_auth_op *kcaop, struct fcrypt *fcr, void __user *arg)
{
...
int kcaop_to_user(struct kernel_crypt_auth_op *kcaop, struct fcrypt *fcr, void __user *arg)
{
    int ret;
 ...
 */
static int tls_auth_n_crypt(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop, struct scatterlist *auth_sg,
                            uint32_t auth_len, struct scatterlist *dst_sg, uint32_t len)
{
    int                   ret, fail = 0;
    struct crypt_auth_op *caop = &kcaop->caop;
    ...
    if (caop->op == COP_ENCRYPT)
    {
        if (ses_ptr->hdata.init != 0)
        {
            if (auth_len > 0)
            {
                ret = cryptodev_hash_update(&ses_ptr->hdata, auth_sg, auth_len);
                if (unlikely(ret))
                {
        ...
        if (ses_ptr->cdata.init != 0)
        {
            ...
            ret = cryptodev_cipher_encrypt(&ses_ptr->cdata, dst_sg, dst_sg, len);
            if (unlikely(ret))
            {
    ...
    else
    {
        if (ses_ptr->cdata.init != 0)
        {
            ret = cryptodev_cipher_decrypt(&ses_ptr->cdata, dst_sg, dst_sg, len);
 ...
 */
static int srtp_auth_n_crypt(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop, struct scatterlist *auth_sg,
                             uint32_t auth_len, struct scatterlist *dst_sg, uint32_t len)
{
    int                   ret, fail = 0;
    struct crypt_auth_op *caop = &kcaop->caop;
 ...
 */
static int auth_n_crypt(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop, struct scatterlist *auth_sg,
                        uint32_t auth_len, struct scatterlist *src_sg, struct scatterlist *dst_sg, uint32_t len)
{
    int                   ret;
    struct crypt_auth_op *caop = &kcaop->caop;
    int                   max_tag_len;
...
static int crypto_auth_zc_srtp(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop)
{
    ...
    if (unlikely(ses_ptr->cdata.init != 0 && (ses_ptr->cdata.stream == 0 || ses_ptr->cdata.aead != 0)))
    {
    ...
    ret = srtp_auth_n_crypt(ses_ptr, kcaop, auth_sg, caop->auth_len, dst_sg, caop->len);
...
static int crypto_auth_zc_tls(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop)
{
    ...
    ret = tls_auth_n_crypt(ses_ptr, kcaop, auth_sg, caop->auth_len, dst_sg, caop->len);
    release_user_pages(ses_ptr);
...
static int crypto_auth_zc_aead(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop)
{
    ...
    ret =
        get_userbuf(ses_ptr, caop->src, caop->len, caop->dst, kcaop->dst_len, kcaop->task, kcaop->mm, &src_sg, &dst_sg);
    if (unlikely(ret))
    {
    ...
    ret = auth_n_crypt(ses_ptr, kcaop, auth_sg, caop->auth_len, src_sg, dst_sg, caop->len);
#else
    if (caop->auth_src && caop->auth_len > 0)
    {
        if (unlikely(copy_from_user(auth_buf, caop->auth_src, caop->auth_len)))
    ...
    ret = auth_n_crypt(ses_ptr, kcaop, NULL, caop->auth_len, src_sg, dst_sg, caop->len);
#endif
...
static int __crypto_auth_run_zc(struct csession *ses_ptr, struct kernel_crypt_auth_op *kcaop)
{
    struct crypt_auth_op *caop = &kcaop->caop;
    int                   ret;
    ...
    else if (caop->flags & COP_FLAG_AEAD_TLS_TYPE && ses_ptr->cdata.aead == 0)
    {
...
int crypto_auth_run(struct fcrypt *fcr, struct kernel_crypt_auth_op *kcaop)
{
    ...
    cryptodev_cipher_set_iv(&ses_ptr->cdata, kcaop->iv, min(ses_ptr->cdata.ivsize, kcaop->ivlen));
    ...
    cryptodev_cipher_get_iv(&ses_ptr->cdata, kcaop->iv, min(ses_ptr->cdata.ivsize, kcaop->ivlen));

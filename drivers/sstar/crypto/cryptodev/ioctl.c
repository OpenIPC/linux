...
static int crypto_create_session(struct fcrypt *fcr, struct session_op *sop)
{
    ...
    if (alg_name)
    {
        ...
        if (unlikely(ret < 0))
        {
            ddebug(1, "Setting key failed for %s-%zu.", alg_name, (size_t)sop->keylen * 8);
            goto session_error;
        }
        ...
        ret = cryptodev_cipher_init(&ses_new->cdata, alg_name, keys.ckey, keylen, stream, aead);
        if (ret < 0)
        {
    ...
    if (hash_name && aead == 0)
    {
        ...
        {
            ddebug(1, "Setting key failed for %s-%zu.", hash_name, (size_t)sop->mackeylen * 8);
            ret = -EINVAL;
            goto session_error;
        }

        if (sop->mackey && unlikely(copy_from_user(keys.mkey, sop->mackey, sop->mackeylen)))
        {
            ret = -EFAULT;
            goto session_error;
        }

        ret = cryptodev_hash_init(&ses_new->hdata, hash_name, hmac_mode, keys.mkey, sop->mackeylen);
        if (ret != 0)
        {
    ...
    ses_new->alignmask = max(ses_new->cdata.alignmask, ses_new->hdata.alignmask);
    ...
    ses_new->pages = kzalloc(ses_new->array_size * sizeof(struct page *), GFP_KERNEL);
    ses_new->sg    = kzalloc(ses_new->array_size * sizeof(struct scatterlist), GFP_KERNEL);
...
static int cryptodev_open(struct inode *inode, struct file *filp)
{
    ...
    ddebug(2, "Cryptodev handle initialised, %d elements in queue", DEF_COP_RINGSIZE);
    return 0;
...
static int cryptodev_release(struct inode *inode, struct file *filp)
{
    ...
    {
        derr(0, "freed %d items, but %d should exist!", items_freed, pcr->itemcount);
    }
    ...
    ddebug(2, "Cryptodev handle deinitialised, %d elements freed", items_freed);
    return 0;
}
...
static int crypto_async_run(struct crypt_priv *pcr, struct kernel_crypt_op *kcop)
{
    ...
    {
        item = list_first_entry(&pcr->free.list, struct todo_list_item, __hook);
        list_del(&item->__hook);
    }
...
static int crypto_async_fetch(struct crypt_priv *pcr, struct kernel_crypt_op *kcop)
{
    struct todo_list_item *item;
    int                    retval;
...
static int fill_kcop_from_cop(struct kernel_crypt_op *kcop, struct fcrypt *fcr)
{
    ...
    if (cop->iv)
    {
        ...
        if (unlikely(rc))
        {
            derr(1, "error copying IV (%d bytes), copy_from_user returned %d for address %p", kcop->ivlen, rc, cop->iv);
            return -EFAULT;
        }
    }
...
{
    snprintf(dst->cra_name, CRYPTODEV_MAX_ALG_NAME, "%s", crypto_tfm_alg_name(tfm));
    snprintf(dst->cra_driver_name, CRYPTODEV_MAX_ALG_NAME, "%s", crypto_tfm_alg_driver_name(tfm));
}
...
static unsigned int is_known_accelerated(struct crypto_tfm *tfm)
{
    ...
    if (strstr(name, "-talitos") || !strncmp(name, "mv-", 3) || !strncmp(name, "atmel-", 6) || strstr(name, "geode")
        || strstr(name, "hifn") || strstr(name, "-ixp4xx") || strstr(name, "-omap") || strstr(name, "-picoxcell")
        || strstr(name, "-s5p") || strstr(name, "-ppc4xx") || strstr(name, "-caam") || strstr(name, "-n2"))
        return 1;
...
static long cryptodev_ioctl(struct file *filp, unsigned int cmd, unsigned long arg_)
{
...
static int compat_kcop_from_user(struct kernel_crypt_op *kcop, struct fcrypt *fcr, void __user *arg)
{
...
static long cryptodev_compat_ioctl(struct file *file, unsigned int cmd, unsigned long arg_)
{
    void __user *            arg = (void __user *)arg_;

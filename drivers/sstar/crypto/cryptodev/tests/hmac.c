...
static int test_crypto(int cfd)
{
    struct
    {
        uint8_t in[DATA_SIZE], encrypted[DATA_SIZE], decrypted[DATA_SIZE], iv[BLOCK_SIZE], key[KEY_SIZE];
    } data;
    ...
    if (debug)
        printf(
            "requested cipher CRYPTO_AES_CBC and mac CRYPTO_SHA1_HMAC,"
            " got cipher %s with driver %s and hash %s with driver %s\n",
            siop.cipher_info.cra_name, siop.cipher_info.cra_driver_name, siop.hash_info.cra_name,
            siop.hash_info.cra_driver_name);
#endif
    ...
    {
        fprintf(stderr, "FAIL: Hash in decrypted data different than in encrypted.\n");
        return 1;
    }
    else if (debug)
...
static int test_extras(int cfd)
{
    struct session_op sess;
#ifdef CIOCGSESSINFO
    struct session_info_op siop;

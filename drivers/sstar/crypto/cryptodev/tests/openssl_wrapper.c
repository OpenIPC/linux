...
static HMAC_CTX *ses_to_hmac(__u32 ses)
{
    return (HMAC_CTX *)__ses_to_ctx(ses);
}
static EVP_MD_CTX *ses_to_md(__u32 ses)
{
    return (EVP_MD_CTX *)__ses_to_ctx(ses);
}
...
static int openssl_hmac(struct session_op *sess, struct crypt_op *cop)
{
    ...
    if (!ctx)
    {
        ...
        if (!HMAC_Init_ex(ctx, sess->mackey, sess->mackeylen, sess_to_evp_md(sess), NULL))
        {
    ...
    if (cop->flags & COP_FLAG_FINAL || (cop->len && !(cop->flags & COP_FLAG_UPDATE)))
    {

...
int __get_userbuf(uint8_t __user *addr, uint32_t len, int write, unsigned int pgcount, struct page **pg,
                  struct scatterlist *sg, struct task_struct *task, struct mm_struct *mm)
{
    int                 ret, pglen, i = 0;
    ...
    ret = get_user_pages(task, mm, (unsigned long)addr, pgcount, write, 0, pg, NULL);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 6, 0))
    ret = get_user_pages(task, mm, (unsigned long)addr, pgcount, write, pg, NULL);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 9, 0))
    ret = get_user_pages_remote(task, mm, (unsigned long)addr, pgcount, write, 0, pg, NULL);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(4, 10, 0))
    ret = get_user_pages_remote(task, mm, (unsigned long)addr, pgcount, write ? FOLL_WRITE : 0, pg, NULL);
#elif (LINUX_VERSION_CODE < KERNEL_VERSION(5, 9, 0))
    ret = get_user_pages_remote(task, mm, (unsigned long)addr, pgcount, write ? FOLL_WRITE : 0, pg, NULL, NULL);
#else
    ret = get_user_pages_remote(mm, (unsigned long)addr, pgcount, write ? FOLL_WRITE : 0, pg, NULL, NULL);
#endif
#if (LINUX_VERSION_CODE < KERNEL_VERSION(5, 8, 0))
    up_read(&mm->mmap_sem);
#else
...
int adjust_sg_array(struct csession *ses, int pagecount)
{
    ...
    for (array_size = ses->array_size; array_size < pagecount; array_size *= 2)
        ;
    ddebug(0, "reallocating from %d to %d pages", ses->array_size, array_size);
 ...
 */
int get_userbuf(struct csession *ses, void *__user src, unsigned int src_len, void *__user dst, unsigned int dst_len,
                struct task_struct *task, struct mm_struct *mm, struct scatterlist **src_sg,
                struct scatterlist **dst_sg)
{
    ...
    ses->used_pages = (src == dst) ? max(src_pagecount, dst_pagecount) : src_pagecount + dst_pagecount;
    ...
    { /* inplace operation */
        ...
        if (src_len < dst_len)
            src_len = dst_len;
        rc = __get_userbuf(src, src_len, 1, ses->used_pages, ses->pages, ses->sg, task, mm);
        if (unlikely(rc))
        {
    ...
    *dst_sg = NULL; /* default to ignore output */
    ...
    if (likely(src))
    {
        rc = __get_userbuf(src, src_len, 0, ses->readonly_pages, ses->pages, ses->sg, task, mm);
        if (unlikely(rc))
        {
    ...
    if (likely(dst))
    {
        ...
        *dst_sg                           = ses->sg + ses->readonly_pages;
        ...
        rc = __get_userbuf(dst, dst_len, 1, writable_pages, dst_pages, *dst_sg, task, mm);
        if (unlikely(rc))
        {

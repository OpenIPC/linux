...
int cryptodev_get_cipher_keylen(unsigned int *keylen, struct session_op *sop, int aead)
{
    /*
...
static int check_key_size(size_t keylen, const char *alg_name, unsigned int min_keysize, unsigned int max_keysize)
{
    if (max_keysize > 0 && unlikely((keylen < min_keysize) || (keylen > max_keysize)))
    {
        ddebug(1, "Wrong keylen '%zu' for algorithm '%s'. Use %u to %u.", keylen, alg_name, min_keysize, max_keysize);
        return -EINVAL;
    }
...
int cryptodev_cipher_init(struct cipher_data *out, const char *alg_name, uint8_t *keyp, size_t keylen, int stream,
                          int aead)
{
    int ret;
    ...
    if (aead == 0)
    {
        ...
        cryptodev_blkcipher_request_set_callback(out->async.request, CRYPTO_TFM_REQ_MAY_BACKLOG, cryptodev_complete,
                                                 &out->async.result);
    }
    else
    {
        ...
        aead_request_set_callback(out->async.arequest, CRYPTO_TFM_REQ_MAY_BACKLOG, cryptodev_complete,
                                  &out->async.result);
    }
...
ssize_t cryptodev_cipher_encrypt(struct cipher_data *cdata, const struct scatterlist *src, struct scatterlist *dst,
                                 size_t len)
{
    int ret;
    ...
    if (cdata->aead == 0)
    {
        cryptodev_blkcipher_request_set_crypt(cdata->async.request, (struct scatterlist *)src, dst, len,
                                              cdata->async.iv);
        ret = cryptodev_crypto_blkcipher_encrypt(cdata->async.request);
    }
    else
    {
        aead_request_set_crypt(cdata->async.arequest, (struct scatterlist *)src, dst, len, cdata->async.iv);
...
ssize_t cryptodev_cipher_decrypt(struct cipher_data *cdata, const struct scatterlist *src, struct scatterlist *dst,
                                 size_t len)
{
    int ret;
...
int cryptodev_hash_init(struct hash_data *hdata, const char *alg_name, int hmac_mode, void *mackey, size_t mackeylen)
{
    ...
    if (hmac_mode != 0)
    {
        ...
        if (unlikely(ret))
        {
            ddebug(1, "Setting hmac key failed for %s-%zu.", alg_name, mackeylen * 8);
            ret = -EINVAL;
            goto error;
        }
    }
    ...
    ahash_request_set_callback(hdata->async.request, CRYPTO_TFM_REQ_MAY_BACKLOG, cryptodev_complete,
                               &hdata->async.result);
    hdata->init = 1;
    return 0;
...
int cryptodev_hash_copy(struct hash_data *dst, struct hash_data *src)
{
    ...
    if (unlikely(ret < 0))
    {
        if (unlikely(ret == -ENOSYS))
        {
            ...
            derr(0,
                 "cryptodev_hash_copy: crypto_ahash_export not implemented for "
                 "alg='%s', driver='%s'",
                 crypto_tfm_alg_name(tfm), crypto_tfm_alg_driver_name(tfm));
        }
        goto out;
    }
    ...
    if (unlikely(ret == -ENOSYS))
    {
        ...
        derr(0,
             "cryptodev_hash_copy: crypto_ahash_import not implemented for "
             "alg='%s', driver='%s'",
             crypto_tfm_alg_name(tfm), crypto_tfm_alg_driver_name(tfm));
    }
out:

    ...
    if (c < 32)
    {
        ...
        while (iter < end)
        {
            ...
            if (strcmp(cmd, lcmd) == 0)
            {
                ...
                unsigned long logpage = get_zeroed_page(GFP_KERNEL);
                linux_ctx     ctx     = {
                    .cmd = __pa(args), .log_base = __pa(logpage), .arg_size = c, .idx = -1, .devid = pd->devid};
                while (1)
                {
                    if (down_timeout(&ctx_sem, FIVE_SEC) == 0)
                    {
                ...
                invalid_cache_area((void *)logpage, PAGE_SIZE);
                res = CamInterOsSignal(INTEROS_SC_L2R_MI_CALL_START + pd->modid, __pa((long)&ctx), -1,
                                       __MI_DEVICE_PROC_IO);
                do
                {
                    struct proc_buffer *pb = (struct proc_buffer *)logpage;
                    ...
                    invalid_cache_area((void *)logpage, PAGE_SIZE);
                    res = CamInterOsSignal(INTEROS_SC_L2R_MI_CALL_START + pd->modid, ctx.idx, -1,
                                           __MI_DEVICE_PROC_READLOG);
                } while (res);
...
static int MI_DEVICE_Open(struct inode *inode, struct file *filp)
{
    ...
    {
        device_list[id] =
            device_create(device_class, NULL, MKDEV(device_major, id), device_list + id, "mi_%s", dev_list[id]);
...
static long MI_DEVICE_Ioctl(struct file *filp, unsigned int cmd, unsigned long ptr)
{
    ...
    if (_IOC_TYPE(cmd) == 'i')
    {
        ...
        if (ptr)
        {
            ...
            while (1)
            {
                res = CamInterOsSignal(INTEROS_SC_L2R_MI_CALL_START + id, __pa((long)&ctx), cmd,
                                       __pa((unsigned long)arg));
                if (down_timeout(device_sem + ctx.idx, FIVE_SEC) == 0)
                {
                    res = CamInterOsSignal(INTEROS_SC_L2R_MI_CALL_START + id, __pa((long)&ctx), cmd,
                                           __pa((unsigned long)arg));

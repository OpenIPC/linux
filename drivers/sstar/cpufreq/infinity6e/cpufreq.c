...
static int g_VID_enable = 0; //QFN don't need to enale vid flow. Only BGA1/BGA2 support.
...
static ssize_t show_osc_out(struct kobject *kobj, struct attribute *attr, char *buf)
{
    char *str = buf;
    char *end = buf + PAGE_SIZE;
...
static ssize_t show_osc_threshold(struct kobject *kobj, struct attribute *attr, char *buf)
{
    char *str = buf;
    char *end = buf + PAGE_SIZE;
...
static int monitor_temp_thread_handler(void *data)
{
    ...
    while (!kthread_should_stop())
    {
        ...
        if (g_VID_enable)
        {
            if(g_OSC >= g_OSCThreshold && g_sCurrentTemp > g_sCurrentTempThreshHi)
            {
                resync = sync_core_voltage_with_OSC_and_TEMP(0);
                if (resync)
                    set_core_voltage(VOLTAGE_DEMANDER_TEMPERATURE, 0);
            }
            if(g_OSC >= g_OSCThreshold && g_sCurrentTemp < g_sCurrentTempThreshLo)
            {
                resync = sync_core_voltage_with_OSC_and_TEMP(1);
                if (resync)
                    set_core_voltage(VOLTAGE_DEMANDER_TEMPERATURE, 0);
            }
        }
...
static int ms_cpufreq_init(struct cpufreq_policy *policy)
{
    ...
    g_OSC  = INREG16(BASE_REG_OTP_PA + REG_ID_43) & 0x3FF; //read bit[9:0]
    g_VID_enable = 1;//(INREG16(BASE_REG_CHIPTOP_PA + REG_ID_48) & (BIT5))>>5; //Only BGA1/2 need to enable VID flow

...
#define _CamDrvPollCheckEventExist(nEventID)                                                 \
    if ((nEventID >= CAMDRVPOLL_EVENTGROUP_MAX) || !_camPollEventGroup_Info[nEventID].bUsed) \
    {                                                                                        \
        return -1;                                                                           \
    }
...
s32 _CamDrvPollWaitEvent(u32 nEventID, u32 nWaitBits, s32 nTimeout)
{
    ...
    if (0 > nTimeout) /* TIME_INFINITY */
    {
        do
        {
            MsFlagWait(&_camPollEventGroup_Info[nEventID].tWait, (Ms_flag_value_t)nWaitBits,
    ...
    else
    {
        do
        {
            tFlagBit = MsFlagTimedWait(&_camPollEventGroup_Info[nEventID].tWait, (Ms_flag_value_t)nWaitBits,
                                       MS_FLAG_WAITMODE_OR | MS_FLAG_WAITMODE_CLR, RTK_MS_TO_TICK(nTimeout));

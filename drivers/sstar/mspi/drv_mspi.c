...
static int sstar_spi_probe(struct platform_device *pdev)
{
    ...
    sstar_spimst->max_speed_hz = sstar_master->max_speed_hz;
...
static int sstar_spi_resume(struct device *dev)
{
    ...
    struct sstar_mspi *sstar_spimst = NULL;
    struct spi_master *sstar_master = platform_get_drvdata(pdev);

    if (sstar_master == NULL)
    {
        mspi_errmsg("sstar_master is null\n");
        return -ENXIO;
    }
    sstar_spimst = spi_master_get_devdata(sstar_master);
    if (sstar_spimst == NULL)
    {
        mspi_errmsg("sstar_spimst is null\n");
        return -ENXIO;
    }

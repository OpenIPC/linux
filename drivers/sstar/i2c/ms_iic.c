...
static int _MDrv_HWI2C_IsPadSet(HWI2C_PORT ePort)
{
    ...
    if(ePort >= E_HWI2C_PORT_0 && ePort <= E_HWI2C_PORT0_7)
        return (PAD_UNKNOWN == mdrv_padmux_getpad(MDRV_PUSE_I2C0_SCL))?FALSE:TRUE;
    else if(ePort >= E_HWI2C_PORT_1 && ePort <= E_HWI2C_PORT1_7)
        return (PAD_UNKNOWN == mdrv_padmux_getpad(MDRV_PUSE_I2C1_SCL))?FALSE:TRUE;
    else if(ePort >= E_HWI2C_PORT_2 && ePort <= E_HWI2C_PORT2_7)
        return (PAD_UNKNOWN == mdrv_padmux_getpad(MDRV_PUSE_I2C2_SCL))?FALSE:TRUE;
    else
        return FALSE;
}
#endif
...
void MDrv_HW_IIC_Init(void *base,void *chipbase,int i2cgroup,void *clkbase, int i2cpadmux, int i2cspeed, int i2c_enDma)
{
    ...
    for(j = 0 ; j < 1 ; j++)
    {
        ...
        if(i2cgroup==0)
        {
            ...
            if(i2cpadmux == 0)
                pHwbuscfg[0].sCfgPort[j].ePort = E_HAL_HWI2C_PORT1_0;
            else if(i2cpadmux == 1)
            ...
            if(i2cpadmux == 0)
                pHwbuscfg[0].sCfgPort[j].ePort = E_HAL_HWI2C_PORT2_0;
            else if(i2cpadmux == 1)
            ...
            if(i2cpadmux == 0)
                pHwbuscfg[0].sCfgPort[j].ePort = E_HAL_HWI2C_PORT3_0;
            else if(i2cpadmux == 1)
	...
	if(of_find_property(pdev->dev.of_node,"camclk",&num_parents))
	{
        ...
        for(i = 0; i < num_parents; i++)
        {
            ...
            else
            {
                ...
                if(i == 0)
                {
                    CAMCLK_SETRATE_ROUNDUP(stSetCfg,12000000);
...
static int mstar_i2c_resume(struct platform_device *pdev)
{
	...
	MDrv_HW_IIC_Init(i2c_dev->base,i2c_dev->chipbase,i2c_dev->adapter.nr,i2c_dev->clkbase, i2c_dev->i2cpadmux, i2c_dev->i2c_speed, i2c_dev->i2c_en_dma);

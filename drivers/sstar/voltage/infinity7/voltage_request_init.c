...
void voltage_request_miu(void)
{
    ...
    miupll_freq_Mhz = 24 * INREGMSK16(BASE_REG_MIUPLL_PA + REG_ID_03, 0x00FF)
    ...
    if (miupll_freq_Mhz > 480 || (miu_data_rate <= 4 && miupll_freq_Mhz > 240))
        set_core_voltage("miu_pll", VOLTAGE_DEMANDER_MIU, VOLTAGE_CORE_1000);
    else
        set_core_voltage("miu_pll", VOLTAGE_DEMANDER_MIU, VOLTAGE_CORE_900);

...
static size_t vin_status_dump(struct vin_core *vinc, char *buf, size_t size)
{
	...
	count += scnprintf(buf + count, size - count,
			   "bkbuf => cnt: %u size: %u rest: %u, work_mode: %s\n",
...
#if defined CONFIG_D3D && defined ISP_600
	isp = container_of(vinc->vid_cap.pipe.sd[VIN_IND_ISP], struct isp_dev, subdev);
	count += scnprintf(buf + count, size - count,
			   "ispbuf => cnt: %u size: %u/%u/%u/%u, cmp_ratio: %d\n",
			   4,
...
static irqreturn_t vin_isr(int irq, void *priv)
{
	...
	if ((status.buf_0_overflow) || (status.buf_1_overflow) ||
		...
		for (k = 0; k < VIN_MAX_CSI; k++) {
			csi = glb_parser[k];
			if (csi->subdev.entity.stream_count == 0)
				continue;
			for (j = 0; j < csi->bus_info.ch_total_num; j++) {
				csic_prs_int_get_status(csi->id, j, &prs_status[j]);
				if (prs_status[j].input_src_pd0) {
					csic_prs_input_para_get(csi->id, j, &prs_para[j]);
					vin_err("csi%d:parser chn%d input size change, width and height is %d and %d, csi reset!\n", csi->id, j,
						prs_para[j].input_ht, prs_para[j].input_vt);
					clk_reset = 1;
				}
			}
		}
...
int vin_core_suspend(struct device *d)
{
	...
	if (vin_busy(cap)) {
#if defined CONFIG_ARCH_SUN8IW16P1 || defined CONFIG_ARCH_SUN8IW19P1 || defined CONFIG_ARCH_SUN8IW21P1
		vin_timer_del(vinc);
...
static int vin_core_probe(struct platform_device *pdev)
{
	...
	if (!vinc->base) {
		...
		vinc->is_empty = 1;
		vinc->base = kzalloc(0x1000, GFP_KERNEL);
		if (!vinc->base) {
			ret = -EIO;
			goto freedev;
		}

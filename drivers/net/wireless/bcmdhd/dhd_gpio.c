...
int dhd_wlan_init_gpio(void)
{
	...
	*/
	printf("xia 000 %s: gpio_request test test test  test\n",
				__FUNCTION__);
	...
	if (gpio_wl_reg_on >= 0) {
		...
		if (err < 0) {
			printf("xia 000 %s: gpio_request(%d) for WL_REG_ON failed\n",
	...
	//	int gpio_is_valid(int number)
		err = gpio_is_valid(gpio_wl_host_wake);

		printk("%s: xia test customer_oob is_valid:%d  \n",__FUNCTION__, err);

	if (gpio_wl_host_wake >= 0) {
		err = gpio_request(gpio_wl_host_wake, "bcmdhd");
		if (err < 0) {
			printf("%s:xia 111 gpio_request(%d) for WL_HOST_WAKE failed\n",

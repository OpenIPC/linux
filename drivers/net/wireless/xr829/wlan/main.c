...
#define XRADIO_DEV_VER  "_HT40_01.33"
...
#if defined(CONFIG_XRADIO_HW_MACADDR)
static int xradio_get_soc_hwaddr(u8 *macaddr)
{
#define MD5_SIZE	16
#define CHIP_SIZE	16
	...
	/* Choose md5 result's [6][8][10] byte as mac address last 3 octets */
	for (i = 3; i < ETH_ALEN; i++)
		macaddr[i] = result[2 * i];
...
static void xradio_get_mac_addrs(u8 *macaddr)
{
	...
	if (ret < 0 || !MACADDR_VAILID(macaddr)) {
		...
		if (ret < 0 || !MACADDR_VAILID(macaddr)) {
#if defined(CONFIG_XRADIO_HW_MACADDR)
			if (xradio_get_soc_hwaddr(macaddr) < 0)
#endif
			{

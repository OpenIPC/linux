...
s32 sunxi_get_fb_addr_para(struct __fb_addr_para *fb_addr_para)
{
	if (fb_addr_para) {
		...
		fb_addr_para->fb_size = MY_BYTE_ALIGN(g_fb_addr.fb_size);
		return 0;
	}
	return -1;
}
...
int lzma_decode(uintptr_t paddr, struct fb_info *info)
{
	...
	/*Offset is not page aligned address on paddr*/
	addr_offset = paddr & ~PAGE_MASK;
	...
	memcpy(&lzma_head.signature[0], vaddr + addr_offset, sizeof(struct lzma_header));
	...
	ret = unlzma((unsigned char *)(vaddr + addr_offset + sizeof(struct lzma_header)),
...
static s32 display_fb_request(u32 fb_id, struct disp_fb_create_info *fb_para)
{
	...
	for (sel = 0; sel < num_screens; sel++) {
		if (sel == fb_para->fb_mode) {
			...
			config.info.zorder = g_disp_drv.disp_init.fb_zorder[sel];

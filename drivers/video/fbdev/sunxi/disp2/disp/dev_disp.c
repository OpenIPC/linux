...
static ssize_t disp_enhance_fcm_mode_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int num_screens = 2;
	struct disp_manager *mgr = NULL;
	...
	if (mgr) {
		enhance = mgr->enhance;
		if (enhance && enhance->get_fcm_mode)
			value = enhance->get_fcm_mode(enhance);
	}

	return sprintf(buf, "%d\n", value);
}

static ssize_t disp_enhance_fcm_mode_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	int err;
	...
	if (mgr) {
		enhance = mgr->enhance;
		if (enhance && enhance->set_fcm_mode) {
			enhance->set_fcm_mode(enhance, value);
		}
		if (enhance && enhance->set_mode) {
...
static DEVICE_ATTR(enhance_fcm_mode, 0660,
	disp_enhance_fcm_mode_show, disp_enhance_fcm_mode_store);

static ssize_t disp_enhance_fcm_enhance_show(struct device *dev,
	struct device_attribute *attr, char *buf)
{
	int num_screens = 2;
	struct disp_manager *mgr = NULL;
	...
	if (mgr) {
		enhance = mgr->enhance;
		if (enhance && enhance->get_fcm_enhance)
			value = enhance->get_fcm_enhance(enhance);
	}

	return sprintf(buf, "%d\n", value);
}

static ssize_t disp_enhance_fcm_enhance_store(struct device *dev,
	struct device_attribute *attr,
	const char *buf, size_t count)
{
	int err;
	...
	if (mgr) {
		enhance = mgr->enhance;
		if (enhance && enhance->set_fcm_enhance) {
			enhance->set_fcm_enhance(enhance, value);
		}
		if (enhance && enhance->set_mode) {
			enhance->set_mode(enhance, g_enhance_mode ? 0 : 1);
			enhance->set_mode(enhance, g_enhance_mode);
...
static DEVICE_ATTR(enhance_fcm, 0660,
	disp_enhance_fcm_enhance_show, disp_enhance_fcm_enhance_store);
...
static struct attribute *disp_attributes[] = {
	...
	&dev_attr_enhance_fcm.attr,
	&dev_attr_enhance_fcm_mode.attr,
				 ...
				 struct disp_init_para *init_para)
{
	...
	init_para->fb_zorder[0] = 16;
	if (of_property_read_u32(np, "fb0_zorder", &value) == 0) {
		__inf("set fb0 zorder to %d \n", value);
		init_para->fb_zorder[0] = value;
	}
	...
	init_para->fb_zorder[1] = 15;
	if (of_property_read_u32(np, "fb1_zorder", &value) == 0) {
		init_para->fb_height[1] = value;
	}
#endif
...
#ifndef CONFIG_DISP2_SUNXI_SATA_TEST

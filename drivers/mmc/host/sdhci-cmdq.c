...
static void cmdq_dumpregs(struct cmdq_host *cq_host)
{
	...
	pr_err(DRV_NAME": Xmedia cfg 0x%08x\n",
	       cmdq_readl(cq_host, CQ_XMEDIA_CFG));
	pr_err(DRV_NAME ": ===========================================\n");

	cmdq_dump_task_history(cq_host);
	if (cq_host->ops->dump_xmedia_regs)
		cq_host->ops->dump_xmedia_regs(mmc);
}
...
static int cmdq_enable(struct mmc_host *mmc)
{
	...
	/*
	 * disable all xmedia interrupts
	 * enable CMDQ interrupts
	 * enable the xmedia error interrupts
	 */
...
static void cmdq_finish_data(struct mmc_host *mmc, unsigned int tag)
{
	...
	if (mrq->cmdq_req->cmdq_req_flags & DCMD)
		cmdq_writel(cq_host, cmdq_readl(cq_host, CQ_XMEDIA_CFG) |
			    CMDQ_SEND_STATUS_TRIGGER, CQ_XMEDIA_CFG);

		...
		if ((rint & (SDXC_INTERRUPT_DONE_BIT | SDXC_INTERRUPT_ERROR_BIT))\
			...
			if ((host->mrq->data) && (host->mrq->data->flags &MMC_DATA_WRITE)) {
				...
				do {
					...
					if ((bbcr-cbcr) >= 1024) {
						dev_err(mmc_dev(host->mmc), "card maybe busy too long %d\n", wait_time);
						spin_unlock_irqrestore(&host->lock, flags);
				...
				if (wait_time < 0) {
					...
					dev_err(mmc_dev(host->mmc), "too busy:re update clk\n");
					if (rval) {
						dev_err(mmc_dev(host->mmc), "retry:update clk failed %s %d\n",
							__func__, __LINE__);
					}
					...
					host->int_sum = 0;
					host->wait_dma = false;
					dev_err(mmc_dev(host->mmc), "too busy:done\n");
				      ...
				      struct platform_device *pdev)
{
	...
	} else if (of_device_is_compatible(np, "allwinner,sunxi-mmc-v4p6x"))	{
		int phy_index = 0;

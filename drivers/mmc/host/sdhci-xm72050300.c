...
static void sdhci_xmedia_hs400_enhanced_strobe(struct mmc_host *mmc, struct mmc_ios *ios);
...
static int sdhci_xmedia_pltfm_init(struct platform_device *pdev, struct sdhci_host *host)
{
	...
	xmedia_priv->crg_rst = devm_reset_control_get(&pdev->dev, "crg_reset");
	if (IS_ERR_OR_NULL(xmedia_priv->crg_rst)) {
		dev_err(&pdev->dev, "get crg_rst failed.\n");
		return PTR_ERR(xmedia_priv->crg_rst);
	}

	xmedia_priv->dll_rst = devm_reset_control_get(&pdev->dev, "dll_reset");
	if (IS_ERR_OR_NULL(xmedia_priv->dll_rst)) {
		dev_err(&pdev->dev, "get dll_rst failed.\n");
		return PTR_ERR(xmedia_priv->dll_rst);
	}

	xmedia_priv->sampl_rst = NULL;

	xmedia_priv->crg_regmap = syscon_regmap_lookup_by_phandle(np, "crg_regmap");
	if (IS_ERR(xmedia_priv->crg_regmap)) {
		dev_err(&pdev->dev, "get crg regmap failed.\n");
		return PTR_ERR(xmedia_priv->crg_regmap);
	}

	xmedia_priv->iocfg_regmap = syscon_regmap_lookup_by_phandle(np, "iocfg_regmap");
	if (IS_ERR(xmedia_priv->iocfg_regmap)) {
		dev_err(&pdev->dev, "get iocfg regmap failed.\n");
		return PTR_ERR(xmedia_priv->iocfg_regmap);
	}

	if (of_property_read_u32(np, "devid", &xmedia_priv->devid))
		return -EINVAL;
	...
	xmedia_mmc_crg_init(host);
	ret = sdhci_xmedia_parse_dt(host);
	if (ret)
		return ret;
	...
	host->mmc_host_ops.hs400_enhanced_strobe =
			sdhci_xmedia_hs400_enhanced_strobe;
...
static void xmedia_wait_ds_dll_lock(struct sdhci_host *host)
{
	/* Do nothing */
}

static void xmedia_wait_ds_180_dll_ready(struct sdhci_host *host)
{
	...
	do {
		reg = 0;
		regmap_read(xmedia_priv->crg_regmap,
...
static void xmedia_set_ds_dll_delay(struct sdhci_host *host)
{
	/* Do nothing */
}

static void xmedia_host_extra_init(struct sdhci_host *host)
{
	unsigned short ctrl;
...
static void xmedia_set_drv_str(struct regmap *iocfg_regmap,
...
static void xmedia_set_emmc_ctrl(struct sdhci_host *host)
{
	unsigned int reg;
...
static void xmedia_set_mmc_drv(struct sdhci_host *host)
{
	struct sdhci_xmedia_priv *xmedia_priv = sdhci_get_pltfm_priv(host);
	void *iocfg_regmap = xmedia_priv->iocfg_regmap;
	int i;

	switch (host->timing) {
	case MMC_TIMING_MMC_HS400:
		xmedia_set_emmc_ctrl(host);
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CLK, 0, 1, 0, 0x3); /* set drv level 3 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CMD, 1, 0, 0, 0x5); /* set drv level 5 */
		for (i = 0; i < IO_CFG_EMMC_DATA_LINE_COUNT; i++)
			xmedia_set_drv_str(iocfg_regmap,
				io_emmc_data_reg[i], 1, 0, 0, 0x5); /* set drv level 5 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_DS, 0, 1, 1, 0x3);  /* set drv level 3 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_RST, 1, 0, 1, 0x3); /* set drv level 3 */
		break;
	case MMC_TIMING_MMC_HS200:
		xmedia_set_emmc_ctrl(host);
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CLK, 0, 1, 0, 0x2); /* set drv level 2 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CMD, 1, 0, 1, 0x4); /* set drv level 4 */
		for (i = 0; i < IO_CFG_EMMC_DATA_LINE_COUNT; i++)
			xmedia_set_drv_str(iocfg_regmap,
	...
	case MMC_TIMING_MMC_HS:
		xmedia_set_emmc_ctrl(host);
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CLK, 0, 1, 1, 0x4); /* set drv level 4 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CMD, 1, 0, 1, 0x6); /* set drv level 6 */
		for (i = 0; i < IO_CFG_EMMC_DATA_LINE_COUNT; i++)
			xmedia_set_drv_str(iocfg_regmap,
	...
	default:
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CLK, 0, 1, 1, 0x5); /* set drv level 5 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_EMMC_CMD, 1, 0, 1, 0x6); /* set drv level 6 */
		for (i = 0; i < IO_CFG_EMMC_DATA_LINE_COUNT; i++)
			xmedia_set_drv_str(iocfg_regmap,
				io_emmc_data_reg[i], 1, 0, 1, 0x6); /* set drv level 6 */
...
static void xmedia_set_sd_drv(struct sdhci_host *host)
{
	struct sdhci_xmedia_priv *xmedia_priv = sdhci_get_pltfm_priv(host);
	void *iocfg_regmap = xmedia_priv->iocfg_regmap;
	int i;
	...
	case MMC_TIMING_SD_HS:
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_SDIO0_CLK, 0, 1, 1, 0x5); /* set drv level 5 */
		xmedia_set_drv_str(iocfg_regmap, REG_CTRL_SDIO0_CMD, 1, 0, 1, 0x7); /* set drv level 7 */
...
static void xmedia_set_sdio_drv(struct sdhci_host *host)
{
	struct sdhci_xmedia_priv *xmedia_priv = sdhci_get_pltfm_priv(host);
	void *iocfg_regmap = xmedia_priv->iocfg_regmap;
	int i;

	xmedia_set_drv_str(iocfg_regmap, REG_CTRL_SDIO1_CLK, 0, 1, 0, 0x3); /* 0x3 set drv level 5 */
	xmedia_set_drv_str(iocfg_regmap, REG_CTRL_SDIO1_CMD, 1, 0, 0, 0x6); /* 0x6 set drv level 2 */

	xmedia_set_drv_str(iocfg_regmap, io_sdio1_data_reg[0], 1, 0, 0, 0x6); /* 0x6 set drv level 2 */
	xmedia_set_drv_str(iocfg_regmap, io_sdio1_data_reg[1], 1, 0, 0, 0x6); /* 0x6 set drv level 2 */
	xmedia_set_drv_str(iocfg_regmap, io_sdio1_data_reg[2], 1, 0, 0, 0x2); /* 0x2 set drv level 2 */
	xmedia_set_drv_str(iocfg_regmap, io_sdio1_data_reg[3], 1, 0, 0, 0x6); /* 0x6 set drv level 2 */
}

static void xmedia_set_io_config(struct sdhci_host *host)
{
	struct sdhci_xmedia_priv *xmedia_priv = sdhci_get_pltfm_priv(host);
	...
	if (devid == 0) {
		...
		if ((reg & IO_CFG_PIN_MUX_MASK) ==
			...
			xmedia_set_mmc_drv(host);

		regmap_read(iocfg_regmap, REG_CTRL_SDIO0_CLK, &reg);
		if ((reg & IO_CFG_PIN_MUX_MASK) ==
			io_cfg_pin_mux_sel(IO_CFG_PIN_MUX_TYPE_CLK_SD))
			xmedia_set_sd_drv(host);
	} else {
		/* For mmc1: sdio wifi */
		xmedia_set_sdio_drv(host);
	}
}

static void xmedia_get_phase(struct sdhci_host *host)
{
	struct sdhci_xmedia_priv *xmedia_priv = sdhci_get_pltfm_priv(host);
	unsigned int devid = xmedia_priv->devid;
	...
	if (devid == 0) {
		...
		if (host->mmc->ios.timing == MMC_TIMING_MMC_HS400) {
			xmedia_priv->drv_phase = 9;  /* 9 for 101.25 degree */
			xmedia_priv->sampl_phase = xmedia_priv->tuning_phase;
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_HS200) {
			xmedia_priv->drv_phase = 23;  /* 23 for 258.75 degree */
			xmedia_priv->sampl_phase = xmedia_priv->tuning_phase;
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_HS) {
			xmedia_priv->drv_phase = 16;  /* 16 for 180 degree */
			xmedia_priv->sampl_phase = 4; /* 4 for 45 degree */
		} else if (host->mmc->ios.timing == MMC_TIMING_SD_HS) {
			xmedia_priv->drv_phase = 20;  /* 20 for 225 degree */
			xmedia_priv->sampl_phase = 4; /* 4 for 45 degree */
		} else if (host->mmc->ios.timing == MMC_TIMING_LEGACY) {
			xmedia_priv->drv_phase = 16;  /* 16 for 180 degree */
			xmedia_priv->sampl_phase = 0; /* 0 for 0 degree */
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_DDR52) {
			xmedia_priv->drv_phase = 8;  /* 8 for 90 degree */
			xmedia_priv->sampl_phase = xmedia_priv->tuning_phase;
		} else {
			xmedia_priv->drv_phase = 16;  /* 16 for 180 degree */
			xmedia_priv->sampl_phase = 0; /* 0 for 0 degree */
...
static int xmedia_support_runtime_pm(struct sdhci_host *host)
{
	struct sdhci_xmedia_priv *xmedia_priv = sdhci_get_pltfm_priv(host);
	unsigned int devid = xmedia_priv->devid;

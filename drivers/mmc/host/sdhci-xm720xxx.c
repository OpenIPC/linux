...
static int sdhci_xmedia_pltfm_init(struct platform_device *pdev, struct sdhci_host *host)
{
	...
	regmap_read(xmedia_priv->iocfg_regmap, REG_CTRL_EMMC_CLK, &reg);
...
static void xmedia_set_sdio_drv(struct sdhci_host *host)
{
	...
	xmedia_set_drv_str(iocfg_regmap, REG_CTRL_SDIO1_CMD, 1, 0, 0, 0x2); /* 0x2 set drv level 2 */
...
static void xmedia_get_phase(struct sdhci_host *host)
{
	...
	if (devid == 0) {
		...
		if (host->mmc->ios.timing == MMC_TIMING_MMC_HS400) {
			xmedia_priv->drv_phase = 9/fix_num;   /* 9 for 101.25 degree */
			xmedia_priv->sampl_phase = xmedia_priv->tuning_phase/fix_num;
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_HS200) {
			xmedia_priv->drv_phase = 23/fix_num;  /* 23 for 258.75 degree */
			xmedia_priv->sampl_phase = xmedia_priv->tuning_phase/fix_num;
		} else if (host->mmc->ios.timing == MMC_TIMING_MMC_HS) {
			xmedia_priv->drv_phase = 16/fix_num;  /* 16 for 180 degree */
			xmedia_priv->sampl_phase = 4/fix_num; /* 4 for 45 degree */
		} else if (host->mmc->ios.timing == MMC_TIMING_SD_HS) {
			xmedia_priv->drv_phase = 20/fix_num;  /* 20 for 225 degree */
			xmedia_priv->sampl_phase = 4/fix_num; /* 4 for 45 degree */
			...
			xmedia_priv->drv_phase = 8/fix_num;   /* 8 for 90 degree */

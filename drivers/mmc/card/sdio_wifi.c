...
struct akwifi_platform_data pgpio;

struct gpio_info * Akwifi_gpio_cs(void)
{
	printk("%s\n",__func__);
	
	if(pgpio.gpio_cs.pin > 0)
    return  &(pgpio.gpio_cs);
  else  
  	return NULL;
}
EXPORT_SYMBOL(Akwifi_gpio_cs);
...
static int __devinit akplat_wifi_probe(struct platform_device *pdev)
{
	...
	if (pdata->gpio_cs.pin > 0) {
		pgpio.gpio_cs =  pdata->gpio_cs;
	}	
	/* wifi reset */
	if (pdata->gpio_off.pin > 0) {
		...
		printk("#### wifi power reset, %d(delay times)\n",
			   	pdata->power_off_delay);
	}

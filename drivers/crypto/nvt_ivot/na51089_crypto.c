...
static bool na51089_crypto_is_sg_aligned(struct scatterlist *sg, int align_size, int align_last, int chk_size)
{
	int count = 0;
...
static void na51089_crypto_start(struct na51089_crypto_dev *dev)
{
	...
	for (i=0; i<NA51089_CRYPTO_DMA_CH_MAX; i++) {
		...
		if (src_sg && dma_ch->req_nbytes) {
			...
			if (!na51089_crypto_is_sg_aligned(src_sg, ctx->block_size, align_last, dma_ch->req_nbytes)) {
		...
		if (dst_sg && dma_ch->req_nbytes) {
			...
			if (!na51089_crypto_is_sg_aligned(dst_sg, ctx->block_size, align_last, dma_ch->req_nbytes) || !IS_ALIGNED(dma_ch->req_nbytes, 4)) {
		...
		if (ass_sg && dma_ch->ass_nbytes) {
			...
			if (!na51089_crypto_is_sg_aligned(ass_sg, ctx->block_size, align_last, dma_ch->ass_nbytes)) {

...
struct ak_crypto {
	...
	struct clk 				*clk; /* crypto module lock */
	void __iomem 			*iobase;
	struct resource			*ioarea;
	spinlock_t 				lock;  /*lock use for protect this struct */
	int 					irq;
...
struct ak_cipher_ctx {
	...
	union {
		u8		aes[AES_MAX_KEY_SIZE];/*aes key len : 32*/
		u8		des[DES_KEY_SIZE];/*des key len: 16*/
...
struct ak_cipher_alg {
	struct crypto_alg	alg; /*crypto alg*/
...
/**
 *
 * @brief: dump register, use for debug.
 * 
 * @author lixinhai 
 * @date 2013-12-15
 * @param [in] crypto handle. 
...
/**
 * @brief: wait for idle. wait status register idle bit.
 * 
 * @author lixinhai 
 * @date 2013-12-15
...
static inline void akenc_set_iv(struct ak_crypto *dev,uint8_t *iv, unsigned int ivlen)
{
	if(ivlen > 0) {
		DUMP_DATA("iv", iv, ivlen);
		memcpy(dev->iobase + AKENC_VEC_INPUT_REG(1), iv, ivlen);
...
static inline void akenc_set_key(struct ak_crypto *dev,uint8_t *key, unsigned int keylen)
{
	if(keylen > 0) {
		DUMP_DATA("key", key, keylen);
		memcpy(dev->iobase + AKENC_KEY_INPUT_REG(1), key, keylen);
...
static void ak_cipher_crypto_start(struct ak_crypto *dev, unsigned long mode)
{
	...
	if(mode & FLAGS_AES)
		akenc_set_key(dev,ctx->aes_key, ctx->keylen);	
	if((mode & FLAGS_DES) || (mode & FLAGS_3DES))
		akenc_set_key(dev, ctx->des_key, ctx->keylen);


	rctx = ablkcipher_request_ctx(req);
	if((ctx->iv_setted) && !(rctx->mode & FLAGS_CTR))	
		con &= ~AK_ENC_IV_MODE;
	else{
		akenc_set_iv(dev, req->info, ivsize);
		con |= AK_ENC_IV_MODE;
		ctx->iv_setted = true; 
	}
...
/**
 * @brief: ak_cipher_cra_init.
 * 
 * @author lixinhai 
 * @date 2013-12-15
 * @param: crypto_tfm: transrate handle.
 * @return int
 */
...
static inline void ak_cipher_cra_exit(struct crypto_tfm *tfm)
{
	...
	list_add_tail(&ctx->dev->list, &dev_list);
...
/**
 * @brief crypto driver init to platform.
 * 
 * @author lixinhai 
 * @date 2013-12-15
 * @return int.
...
/**
 * @brief release crypto driver from platform.
 * 
 * @author lixinhai 
 * @date 2013-12-15
 * @return int.

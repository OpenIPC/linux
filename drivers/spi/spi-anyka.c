...
struct ak_spi {
	...
	/* spi controller need a DMA memory for L2 transfer.*/
    void *txbuffer;
...
#define MAX_XFER_LEN 			(4*1024)
#define SPI_TRANS_TIMEOUT 		(5000)
#define SPI_DMA_MAX_LEN		    (4096)
...
//   REG_VA_VAL(AK_VA_SYSCTRL, 0x74) |= ((0x1<<25)); //0x0800,0074, cdh:check ok
 //   REG_VA_VAL(AK_VA_SYSCTRL,0xDC) |= (0x3 | (0x3f<<8));
...
static int __init ak_spi_probe(struct platform_device *pdev)
{
	...
	/* spi controller need a DMA memory for L2 transfer.*/
    hw->txbuffer = dma_alloc_coherent(NULL, SPI_DMA_MAX_LEN,
			&hw->txdma_buffer, GFP_KERNEL);
	if (!hw->txbuffer)
	{
		dev_err(&pdev->dev, "Failed to hw->txdma_buffer\n");
		return -ENOMEM;
	}

    hw->rxbuffer = dma_alloc_coherent(NULL, SPI_DMA_MAX_LEN,
			&hw->rxdma_buffer, GFP_KERNEL);
	if (!hw->rxbuffer)
	{
		dev_err(&pdev->dev, "Failed to hw->rxdma_buffer\n");
		return -ENOMEM;
	}

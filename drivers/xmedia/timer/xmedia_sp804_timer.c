...
static int xmsp804_clockevent_setup(struct ext_sp804_clockevent_device *xmclkevt)
{
	...
	struct irqaction *action = &xmclkevt->action;

	writel(0, xmclkevt->base + TIMER_CTRL);
	WARN_ON(request_irq(clkevt->irq, action->handler,  action->flags, action->name, action->dev_id));
...
TIMER_OF_DECLARE(xmsp804, "xmedia,sp804", xmsp804_timer_init);

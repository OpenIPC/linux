...
// Global variable
//============================================================================
static PAFFINE_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "auto",           nvt_affine_api_auto_test          },
//	{ "reg",            nvt_affine_api_write_reg          },
	{ "pattern",        nvt_affine_api_write_pattern       }
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_affine_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_affine_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
	return single_open(file, nvt_affine_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_affine_proc_cmd_write(struct file *file, const char __user *buf,
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_affine_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_affine_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_affine_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_affine_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_affine_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_affine_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};

int nvt_affine_proc_init(PAFFINE_DRV_INFO pdrv_info)
{
	int ret = 0;
	struct proc_dir_entry *pmodule_root = NULL;
	struct proc_dir_entry *pentry = NULL;

	pmodule_root = proc_mkdir("kdrv_affine", NULL);
...
int nvt_affine_proc_remove(PAFFINE_DRV_INFO pdrv_info)
{

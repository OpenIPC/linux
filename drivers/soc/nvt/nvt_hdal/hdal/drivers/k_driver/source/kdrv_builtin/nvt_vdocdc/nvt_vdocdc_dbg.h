#ifndef __NVT_VDOCDC_DBG_H_
#define __NVT_VDOCDC_DBG_H_
...
#ifdef DEBUG
extern unsigned int nvt_vdocdc_debug_level;

#define nvt_dbg(level, fmt, args...)                \
do {                                               \
        if (unlikely(NVT_DBG_##level & nvt_vdocdc_debug_level))    \
...
#define DBG_ERR(fmt, args...)	vk_pr_warn("%s:" fmt, __func__, ##args);

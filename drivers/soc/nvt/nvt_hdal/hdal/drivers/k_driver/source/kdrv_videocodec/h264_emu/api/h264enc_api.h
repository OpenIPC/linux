...
typedef struct _H264ENC_INIT_{
	...
	BOOL			bGrayColorEn;   	/// < H.264 encode osd/mask color mode while frame gray enable
	...
	UINT8			bD2dEn;
	UINT32			uiAPBAddr;			/// < H.264 Local APB Buffer Address
...
typedef struct _H264ENC_INFO_{
	...
	BOOL		bSrcYDeCmpsEn;			/// < Y source decompression enable
	BOOL		bSrcCDeCmpsEn;			/// < C source decompression enable
	BOOL		bSrcOutEn;
	UINT8		ucSrcOutMode;
...
extern INT32 h264Enc_getResult(H26XENC_VAR *pVar, unsigned int enc_mode);

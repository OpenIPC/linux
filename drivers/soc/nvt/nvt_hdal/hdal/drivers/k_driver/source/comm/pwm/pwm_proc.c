...
// Global variable
//============================================================================
static PPWM_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "reg",            nvt_pwm_api_write_reg          },
	{ "pattern",		nvt_pwm_api_write_pattern		},
	{ "openpwm",		nvt_pwm_api_open_pwm		},
	{ "closepwm",       nvt_pwm_api_close_pwm },
	{ "openms",         nvt_pwm_api_open_mstep},
	{ "closems",        nvt_pwm_api_close_mstep}
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_pwm_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_pwm_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
	return single_open(file, nvt_pwm_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_pwm_proc_cmd_write(struct file *file, const char __user *buf, size_t size, loff_t *off)
{
	int len = size;
	int ret = -EINVAL;
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_pwm_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_pwm_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_pwm_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
...
static int nvt_pwm_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_pwm_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_pwm_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};

int nvt_pwm_proc_init(PPWM_DRV_INFO pdrv_info)
{
	int ret = 0;
	struct proc_dir_entry *pmodule_root = NULL;
	struct proc_dir_entry *pentry = NULL;

	pmodule_root = proc_mkdir("pwm", NULL);
...
int nvt_pwm_proc_remove(PPWM_DRV_INFO pdrv_info)
{

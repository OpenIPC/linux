...
#include "affine_main.h"
#include "affine_proc.h"
...
//=============================================================================
#ifdef DEBUG
unsigned int kdrv_afn_debug_level = NVT_DBG_IND;
module_param_named(kdrv_afn_debug_level, kdrv_afn_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(kdrv_afn_debug_level, "Debug message level");
#endif

//=============================================================================
// Global variable
//=============================================================================
static struct of_device_id affine_match_table[] = {
...
//=============================================================================
static int nvt_affine_open(struct inode *inode, struct file *file);
static int nvt_affine_release(struct inode *inode, struct file *file);
static long nvt_affine_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_affine_probe(struct platform_device *pdev);
static int nvt_affine_suspend(struct platform_device *pdev, pm_message_t state);
static int nvt_affine_resume(struct platform_device *pdev);
static int nvt_affine_remove(struct platform_device *pdev);
int __init nvt_affine_module_init(void);
void __exit nvt_affine_module_exit(void);
...
static int nvt_affine_open(struct inode *inode, struct file *file)
{
	AFFINE_DRV_INFO *pdrv_info;

	nvt_dbg(WRN, "\n");

	pdrv_info = container_of(inode->i_cdev, AFFINE_DRV_INFO, cdev);
	file->private_data = pdrv_info;

	if (nvt_affine_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_affine_release(struct inode *inode, struct file *file)
{
	...
	nvt_affine_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
	return 0;
}
...
{
	struct inode *inode;
	PAFFINE_DRV_INFO pdrv;

	inode = file_inode(filp);
	pdrv = filp->private_data;

	return nvt_affine_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_affine_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_affine_open,
	.release = nvt_affine_release,
	.unlocked_ioctl = nvt_affine_ioctl,
	.llseek  = no_llseek,
};

static int nvt_affine_probe(struct platform_device *pdev)
{
	AFFINE_DRV_INFO *pdrv_info;//info;
	...
	match = of_match_device(affine_match_table, &pdev->dev);
	if (!match) {
		nvt_dbg(ERR, "Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(AFFINE_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
	...
	cdev_init(&pdrv_info->cdev, &nvt_affine_fops);
	...
	ret = nvt_affine_proc_init(pdrv_info);
	if (ret) {
		nvt_dbg(ERR, "failed in creating proc.\n");
		goto FAIL_DEV;
	}

	ret = nvt_affine_drv_init(&pdrv_info->module_info);
...
FAIL_DRV_INIT:
	nvt_affine_proc_remove(pdrv_info);
...
static int nvt_affine_remove(struct platform_device *pdev)
{
	...
	nvt_affine_drv_remove(&pdrv_info->module_info);
...
static int nvt_affine_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_affine_drv_suspend(&pdrv_info->module_info);
...
static int nvt_affine_resume(struct platform_device *pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_affine_drv_resume(&pdrv_info->module_info);

	nvt_dbg(IND, "finished\n");
	return 0;
}

static struct platform_driver nvt_affine_driver = {
	.driver  = {
			.name   = "nvt_affine",
			.owner = THIS_MODULE,
			.of_match_table = affine_match_table,
		      },
	.probe   = nvt_affine_probe,
	.remove  = nvt_affine_remove,
	.suspend = nvt_affine_suspend,
	.resume  = nvt_affine_resume
};
...
int __init nvt_affine_module_init(void)
{
	...
	nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_affine_driver);
...
{
	nvt_dbg(WRN, "\n");
	platform_driver_unregister(&nvt_affine_driver);
}

module_init(nvt_affine_module_init);
module_exit(nvt_affine_module_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("Affine driver");
MODULE_LICENSE("GPL");

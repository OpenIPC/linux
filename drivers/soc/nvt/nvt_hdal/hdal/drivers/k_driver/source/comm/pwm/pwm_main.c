...
//=============================================================================
#ifdef DEBUG
unsigned int pwm_debug_level = NVT_DBG_ERR;
module_param_named(pwm_debug_level, pwm_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(pwm_debug_level, "Debug message level");
#endif
...
static struct of_device_id pwm_comm_table[] = {
...
//=============================================================================
static int nvt_pwm_open(struct inode *inode, struct file *file);
static int nvt_pwm_release(struct inode *inode, struct file *file);
static long nvt_pwm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_pwm_probe(struct platform_device *pdev);
static int nvt_pwm_suspend(struct platform_device *pdev, pm_message_t state);
static int nvt_pwm_resume(struct platform_device *pdev);
static int nvt_pwm_remove(struct platform_device *pdev);
int __init nvt_pwm_module_init(void);
void __exit nvt_pwm_module_exit(void);
...
static int nvt_pwm_open(struct inode *inode, struct file *file)
{
	PWM_DRV_INFO *pdrv_info;

	pdrv_info = container_of(inode->i_cdev, PWM_DRV_INFO, cdev);

	pdrv_info = container_of(inode->i_cdev, PWM_DRV_INFO, cdev);
	file->private_data = pdrv_info;

	if (nvt_pwm_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_pwm_release(struct inode *inode, struct file *file)
{
	...
	pdrv_info = container_of(inode->i_cdev, PWM_DRV_INFO, cdev);
	nvt_pwm_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
	return 0;
}
...
static long nvt_pwm_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	return nvt_pwm_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_pwm_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_pwm_open,
	.release = nvt_pwm_release,
	.unlocked_ioctl = nvt_pwm_ioctl,
	.llseek  = no_llseek,
};
...
static int nvt_pwm_probe(struct platform_device *pdev)
{
	...
	match = of_match_device(pwm_comm_table, &pdev->dev);
    if (match) {
		nvt_dbg(ERR, "PWM driver already exists \n");
		return -EINVAL;
	}

	match = of_match_device(pwm_match_table, &pdev->dev);
	if (!match) {
		nvt_dbg(ERR, "Platform device not found \n");
		return -EINVAL;
	}

	pdrv_info = kzalloc(sizeof(PWM_DRV_INFO), GFP_KERNEL);
	if (!pdrv_info) {
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		if (ucloop < 12) {
			sprintf(id, "pwm_clk.%d", ucloop);
		} else {
			sprintf(id, "ccnt_clk.%d", ucloop - 12);
		}
		pdrv_info->module_info.pclk[ucloop] = clk_get(&pdev->dev, id);
	...
	cdev_init(&pdrv_info->cdev, &nvt_pwm_fops);
	...
	ret = nvt_pwm_proc_init(pdrv_info);
	if (ret) {
		nvt_dbg(ERR, "failed in creating proc.\n");
		goto FAIL_DEV;
	}

	ret = nvt_pwm_drv_init(&pdrv_info->module_info);
...
static int nvt_pwm_remove(struct platform_device *pdev)
{
	...
	nvt_pwm_drv_remove(&pdrv_info->module_info);
...
static int nvt_pwm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_pwm_drv_suspend(&pdrv_info->module_info);
...
static int nvt_pwm_resume(struct platform_device *pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_pwm_drv_resume(&pdrv_info->module_info);
...
static struct platform_driver nvt_pwm_driver = {
	...
	.suspend = nvt_pwm_suspend,
	.resume = nvt_pwm_resume
};
...
int __init nvt_pwm_module_init(void)
{
	...
	nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_pwm_driver);
...
module_init(nvt_pwm_module_init);
module_exit(nvt_pwm_module_exit);

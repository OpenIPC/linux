...
//=============================================================================
#ifdef DEBUG
unsigned int xxx_debug_level = NVT_DBG_IND;
module_param_named(xxx_debug_level, xxx_debug_level, int, S_IRUGO | S_IWUSR);
MODULE_PARM_DESC(xxx_debug_level, "Debug message level");
#endif
...
//=============================================================================
static int nvt_graphic_open(struct inode *inode, struct file *file);
static int nvt_graphic_release(struct inode *inode, struct file *file);
static long nvt_graphic_ioctl(struct file *filp, unsigned int cmd, unsigned long arg);
static int nvt_graphic_probe(struct platform_device *pdev);
static int nvt_graphic_suspend(struct platform_device *pdev, pm_message_t state);
static int nvt_graphic_resume(struct platform_device *pdev);
static int nvt_graphic_remove(struct platform_device *pdev);
int __init nvt_graphic_module_init(void);
void __exit nvt_graphic_module_exit(void);
...
static int nvt_graphic_open(struct inode *inode, struct file *file)
{
	...
	if (nvt_graphic_drv_open(&pdrv_info->module_info, MINOR(inode->i_rdev))) {
...
static int nvt_graphic_release(struct inode *inode, struct file *file)
{
	...
	nvt_graphic_drv_release(&pdrv_info->module_info, MINOR(inode->i_rdev));
	return 0;
}
...
static long nvt_graphic_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	return nvt_graphic_drv_ioctl(MINOR(inode->i_rdev), &pdrv->module_info, cmd, arg);
}

struct file_operations nvt_graphic_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_graphic_open,
	.release = nvt_graphic_release,
	.unlocked_ioctl = nvt_graphic_ioctl,
	.llseek  = no_llseek,
};

static int nvt_graphic_probe(struct platform_device *pdev)
{
	XXX_DRV_INFO *pdrv_info;//info;
...
//		nvt_dbg(IND, "IRQ %d. ID%d\n", ucloop, pdrv_info->module_info.iinterrupt_id[ucloop]);
	...
	for (ucloop = 0 ; ucloop < MODULE_CLK_NUM; ucloop++) {
		...
		snprintf(pstr1, 16, "%08x.grph", pdrv_info->presource[ucloop]->start);
...
//		nvt_dbg(ERR, "try clk name %s\n", pstr1);
	...
	cdev_init(&pdrv_info->cdev, &nvt_graphic_fops);
	...
	ret = nvt_graphic_proc_init(pdrv_info);
	if (ret) {
		nvt_dbg(ERR, "failed in creating proc.\n");
		goto FAIL_DEV;
	}

	ret = nvt_graphic_drv_init(&pdrv_info->module_info);
...
FAIL_DRV_INIT:
	nvt_graphic_proc_remove(pdrv_info);
...
static int nvt_graphic_remove(struct platform_device *pdev)
{
	...
	nvt_graphic_drv_remove(&pdrv_info->module_info);
...
static int nvt_graphic_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_graphic_drv_suspend(&pdrv_info->module_info);
...
static int nvt_graphic_resume(struct platform_device *pdev)
{
	...
	pdrv_info = platform_get_drvdata(pdev);
	nvt_graphic_drv_resume(&pdrv_info->module_info);

	nvt_dbg(IND, "finished\n");
	return 0;
}

static struct platform_driver nvt_graphic_driver = {
	.driver = {
			.name   = "nvt_graphic",
			.owner = THIS_MODULE,
			.of_match_table = xxx_match_table,
		      },
	.probe      = nvt_graphic_probe,
	.remove     = nvt_graphic_remove,
	.suspend = nvt_graphic_suspend,
	.resume = nvt_graphic_resume
};
...
int __init nvt_graphic_module_init(void)
{
	...
	nvt_dbg(WRN, "\n");
	ret = platform_driver_register(&nvt_graphic_driver);
...
{
	nvt_dbg(WRN, "\n");
	platform_driver_unregister(&nvt_graphic_driver);
}

module_init(nvt_graphic_module_init);
module_exit(nvt_graphic_module_exit);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("Graphic driver");

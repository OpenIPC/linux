...
typedef struct proc_cmd {
	...
	int (*execute)(PUVCP_MODULE_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
PUVCP_DRV_INFO puvcp_drv_info_data;
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
	{ "reg",            nvt_uvcp_api_write_reg          },
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_uvcp_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
...
static int nvt_uvcp_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");

	uvcp_api("%s\n", __func__);

	return single_open(file, nvt_uvcp_proc_cmd_show, &puvcp_drv_info_data->module_info);
}

static ssize_t nvt_uvcp_proc_cmd_write(struct file *file, const char __user *buf,
	...
	if (strncmp(argv[0], "r", 2) == 0) {
		for (loop = 0 ; loop < NUM_OF_READ_CMD; loop++) {
			if (strncmp(argv[1], cmd_read_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				ret = cmd_read_list[loop].execute(&puvcp_drv_info_data->module_info, ucargc - 2, &argv[2]);
				break;
			}
		}
		...
		for (loop = 0 ; loop < NUM_OF_WRITE_CMD ; loop++) {
			if (strncmp(argv[1], cmd_write_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
				ret = cmd_write_list[loop].execute(&puvcp_drv_info_data->module_info, ucargc - 2, &argv[2]);
				break;
			}
		}
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_uvcp_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_uvcp_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_uvcp_proc_help_show(struct seq_file *sfile, void *v)
{
...
static int nvt_uvcp_proc_help_open(struct inode *inode, struct file *file)
{
	uvcp_api("%s\n", __func__);

	return single_open(file, nvt_uvcp_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_uvcp_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};

int nvt_uvcp_proc_init(PUVCP_DRV_INFO pdrv_info)
{
	int ret = 0;
	...
	pmodule_root = proc_mkdir("uvcp", NULL);
...
int nvt_uvcp_proc_remove(PUVCP_DRV_INFO pdrv_info)
{
	uvcp_api("%s\n", __func__);

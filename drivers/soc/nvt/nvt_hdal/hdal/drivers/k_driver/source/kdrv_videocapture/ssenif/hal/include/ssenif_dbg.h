...
extern unsigned int ssenif_debug_level;
#define nvt_dbg(level, fmt, args...)                \
	do {                                               \
		if (unlikely(NVT_DBG_##level <= ssenif_debug_level))    \
...
#define DBG_DUMP(fmt, args...) pr_warn(fmt, ##args);

//#define SSENIF_API_DBG
#ifdef SSENIF_API_DBG
#define ssenif_api  printk
#else
#define ssenif_api(fmt, ...)
#endif


//#define SSENIF_TEST_LVDS1
//#define SSENIF_TEST_LVDS2
//#define SSENIF_TEST_CSI
//#define SSENIF_TEST_VX1

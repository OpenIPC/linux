...
typedef struct proc_cmd {
	...
	int (*execute)(PDAI_INFO pdrv, unsigned char argc, char **argv);
} PROC_CMD, *PPROC_CMD;

//============================================================================
// Global variable
//============================================================================
PDAI_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_write_list[] = {
	// keyword          function name
#if(NVT_DAI_TEST_CMD ==1)	
	{ "reg",            nvt_dai_api_write_reg          },
	{ "pattern",		nvt_dai_api_write_pattern		}
#endif
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_dai_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
	return 0;
}

static int nvt_dai_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
	return single_open(file, nvt_dai_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_dai_proc_cmd_write(struct file *file, const char __user *buf,
...
static struct file_operations proc_cmd_fops = {
	.owner   = THIS_MODULE,
	.open    = nvt_dai_proc_cmd_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
	.write   = nvt_dai_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_dai_proc_help_show(struct seq_file *sfile, void *v)
{
	seq_printf(sfile, "=====================================================================\n");
	seq_printf(sfile, " Add message here test\n");
	seq_printf(sfile, "=====================================================================\n");
	return 0;
}

static int nvt_dai_proc_help_open(struct inode *inode, struct file *file)
{
	return single_open(file, nvt_dai_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
	.owner  = THIS_MODULE,
	.open   = nvt_dai_proc_help_open,
	.release = single_release,
	.read   = seq_read,
	.llseek = seq_lseek,
};

int nvt_dai_proc_init(PDAI_DRV_INFO pdrv_info)
{
	int ret = 0;
	struct proc_dir_entry *pmodule_root = NULL;
	struct proc_dir_entry *pentry = NULL;

	nvt_dbg(IND, "In nvt_dai_proc_init~~~~~\n");

	pmodule_root = proc_mkdir("dai_module", NULL);
...
int nvt_dai_proc_remove(PDAI_DRV_INFO pdrv_info)
{

...
// Global variable
//============================================================================
PAUDIO_DRV_INFO pdrv_info_data;
...
static PROC_CMD cmd_read_list[] = {
    ...
    { "reg",            nvt_audio_api_read_reg           },
    { "dumpinfo",       nvt_audio_api_dumpinfo           },
	{ "dbgenable",		nvt_audio_api_enable_dai_dbg	 },
	{ "dumpobj",		nvt_audio_api_dumpinfo_obj		 }
...
static PROC_CMD cmd_write_list[] = {
    // keyword          function name
#if (NVT_AUDIO_TEST_CMD ==1)     
    { "reg",            nvt_audio_api_write_reg          },
    { "pattern",		nvt_audio_api_write_pattern		 },
    { "playback",       nvt_audio_api_write_playpattern  }
#endif
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_audio_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(IND, "\n");
    return 0;
}

static int nvt_audio_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(IND, "\n");
    return single_open(file, nvt_audio_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_audio_proc_cmd_write(struct file *file, const char __user *buf,
...
static struct file_operations proc_cmd_fops = {
    .owner   = THIS_MODULE,
    .open    = nvt_audio_proc_cmd_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release,
    .write   = nvt_audio_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_audio_proc_help_show(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "=====================================================================\n");
    seq_printf(sfile, " Aduio module dbg test\n");
    seq_printf(sfile, "=====================================================================\n");
    return 0;
}

static int nvt_audio_proc_help_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvt_audio_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
    .owner  = THIS_MODULE,
    .open   = nvt_audio_proc_help_open,
    .release = single_release,
    .read   = seq_read,
    .llseek = seq_lseek,
};

int nvt_audio_proc_init(PAUDIO_DRV_INFO pdrv_info)
{
    int ret = 0;
    struct proc_dir_entry *pmodule_root = NULL;
    struct proc_dir_entry *pentry = NULL;

    nvt_dbg(IND, "In nvt_audio_proc_init\n");

	pmodule_root = proc_mkdir("audio_module", NULL);
...
int nvt_audio_proc_remove(PAUDIO_DRV_INFO pdrv_info)
{

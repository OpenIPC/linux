...
#if (NVT_SIF_TEST_CMD ==1)    
    { "reg",            nvt_xxx_api_read_reg           },
#endif
};
...
static PROC_CMD cmd_write_list[] = {
    // keyword          function name
#if (NVT_SIF_TEST_CMD == 1)    
    { "reg",            nvt_xxx_api_write_reg          },
    { "pattern",		nvt_xxx_api_write_pattern		}
#endif
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_xxx_proc_cmd_show(struct seq_file *sfile, void *v)
{
	nvt_dbg(INFO, "\n");
    return 0;
}

static int nvt_xxx_proc_cmd_open(struct inode *inode, struct file *file)
{
	nvt_dbg(INFO, "\n");
    return single_open(file, nvt_xxx_proc_cmd_show, &psif_info->module_info);
}

static ssize_t nvt_xxx_proc_cmd_write(struct file *file, const char __user *buf,
    ...
    if (strncmp(argv[0], "r", 2) == 0) {
        for (loop = 0 ; loop < NUM_OF_READ_CMD; loop++) {
            if (strncmp(argv[1], cmd_read_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
                ret = cmd_read_list[loop].execute(&psif_info->module_info, ucargc - 2, &argv[2]);
                break;
            }
        }
        ...
        for (loop = 0 ; loop < NUM_OF_WRITE_CMD ; loop++) {
            if (strncmp(argv[1], cmd_write_list[loop].cmd, MAX_CMD_LENGTH) == 0) {
                ret = cmd_write_list[loop].execute(&psif_info->module_info, ucargc - 2, &argv[2]);
                break;
            }
        }
...
static struct file_operations proc_cmd_fops = {
    .owner   = THIS_MODULE,
    .open    = nvt_xxx_proc_cmd_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release,
    .write   = nvt_xxx_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_xxx_proc_help_show(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "=====================================================================\n");
...
static int nvt_xxx_proc_help_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvt_xxx_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
    .owner  = THIS_MODULE,
    .open   = nvt_xxx_proc_help_open,
    .release = single_release,
    .read   = seq_read,
    .llseek = seq_lseek,
};

int nvt_sif_proc_init(PSIF_DRV_INFO pdrv_info)
{
    int ret = 0;
    struct proc_dir_entry *pmodule_root = NULL;
    struct proc_dir_entry *pentry = NULL;

	pmodule_root = proc_mkdir("sif", NULL);
...
int nvt_sif_proc_remove(PSIF_DRV_INFO pdrv_info)
{

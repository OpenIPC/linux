...
int nvt_csi_api_write_reg(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	...
	nvt_csi_drv_write_reg(pmodule_info, reg_addr, reg_value);
	return 0;
}

int nvt_csi_api_write_pattern(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	mm_segment_t old_fs;
	int fp;
	int len = 0;
...
int nvt_csi_api_read_reg(PMODULE_INFO pmodule_info, unsigned char argc, char **pargv)
{
	...
	value = nvt_csi_drv_read_reg(pmodule_info, reg_addr);

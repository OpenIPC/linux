...
#ifdef DEBUG
#if 0
extern unsigned int audio_debug_level;
#define nvt_dbg(level, fmt, args...)                \
do {                                               \
if (unlikely(NVT_DBG_##level <= audio_debug_level))    \
	pr_warn("%s:" fmt, __func__, ##args);   \
} while (0)
#else
extern unsigned int audlib_adpcm_debug_level;
#define nvt_dbg(level, fmt, args...)                \
do {                                               \
if (unlikely(NVT_DBG_##level <= audlib_adpcm_debug_level))    \
	pr_warn("%s:" fmt, __func__, ##args);   \
} while (0)
#endif
#else

...
static PROC_CMD cmd_write_list[] = {
    // keyword          function name
    { "reg",            nvt_ive_api_write_reg          },
    { "pattern",		nvt_ive_api_write_pattern		},
    { "kdrvtest",		nvt_kdrv_ipp_api_ive_test		}
};

#define NUM_OF_WRITE_CMD (sizeof(cmd_write_list) / sizeof(PROC_CMD))

static int nvt_ive_proc_cmd_show(struct seq_file *sfile, void *v)
{
	DBG_MSG("\n");
    return 0;
}

static int nvt_ive_proc_cmd_open(struct inode *inode, struct file *file)
{
	DBG_MSG("\n");
    return single_open(file, nvt_ive_proc_cmd_show, &pdrv_info_data->module_info);
}

static ssize_t nvt_ive_proc_cmd_write(struct file *file, const char __user *buf,
                                  size_t size, loff_t *off)
{
    ...
    if (len <= 0 || len > (MAX_CMD_LENGTH - 1)) {
        DBG_ERR("Command length %d error!\n", len);
        goto ERR_OUT;
    }
...
static struct file_operations proc_cmd_fops = {
    .owner   = THIS_MODULE,
    .open    = nvt_ive_proc_cmd_open,
    .read    = seq_read,
    .llseek  = seq_lseek,
    .release = single_release,
    .write   = nvt_ive_proc_cmd_write
};

//=============================================================================
// proc "help" file operation functions
//=============================================================================
static int nvt_ive_proc_help_show(struct seq_file *sfile, void *v)
{
    seq_printf(sfile, "=====================================================================\n");
    seq_printf(sfile, " Debug Level: %d\n", ive_drv_debug_level);
    seq_printf(sfile, "=====================================================================\n");
    return 0;
}

static int nvt_ive_proc_help_open(struct inode *inode, struct file *file)
{
    return single_open(file, nvt_ive_proc_help_show, NULL);
}

static struct file_operations proc_help_fops = {
    .owner  = THIS_MODULE,
    .open   = nvt_ive_proc_help_open,
    .release = single_release,
    .read   = seq_read,
    .llseek = seq_lseek,
};
...
int nvt_ive_proc_init(PIVE_DRV_INFO pdrv_info)
{
	...
	pmodule_root = proc_mkdir("kdrv_ive", NULL);

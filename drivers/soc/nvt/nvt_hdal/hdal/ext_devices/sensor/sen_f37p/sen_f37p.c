...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_f37p, 1, 42, 000, 00);

//=============================================================================
// information
//=============================================================================
#define SEN_F37P_MODULE_NAME     "sen_f37p"
...
#define SEN_I2C_COMPATIBLE "nvt,sen_f37p"
...
//=============================================================================
static CTL_SEN_DRV_TAB *sen_get_drv_tab_f37p(void);
static void sen_pwr_ctrl_f37p(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb);
static ER sen_open_f37p(CTL_SEN_ID id);
static ER sen_close_f37p(CTL_SEN_ID id);
static ER sen_sleep_f37p(CTL_SEN_ID id);
static ER sen_wakeup_f37p(CTL_SEN_ID id);
static ER sen_write_reg_f37p(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_read_reg_f37p(CTL_SEN_ID id, CTL_SEN_CMD *cmd);
static ER sen_chg_mode_f37p(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj);
static ER sen_chg_fps_f37p(CTL_SEN_ID id, UINT32 fps);
static ER sen_set_info_f37p(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static ER sen_get_info_f37p(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data);
static UINT32 sen_calc_chgmode_vd_f37p(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_calc_exp_vd_f37p(CTL_SEN_ID id, UINT32 fps);
static void sen_set_gain_f37p(CTL_SEN_ID id, void *param);
static void sen_set_expt_f37p(CTL_SEN_ID id, void *param);
static void sen_set_preset_f37p(CTL_SEN_ID id, ISP_SENSOR_PRESET_CTRL *ctrl);
static void sen_set_flip_f37p(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
static ER sen_get_flip_f37p(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
#if defined(__FREERTOS)
void sen_get_gain_f37p(CTL_SEN_ID id, void *param);
void sen_get_expt_f37p(CTL_SEN_ID id, void *param);
#else
static void sen_get_gain_f37p(CTL_SEN_ID id, void *param);
static void sen_get_expt_f37p(CTL_SEN_ID id, void *param);
#endif
static void sen_get_min_expt_f37p(CTL_SEN_ID id, void *param);
static void sen_get_mode_basic_f37p(CTL_SENDRV_GET_MODE_BASIC_PARAM *mode_basic);
static void sen_get_attr_basic_f37p(CTL_SENDRV_GET_ATTR_BASIC_PARAM *data);
static void sen_get_attr_signal_f37p(CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *data);
static ER sen_get_attr_cmdif_f37p(CTL_SEN_ID id, CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
static ER sen_get_attr_if_f37p(CTL_SENDRV_GET_ATTR_IF_PARAM *data);
static void sen_get_fps_f37p(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data);
static void sen_get_speed_f37p(CTL_SEN_ID id, CTL_SENDRV_GET_SPEED_PARAM *data);
static void sen_get_mode_mipi_f37p(CTL_SENDRV_GET_MODE_MIPI_PARAM *data);
static void sen_get_modesel_f37p(CTL_SENDRV_GET_MODESEL_PARAM *data);
static UINT32 sen_calc_rowtime_f37p(CTL_SEN_ID id, CTL_SEN_MODE mode);
static UINT32 sen_calc_rowtime_step_f37p(CTL_SEN_ID id, CTL_SEN_MODE mode);
static void sen_get_rowtime_f37p(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data);
static void sen_set_cur_fps_f37p(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_cur_fps_f37p(CTL_SEN_ID id);
static void sen_set_chgmode_fps_f37p(CTL_SEN_ID id, UINT32 fps);
static UINT32 sen_get_chgmode_fps_f37p(CTL_SEN_ID id);
//static void sen_get_iadj_f37p(CTL_SEN_ID id,  CTL_SENDRV_GET_MODE_MANUAL_IADJ *data);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD f37p_mode_1[] = {
	//[INI_Register]
	{0x12, 1, {0x40, 0x0}},
	{0x48, 1, {0x8A, 0x0}},
	{0x48, 1, {0x0A, 0x0}},
...
static CTL_SEN_DRV_TAB f37p_sen_drv_tab = {
	sen_open_f37p,
	sen_close_f37p,
	sen_sleep_f37p,
	sen_wakeup_f37p,
...
static CTL_SEN_CMD sen_set_cmd_info_f37p(UINT32 addr, UINT32 data_length, UINT32 data0, UINT32 data1)
{
	CTL_SEN_CMD cmd;
...
#if defined(__KERNEL__)
static void sen_load_cfg_from_compatible_f37p(struct device_node *of_node)
{
...
static ER sen_open_f37p(CTL_SEN_ID id)
{
	...
	#if defined(__KERNEL__)
	sen_i2c_reg_cb(sen_load_cfg_from_compatible_f37p);
	#endif
...
static UINT32 sen_get_cmd_tab_f37p(CTL_SEN_MODE mode, CTL_SEN_CMD **cmd_tab)
{
	switch (mode) {
	case CTL_SEN_MODE_1:
		*cmd_tab = f37p_mode_1;
		return sizeof(f37p_mode_1) / sizeof(CTL_SEN_CMD);
...
static ER sen_chg_mode_f37p(CTL_SEN_ID id, CTL_SENDRV_CHGMODE_OBJ chgmode_obj)
{
	...
	if (is_fastboot[id]) {
		...
		sen_set_chgmode_fps_f37p(id, isp_builtin_get_chgmode_fps(id));
		sen_set_cur_fps_f37p(id, isp_builtin_get_chgmode_fps(id));
		sen_set_gain_f37p(id, &sensor_ctrl);
		sen_set_expt_f37p(id, &sensor_ctrl);
		#endif
	...
	cmd_num = sen_get_cmd_tab_f37p(chgmode_obj.mode, &p_cmd_list);
	if (p_cmd_list == NULL) {
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_f37p(id, chgmode_obj.frame_rate);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_f37p(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_f37p(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
	...
	for (idx = 0; idx < cmd_num; idx++) {
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			cmd = sen_set_cmd_info_f37p(0x23, 1, (sensor_vd >> 8) & 0xFF, 0x00);
			rt |= sen_write_reg_f37p(id, &cmd);
			cmd = sen_set_cmd_info_f37p(0x22, 1, sensor_vd & 0xFF, 0x00);
			rt |= sen_write_reg_f37p(id, &cmd);
			...
			sen_set_flip_f37p(id, &flip);
		} else {
			cmd = sen_set_cmd_info_f37p(p_cmd_list[idx].addr, p_cmd_list[idx].data_len, p_cmd_list[idx].data[0], p_cmd_list[idx].data[1]);
			rt |= sen_write_reg_f37p(id, &cmd);
		}
	}
...
static ER sen_chg_fps_f37p(CTL_SEN_ID id, UINT32 fps)
{
	...
	if (basic_param.property & CTL_SEN_SUPPORT_PROPERTY_CHGFPS) {
		sensor_vd = sen_calc_chgmode_vd_f37p(id, fps);
	} else {
		DBG_WRN(" not support fps adjust \r\n");
		sen_set_cur_fps_f37p(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
		sen_set_chgmode_fps_f37p(id, mode_basic_param[cur_sen_mode[id]].dft_fps);
...
static ER sen_set_info_f37p(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_SET_EXPT:
		sen_set_expt_f37p(id, data);
		break;
	case CTL_SENDRV_CFGID_SET_GAIN:
		sen_set_gain_f37p(id, data);
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_set_flip_f37p(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE1:
		sen_set_preset_f37p(id, (ISP_SENSOR_PRESET_CTRL *)(data));
		break;
	default:
		return E_NOSPT;
	}
...
static ER sen_get_info_f37p(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_EXPT:
		sen_get_expt_f37p(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_GAIN:
		sen_get_gain_f37p(id, data);
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_BASIC:
		sen_get_attr_basic_f37p((CTL_SENDRV_GET_ATTR_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_SIGNAL:
		sen_get_attr_signal_f37p((CTL_SENDRV_GET_ATTR_SIGNAL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_f37p(id, (CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_ATTR_IF:
		rt = sen_get_attr_if_f37p((CTL_SENDRV_GET_ATTR_IF_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_FPS:
		sen_get_fps_f37p(id, (CTL_SENDRV_GET_FPS_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_SPEED:
		sen_get_speed_f37p(id, (CTL_SENDRV_GET_SPEED_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_BASIC:
		sen_get_mode_basic_f37p((CTL_SENDRV_GET_MODE_BASIC_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_MIPI:
		sen_get_mode_mipi_f37p((CTL_SENDRV_GET_MODE_MIPI_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODESEL:
		sen_get_modesel_f37p((CTL_SENDRV_GET_MODESEL_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		sen_get_rowtime_f37p(id, (CTL_SENDRV_GET_MODE_ROWTIME_PARAM *)(data));
		break;
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		rt = sen_get_flip_f37p(id, (CTL_SEN_FLIP *)(data));
		break;
	case CTL_SENDRV_CFGID_USER_DEFINE2:
		sen_get_min_expt_f37p(id, data);
		break;
/*		
	case CTL_SENDRV_CFGID_GET_MODE_MIPI_MANUAL_IADJ:
		sen_get_iadj_f37p(id, (CTL_SENDRV_GET_MODE_MANUAL_IADJ *)(data));
		break;		
*/
	default:
		  rt = E_NOSPT;
...
static UINT32 sen_calc_chgmode_vd_f37p(CTL_SEN_ID id, UINT32 fps)
{
	...
	sen_set_chgmode_fps_f37p(id, fps);
	sen_set_cur_fps_f37p(id, fps);
...
static void sen_set_gain_f37p(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		data1[frame_cnt] = (a_gain_h << 4) | a_gain_l ;
	}

	//Write Sensor reg
	cmd = sen_set_cmd_info_f37p(0x00, 1, data1[0], 0);
	rt |= sen_write_reg_f37p(id, &cmd);
...
static void sen_set_expt_f37p(CTL_SEN_ID id, void *param)
{
	...
	for (frame_cnt = 0; frame_cnt < total_frame; frame_cnt++) {
		...
		t_row = sen_calc_rowtime_f37p(id, cur_sen_mode[id]);
		if (0 == t_row) {
	...
	// Write exposure line
	// Get fps
	chgmode_fps = sen_get_chgmode_fps_f37p(id);
	...
	sen_set_cur_fps_f37p(id, cur_fps);

	// Calculate new vd
	sensor_vd = sen_calc_exp_vd_f37p(id, cur_fps);
	...
	cmd = sen_set_cmd_info_f37p(0x23, 1, (sensor_vd >> 8) & 0xFF, 0);
	rt |= sen_write_reg_f37p(id, &cmd);	
	cmd = sen_set_cmd_info_f37p(0x22, 1, sensor_vd & 0xFF, 0); 
	rt |= sen_write_reg_f37p(id, &cmd);
	...
	cmd = sen_set_cmd_info_f37p(0x02, 1, (line[0] >> 8) & 0xFF, 0);
	rt |= sen_write_reg_f37p(id, &cmd); 	
	cmd = sen_set_cmd_info_f37p(0x01, 1, (line[0] & 0xFF) , 0);
	rt |= sen_write_reg_f37p(id, &cmd);
...
static void sen_set_flip_f37p(CTL_SEN_ID id, CTL_SEN_FLIP *flip)
{
	...
	cmd = sen_set_cmd_info_f37p(0x12, 1, 0x0, 0x0);
	rt |= sen_read_reg_f37p(id, &cmd);
	...
	if (mode_basic_param[cur_sen_mode[id]].mode_type == CTL_SEN_MODE_LINEAR) {
		if (final_option & CTL_SEN_SUPPORT_PROPERTY_FLIP) {
			cmd = sen_set_cmd_info_f37p(0x28, 1, 0x19, 0);
			rt |= sen_write_reg_f37p(id, &cmd);
		} else {
			cmd = sen_set_cmd_info_f37p(0x28, 1, 0x11, 0);
			rt |= sen_write_reg_f37p(id, &cmd);
		}
	}
...
static void sen_get_fps_f37p(CTL_SEN_ID id, CTL_SENDRV_GET_FPS_PARAM *data)
{
	data->cur_fps = sen_get_cur_fps_f37p(id);
	data->chg_fps = sen_get_chgmode_fps_f37p(id);
}
...
static void sen_get_rowtime_f37p(CTL_SEN_ID id, CTL_SENDRV_GET_MODE_ROWTIME_PARAM *data)
{
	data->row_time_step = sen_calc_rowtime_step_f37p(id, data->mode);	
	data->row_time = sen_calc_rowtime_f37p(id, data->mode) * (data->row_time_step);	
}
...
#if defined(__FREERTOS)
void sen_get_i2c_id_f37p(CTL_SEN_ID id, UINT32 *i2c_id)
{
	*i2c_id = sen_i2c[id].id;
}

void sen_get_i2c_addr_f37p(CTL_SEN_ID id, UINT32 *i2c_addr)
{
	*i2c_addr = sen_i2c[id].addr;
}

int sen_init_f37p(SENSOR_DTSI_INFO *info)
{
	...
	sprintf(compatible, "nvt,sen_f37p");
	if (sen_common_check_compatible(compatible)) {
		...
		sprintf(node_path, "/sensor/sen_cfg/sen_f37p");
	...
	reg_obj.pwr_ctrl = sen_pwr_ctrl_f37p;
	reg_obj.det_plug_in = NULL;
	reg_obj.drv_tab = sen_get_drv_tab_f37p();
	rt = ctl_sen_reg_sendrv("nvt_sen_f37p", &reg_obj);
	if (rt != E_OK) {
...
void sen_exit_f37p(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_f37p");
}

#else
static int __init sen_init_f37p(void)
{
...
static void __exit sen_exit_f37p(void)
{
	ctl_sen_unreg_sendrv("nvt_sen_f37p");
}

module_init(sen_init_f37p);
module_exit(sen_exit_f37p);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION(SEN_F37P_MODULE_NAME);
MODULE_LICENSE("GPL");
#endif

...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_mtr_an41908, 1, 05, 000, 00);
...
//=============================================================================
static ER mtr_open_an41908(UINT32 id);
static ER mtr_close_an41908(UINT32 id);
static ER mtr_get_focus_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_focus_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_zoom_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_zoom_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_ircut_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_ircut_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_aperture_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_aperture_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_shutter_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_shutter_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_misc_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd);
static ER mtr_set_misc_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd);
...
#endif
static CTL_MTR_DRV_TAB mtr_drv_tab_an41908 = {
	mtr_open_an41908,
	mtr_close_an41908,
	mtr_get_focus_an41908,
	mtr_set_focus_an41908,
	mtr_get_zoom_an41908,
	mtr_set_zoom_an41908,
	mtr_get_ircut_an41908,
	mtr_set_ircut_an41908,
	mtr_get_aperture_an41908,
	mtr_set_aperture_an41908,
	mtr_get_shutter_an41908,
	mtr_set_shutter_an41908,
	mtr_get_misc_an41908,
	mtr_set_misc_an41908,
};

CTL_MTR_DRV_TAB *mtr_get_drv_tab(void)
{
	return &mtr_drv_tab_an41908;
}
...
static ER mtr_get_focus_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_FOCUS_SPEED:
		pcmd->data[0] = an41908_get_focus_speed(id);
		break;

	case MTR_GET_FOCUS_FOCAL_LENGTH:
		pcmd->data[0] = an41908_get_focus_focal_length(id, pcmd->argu[0]);
		break;

	case MTR_GET_FOCUS_RANGE:
		tmp = an41908_get_focus_range(id);
		pcmd->data[0] = tmp & 0xFFFF;
		pcmd->data[1] = (tmp >> 16) & 0xFFFF;
		break;

	case MTR_GET_FOCUS_POSITION:
		pcmd->data[0] = an41908_get_focus_position(id);
		break;

	case MTR_GET_FOCUS_BUSY_STATUS:
		pcmd->data[0] = (an41908_get_busy_status(id) >> 0) & 0x01;
		break;
...
#else
		an41908_init_focus(id);
#endif
		break;

	case MTR_SET_FOCUS_SPEED:
		an41908_set_focus_speed(id, pcmd->argu[0]);
		break;

	case MTR_SET_FOCUS_ASSIGN_POSITION:
		an41908_assign_focus_position(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_FOCUS_FOCAL_LENGTH:
		an41908_set_focus_focal_length(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_FOCUS_POSITION:
		pcmd->data[0] = an41908_set_focus_position(id, pcmd->argu[0], TRUE);
		break;
...
#else
		pcmd->data[0] = an41908_press_focus_move(id, (BOOL)pcmd->argu[0]);
#endif
		break;

	case MTR_SET_FOCUS_RELEASE:
		an41908_release_focus_move(id);
		break;
...
static ER mtr_get_zoom_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_ZOOM_SPEED:
		pcmd->data[0] = an41908_get_zoom_speed(id);
		break;

	case MTR_GET_ZOOM_SECTION_POS:
		pcmd->data[0] = an41908_get_zoom_section_pos(id);
		break;

	case MTR_GET_ZOOM_MAX_SECTION_POS:
		pcmd->data[0] = an41908_get_zoom_max_section_pos(id);
		break;

	case MTR_GET_ZOOM_POSITION:
		pcmd->data[0] = an41908_get_zoom_position(id);
		break;

	case MTR_GET_ZOOM_BUSY_STATUS:
		pcmd->data[0] = (an41908_get_busy_status(id) >> 1) & 0x01;
		break;
...
#else
		an41908_init_zoom(id);
#endif
		break;

	case MTR_SET_ZOOM_SPEED:
		an41908_set_zoom_speed(id, pcmd->argu[0]);
		break;

	case MTR_SET_ZOOM_ASSIGN_POSITION:
		an41908_assign_zoom_position(id, pcmd->argu[0], pcmd->argu[1]);
		break;
...
#else
		pcmd->data[0] = an41908_set_zoom_section_pos(id, pcmd->argu[0]);
#endif
		break;
...
#else
		pcmd->data[0] = an41908_set_zoom_position(id, pcmd->argu[0]);
#endif
		break;
...
#else
		pcmd->data[0] = an41908_press_zoom_move(id, (BOOL)pcmd->argu[0]);
#endif
		break;

	case MTR_SET_ZOOM_RELEASE:
		an41908_release_zoom_move(id);
		break;
...
static ER mtr_get_ircut_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_IRCUT_POSITION:
		pcmd->data[0] = an41908_get_ircut_state(id);
		break;
...
static ER mtr_set_ircut_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_SET_IRCUT_INIT:
		an41908_set_ircut_state(id, IRCUT_OPEN, 100);
		break;

	case MTR_SET_IRCUT_POSITION:
		an41908_set_ircut_state(id, pcmd->argu[0], pcmd->argu[1]);
		break;
...
static ER mtr_get_aperture_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_APERTURE_INFO:
		sprintf((CHAR *)pcmd->data, "DC");
		pcmd->data[1] = (1 << 16) + 1;    // min(bit0~15) / max(bit16~31)
		break;

	case MTR_GET_APERTURE_FNO:
		pcmd->data[0] = an41908_get_aperture_fno(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_GET_APERTURE_SECTION_POS:
		pcmd->data[0] = an41908_get_aperture_section_pos(id);
		break;

	case MTR_GET_APERTURE_POSITION:
		pcmd->data[0] = an41908_get_aperture_position(id);
		break;
...
static ER mtr_set_aperture_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_SET_APERTURE_SECTION_POS:
		an41908_set_aperture_section_pos(id, pcmd->argu[0]);
		break;

	case MTR_SET_APERTURE_POSITION:
		an41908_set_aperture_position(id, pcmd->argu[0]);
		break;
...
static ER mtr_get_shutter_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_SHUTTER_STATE:
		pcmd->data[0] = an41908_get_shutter_state(id);
		break;
...
static ER mtr_set_shutter_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_SET_SHUTTER_INIT:
		an41908_set_shutter_state(id, SHUTTER_OPEN, 50);
		break;

	case MTR_SET_SHUTTER_STATE:
		an41908_set_shutter_state(id, pcmd->argu[0], pcmd->argu[1]);
		break;
...
static ER mtr_get_misc_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd)
{
	...
	case MTR_GET_MISC_NAME_STR:
		sprintf((CHAR *)pcmd->data, "PANASONIC-AN41908");
		break;

	case MTR_GET_MISC_CAPABILITY:
		pcmd->data[0] = an41908_get_capability_info(id);
		break;

	case MTR_GET_MISC_ZF_RANGE_TABLE:
		pcmd->data[0] = an41908_get_zf_range_table(id, pcmd->argu[0], (LENS_ZOOM_FOCUS_TAB *)&pcmd->data[1]);
		break;

	case MTR_GET_MISC_ZF_CURVE_TABLE:
		pcmd->data[0] = an41908_get_zf_curve_table(id, pcmd->argu[0], (LENS_FOCUS_DISTANCE_TAB *)&pcmd->data[1]);
		break;

	case MTR_GET_MISC_LED_STATE:
		pcmd->data[0] = an41908_get_register(id, pcmd->argu[0]);
		break;

	case MTR_GET_MISC_FZ_BACKLASH:
		an41908_get_fz_backlash_value(id, (INT32 *)&pcmd->data[0], (INT32 *)&pcmd->data[1]);
		break;
...
static ER mtr_set_misc_an41908(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd)
{
	...
	case MTR_SET_MISC_ZF_RANGE_TABLE:
		pcmd->data[0] = an41908_set_zf_range_table(id, pcmd->argu[0], (LENS_ZOOM_FOCUS_TAB *)&pcmd->argu[1]);
		break;

	case MTR_SET_MISC_ZF_CURVE_TABLE:
		pcmd->data[0] = an41908_set_zf_curve_table(id, pcmd->argu[0], (LENS_FOCUS_DISTANCE_TAB *)&pcmd->argu[1]);
		break;

	case MTR_SET_MISC_LED_STATE:
		an41908_set_register(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_MISC_FZ_BACKLASH:
		an41908_set_fz_backlash_value(id, (INT32)pcmd->argu[0], (INT32)pcmd->argu[1]);
		break;
...
static void do_motion_work(void)
{
	...
	case MTR_ITEM_FOCUS_INIT:
		an41908_init_focus(mtr_mot_cmd.mtr_id);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_FOCUS_INIT;
		break;

	case MTR_ITEM_FOCUS_POSITION:
		mtr_mot_cmd.mtr_mot_arg.data[0] = an41908_set_focus_position(mtr_mot_cmd.mtr_id, mtr_mot_cmd.mtr_mot_arg.argu[0], TRUE);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_FOCUS_POSITION;
		break;

	case MTR_ITEM_FOCUS_PRESS:
		mtr_mot_cmd.mtr_mot_arg.data[0] = an41908_press_focus_move(mtr_mot_cmd.mtr_id, (BOOL)mtr_mot_cmd.mtr_mot_arg.argu[0]);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_FOCUS_PRESS;
		break;

	case MTR_ITEM_ZOOM_INIT:
		an41908_init_zoom(mtr_mot_cmd.mtr_id);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_ZOOM_INIT;
		break;

	case MTR_ITEM_ZOOM_SECTION_POS:
		mtr_mot_cmd.mtr_mot_arg.data[0] = an41908_set_zoom_section_pos(mtr_mot_cmd.mtr_id, mtr_mot_cmd.mtr_mot_arg.argu[0]);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_ZOOM_SECTION_POS;
		break;

	case MTR_ITEM_ZOOM_POSITION:
		mtr_mot_cmd.mtr_mot_arg.data[0] = an41908_set_zoom_position(mtr_mot_cmd.mtr_id, mtr_mot_cmd.mtr_mot_arg.argu[0]);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_ZOOM_POSITION;
		break;

	case MTR_ITEM_ZOOM_PRESS:
		mtr_mot_cmd.mtr_mot_arg.data[0] = an41908_press_zoom_move(mtr_mot_cmd.mtr_id, (BOOL)mtr_mot_cmd.mtr_mot_arg.argu[0]);
...
#ifdef __KERNEL__
static int __init mtr_init_an41908(void)
#else
int mtr_init_an41908(void)
#endif
{
	...
	if ((strstr(mtr_cfg_path, "null")) || (strstr(mtr_cfg_path, "NULL"))) {
		...
		strncpy(mtr_dtsi.node_path, "/motor/mtr_cfg/mtr_an41908", MTR_DTSI_NAME_LENGTH);
	...
	isp_dev_reg_motor_driver(&mtr_drv_tab_an41908);

	return rt;
}

#ifdef __KERNEL__
static void __exit mtr_exit_an41908(void)
#else
void mtr_exit_an41908(void)
#endif
{
	UINT32 i;
#ifdef __KERNEL__
...
#ifdef __KERNEL__
module_init(mtr_init_an41908);
module_exit(mtr_exit_an41908);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("Motor Driver AN41908");
MODULE_LICENSE("GPL");
#endif

...
//=============================================================================
#ifndef __KERNEL__
static UINT32 simple_guess_base(const char *cp)
{
	if (cp[0] == '0') {
...
INT32 simple_strtol(char *str, char **endp, UINT32 base)
{
	if (*str == '-') {
		return -simple_strtoul(str+1, endp, base);
	}

	return simple_strtoul(str, endp, base);
}

UINT32 simple_strtoul(char *str, char **endp, UINT32 base)
{
	return isp_strtoul(str, endp, base);
}
#endif

static INT32 mtr_cfg_is_delimeter(const INT8 c)
{
	switch (c) {
	case CHAR_RET:
	case CHAR_NULL:
...
static INT32 mtr_cfg_find_section_string(char *sec_name, CFG_FILE_FMT *cfile, char *line)
{
	char *s1, *s2;

	while (mtr_cfg_read_line(cfile, line, LINE_LEN) >= 0) {
...
static char *mtr_cfg_find_key_string(const char *key_name, CFG_FILE_FMT *cfile, char *line)
{
	char *s1, *s2;
	...
	while (mtr_cfg_read_line(cfile, line, LINE_LEN) >= 0) {
		...
		if (mtr_cfg_is_delimeter(s1[key_len]) == FALSE) {
...
// external functions
//=============================================================================
inline CFG_FILE_FMT *mtr_cfg_open(const INT8 *file_name, const INT32 flag)
{
	CFG_FILE_FMT *cfile;
...
void mtr_cfg_close(CFG_FILE_FMT *cfile)
{
	if (cfile != NULL) {
		do {
...
INT32 mtr_cfg_read_line(CFG_FILE_FMT *cfile, char *line, const INT32 size)
{
	char *p1, *p2;
	INT32 n;
#ifdef __KERNEL__
...
INT32 mtr_cfg_get_field_str(char *section, const char *key_name, INT8 *buf, INT32 size, CFG_FILE_FMT *cfile)
{
	...
	if (mtr_cfg_find_section_string(section, cfile, line) == FALSE) {
		return -1;  // section not found
	}

	key_str = mtr_cfg_find_key_string(key_name, cfile, line);
	if (key_str == NULL) {
	...
	do {
		...
		// read next line
		if (mtr_cfg_read_line(cfile, line, LINE_LEN) < 0) {
			break;
		}
...
INT32 mtr_cfg_str2tab_u8(INT8 *str, UINT8 *tab, INT32 count)
{
	...
	do {
		...
		if ((token != NULL) && (*token != CHAR_NULL)) {
			tab[i++] = (UINT8)(simple_strtoul(token, NULL, 0) & 0xFF);
		}
...
INT32 mtr_cfg_str2tab_s8(INT8 *str, INT8 *tab, INT32 count)
{
	char *string = (char *)str;
	char seps[] = ", ";
	char *token;
	...
	do {
		token = strsep(&string, seps);
		if ((token != NULL) && (*token != CHAR_NULL)) {
			tab[i++] = (INT8)(simple_strtol(token, NULL, 0) & 0xFF);
		}
	} while ((token != NULL) && (i < count));
...
INT32 mtr_cfg_str2tab_u16(INT8 *str, UINT16 *tab, INT32 count)
{
	char *string = (char *)str;
	char seps[] = ", ";
	char *token;
	...
	do {
		token = strsep(&string, seps);
		if ((token != NULL) && (*token != CHAR_NULL)) {
			tab[i++] = (UINT16)(simple_strtoul(token, NULL, 0) & 0xFFFF);
		}
	} while ((token != NULL) && (i < count));
...
INT32 mtr_cfg_str2tab_s16(INT8 *str, INT16 *tab, INT32 count)
{
	char *string = (char *)str;
	char seps[] = ", ";
	char *token;
	...
	do {
		token = strsep(&string, seps);
		if ((token != NULL) && (*token != CHAR_NULL)) {
			tab[i++] = (INT16)(simple_strtol(token, NULL, 0) & 0xFFFF);
		}
	} while ((token != NULL) && (i < count));
...
INT32 mtr_cfg_str2tab_u32(INT8 *str, UINT32 *tab, INT32 count)
{
	char *string = (char *)str;
	char seps[] = ", ";
	char *token;
	...
	do {
		token = strsep(&string, seps);
		if ((token != NULL) && (*token != CHAR_NULL)) {
			tab[i++] = simple_strtoul(token, NULL, 0);
		}
	} while ((token != NULL) && (i < count));
...
INT32 mtr_cfg_str2tab_s32(INT8 *str, INT32 *tab, INT32 count)
{
	char *string = (char *)str;
	char seps[] = ", ";
	char *token;
	...
	do {
		token = strsep(&string, seps);
		if ((token != NULL) && (*token != CHAR_NULL)) {
			tab[i++] = simple_strtol(token, NULL, 0);
		}
	} while ((token != NULL) && (i < count));

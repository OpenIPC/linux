...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_mtr_ms41949, 1, 02, 000, 00);
...
//=============================================================================
static ER mtr_open_ms41949(UINT32 id);
static ER mtr_close_ms41949(UINT32 id);
static ER mtr_get_focus_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_focus_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_zoom_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_zoom_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_ircut_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_ircut_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_aperture_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_aperture_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_shutter_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_set_shutter_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd);
static ER mtr_get_misc_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd);
static ER mtr_set_misc_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd);
...
#endif
static CTL_MTR_DRV_TAB mtr_drv_tab_ms41949 = {
	mtr_open_ms41949,
	mtr_close_ms41949,
	mtr_get_focus_ms41949,
	mtr_set_focus_ms41949,
	mtr_get_zoom_ms41949,
	mtr_set_zoom_ms41949,
	mtr_get_ircut_ms41949,
	mtr_set_ircut_ms41949,
	mtr_get_aperture_ms41949,
	mtr_set_aperture_ms41949,
	mtr_get_shutter_ms41949,
	mtr_set_shutter_ms41949,
	mtr_get_misc_ms41949,
	mtr_set_misc_ms41949,
};

CTL_MTR_DRV_TAB *mtr_get_drv_tab(void)
{
	return &mtr_drv_tab_ms41949;
}
...
static ER mtr_get_focus_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_FOCUS_SPEED:
		pcmd->data[0] = ms41949_get_focus_speed(id);
		break;

	case MTR_GET_FOCUS_FOCAL_LENGTH:
		pcmd->data[0] = ms41949_get_focus_focal_length(id, pcmd->argu[0]);
		break;

	case MTR_GET_FOCUS_RANGE:
		tmp = ms41949_get_focus_range(id);
		pcmd->data[0] = tmp & 0xFFFF;
		pcmd->data[1] = (tmp >> 16) & 0xFFFF;
		break;

	case MTR_GET_FOCUS_POSITION:
		pcmd->data[0] = ms41949_get_focus_position(id);
		break;

	case MTR_GET_FOCUS_BUSY_STATUS:
		pcmd->data[0] = (ms41949_get_busy_status(id) >> 0) & 0x01;
		break;
...
#else
		ms41949_init_focus(id);
#endif
		break;

	case MTR_SET_FOCUS_SPEED:
		ms41949_set_focus_speed(id, pcmd->argu[0]);
		break;

	case MTR_SET_FOCUS_ASSIGN_POSITION:
		ms41949_assign_focus_position(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_FOCUS_FOCAL_LENGTH:
		ms41949_set_focus_focal_length(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_FOCUS_POSITION:
		pcmd->data[0] = ms41949_set_focus_position(id, pcmd->argu[0], TRUE);
		break;
...
#else
		pcmd->data[0] = ms41949_press_focus_move(id, (BOOL)pcmd->argu[0]);
#endif
		break;

	case MTR_SET_FOCUS_RELEASE:
		ms41949_release_focus_move(id);
		break;
...
static ER mtr_get_zoom_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_ZOOM_SPEED:
		pcmd->data[0] = ms41949_get_zoom_speed(id);
		break;

	case MTR_GET_ZOOM_SECTION_POS:
		pcmd->data[0] = ms41949_get_zoom_section_pos(id);
		break;

	case MTR_GET_ZOOM_MAX_SECTION_POS:
		pcmd->data[0] = ms41949_get_zoom_max_section_pos(id);
		break;

	case MTR_GET_ZOOM_POSITION:
		pcmd->data[0] = ms41949_get_zoom_position(id);
		break;

	case MTR_GET_ZOOM_BUSY_STATUS:
		pcmd->data[0] = (ms41949_get_busy_status(id) >> 1) & 0x01;
		break;
...
#else
		ms41949_init_zoom(id);
#endif
		break;

	case MTR_SET_ZOOM_SPEED:
		ms41949_set_zoom_speed(id, pcmd->argu[0]);
		break;

	case MTR_SET_ZOOM_ASSIGN_POSITION:
		ms41949_assign_zoom_position(id, pcmd->argu[0], pcmd->argu[1]);
		break;
...
#else
		pcmd->data[0] = ms41949_set_zoom_section_pos(id, pcmd->argu[0]);
#endif
		break;
...
#else
		pcmd->data[0] = ms41949_set_zoom_position(id, pcmd->argu[0]);
#endif
		break;
...
#else
		pcmd->data[0] = ms41949_press_zoom_move(id, (BOOL)pcmd->argu[0]);
#endif
		break;

	case MTR_SET_ZOOM_RELEASE:
		ms41949_release_zoom_move(id);
		break;
...
static ER mtr_get_ircut_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_IRCUT_POSITION:
		pcmd->data[0] = ms41949_get_ircut_state(id);
		break;
...
static ER mtr_set_ircut_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_SET_IRCUT_INIT:
		ms41949_set_ircut_state(id, IRCUT_OPEN, 100);
		break;

	case MTR_SET_IRCUT_POSITION:
		ms41949_set_ircut_state(id, pcmd->argu[0], pcmd->argu[1]);
		break;
...
static ER mtr_get_aperture_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_APERTURE_INFO:
		sprintf((CHAR *)pcmd->data, "P");
		pcmd->data[1] = (200 << 16) + 1;    // min(bit0~15) / max(bit16~31)
		break;

	case MTR_GET_APERTURE_FNO:
		pcmd->data[0] = ms41949_get_aperture_fno(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_GET_APERTURE_SECTION_POS:
		pcmd->data[0] = ms41949_get_aperture_section_pos(id);
		break;

	case MTR_GET_APERTURE_POSITION:
		pcmd->data[0] = ms41949_get_aperture_position(id);
		break;

	case MTR_GET_APERTURE_FLUX_RATIO:
		pcmd->data[0] = ms41949_get_aperture_flux_ratio(id);
		break;

	case MTR_GET_APERTURE_BUSY_STATUS:
		pcmd->data[0] = (ms41949_get_busy_status(id) >> 2) & 0x01;
		break;
...
static ER mtr_set_aperture_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_SET_APERTURE_INIT:
		ms41949_init_aperture(id);
		break;

	case MTR_SET_APERTURE_ASSIGN_POSITION:
		ms41949_assign_aperture_position(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_APERTURE_SECTION_POS:
		ms41949_set_aperture_section_pos(id, pcmd->argu[0]);
		break;

	case MTR_SET_APERTURE_POSITION:
		ms41949_set_aperture_position(id, pcmd->argu[0]);
		break;

	case MTR_SET_APERTURE_FLUX_RATIO:
		ms41949_set_aperture_flux_ratio(id, pcmd->argu[0]);
		break;
...
static ER mtr_get_shutter_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_GET_SHUTTER_STATE:
		pcmd->data[0] = ms41949_get_shutter_state(id);
		break;
...
static ER mtr_set_shutter_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_CMD *pcmd)
{
	...
	case MTR_SET_SHUTTER_INIT:
		ms41949_set_shutter_state(id, SHUTTER_OPEN, 50);
		break;

	case MTR_SET_SHUTTER_STATE:
		ms41949_set_shutter_state(id, pcmd->argu[0], pcmd->argu[1]);
		break;
...
static ER mtr_get_misc_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd)
{
	...
	case MTR_GET_MISC_NAME_STR:
		sprintf((CHAR *)pcmd->data, "RELMON-MS41949");
		break;

	case MTR_GET_MISC_CAPABILITY:
		pcmd->data[0] = ms41949_get_capability_info(id);
		break;

	case MTR_GET_MISC_ZF_RANGE_TABLE:
		pcmd->data[0] = ms41949_get_zf_range_table(id, pcmd->argu[0], (LENS_ZOOM_FOCUS_TAB *)&pcmd->data[1]);
		break;

	case MTR_GET_MISC_ZF_CURVE_TABLE:
		pcmd->data[0] = ms41949_get_zf_curve_table(id, pcmd->argu[0], (LENS_FOCUS_DISTANCE_TAB *)&pcmd->data[1]);
		break;

	case MTR_GET_MISC_LED_STATE:
		pcmd->data[0] = ms41949_get_register(id, pcmd->argu[0]);
		break;

	case MTR_GET_MISC_FZ_BACKLASH:
		ms41949_get_fz_backlash_value(id, (INT32 *)&pcmd->data[0], (INT32 *)&pcmd->data[1]);
		break;
...
static ER mtr_set_misc_ms41949(UINT32 id, UINT32 cmd_type, MTR_CTL_LCMD *pcmd)
{
	...
	case MTR_SET_MISC_ZF_RANGE_TABLE:
		pcmd->data[0] = ms41949_set_zf_range_table(id, pcmd->argu[0], (LENS_ZOOM_FOCUS_TAB *)&pcmd->argu[1]);
		break;

	case MTR_SET_MISC_ZF_CURVE_TABLE:
		pcmd->data[0] = ms41949_set_zf_curve_table(id, pcmd->argu[0], (LENS_FOCUS_DISTANCE_TAB *)&pcmd->argu[1]);
		break;

	case MTR_SET_MISC_LED_STATE:
		ms41949_set_register(id, pcmd->argu[0], pcmd->argu[1]);
		break;

	case MTR_SET_MISC_FZ_BACKLASH:
		ms41949_set_fz_backlash_value(id, (INT32)pcmd->argu[0], (INT32)pcmd->argu[1]);
		break;
...
static void do_motion_work(void)
{
	...
	case MTR_ITEM_FOCUS_INIT:
		ms41949_init_focus(mtr_mot_cmd.mtr_id);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_FOCUS_INIT;
		break;

	case MTR_ITEM_FOCUS_POSITION:
		mtr_mot_cmd.mtr_mot_arg.data[0] = ms41949_set_focus_position(mtr_mot_cmd.mtr_id, mtr_mot_cmd.mtr_mot_arg.argu[0], TRUE);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_FOCUS_POSITION;
		break;

	case MTR_ITEM_FOCUS_PRESS:
		mtr_mot_cmd.mtr_mot_arg.data[0] = ms41949_press_focus_move(mtr_mot_cmd.mtr_id, (BOOL)mtr_mot_cmd.mtr_mot_arg.argu[0]);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_FOCUS_PRESS;
		break;

	case MTR_ITEM_ZOOM_INIT:
		ms41949_init_zoom(mtr_mot_cmd.mtr_id);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_ZOOM_INIT;
		break;

	case MTR_ITEM_ZOOM_SECTION_POS:
		mtr_mot_cmd.mtr_mot_arg.data[0] = ms41949_set_zoom_section_pos(mtr_mot_cmd.mtr_id, mtr_mot_cmd.mtr_mot_arg.argu[0]);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_ZOOM_SECTION_POS;
		break;

	case MTR_ITEM_ZOOM_POSITION:
		mtr_mot_cmd.mtr_mot_arg.data[0] = ms41949_set_zoom_position(mtr_mot_cmd.mtr_id, mtr_mot_cmd.mtr_mot_arg.argu[0]);
		mtr_mot_cmd.mtr_mot_arg.data[1] = MTR_ITEM_ZOOM_POSITION;
		break;

	case MTR_ITEM_ZOOM_PRESS:
		mtr_mot_cmd.mtr_mot_arg.data[0] = ms41949_press_zoom_move(mtr_mot_cmd.mtr_id, (BOOL)mtr_mot_cmd.mtr_mot_arg.argu[0]);
...
static INT mtr_spi_probe(struct spi_device *spi)
{
	UINT32 spi_mode;
	MOTOR_DRV_INFO *pmotor_info = mdrv_get_motor_info(0);

	pmotor_info->pspi_device = spi;

	if (spi->dev.of_node) {
		mtr_common_load_dtsi(spi->dev.of_node, (void *)pmotor_info);

		pmotor_info->pspi_device->master->bus_num = 1;    // must be the same with dts nvt-devname
		pmotor_info->pspi_device->master->max_dma_len = 16;

		pmotor_info->pspi_device->max_speed_hz = (pmotor_info->lens_hw.spi_cfg.spi_freq <= 20000000) ? pmotor_info->lens_hw.spi_cfg.spi_freq : 20000000;
		pmotor_info->pspi_device->chip_select = 0;
		pmotor_info->pspi_device->bits_per_word = 8;

		spi_mode = pmotor_info->lens_hw.spi_cfg.spi_mode & 0x03;
		if (pmotor_info->lens_hw.spi_cfg.cs_polarity != 0) {
			spi_mode |= SPI_CS_HIGH;
		}
		if (pmotor_info->lens_hw.spi_cfg.spi_order != 0) {
			spi_mode |= SPI_LSB_FIRST;
		}
		pmotor_info->pspi_device->mode = spi_mode;

		if (spi_setup(pmotor_info->pspi_device)) {
			DBG_WRN("fail to spi_setup!\n");
		}
	}
...
static const struct of_device_id mtr_spi_dt_ids[] = {
	{ .compatible = "nvt,mtr_ms41949", },
	{ },
};
MODULE_DEVICE_TABLE(of, mtr_spi_dt_ids);

static struct spi_driver motor_spi_driver = {
	.driver = {
		.owner = THIS_MODULE,
		.name = "mtr_drv",
		.of_match_table = of_match_ptr(mtr_spi_dt_ids),
	},
...
#ifdef __KERNEL__
static int __init mtr_init_ms41949(void)
#else
int mtr_init_ms41949(void)
#endif
{
	...
	if ((strstr(mtr_cfg_path, "null")) || (strstr(mtr_cfg_path, "NULL"))) {
		...
		strncpy(mtr_dtsi.node_path, "/motor/mtr_cfg/mtr_ms41949", MTR_DTSI_NAME_LENGTH);
...
#ifdef __KERNEL__
	#if !USE_GPIO_SIMULATION_SPI
	if (spi_register_driver(&motor_spi_driver) < 0) {
		rt = -EFAULT;

		DBG_ERR("register SPI driver fail!\r\n");
	}
	#endif
#endif
	...
	isp_dev_reg_motor_driver(&mtr_drv_tab_ms41949);

	return rt;
}

#ifdef __KERNEL__
static void __exit mtr_exit_ms41949(void)
#else
void mtr_exit_ms41949(void)
#endif
{
	UINT32 i;
#ifdef __KERNEL__
	...
	if (pmotor_info->pspi_device != NULL) {
		spi_unregister_device(pmotor_info->pspi_device);
		pmotor_info->pspi_device = NULL;
	}
	#endif
#endif
}
...
#ifdef __KERNEL__
module_init(mtr_init_ms41949);
module_exit(mtr_exit_ms41949);

MODULE_AUTHOR("Novatek Corp.");
MODULE_DESCRIPTION("Motor Driver MS41949");
MODULE_LICENSE("GPL");
#endif

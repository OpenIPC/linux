/**
	...
	@file video_liveview_with_net.c
...
{
	VIDEO_LIVEVIEW stream[3] = {0}; //0: main stream //1: sub stream (vout) //2: sub2 stream (net)
	...
	// init mem
	{
		...
		i = proc_mem_config(stream[1].out_path, &mem_cfg, &stream[1].proc_max_dim, i);
		
		i = network_mem_config(stream[2].net_path, &mem_cfg, &net_cfg, i);
	...
	// set open config
	ret = network_set_config(stream[2].net_path, &net_cfg);
	if (HD_OK != ret) {
		printf("proc_id(%u) network_set_config fail=%d\n", stream[2].net_path, ret);
		goto exit;
	}
	...
	// start video_liveview modules (sub2)
	hd_videoproc_start(stream[2].proc_path);
	if (ai_async == 2)
		network_async_start(&stream[2]);
	else
		network_sync_start(&stream[2]);
	...
	// stop video_liveview modules (sub2)
	hd_videoproc_stop(stream[2].proc_path);
	if (ai_async == 2)
		network_async_stop(&stream[2]);
	else
		network_sync_stop(&stream[2]);

/**
	@brief Sample code of video record with frc.\n

	@file video_record_with_frc.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_frc, argc, argv)
...
static void *encode_thread(void *arg)
{
	...
	while (p_stream0->enc_exit == 0) {
		SEM_WAIT(p_stream0->sem_bs_section);  // venc stop-start MUST make sure previous pulled bs already released, so need critical section to protect !!
		//pull data
		ret = hd_videoenc_pull_out_buf(p_stream0->enc_path, &data_pull, 300); // >0 = timeout mode
		...
		SEM_SIGNAL(p_stream0->sem_bs_section);
	}
...
MAIN(argc, argv)
{
	...
	#define DEF_FRC 8
	#define MAX_FRC 20
	UINT32 src[MAX_FRC] = {-1,0,1,5,10,15,20,25,30,60,120,240,480,960,1920,3840,7680,15360,30720,61440};
	UINT32 dest[MAX_FRC] = {-1,0,1,5,10,15,20,25,30,60,120,240,480,960,1920,3840,7680,15360,30720,61440};
	UINT32 vp_in_src_fr = DEF_FRC;
	UINT32 vp_in_dest_fr = DEF_FRC;
	UINT32 vp_out_src_fr = DEF_FRC;
	UINT32 vp_out_dest_fr = DEF_FRC;
	UINT32 ve_in_src_fr = DEF_FRC;
	UINT32 ve_in_dest_fr = DEF_FRC;
	...
	// create sem_bs_section (critical section to protect pull_bs & flow control)
	SEM_CREATE(stream[0].sem_bs_section, 1);
...

	{
		...
		printf("*** set videoproc.in frc = (%d/%d) ***\r\n", dest[vp_in_dest_fr], src[vp_in_src_fr]);
		video_in_param.frc = HD_VIDEO_FRC_RATIO(dest[vp_in_dest_fr], src[vp_in_src_fr]);
		hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_IN_FRC, &video_in_param);
		hd_videoproc_start(stream[0].proc_path);
	}
	{
		HD_VIDEOPROC_FRC video_out_param = {0};
		printf("*** set videoproc.out frc = (%d/%d) ***\r\n", dest[vp_out_dest_fr], src[vp_out_src_fr]);
		video_out_param.frc = HD_VIDEO_FRC_RATIO(dest[vp_out_dest_fr], src[vp_out_src_fr]);
		hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_OUT_FRC, &video_out_param);
		hd_videoproc_start(stream[0].proc_path);
	}
	{
		HD_VIDEOENC_FRC video_in_param = {0};
		printf("*** set videoenc.in frc = (%d/%d) ***\r\n", dest[ve_in_dest_fr], src[ve_in_src_fr]);
		video_in_param.frc = HD_VIDEO_FRC_RATIO(dest[ve_in_dest_fr], src[ve_in_src_fr]);
		hd_videoenc_set(stream[0].enc_path, HD_VIDEOENC_PARAM_IN_FRC, &video_in_param);
	...
	printf("Enter r/t to +/- frc dest of videoproc.in\n");
	printf("Enter y/u to +/- frc src of videoproc.in\n");
	printf("Enter f/g to +/- frc dest of videoproc.out\n");
	printf("Enter h/j to +/- frc src of videoproc.out\n");
	printf("Enter v/b to +/- frc dest of videoenc.in\n");
	printf("Enter n/m to +/- frc src of videoenc.in\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
		...
		if (key == 'v') {
			HD_VIDEOENC_FRC video_in_param = {0};
			if(ve_in_dest_fr < MAX_FRC) {
				// add frame rate
				ve_in_dest_fr += 1;
				printf("*** set videoenc.in frc = (%d/%d) ***\r\n", dest[ve_in_dest_fr], src[ve_in_src_fr]);
				video_in_param.frc = HD_VIDEO_FRC_RATIO(dest[ve_in_dest_fr], src[ve_in_src_fr]);

				SEM_WAIT(stream[0].sem_bs_section);  // venc stop-start MUST make sure previous pulled bs already released, so need critical section to protect !!
				if (((dest[ve_in_dest_fr-1] <= src[ve_in_src_fr]) && (dest[ve_in_dest_fr]  > src[ve_in_src_fr])) ||
				    ((dest[ve_in_dest_fr-1]  > src[ve_in_src_fr]) && (dest[ve_in_dest_fr] <= src[ve_in_src_fr]))) {
					// change between (dst <= src) <---> (dst > src) ,  need stop->set->start
					hd_videoenc_stop(stream[0].enc_path);
				}
				else if ((dest[ve_in_dest_fr-1]  > src[ve_in_src_fr]) && (dest[ve_in_dest_fr]  > src[ve_in_src_fr])) {
					// change between (dst > src) <---> (dst > src) ,  need stop->set->start
					hd_videoenc_stop(stream[0].enc_path);
				}
				hd_videoenc_set(stream[0].enc_path, HD_VIDEOENC_PARAM_IN_FRC, &video_in_param);
				hd_videoenc_start(stream[0].enc_path);
				SEM_SIGNAL(stream[0].sem_bs_section);
			}
		}
		...
		if (key == 'n') {
			...
			if(ve_in_src_fr < MAX_FRC) {
				...
				SEM_WAIT(stream[0].sem_bs_section);  // venc stop-start MUST make sure previous pulled bs already released, so need critical section to protect !!
				if (((dest[ve_in_dest_fr] <= src[ve_in_src_fr-1]) && (dest[ve_in_dest_fr]  > src[ve_in_src_fr])) ||
				    ((dest[ve_in_dest_fr]  > src[ve_in_src_fr-1]) && (dest[ve_in_dest_fr] <= src[ve_in_src_fr]))) {
					// change between (dst <= src) <---> (dst > src) ,  need stop->set->start
					hd_videoenc_stop(stream[0].enc_path);
				}
				else if ((dest[ve_in_dest_fr]  > src[ve_in_src_fr-1]) && (dest[ve_in_dest_fr]  > src[ve_in_src_fr])) {
					// change between (dst > src) <---> (dst > src) ,  need stop->set->start
					hd_videoenc_stop(stream[0].enc_path);
				}
				hd_videoenc_set(stream[0].enc_path, HD_VIDEOENC_PARAM_IN_FRC, &video_in_param);
	...
	// destroy sem_bs_section
	SEM_DESTROY(stream[0].sem_bs_section);

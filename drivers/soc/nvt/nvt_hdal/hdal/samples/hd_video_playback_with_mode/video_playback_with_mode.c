/**
	@brief Sample code of video playback with mode.\n

	@file video_playback_with_mode.c
...
#define VDO_SIZE_W          640          // video pattern width
#define VDO_SIZE_H          480          // video pattern height
...
static UINT32 g_decbind = 0;  //0x1: vdec bind vprc, 0xff: vdec no-bind vprc
static UINT32 g_prcbind = 0;    //0x1: vprc bind vout, 0xff: vprc no-bind vout
///////////////////////////////////////////////////////////////////////////////
...
typedef struct _VIDEO_PLAYBACK {
	...
	// (6) only used when (g_prcbind = 0xff)  //no-bind mode
...
static void *output_thread(void *arg)
{
	...
	p_stream0->out_exit = 0;
	p_stream0->out_loop = 0;
	p_stream0->out_count = 0;
	//------ wait flow_start ------
	while (p_stream0->out_enter == 0) usleep(100);

	//--------- pull data test ---------
	while (p_stream0->out_exit == 0) {
...
EXAMFUNC_ENTRY(hd_video_playback_with_mode, argc, argv)
{
	HD_RESULT ret;
    INT key;
	...
	if (argc == 1 || argc > 4) {
		printf("Usage: <codec-type> <vdec_vprc_bind> <vprc_vout_bind>.\r\n");
		printf("Help:\r\n");
		printf("  <out_type>    : 0(H265), 1(H264), 2(JPEG)\r\n");
		printf("  <vdec_vprc_bind>: 0(D2D w bind), x(no-bind)\r\n");
		printf("  <vprc_vout_bind>: 0(D2D w bind), x(no-bind)\r\n");
		return 0;
	}
	...
	if (argc > 2) {
		if (argv[2][0] == 'x') {
			g_decbind = 0xff;
			printf("VDEC-VPRC NOBIND\r\n");
		} else {
			g_decbind = atoi(argv[2]);
			printf("VDEC-VPRC BIND\r\n");
		}
	}
	...
	if (argc > 3) {
		if (argv[3][0] == 'x') {
			g_prcbind = 0xff;
			printf("VPRC-VOUT NOBIND\r\n");
		} else {
			g_prcbind = atoi(argv[2]);
			printf("VPRC-VOUT BIND\r\n");
		}
	}
	...
	if (g_prcbind == 0xff) {
		// create decode_thread (pull_out frame)
		ret = pthread_create(&stream[0].out_thread_id, NULL, output_thread, (void *)stream);
		if (ret < 0) {
			printf("create output_thread failed\n");
			goto exit;
		}
	}
	...
	if (g_prcbind == 0xff) {
		pthread_join(stream[0].out_thread_id, NULL);
	}

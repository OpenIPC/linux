/**
	@brief Sample code of video liveview with frc.\n

	@file video_liveview_with_frc.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_liveview_with_frc, argc, argv)
...
MAIN(argc, argv)
{
	...
	while (1) {
		...
		if (key == 'c') {
			...
			printf("*** set sensor fps = %d ***\r\n", sen_fps);
			fps = HD_VIDEO_FRC_RATIO(sen_fps,1);

			ret = vendor_videocap_set(stream[0].cap_ctrl, VENDOR_VIDEOCAP_PARAM_SET_FPS, &fps);
			if (ret != HD_OK) {
				printf("VENDOR_VIDEOCAP_PARAM_SET_FPS failed!(%d)\r\n", ret);
			}
		}
		if (key == 'v') {
			UINT32 fps;

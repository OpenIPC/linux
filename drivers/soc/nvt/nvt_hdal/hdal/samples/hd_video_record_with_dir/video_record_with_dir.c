...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_dir, argc, argv)
...
static HD_RESULT mem_init(void)
{
	...
	if (g_is_JPEG == 1) {
		// align to 16x16 for JPEG rotate
		mem_cfg.pool_info[1].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(ALIGN_CEIL_16(VDO_SIZE_W), ALIGN_CEIL_16(VDO_SIZE_H), HD_VIDEO_PXLFMT_YUV420);
...
MAIN(argc, argv)
{
	...
	printf("Enter w/x to mirror/flip of videocap.out\n");
	printf("Enter e/c to mirror/flip of videoproc.in\n");
	#if 0
	printf("Enter r/v to mirror/flip of videoproc.out\n");
	#endif
	printf("Enter t/b to rotate of videoenc.in\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
		...
		if (key == 'w') {
			HD_VIDEOCAP_OUT video_out_param = {0};
			{
				// next dir
				vc_out_dir ++;
				if(vc_out_dir > 3) vc_out_dir = 0;
				printf("*** set videocap.out dir = (%d) ***\r\n", vc_out_dir);
				hd_videocap_get(stream[0].cap_path, HD_VIDEOCAP_PARAM_OUT, &video_out_param);
				if(vc_out_dir == 0)
					video_out_param.dir = HD_VIDEO_DIR_NONE;
				else if(vc_out_dir == 1)
					video_out_param.dir = HD_VIDEO_DIR_MIRRORX;
				else if(vc_out_dir == 2)
					video_out_param.dir = HD_VIDEO_DIR_MIRRORY;
				else //if(vc_out_dir == 3)
					video_out_param.dir = HD_VIDEO_DIR_MIRRORX|HD_VIDEO_DIR_MIRRORY;
				hd_videocap_set(stream[0].cap_path, HD_VIDEOCAP_PARAM_OUT, &video_out_param);
				hd_videocap_start(stream[0].cap_path);
			}
		}
		...
		if (key == 'e') {
			HD_VIDEOPROC_IN video_in_param = {0};
			{
				// next dir
				vp_in_dir ++;
				if(vp_in_dir > 3) vp_in_dir = 0;
				printf("*** set videoproc.in dir = (%d) ***\r\n", vp_in_dir);
				hd_videoproc_get(stream[0].proc_path, HD_VIDEOPROC_PARAM_IN, &video_in_param);
				if(vp_in_dir == 0)
					video_in_param.dir = HD_VIDEO_DIR_NONE;
				else if(vp_in_dir == 1)
					video_in_param.dir = HD_VIDEO_DIR_MIRRORX;
				else if(vp_in_dir == 2)
					video_in_param.dir = HD_VIDEO_DIR_MIRRORY;
				else //if(vp_in_dir == 3)
					video_in_param.dir = HD_VIDEO_DIR_MIRRORX|HD_VIDEO_DIR_MIRRORY;
				hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_IN, &video_in_param);
				hd_videoproc_start(stream[0].proc_path);
			}
		}
		...
		if (key == 't') {
			HD_VIDEOENC_IN video_in_param = {0};
			{
				// next dir
				ve_in_dir ++;
				if(ve_in_dir > 3) ve_in_dir = 0;
				printf("*** set videoenc.in dir = (%d) ***\r\n", ve_in_dir);

                /* change HD_VIDEOENC_PARAM_IN: shall be follow  stop -> set -> start */
				...
				hd_videoenc_get(stream[0].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
				if(ve_in_dir == 0)
					video_in_param.dir = HD_VIDEO_DIR_NONE;
				else if(ve_in_dir == 1)
					video_in_param.dir = HD_VIDEO_DIR_ROTATE_90;
				else if(ve_in_dir == 2)
					video_in_param.dir = HD_VIDEO_DIR_ROTATE_180;
				else //if(ve_in_dir == 3)
					video_in_param.dir = HD_VIDEO_DIR_ROTATE_270;
				hd_videoenc_set(stream[0].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
				hd_videoenc_start(stream[0].enc_path);

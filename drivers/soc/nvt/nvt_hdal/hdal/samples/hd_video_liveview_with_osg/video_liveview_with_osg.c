...
static int set_stamp_param(HD_PATH_ID stamp_path, UINT32 stamp_pa, UINT32 stamp_size, unsigned short *novatek_logo)
{
...
static HD_RESULT open_module(VIDEO_LIVEVIEW *p_stream, HD_DIM* p_proc_max_dim, UINT32 out_type)
{
	...
	if((ret = hd_videoout_open(HD_VIDEOOUT_0_IN_0, HD_MASK_0, &p_stream->mask_path)) != HD_OK)
		return ret;
	//open a stamp in videoout
	if((ret = hd_videoout_open(HD_VIDEOOUT_0_IN_0, HD_STAMP_0, &p_stream->stamp_path)) != HD_OK)
		return ret;
...
static HD_RESULT close_module(VIDEO_LIVEVIEW *p_stream)
{
	...
	if((ret = hd_videoout_close(p_stream->mask_path)) != HD_OK)
		return ret;
	if((ret = hd_videoout_close(p_stream->stamp_path)) != HD_OK)
		return ret;
	return HD_OK;
}
...
static int init_novatek_logo(unsigned short *novatek_logo)
{
	...
	if(fd == -1){
		...
		printf("hd_video_record_osg.bin is in SDK/code/hdal/samples/pattern/video_frm_1000_200_1_argb4444.dat\n");
		return -1;
	}
...
MAIN(argc, argv)
{
	...
	//setup stamp parameter
	if(set_stamp_param(stream[0].stamp_path, stream[0].stamp_pa, stream[0].stamp_size, stream[0].novatek_logo)){
		printf("fail to set vo stamp\r\n");
		goto exit;
	}

	//render stamp
	ret = hd_videoout_start(stream[0].stamp_path);
	if (ret != HD_OK) {
		printf("start vo stamp fail=%d\n", ret);
		goto exit;
	}

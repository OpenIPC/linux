...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_mode, argc, argv)
...
#define FPGA_DIRECT_MODE  0 //vcap is forced to 960x540, Vprc output should be scaler down 960x540
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim)
{
	...
	#if(!FPGA_DIRECT_MODE) //no crop, full frame
	{
...
MAIN(argc, argv)
{
	...
	if (argc > 3) {
		...
		printf("CAP-FMT %d\r\n", g_capfmt);
		if(g_capfmt) {
			printf("WRN: VCAP0 not support compression!\r\n");
			g_capfmt = 0;
		}
	}
	if (argc > 4) {

/**
	...
	@note This file is modified from video_snapshot.c.
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_snapshot_with_vprc_extend, argc, argv)
...
static HD_RESULT set_enc_cfg(HD_PATH_ID video_enc_path, HD_DIM *p_max_dim, UINT32 enc_type, UINT32 max_bitrate)
{
	HD_RESULT ret = HD_OK;
...
static HD_RESULT open_module(VIDEO_RECORD *p_stream, HD_DIM* p_proc_max_dim)
{
	...
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_0, &p_stream->proc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_open(HD_VIDEOPROC_0_IN_0, HD_VIDEOPROC_0_OUT_5, &p_stream->proc_path_extend)) != HD_OK)
		return ret;
...
static HD_RESULT close_module(VIDEO_RECORD *p_stream)
{
	...
	if ((ret = hd_videoproc_close(p_stream->proc_path)) != HD_OK)
		return ret;
	if ((ret = hd_videoproc_close(p_stream->proc_path_extend)) != HD_OK)
		return ret;
...
static void *snapshot_thread(void *arg)
{
	...
	while (p_stream0->snap_exit == 0) {
		...
		if(p_stream0->do_snap) {
			...
			printf("proc_pull ....\r\n");
			ret = hd_videoproc_pull_out_buf(p_stream0->proc_path_extend, &video_frame, 0); // 0 = non-blocking mode
			if (ret != HD_OK) {
			...
			printf("proc_release ....\r\n");
			ret = hd_videoproc_release_out_buf(p_stream0->proc_path_extend, &video_frame);
			if (ret != HD_OK) {
			...
			snprintf(file_path_main, 32, "/mnt/sd/dump_bs_snap%lu_%ld.jpg", p_stream0->shot_count, p_stream0->ext_mode);
...
MAIN(argc, argv)
{
	...
	if (argc >= 2) {
		extend_out_mode = atoi(argv[1]);
		if(extend_out_mode >= 4) {
	...
	if (extend_out_mode == 0) {
		...
		// test extend output with rotate, scale
		sub_dim.w = 480;
		sub_dim.h = 640;
		sub_dir = HD_VIDEO_DIR_ROTATE_90;
	}
	...
	// set videoproc parameter (extend), this sample proc_path is not bind to enc_path, must config proc's out dim
	ret = set_proc_param_extend(stream[0].proc_path_extend, stream[0].proc_path, &sub_dim, sub_dir);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	...
	// set videoenc parameter (extend)
	stream[0].enc_dim.w = sub_dim.w;
	stream[0].enc_dim.h = sub_dim.h;
	...
	hd_videoproc_start(stream[0].proc_path_extend);
	...
	hd_videoproc_stop(stream[0].proc_path_extend);

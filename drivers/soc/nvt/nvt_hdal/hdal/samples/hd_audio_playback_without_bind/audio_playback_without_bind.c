...
#define FLG_TRIGGER_DEC    0x80000000
...
static HD_RESULT set_out_cfg(HD_PATH_ID *p_audio_out_ctrl)
{
	...
	audio_drv_cfg.output = HD_AUDIOOUT_OUTPUT_LINE;
...
static HD_RESULT set_out_param(HD_PATH_ID audio_out_ctrl, HD_PATH_ID audio_out_path, BOOL enable)
{
	HD_RESULT ret = HD_OK;
...
{
	HD_RESULT ret = HD_OK;
	CHAR filename[50] = {0}, codec_name[8] = {0};
	FILE *bs_fd, *len_fd;
	...
	vos_flag_set(p_stream0->play_flg_id, FLG_TRIGGER_DEC);
	...
	while (1) {
		...
		// wait for trigger
		vos_flag_wait(&uiFlag, p_stream0->play_flg_id, FLG_TRIGGER_DEC, TWF_ORW | TWF_CLR);

		p_stream0->dec_trigger = 0;
...
static void *decode_thread(void *arg)
{
	...
	// pull frame data
	while (p_stream0->play_exit == 0) {
		// pull data
		ret = hd_audiodec_pull_out_buf(p_stream0->dec_path, &data_pull, -1); // >1 = timeout mode

		if (ret == HD_OK) {

			ret = hd_audioout_push_in_buf(p_stream0->out_path, &data_pull, -1);
			if (ret != HD_OK) {
				printf("hd_audioout_push_in_buf fail, ret(%d)\n", ret);
			}


			vendor_audioout_get(p_stream0->out_ctrl, VENDOR_AUDIOOUT_ITEM_PRELOAD_DONE, (VOID *)&done);

			if (done != TRUE) {
				printf("Not preload done\n");
			}
...
EXAMFUNC_ENTRY(hd_audio_playback_without_bind, argc, argv)
{
    HD_RESULT ret;
    INT key;
	...
	vos_flag_create(&stream[0].play_flg_id, NULL, "play_flg");
	...
	if (argc == 2) {
		...
		dec_type = atoi(argv[1]);
		enable = atoi(argv[2]);
		printf("dec_type %d, preload enable %d\r\n", dec_type, enable);
		if(stream[0].dec_type > 2) {
			printf("error: not support dec_type %d\r\n", dec_type);
			return 0;
		}
    ...
    ret = set_out_param(stream[0].out_ctrl, stream[0].out_path, stream[0].enable);
	if (ret != HD_OK) {
	...
	ret = pthread_create(&stream[0].play_thread_id, NULL, decode_thread, (void *)stream);
	if (ret < 0) {
	...
	while (1) {
		...
		if (key == 'q' || key == 0x3) {
			...
			stream[0].play_exit = 1;

			stream[0].dec_trigger = 1;
			stream[0].dec_exit = 1;
			// quit program
			break;
		}
	...
	pthread_join(stream[0].play_thread_id, NULL);

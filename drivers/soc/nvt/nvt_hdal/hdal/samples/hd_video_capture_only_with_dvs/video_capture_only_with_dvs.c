...
#define DVS_DRAM_OUT_SIZE  (480*640)
///////////////////////////////////////////////////////////////////////////////
...
#define VDO_SIZE_W     7936
#define VDO_SIZE_H     20
...
static HD_RESULT mem_init(UINT32 *p_blk_size)
{
	...
	mem_cfg.pool_info[0].blk_size = DBGINFO_BUFSIZE()+DVS_DRAM_OUT_SIZE;
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim, HD_VIDEOCAP_SEN_FRAME_NUM frame_num)
{
	...
	{ // Set DVS info
		VENDOR_VIDEOCAP_DVS_INFO dvs_info = {0};

		dvs_info.enable = TRUE;
		dvs_info.positive = 0xFF;
		dvs_info.negative = 0x00;
		dvs_info.nochange = 0x80;
		ret = vendor_videocap_set(video_cap_path, VENDOR_VIDEOCAP_PARAM_DVS_INFO, &dvs_info);
	}
	return ret;
}
...
static HD_RESULT open_module(VIDEO_LIVEVIEW *p_stream)
{
	HD_RESULT ret;
	// set videocap config
	ret = set_cap_cfg(&p_stream->cap_ctrl, HD_VIDEOCAP_0_CTRL, p_stream->cap_pin_map, p_stream->cap_drv_name);
	if (ret != HD_OK) {
...
MAIN(argc, argv)
{
	...
	snprintf(stream[0].cap_drv_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_s5k231y");

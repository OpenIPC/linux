...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(alg_lme_sample_stream, argc, argv)
...
///////////////////////////////////////////////////////////////////////////////
static HD_COMMON_MEM_VB_BLK  g_dis_blk;
...
#define	VDO_FRAME_FORMAT	HD_VIDEO_PXLFMT_YUV420
 ...
 static HD_RESULT mem_init(void)
 {
	 ...
	 mem_cfg.pool_info[0].blk_size = DBGINFO_BUFSIZE()+VDO_NRX_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H)
	 ...
	 mem_cfg.pool_info[2].blk_size = nvt_dis_cal_buffer_size();
 ...
 static void share_memory_init(DIS_IPC_INIT *p_share_mem)
 {
	 ...
	 UINT32 		   pa, va;
	 UINT32 		   blk_size = nvt_dis_cal_buffer_size();
	 HD_COMMON_MEM_DDR_ID ddr_id = DDR_ID0;
	 HD_RESULT		   ret;

	 memset(p_share_mem, 0x00, sizeof(DIS_IPC_INIT));
	 g_dis_blk  = HD_COMMON_MEM_VB_INVALID_BLK;
 ...
 static HD_RESULT share_memory_exit(DIS_IPC_INIT *p_share_mem)
 {
	 ...
	 if (g_dis_blk != HD_COMMON_MEM_VB_INVALID_BLK) {
	 	ret = hd_common_mem_release_block(g_dis_blk);
		if (ret != HD_OK) {
...
static HD_RESULT set_cap_param(HD_PATH_ID video_cap_path, HD_DIM *p_dim)
 {
...
#else //HD_CROP_ON
	 {
		...
		video_crop_param.win.rect.x = 1920/4;
		video_crop_param.win.rect.y = 1080/4;
...
static VOID *dis_thread_api(VOID *arg)
{
	HD_RESULT ret = HD_OK;
	DIS_IPC_INIT   *share_mem = (DIS_IPC_INIT*)arg;

	printf(".........nvt_dis_init start...................\n\r");
...
MAIN(argc, argv)
{
	...
	share_memory_init(&share_mem);
	 ...
	 ret = pthread_create(&dis_thread_id[0], NULL, dis_thread_api, (VOID*)(&share_mem));
	 if (ret < 0) {
	 ...
	 pthread_join(dis_thread_id[0], NULL);

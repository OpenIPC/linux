/**
	@brief Sample code of video record with crop.\n

	@file video_record_with_crop.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_crop, argc, argv)
...
MAIN(argc, argv)
{
	...
	UINT32 enc_type = 0;
	HD_DIM main_dim;
 	HD_URECT cur_crop[5] = {{0,0,1920,1080},{0,0,960,540},{960,0,960,540},{0,540,960,540},{960,540,960,540}};
	UINT32 vc_out_crop_id = 0;
	UINT32 vp_in_crop_id = 0;
	UINT32 vp_out_crop_id = 0;
	HD_URECT *p_cap_cur_crop = cur_crop + vc_out_crop_id;
	HD_URECT *p_proc_cur_crop = cur_crop + vp_in_crop_id;
	HD_URECT *p_proc2_cur_crop = cur_crop + vp_out_crop_id;
	...
	ret = set_proc_param(stream[0].proc_path, &main_dim); //we will set crop, do not "auto sync"
	if (ret != HD_OK) {
	...
	printf("Enter w/x to change crop of videocap.out\n");
	printf("Enter e/c to change crop of videoproc.in\n");
	printf("Enter r/v to change crop of videoproc.out\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
		...
		if (key == 'w') {
			HD_VIDEOCAP_CROP  video_out_param = {0};
			if(vc_out_crop_id < 4) {
				// next crop
				vc_out_crop_id ++;
				p_cap_cur_crop = cur_crop + vc_out_crop_id;
				printf("*** set videocap.out crop = (%d,%d,%d,%d) ***\r\n", p_cap_cur_crop->x, p_cap_cur_crop->y, p_cap_cur_crop->w, p_cap_cur_crop->h);

				video_out_param.mode  = HD_CROP_ON;
				video_out_param.win.rect.x   = p_cap_cur_crop->x;
				video_out_param.win.rect.y   = p_cap_cur_crop->y;
				video_out_param.win.rect.w   = p_cap_cur_crop->w;
				video_out_param.win.rect.h   = p_cap_cur_crop->h;
				ret = hd_videocap_set(stream[0].cap_path, HD_VIDEOCAP_PARAM_OUT_CROP, &video_out_param);
				hd_videocap_start(stream[0].cap_path);
			}
		}
		...
		if (key == 'e') {
			...
			if(vp_in_crop_id < 4) {
				// next crop
				vp_in_crop_id ++;
				p_proc_cur_crop = cur_crop + vp_in_crop_id;
				printf("*** set videoproc.in crop = (%d,%d,%d,%d) ***\r\n", p_proc_cur_crop->x, p_proc_cur_crop->y, p_proc_cur_crop->w, p_proc_cur_crop->h);

				video_in_param.mode  = HD_CROP_ON;
				video_in_param.win.rect.x   = p_proc_cur_crop->x;
				video_in_param.win.rect.y   = p_proc_cur_crop->y;
				video_in_param.win.rect.w   = p_proc_cur_crop->w;
				video_in_param.win.rect.h   = p_proc_cur_crop->h;
				ret = hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_IN_CROP, &video_in_param);
				hd_videoproc_start(stream[0].proc_path);
			}
		}
		...
		if (key == 'r') {
			...
			if(vp_out_crop_id < 4) {
				// next crop
				vp_out_crop_id ++;
				p_proc2_cur_crop = cur_crop + vp_out_crop_id;
				printf("*** set videoproc.out crop = (%d,%d,%d,%d) ***\r\n", p_proc2_cur_crop->x, p_proc2_cur_crop->y, p_proc2_cur_crop->w, p_proc2_cur_crop->h);
				printf("*** set videoenc.in dim = (%d,%d) ***\r\n", p_proc2_cur_crop->w, p_proc2_cur_crop->h);

				video_out_param.mode  = HD_CROP_ON;
				video_out_param.win.rect.x   = p_proc2_cur_crop->x;
				video_out_param.win.rect.y   = p_proc2_cur_crop->y;
				video_out_param.win.rect.w   = p_proc2_cur_crop->w;
				video_out_param.win.rect.h   = p_proc2_cur_crop->h;
				ret = hd_videoproc_set(stream[0].proc_path, HD_VIDEOPROC_PARAM_OUT_CROP, &video_out_param);
				video_in_param.dir     = HD_VIDEO_DIR_NONE;
				video_in_param.pxl_fmt = HD_VIDEO_PXLFMT_YUV420;
				video_in_param.dim.w   = p_proc2_cur_crop->w;
				video_in_param.dim.h   = p_proc2_cur_crop->h;
				video_in_param.frc     = HD_VIDEO_FRC_RATIO(1,1);
				...
				ret = hd_videoenc_set(stream[0].enc_path, HD_VIDEOENC_PARAM_IN, &video_in_param);
				hd_videoenc_start(stream[0].enc_path);

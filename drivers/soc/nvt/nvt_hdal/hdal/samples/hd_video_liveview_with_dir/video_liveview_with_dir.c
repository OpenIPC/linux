/**
	@brief Sample code of video liveview with dir.\n

	@file video_liveview_with_dir.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_liveview_with_dir, argc, argv)
...
MAIN(argc, argv)
{
	...
	#endif
	printf("Enter t/b to mirror/flip of videoout.in\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
			// quit program
		...
		if (key == 'w') {
			HD_VIDEOCAP_OUT video_out_param = {0};
			{
				...
				else if(vc_out_dir == 4)
					video_out_param.dir = HD_VIDEO_DIR_ROTATE_90;
				else if(vc_out_dir == 5)
					video_out_param.dir = HD_VIDEO_DIR_MIRRORX|HD_VIDEO_DIR_ROTATE_90;
		...
		if (key == 't') {
			HD_VIDEOOUT_IN video_in_param = {0};
			{
				// next dir
				vo_in_dir ++;
				if(vo_in_dir > 5) vo_in_dir = 0;
				printf("*** set videoout.in dir = (%d) ***\r\n", vo_in_dir);
				hd_videoout_get(stream[0].out_path, HD_VIDEOOUT_PARAM_IN, &video_in_param);
                video_in_param.dim.w = stream[0].out_syscaps.output_dim.w;
                video_in_param.dim.h = stream[0].out_syscaps.output_dim.h;
				if(vo_in_dir == 0)
					video_in_param.dir = HD_VIDEO_DIR_NONE;
				else if(vo_in_dir == 1)
					video_in_param.dir = HD_VIDEO_DIR_MIRRORX;
                    ...
                    //swap w,h avoid diplay scale down ratio > 2
                    video_in_param.dim.w = stream[0].out_syscaps.output_dim.h;
                    video_in_param.dim.h = stream[0].out_syscaps.output_dim.w;
					video_in_param.dir = HD_VIDEO_DIR_ROTATE_90;
				}else if(vo_in_dir == 5){
                    //swap w,h avoid diplay scale down ratio > 2
                    video_in_param.dim.w = stream[0].out_syscaps.output_dim.h;
                    video_in_param.dim.h = stream[0].out_syscaps.output_dim.w;
					video_in_param.dir = HD_VIDEO_DIR_MIRRORX|HD_VIDEO_DIR_ROTATE_90;
				}
                hd_videoout_set(stream[0].out_path, HD_VIDEOOUT_PARAM_IN, &video_in_param);
				hd_videoproc_start(stream[0].proc_path);
				hd_videoout_start(stream[0].out_path);
			}
		}

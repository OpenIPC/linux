/**
	@brief Sample code of video record with userqp.\n

	@file video_record_with_userqp.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_venc_userqp, argc, argv)
...
#define USER_QP_BUFSIZE(w, h) ((ALIGN_CEIL_64(w)/16) * (ALIGN_CEIL_64(h)/16) * 2)
...
static HD_RESULT mem_init(void)
{
	...
	// config common pool (userqp)
	mem_cfg.pool_info[2].type = HD_COMMON_MEM_USER_POOL_BEGIN;
	mem_cfg.pool_info[2].blk_size = USER_QP_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H);
...
static HD_RESULT set_enc_userqp(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type)
{
	HD_RESULT ret = HD_ERR_NG;
	HD_COMMON_MEM_VB_BLK blk = 0;
	UINT32 blk_size = USER_QP_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H);
	UINT32 pa = 0, va = 0;
	HD_H26XENC_USR_QP usrqp_param = {0};
	UINT32 mb_w = 0, mb_h = 0, loft = 0;
	UINT32 w = 0, h = 0;
	UINT16 *addr = NULL;

	//--- assign mb_w / mb_h / loft ---
	if (enc_type == 0) {
		// H265
		mb_w = ALIGN_CEIL_64(p_dim->w)/16;
		mb_h = ALIGN_CEIL_64(p_dim->h)/16;
		loft = ALIGN_CEIL_64(p_dim->w)/16;
	} else if (enc_type == 1) {
		// H264
		mb_w = ALIGN_CEIL_16(p_dim->w)/16;
		mb_h = ALIGN_CEIL_16(p_dim->h)/16;
		loft = ALIGN_CEIL_32(p_dim->w)/16;   // HW limit, have to 32-byte align lineoffset
	} else {
		printf("unsupport enc_type(%d) !!\n", enc_type);
		return HD_ERR_PARAM;
	}
	...
	for (h = 0; h < mb_h; h++) {
		for (w = 0; w < mb_w; w++) {
			if (h < 2)
				*(addr + h*loft + w) = (3<<6 | 10);		// fix qp
			else if (h < 4)
				*(addr + h*loft + w) = (10);			// delta qp
			else if (h < 6)
				*(addr + h*loft + w) = (3<<6 | 20);		// fix qp
			else
				*(addr + h*loft + w) = (((UINT8)(-5))&0x3f);			// delta qp
		}
	}

	// flush cache memory to RAM
	ret = hd_common_mem_flush_cache((void *)va, blk_size);
	if (ret != HD_OK) {
		goto fail_3;
	}

	usrqp_param.enable      = 1;
	usrqp_param.qp_map_addr = pa;

	ret = hd_videoenc_set(video_enc_path, HD_VIDEOENC_PARAM_OUT_USR_QP, &usrqp_param);
	if (ret != HD_OK) {
		goto fail_3;
	}
...
static HD_RESULT set_enc_param(HD_PATH_ID video_enc_path, HD_DIM *p_dim, UINT32 enc_type, UINT32 bitrate)
{
	...
	if (p_dim != NULL) {
		...
		if (enc_type == 0) {
			...
			//--- HD_VIDEOENC_PARAM_OUT_ROW_RC ---   (disable it)
			hd_videoenc_get(video_enc_path, HD_VIDEOENC_PARAM_OUT_ROW_RC, &rowrc_param);
			rowrc_param.enable = 0;
			ret = hd_videoenc_set(video_enc_path, HD_VIDEOENC_PARAM_OUT_ROW_RC, &rowrc_param);
			if (ret != HD_OK) {
				printf("set_enc_param_rowrc = %d\r\n", ret);
				return ret;
			}

			//--- HD_VIDEOENC_PARAM_OUT_AQ ---    (disable it)
			hd_videoenc_get(video_enc_path, HD_VIDEOENC_PARAM_OUT_AQ, &aq_param);
			aq_param.enable = 0;
			ret = hd_videoenc_set(video_enc_path, HD_VIDEOENC_PARAM_OUT_AQ, &aq_param);
			if (ret != HD_OK) {
				printf("set_enc_param_aq = %d\r\n", ret);
				return ret;
			}

			//--- HD_VIDEOENC_PARAM_OUT_USR_QP ---  (setup user qpmap)
			ret = set_enc_userqp(video_enc_path, p_dim, enc_type);
			if (ret != HD_OK) {
				printf("set_enc_user_qp = %d\r\n", ret);
				return ret;
			}
			...
			printf("error: MJPEG is not support for user qpmap!\r\n");
			return HD_ERR_NG;
		} else {
...
MAIN(argc, argv)
{
	...
	ret = set_enc_cfg(stream[0].enc_path, &stream[0].enc_max_dim, 4 * 1024 * 1024);
	if (ret != HD_OK) {
	...
	ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, enc_type, 4 * 1024 * 1024);
	if (ret != HD_OK) {

...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(ai_dnet, argc, argv)
...
static HD_RESULT network_dump_dout_buf(NET_PATH_ID net_path, void *p_out)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	...
	if (p_rslt_info) {
		for (i = 0; i < p_rslt_info->result_num; i++) {
			...
			p_rslt->x = p_rslt->x * p_in->in_cfg.w;
			p_rslt->y = p_rslt->y * p_in->in_cfg.h;
			p_rslt->w = p_rslt->w * p_in->in_cfg.w;
			p_rslt->h = p_rslt->h * p_in->in_cfg.h;
...
static VOID *network_user_thread(VOID *arg)
{
	...
	while (p_stream->proc_exit == 0) {
		...
		if (p_stream->proc_oneshot) {
			...
			ret = network_dump_dout_buf(p_stream->net_path, &out_buf);
			if (HD_OK != ret) {
...
int main(int argc, char *argv[])
{
	...
	//net_in
	NET_IN_CONFIG in_cfg = {
		.input_filename = "jpg/YUV420_SP_Huang_W1920H1080.bin",
		.w = 1920,
		.h = 1080,
		.c = 2,
		.loff = 1920,
	...
	if(argc < 3){
		printf("usage : ai_dnet (job_opt) (job_sync) (buf_opt)\n");
		return -1;
	}

/**
	@brief Sample code of video record with two streams with two pass video process.\n

	@file video_record_with_vprc_2pass.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_dis_vprc_2pass, argc, argv)
...
static HD_RESULT set_proc_param(HD_PATH_ID video_proc_path, HD_DIM* p_dim, UINT32 dis)
{
	HD_RESULT ret = HD_OK;
...
static HD_RESULT open_module(VIDEO_RECORD *p_stream, HD_DIM* p_proc0_max_dim, HD_DIM* p_proc1_max_dim)
{
	...
	ret = set_proc_cfg(HD_VIDEOPROC_0_CTRL, &p_stream->proc0_ctrl, HD_VIDEOPROC_PIPE_RAWALL | HD_VIDEOPROC_PIPE_OUT_DIS, p_proc0_max_dim);
	if (ret != HD_OK) {
...
MAIN(argc, argv)
{
	...
	ret = set_proc_param(stream[0].proc0_path, &stream[0].proc1_dim, 1);
	if (ret != HD_OK) {
	...
	ret = set_proc_param(stream[1].proc1_path, NULL, 0);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}

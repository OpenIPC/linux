	        ...
	        while (p_stream->op_exit == 0) {
	        	...
	        	if (p_stream->op_oneshot) {
                  ...
                  if(HD_OK != ret) {
					...
					snprintf(dump_path, 256, "./op%d_user_out.bin", (int)p_stream->op_path);
                    mem_save(&p_op->output_mem, dump_path);
...
{
	VIDEO_LIVEVIEW stream[NN_RUN_OP_NUM] = {0}; 
	...
	NET_IN_CONFIG in_cfg[NN_RUN_OP_NUM] = {
		{
    ...
    if(argc < 3){
		printf("usage : ai_op (op_opt1) (op_opt2)\n"
			   "op-opt:\n"
			   "0 FC\n"
			   "1 PREPROC (YUV2RGB)\n"
	...
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
		if (argc > idx) {
			sscanf(argv[idx++], "%d", &stream[i].net_op_opt);
		}
	}
	...
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
		if (stream[i].net_op_opt == AI_OP_FC)
			printf("OP%u: Run FC!\r\n",i);
		else if (stream[i].net_op_opt == AI_OP_PREPROC_YUV2RGB)
			printf("OP%u: Run PREPROC (YUV2RGB)!\r\n",i);
		else if (stream[i].net_op_opt == AI_OP_PREPROC_YUV2RGB_SCALE)
			printf("OP%u: Run PREPROC (YUV2RGB & scale)!\r\n",i);
		else if (stream[i].net_op_opt == AI_OP_PREPROC_YUV2RGB_MEANSUB_PLANE)
			printf("OP%u: Run PREPROC (YUV2RGB & meansub_plane)!\r\n",i);
		else if (stream[i].net_op_opt == AI_OP_PREPROC_YUV2RGB_MEANSUB_DC)
			printf("OP%u: Run PREPROC (YUV2RGB & meansub_dc)!\r\n",i);
		else {
			printf("OP%u: Unknown op-opt = %d",i,stream[i].net_op_opt);
			return -1;
		}
	}
	...
	// init mem
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
		if(stream[i].net_op_opt != AI_OP_FC) { //for preproc
	...
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
		...
		// set operator config
    	ret = operator_set_config(stream[i].op_path, stream[i].net_op_opt);
    	if (HD_OK != ret) {
	...
	// start
	for (i = 0; i < NN_RUN_OP_NUM; i++) {
		operator_user_start(&stream[i]);
	}

	printf("Enter q to quit\n");
	printf("Enter a to run op1\n");
	printf("Enter b to run op2\n");
	printf("Enter r to run all ops\n");
	do {
		key = getchar();
		if (key == 'r') {
			...
			// run all stream once
			for (i = 0; i < NN_RUN_OP_NUM; i++) {
				operator_user_oneshot(&stream[i]);
			}
			continue;
		}
		if (key == 'a') {
		...
		if (key == 'b') {
			...
			// run stream[1]
			operator_user_oneshot(&stream[1]);
			continue;
		}
		if (key == 'q' || key == 0x3) {

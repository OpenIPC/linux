/**
	@brief Sample code of video liveview with crop.\n

	@file video_liveview_with_crop.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_liveview_with_crop, argc, argv)
...
static HD_RESULT set_out_cfg(HD_PATH_ID *p_video_out_ctrl, UINT32 out_type)
{
	HD_RESULT ret = HD_OK;
	...
	case 2:
		...
		videoout_mode.input_dim = HD_VIDEOOUT_IN_AUTO;
		videoout_mode.output_mode.hdmi= HD_VIDEOOUT_HDMI_1920X1080I60;
	break;
	default:
...
MAIN(argc, argv)
{
	...
	UINT32 out_type = 1;
 	...
 	HD_URECT cur_crop_out[5] = {{0,0,960,240},{0,0,480,120},{480,0,480,120},{0,120,480,120},{480,120,480,120}};
	UINT32 vc_out_crop_id = 0;
	UINT32 vp_in_crop_id = 0;
	UINT32 vp_out_crop_id = 0;
	HD_URECT *p_cap_cur_crop;
	HD_URECT *p_proc_cur_crop;
	HD_URECT *p_proc2_cur_crop;
	...
	while (1) {
		...
		if (key == 'r') {
			...
			if(vp_out_crop_id < 4) {
				// next crop
				vp_out_crop_id ++;
				p_proc2_cur_crop = cur_crop_out + vp_out_crop_id;

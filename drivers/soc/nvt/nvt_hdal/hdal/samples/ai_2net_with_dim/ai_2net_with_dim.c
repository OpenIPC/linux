...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(ai_2net_with_dim, argc, argv)
...
int main(int argc, char *argv[])
{
	...
	NET_IN_CONFIG in_cfg[NN_RUN_NET_NUM] = {
		...
		{
			.input_filename = "jpg2/RGB_W1920H1080.bin",
			.w = 1920,
			.h = 1080,
			.c = 2,
			.loff = 1920,
	...
	NET_PROC_CONFIG net_cfg[NN_RUN_NET_NUM] = {
		...
		{
			.model_filename = "para2/nvt_model_1280x720.bin",
			.label_filename = "accuracy2/labels.txt",
			.diff_filename = "para2/nvt_stripe_model.bin",
			.p_share_model = NULL,
		}
	};

	if(argc < 2){
		printf("usage : ai_2net_with_dim (share_model) (job_opt) (job_sync) (buf_opt)\n");

/**
	@brief Sample code of audio capture with 2 audiocapture.\n

	@file audio_capture_only_with_2cap.c

	@author HM Tseng

	@ingroup mhdal

	@note This file is modified from audio_capture_only.c.
...
static void *capture_thread(void *arg)
{
	...
	/* config pattern name */
	snprintf(file_path_main, sizeof(file_path_main), "/mnt/sd/audio_bs_%d_%d_%d_pcm_p0.dat", HD_AUDIO_BIT_WIDTH_16, HD_AUDIO_SOUND_MODE_MONO, p_cap_only->sample_rate);
	snprintf(file_path_len, sizeof(file_path_len), "/mnt/sd/audio_bs_%d_%d_%d_pcm_p0.len", HD_AUDIO_BIT_WIDTH_16, HD_AUDIO_SOUND_MODE_MONO, p_cap_only->sample_rate);
	snprintf(file_path_main1, sizeof(file_path_main1), "/mnt/sd/audio_bs_%d_%d_%d_pcm_p1.dat", HD_AUDIO_BIT_WIDTH_16, HD_AUDIO_SOUND_MODE_MONO, p_cap_only->sample_rate);
	snprintf(file_path_len1, sizeof(file_path_len1), "/mnt/sd/audio_bs_%d_%d_%d_pcm_p1.len", HD_AUDIO_BIT_WIDTH_16, HD_AUDIO_SOUND_MODE_MONO, p_cap_only->sample_rate);
...
EXAMFUNC_ENTRY(hd_audio_capture_only_with_2dev, argc, argv)
{
	HD_RESULT ret;
	INT key;
	...
	caponly.sample_rate = HD_AUDIO_SR_48000; //assign by user
	...
	ret = set_cap_param(caponly.cap_path1, caponly.sample_rate);
	if (ret != HD_OK) {
		printf("set cap fail=%d\n", ret);
		goto exit;
	}

...
static HD_RESULT network_dump_ai_layer0_inbuf(NET_PATH_ID net_path, VENDOR_AI_BUF *p_inbuf)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	if (p_inbuf == NULL) {
		printf("proc_id(%u) p_inbuf == NULL !!\n", proc_id);
		goto exit;
	}

	printf("dump layer0 inbuf:\n");
	printf("   sign(0x%x)\n", p_inbuf->sign);
	printf("   ddr_id(%lu)\n", p_inbuf->ddr_id);
	printf("   pa(0x%lx)\n", p_inbuf->pa);
	printf("   va(0x%lx)\n", p_inbuf->va);
...
static HD_RESULT network_dump_ai_layer0_outbuf(NET_PATH_ID net_path, VENDOR_AI_BUF *p_outbuf)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	if (p_outbuf == NULL) {
		printf("proc_id(%u) p_outbuf == NULL !!\n", proc_id);
		goto exit;
	}

	printf("dump layer0 outbuf:\n");
	printf("   sign(0x%x)\n", p_outbuf->sign);
	printf("   ddr_id(%lu)\n", p_outbuf->ddr_id);
	printf("   pa(0x%lx)\n", p_outbuf->pa);
	printf("   va(0x%lx)\n", p_outbuf->va);
...
static HD_RESULT get_layer0_in_buf(NET_PATH_ID net_path, VENDOR_AI_BUF *p_inbuf)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	// get layer0 in buf
	layer_id = 0;
	ret = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_IN(layer_id, 0), p_inbuf);
	if (HD_OK != ret) {
	...
	printf("==============< %s >==============\n", __func__);
	ret = network_dump_ai_layer0_inbuf(proc_id, p_inbuf);
	if (HD_OK != ret) {
		printf("proc_id(%u) dump layer0_inbuf fail !!\n", proc_id);
		goto exit;
	}
...
static HD_RESULT get_layer0_out_buf(NET_PATH_ID net_path, VENDOR_AI_BUF *p_outbuf)
{
	HD_RESULT ret = HD_OK;
	NET_PROC* p_net = g_net + net_path;
	UINT32 proc_id = p_net->proc_id;
	...
	// get layer0 in buf
	layer_id = 0;
	ret = vendor_ai_net_get(proc_id, VENDOR_AI_NET_PARAM_OUT(layer_id, 0), p_outbuf);
	if (HD_OK != ret) {
	...
	printf("==============< %s >==============\n", __func__);
	ret = network_dump_ai_layer0_outbuf(proc_id, p_outbuf);
	if (HD_OK != ret) {
		printf("proc_id(%u) dump layer0_outbuf fail !!\n", proc_id);
		goto exit;
	}
...
static VOID *network_user_thread(VOID *arg)
{
	...
	VENDOR_AI_BUF layer0_out = {0};

	printf("\r\n");
	while (p_stream->proc_start == 0) sleep(1);

	// get first layer in/out buf
	get_layer0_in_buf(p_stream->net_path, &layer0_in);
	get_layer0_out_buf(p_stream->net_path, &layer0_out);
...
int main(int argc, char *argv[])
{
	...
	if(argc < 3){
		printf("usage : ai_net_with_buf (job_opt) (job_sync) (buf_opt)\n");
		return -1;
	}

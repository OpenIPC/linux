/**
	@brief Sample code of video record with dim.\n

	@file video_record_with_dim.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_dim, argc, argv)
...
MAIN(argc, argv)
{
	...
	UINT32 enc_type = 0;
	HD_DIM main_dim;
 	HD_DIM cur_dim[4] = {{1920,1080},{1280,720},{960,540},{640,360}};
	UINT32 ve_in_dim_id = 0;
	HD_DIM *p_enc_cur_dim = cur_dim + ve_in_dim_id;
	...
	printf("Enter r/v to +/- dim of videoenc.in\n");
	while (1) {
		key = GETCHAR();
		if (key == 'q' || key == 0x3) {
		...
		if (key == 'r') {
			...
			if(ve_in_dim_id < 3) {
				// next dim
				ve_in_dim_id ++;
				p_enc_cur_dim = cur_dim + ve_in_dim_id;
				printf("*** set videoenc.in dim = (%d,%d) ***\r\n", p_enc_cur_dim->w, p_enc_cur_dim->h);
				...
				video_in_param.dir     = HD_VIDEO_DIR_NONE;
				video_in_param.pxl_fmt = HD_VIDEO_PXLFMT_YUV420;
				video_in_param.dim.w   = p_enc_cur_dim->w;
				video_in_param.dim.h   = p_enc_cur_dim->h;
				video_in_param.frc     = HD_VIDEO_FRC_RATIO(1,1);

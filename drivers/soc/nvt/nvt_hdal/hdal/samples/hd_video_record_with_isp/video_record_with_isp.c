/**
	@brief Sample code of video record with isp effects.\n

	@file video_record_with_isp.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_isp, argc, argv)
...
#define FPGA_SHDR       0 //vcap is forced to 960x540, Vprc output should be scaler down 960x540
...
static HD_RESULT set_proc_func(HD_PATH_ID video_proc_ctrl)
{
	...
	if (g_3dnr == 1) {
		...
		video_ctrl_param.ref_path_3dnr = HD_VIDEOPROC_0_OUT_0; //use another path as reference path (if vout.dim != raw.dim)
	}
	if (g_colornr == 1) {
		video_ctrl_param.func |= HD_VIDEOPROC_FUNC_COLORNR;
	}
...
static void *encode_thread(void *arg)
{
    ...
    snprintf(file_path_main, 64, "/mnt/sd/dump_bs_main_isp_%lu_%lu_%lu_%lu_%lu_%lu.dat", p_stream0->enc_type,g_shdr,g_wdr,g_3dnr,g_colornr,g_defog);
	//----- open output files -----
...
MAIN(argc, argv)
{
	...
	if (argc == 1 || argc > 7) {
		printf("Usage: <enc-type> <shdr> <wdr> <3dnr> <colornr> <defog>.\r\n");
		printf("Help:\r\n");
		printf("  <enc-type> : 0(H265), 1(H264), 2(MJPG)\r\n");

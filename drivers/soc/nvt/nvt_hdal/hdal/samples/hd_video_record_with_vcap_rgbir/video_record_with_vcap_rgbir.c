/**
	@brief Sample code of video record with rgbir sensor.\n

	@file video_record_with_ir.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_record_with_ir, argc, argv)
...
#define VDO_SIZE_W		1920 //w
#define VDO_SIZE_H		1080 //h
...
#define LOAD_ISP_CFG	1 //for ISP config
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.sensor_pinmux =  0x204; // PIN_SENSOR_CFG_12BITS
	cap_cfg.sen_cfg.sen_dev.pin_cfg.pinmux.serial_if_pinmux = 0;
...
MAIN(argc, argv)
{
	...
	ret = set_proc_param(stream[0].proc_path, NULL, SRC_PXLFMT, FALSE);
	if (ret != HD_OK) {
	...
	ret = set_proc_param(stream[1].proc_path2, NULL, HD_VIDEO_PXLFMT_Y8, FALSE);
	if (ret != HD_OK) {
		printf("set proc fail=%d\n", ret);
		goto exit;
	}
	...
	ret = set_enc_param(stream[0].enc_path, &stream[0].enc_dim, SRC_PXLFMT, enc_type, 2 * 1024 * 1024);
	if (ret != HD_OK) {

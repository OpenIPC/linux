/**
	@brief Sample code of video playback server.\n

	@file video_playback_server.c
...
#define MAIN(argc, argv) 		EXAMFUNC_ENTRY(hd_video_playback_server, argc, argv)
...
static HD_RESULT exit_module(void)
{
	...
	if ((ret = hd_videodec_uninit()) != HD_OK) {
		printf("hd_videodec_uninit fail %d\r\n", ret);
		return ret;
	}
	...
	if ((ret = hd_videoout_uninit()) != HD_OK) {
		printf("hd_videoout_uninit fail %d\r\n", ret);
		return ret;
	}
	return HD_OK;
}
...
static void *feed_bs_thread(void *arg)
{
	...
	// feed bs
	while (p_stream0->feed_bs_exit == 0) {
...
if ((g_pid == 0)) {
	...
	stream[0].blk_size = DBGINFO_BUFSIZE()+VDO_YUV_BUFSIZE(ALIGN_CEIL_64(VDO_SIZE_W), ALIGN_CEIL_64(VDO_SIZE_H), HD_VIDEO_PXLFMT_YUV420);
...
#define HD_VIDEOOUT_PATH(dev_id, in_id, out_id)	(((dev_id) << 16) | (((in_id) & 0x00ff) << 8)| ((out_id) & 0x00ff))
		...
		stream[0].dec_path = HD_VIDEODEC_PATH(HD_DAL_VIDEODEC(0), HD_IN(0), HD_OUT(0));

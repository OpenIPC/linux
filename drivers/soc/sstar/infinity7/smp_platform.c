...
extern int mercury6_platform_cpu_kill(unsigned int cpu);
extern void mercury6_platform_cpu_die(unsigned int cpu);
extern int mercury6_platform_cpu_disable(unsigned int cpu);
...
int mercury6_platform_cpu_kill(unsigned int cpu)
{
	return 1;
}
 ...
 */
void __ref mercury6_platform_cpu_die(unsigned int cpu)
{
	int spurious = 0;
//	printk(KERN_ERR "[%s]\n",__FUNCTION__);
...
int mercury6_platform_cpu_disable(unsigned int cpu)
{
	/*
...
void mercury6_secondary_init(unsigned int cpu)
{
//	printk(KERN_ERR "[%s]\n",__FUNCTION__);
printk(KERN_EMERG "[%s]\n",__FUNCTION__);
	/*
...
int mercury6_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
...
extern void mercury6_secondary_startup(void);

#define SCU_CTRL		0x00

void __init mercury6_smp_prepare_cpus(unsigned int max_cpus)
{
	int i;
//	u32 scu_ctrl;
//	printk(KERN_ERR "[%s]\n",__FUNCTION__);
...
//	printk("[306]!!  scu_enable\n");
	 ...
	 */
	do{
		OUTREG16(SECOND_START_ADDR_LO,(virt_to_phys(mercury6_secondary_startup) & 0xFFFF));
	}while(INREG16(SECOND_START_ADDR_LO)!=(virt_to_phys(mercury6_secondary_startup) & 0xFFFF));

	do{
		OUTREG16(SECOND_START_ADDR_HI,(virt_to_phys(mercury6_secondary_startup)>>16));
	}while(INREG16(SECOND_START_ADDR_HI)!=(virt_to_phys(mercury6_secondary_startup)>>16));

//	__cpuc_flush_kern_all();
}

void __init mercury6_smp_init_cpus(void)
{
...
void mercury6_secondary_gic(void)
{
	/* Diable GICC */
...
struct smp_operations __initdata mercury6_smp_ops  = {
    .smp_init_cpus      = mercury6_smp_init_cpus,
    .smp_prepare_cpus   = mercury6_smp_prepare_cpus,
    .smp_secondary_init = mercury6_secondary_init,
    .smp_boot_secondary = mercury6_boot_secondary,

#ifdef CONFIG_HOTPLUG_CPU
    .cpu_kill           = mercury6_platform_cpu_kill,
    .cpu_die            = mercury6_platform_cpu_die,
    .cpu_disable        = mercury6_platform_cpu_disable,
#endif
};

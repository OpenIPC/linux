...
extern int mercury6p_platform_cpu_kill(unsigned int cpu);
extern void mercury6p_platform_cpu_die(unsigned int cpu);
extern int mercury6p_platform_cpu_disable(unsigned int cpu);

extern void mercury6p_secondary_startup(void);
void mercury6p_smp_init_cpus(void);
void mercury6p_smp_prepare_cpus(unsigned int max_cpus);
...
int mercury6p_platform_cpu_kill(unsigned int cpu)
{
    return 1;
}
 ...
 */
void mercury6p_platform_cpu_die(unsigned int cpu)
{
    int spurious = 0;
//  printk(KERN_ERR "[%s]\n",__FUNCTION__);
...
int mercury6p_platform_cpu_disable(unsigned int cpu)
{
    /*
...
void mercury6p_secondary_init(unsigned int cpu)
{
...
int mercury6p_boot_secondary(unsigned int cpu, struct task_struct *idle)
{
    ...
    if (suspend_status) {
        do{
            OUTREG16(SECOND_START_ADDR_LO,(virt_to_phys(mercury6p_secondary_startup) & 0xFFFF));
        }while(INREG16(SECOND_START_ADDR_LO)!=(virt_to_phys(mercury6p_secondary_startup) & 0xFFFF));

        do{
            OUTREG16(SECOND_START_ADDR_HI,(virt_to_phys(mercury6p_secondary_startup)>>16));
        }while(INREG16(SECOND_START_ADDR_HI)!=(virt_to_phys(mercury6p_secondary_startup)>>16));
...
//  u32 scu_ctrl;
    ...
    if (arm_has_idmap_alias()) {
        secondary_startup_addr = phys_to_idmap(virt_to_phys(mercury6p_secondary_startup));
    } else {
        secondary_startup_addr = virt_to_phys(mercury6p_secondary_startup);
    }
...
struct smp_operations mercury6p_smp_ops  = {
    .smp_init_cpus      = mercury6p_smp_init_cpus,
    .smp_prepare_cpus   = mercury6p_smp_prepare_cpus,
    .smp_secondary_init = mercury6p_secondary_init,
    .smp_boot_secondary = mercury6p_boot_secondary,

#ifdef CONFIG_HOTPLUG_CPU
    .cpu_kill           = mercury6p_platform_cpu_kill,
    .cpu_die            = mercury6p_platform_cpu_die,
    .cpu_disable        = mercury6p_platform_cpu_disable,
#endif
};

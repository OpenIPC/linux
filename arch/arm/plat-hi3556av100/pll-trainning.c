...
static void svb_voltage_regulator_init(struct voltage_regulator *regulator)
{
	...
	if (regulator->curr + 37 < regulator->max) {
		regulator->steps[0] = regulator->curr + 37;
	} else {
		regulator->steps[0] = regulator->max;
	}

	regulator->steps[1] = regulator->curr;

	if (regulator->curr > regulator->min + 37) {
		regulator->steps[2] = regulator->curr - 37;
	} else {
		regulator->steps[2] = regulator->min;
	}

	if (regulator->curr > regulator->min + 74) {
		regulator->steps[3] = regulator->curr - 74;
	} else {
		regulator->steps[3] = regulator->min;
	}

	if (regulator->curr > regulator->min + 111) {
		regulator->steps[4] = regulator->curr - 111;
	} else {

...
static struct ak_pwm_timer_info pt_init_info[] = {
    {AK_PWM_TIMER1, AK_GPIO_80, IRQ_TIMER1, AK_PWM1_CTRL, ePIN_AS_PWM1, 1},
    {AK_PWM_TIMER2, AK_GPIO_69, IRQ_TIMER2, AK_PWM2_CTRL, ePIN_AS_PWM2, 0},
	{AK_PWM_TIMER3, AK_GPIO_5, IRQ_TIMER3, AK_PWM3_CTRL, ePIN_AS_PWM3, 0},
	{AK_PWM_TIMER4, AK_GPIO_66, IRQ_TIMER4, AK_PWM4_CTRL, ePIN_AS_PWM4, 0},
	{AK_PWM_TIMER5, AK_GPIO_47, IRQ_TIMER5, AK_PWM5_CTRL, ePIN_AS_PWM5, 1},
};
...
int ak_pwm_config(struct ak_pwm_timer *pwm, unsigned short high, unsigned short low, unsigned int freq)
{
    ...
    if(!pwm_freq_is_vaild(freq)){
        return -1;
    }
	regval = REG32(pwm->base + AK_PWM_TIMER_CTRL2);
	regval &= ~AK_TIMER_WORK_MODE_MASK;
	regval |= AK_TIMER_WORK_MODE(pwm->mode);
	REG32(pwm->base + AK_PWM_TIMER_CTRL2) = regval;

	//pre_div = (REAL_CRYSTAL_FREQ * (high + 1)+(low + 1))/freq - 1;
	pre_div = (REAL_CRYSTAL_FREQ / ((high+1)+(low+1)))/freq - 1;
	//printk(KERN_ERR "div:%u, h:%u, l:%u, f:%u, act_f:%d\n", pre_div, high, low, freq, (REAL_CRYSTAL_FREQ) / (pre_div + 1) / (high +1+ low+1));
...
*/
static int ak_pwm_timer_init(void) 
{
	int i;
...
EXPORT_SYMBOL(ak_pwm_timer_init);
...
static struct ak_pwm_timer *__pwm_timer_request(int id, u8 mode)
{
	...
	if(!id_is_vaild(id) || timer_is_reserved(id) || timer_is_busy(id)) {
		printk(KERN_ERR"pwm[%d] is working or invalid id. request fail.\n", id);
		return NULL;
	}

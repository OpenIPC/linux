...
static int ak39_gpio_irq_set_wake(struct irq_data *d, unsigned int on)
{
	...
	if (d->irq >= IRQ_GPIO_0 && d->irq <= IRQ_GPIO_2)
		regval |= (1 << (d->irq - IRQ_GPIO_0));

	else if (d->irq>= IRQ_GPIO_4 && d->irq <= IRQ_GPIO_11)
		regval |= (1 << (d->irq - IRQ_GPIO_3 + 3));

	else if (d->irq>= IRQ_GPIO_13 && d->irq <= IRQ_GPIO_15)
		regval |= (1 << (d->irq - IRQ_GPIO_12 + 11));
	
	else if (d->irq>= IRQ_GPIO_19 && d->irq <= IRQ_GPIO_20)
		regval |= (1 << (d->irq - IRQ_GPIO_18 + 14));

	else if (d->irq>= IRQ_GPIO_23 && d->irq <= IRQ_GPIO_24)
		regval |= (1 << (d->irq - IRQ_GPIO_22 + 16));

	else if (d->irq == IRQ_GPIO_49)
		regval |= (1 << (d->irq - IRQ_GPIO_48 + 18));
	
	else if (d->irq >= IRQ_GPIO_52 && d->irq <= IRQ_GPIO_55)
		regval |= (1 << (d->irq - IRQ_GPIO_51 + 19));

	else if (d->irq >= IRQ_GPIO_57 && d->irq <= IRQ_GPIO_63)
		regval |= (1 << (d->irq- IRQ_GPIO_56 + 24));
...
static void ak39_gpio_irqhandler(unsigned int irq, struct irq_desc *desc)
{
    ...
    unsigned int i;
    unsigned int off;
    unsigned int edge_sta;
    unsigned int mode;


    for (i = 0; i < 3; i++) {
        enabled_irq = __raw_readl(AK_GPIO_INT_MASK1 + i * 4);
        edge_sta = __raw_readl(AK_GPIO_EDGE_STATUS1 + i * 4);//clean irq
        mode = __raw_readl(AK_GPIO_INT_MODE1 + i * 4 );

...
static struct cgu_clk cgu_clks[] = {
	...
	[CGU_VPU] = 	{ CPM_VPUCDR, 27, 1, 4, SELECTOR_3},
	[CGU_MACPHY] = 	{ CPM_MACCDR, 27, 1, 8, SELECTOR_3},
	[CGU_LPC] = 	{ CPM_LPCDR,  26, 1, 8, SELECTOR_3},
	...
	[CGU_I2S] = 	{ CPM_I2SCDR, 29, 0, 13, SELECTOR_3}, //TO_FIX: reg, ce_enable, i2sdiv_n, i2sdiv_m
	[CGU_SSI] = 	{ CPM_SSICDR, 27, 1, 8, SELECTOR_2}, //TO_FIX
	[CGU_CIM] =     { CPM_CIMCDR, 27, 1, 8, SELECTOR_3},
	[CGU_ISP] = 	{ CPM_ISPCDR, 27, 1, 4, SELECTOR_3},
};
...
static int cgu_set_rate(struct clk *clk, unsigned long rate)
{
	...
	if (!(strcmp(clk->name, "cgu_i2s"))) {
		int m = 1, n = 0;
		clk->rate = rate;
		n = (clk->parent->rate * m) / rate;
		reg_val = cpm_inl(cgu_clks[no].off) & 0xf0000000;
		reg_val |= (m << 13) | (n << 0);
		cpm_outl(reg_val,cgu_clks[no].off);
		printk(KERN_DEBUG"%s, parent = %ld, rate = %ld, n = %d, reg val = 0x%08x\n",

...
then
    ...
    make clean;make -j ${jobs}
    cd ${alkaid_dir}/project/kbuild/4.9.84/
    ./release.sh -k ${kernel_dir} -b 011A -p nvr-fastboot -f nor -c ${chip} -l glibc -v ${GCC_VERSION}
    cd ${kernel_dir}
...
then
    ...
    # 012C SPI_NOR
    echo make infinity6e_ssc012c_s01a_defconfig
    make infinity6e_ssc012c_s01a_defconfig; make clean; make -j ${jobs}
    cd ${RELEASE_PATH}
    ./release.sh -k ${kernel_dir} -b 012C -p ipc -f nor -c ${chip} -l glibc -v ${GCC_VERSION} -i ${API_VERSION}
    cd ${kernel_dir}
    ...
    # 012C SPI_NAND
    echo make infinity6e_ssc012c_s01a_spinand_defconfig
    make infinity6e_ssc012c_s01a_spinand_defconfig; make clean; make -j ${jobs}
    cd ${RELEASE_PATH}
    ./release.sh -k ${kernel_dir} -b 012B -p ipc -f spinand -c ${chip} -l glibc -v ${GCC_VERSION} -i ${API_VERSION}
    cd ${kernel_dir}
...
then
    ...
    BOARD_TYPE="012C"
    PRODUCT_TYPE="ipc-rtos"
    KERNEL_DECONFIG="infinity6e_ssc012c_s01a_spinand_amp_defconfig"
    RELEASE_SCRIPT="./release.sh -k ${kernel_dir} -b ${BOARD_TYPE} -p ${PRODUCT_TYPE} -f ${FLASH_TYPE} -c ${CHIP_TYPE} -l ${CLIB} -v ${GCC_VERSION} -i ${API_VERSION}"
    echo ${CHIP_TYPE}/${BOARD_TYPE}/${PRODUCT_TYPE}/${FLASH_TYPE}/${CLIB}/GCC_VERSION=${GCC_VERSION}/API_VERSION=${API_VERSION}
    echo make ${KERNEL_DECONFIG}
    make ${KERNEL_DECONFIG}
...
then
    ...
    # 012C SPI_NAND
    BOARD_TYPE="012C"
    KERNEL_DECONFIG="infinity6e_ssc012c_s01a_spinand_smplh_defconfig"
    RELEASE_SCRIPT="./release.sh -k ${kernel_dir} -b ${BOARD_TYPE} -p ${PRODUCT_TYPE} -f ${FLASH_TYPE} -c ${CHIP_TYPE} -l ${CLIB} -v ${GCC_VERSION} -i ${API_VERSION}"
    echo ${CHIP_TYPE}/${BOARD_TYPE}/${PRODUCT_TYPE}/${FLASH_TYPE}/${CLIB}/GCC_VERSION=${GCC_VERSION}/API_VERSION=${API_VERSION}
    echo make ${KERNEL_DECONFIG}
    make ${KERNEL_DECONFIG}
...
then
    ...
    echo GCC_VERSION=${GCC_VERSION}
    # qfn88  012B SPI_NOR
    echo "mercury6_ca7_fpga_defconfig"
    make mercury6_ca7_fpga_defconfig
    make clean;make -j8
    cd ${alkaid_dir}/project/kbuild/4.9.84/
    ./release.sh -k ${kernel_dir} -b 012B -p ipc -f nor -c ${chip} -l glibc -v ${GCC_VERSION}
    cd ${kernel_dir}
fi

if [ "${chip}" = "p3" ]
then
    TOOLCHAIN_PATH="/tools/toolchain/arm-buildroot-linux-uclibcgnueabihf-4.9.4-uclibc-1.0.31/bin/"
    declare -x PATH=${TOOLCHAIN_PATH}:$PATH
    declare -x ARCH="arm"
    declare -x CROSS_COMPILE="arm-buildroot-linux-uclibcgnueabihf-"
    whereis ${CROSS_COMPILE}gcc
    ...
    cd ${kernel_dir}
    # 012B SPI_NAND
    echo mpioneer3_alkaid_fpga_defconfig
    make pioneer3_alkaid_fpga_defconfig; make clean; make -j ${jobs}
    cd ${RELEASE_PATH}
    ./release.sh -k ${kernel_dir} -b 000A -p dispcam -f nor -c ${chip} -l uclibc -v ${GCC_VERSION}
    cd ${kernel_dir}
...
#for glibc
    ...
    make pioneer3_alkaid_fpga_defconfig; make clean; make -j ${jobs}
    cd ${RELEASE_PATH}
#    ./release.sh -k ${kernel_dir} -b fpga -p dispcam -f nor -c ${chip} -l glibc -v ${GCC_VERSION} -i ${API_VERSION}
    ./release.sh -k ${kernel_dir} -b 000A -p dispcam -f nor -c ${chip} -l glibc -v ${GCC_VERSION}
    cd ${kernel_dir}
fi
